"""Apply reviewed fail-closed hardening to PR 399 exactly once."""
...
def replace_once(
    ...
    *,
    ...
    if count != 1:
        raise SystemExit(f"{path}: {marker} expected once, found {count}")
    path.write_text(text.replace(old, new), encoding="utf-8")


def main() -> None:
    source = Path("src/bayesian_phystwin/posterior_covariance_portfolio.py")
    replace_once(
        source,
    ...
    replace_once(
        ...
        "                raise ValueError(\n"
        "                    \"portfolio projected covariance semantics changed\"\n"
        "                )\n"
        "            semantics_metadata = semantics.metadata\n",
        marker="projected semantics validation",
    )
    replace_once(
        source,
        "            if unavailable:\n"
        ...
        "            fallback_reason = (\n"
        "                fallback_source.covariance_semantics.metadata.get(\n"
        "                    \"fallback_reason\"\n"
        "                )\n"
        "            )\n"
        ...
        "                )\n",
        marker="rejected fallback reason validation",
    )
...
def test_portfolio_hardening_rejects_semantic_and_reason_drift() -> None:
    ...
    entries = tuple(
        tampered_entry if entry is working else entry
...
'''
    ...
    docs = Path("docs/posterior_covariance_portfolio.md")
    replace_once(
        docs,
        "The final portfolio retains the query matrix and every source. "
        "Construction\ntherefore recomputes each projected covariance and "
        "independently verifies the\nquery, source, semantics, and estimator "
        "identities. A manually assembled entry\ncannot substitute a different "
        "covariance with matching labels.\n",
        "The final portfolio retains the query matrix and every source. "
        "Construction\ntherefore recomputes each projected covariance and "
        "the complete projected\nsemantics, then independently verifies the "
        "query, source, semantics, and\nestimator identities. A manually assembled "
        "entry cannot substitute a different\ncovariance or interpretation with "
        "matching labels.\n",
        marker="projected semantics documentation",
    )
    replace_once(
        docs,
        "A rejected inference result has a different contract. It contains exactly "
        "one\n`exact_prior_fallback` covariance, retains the rejection reason, and "
        "cannot list\naccepted-update alternatives. Use\n",
        "A rejected inference result has a different contract. It contains exactly "
        "one\n`exact_prior_fallback` covariance, requires the portfolio reason to "
        "match the\nfallback covariance's recorded reason, and cannot list accepted-"
        "update\nalternatives. Use\n",
        marker="fallback reason documentation",
    )
