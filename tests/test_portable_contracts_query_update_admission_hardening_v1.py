...
def test_legacy_uniform_tolerance_remains_the_split_default() -> None:
    default = QueryUpdateAdmissionPolicyV1()
    assert default.numerical_tolerance == 1e-12
    ...
    assert uniform.descriptor()["numerical_tolerance"] == 1e-6

    mixed = QueryUpdateAdmissionPolicyV1(
        numerical_tolerance=1e-6,
        regret_tolerance=1e-5,
    )
    assert mixed.identifiable_fraction_tolerance == 1e-6
    assert mixed.regret_tolerance == 1e-5
    assert mixed.information_gain_tolerance == 1e-6
