...
SOURCE = (
    ...
    / "scripts"
    / "held"
    / "prepare_deform360_v82_lock.py"
)
SPEC = importlib.util.spec_from_file_location("deform360_v82_lock_preparer", SOURCE)
...
def test_qualification_must_bind_exact_deployed_worker_sources(
    ...
    local["held_v82_process_isolation_worker_source"] = "f" * 64
    with pytest.raises(
        ValueError,
        match="held_v82_process_isolation_worker_source differs",
    ):
