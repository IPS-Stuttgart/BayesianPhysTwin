...
def test_grouped_only_stable_command_has_no_legacy_alias(capsys) -> None:
    assert (
        cli_main.main(["commands", "describe", "decisive-evidence", "--json"])
        == 0
    )
    payload = json.loads(capsys.readouterr().out)
    assert payload["route"] == ["evidence", "summarize"]
    assert payload["legacy_alias"] is None
    assert payload["status"] == "stable"
    assert payload["owner"] == "bayesian-phystwin-decisive-evidence-v1"
