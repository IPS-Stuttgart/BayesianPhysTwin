...
def test_component_admission_modules_are_stable_direct_imports() -> None:
...
def test_provider_neutral_inference_session_is_a_stable_direct_import() -> None:
    manifest = tool.load_lifecycle_manifest()
    stable = set(manifest["stable_modules"])

    assert "bayesian_phystwin.inference.v2" in stable
...
def test_structured_covariance_and_identifiability_are_stable_direct_imports() -> None:
    manifest = tool.load_lifecycle_manifest()
    stable = set(manifest["stable_modules"])

    assert {
        "bayesian_phystwin.identifiability_report_v1",
