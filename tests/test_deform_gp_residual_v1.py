...
class GPTests(unittest.TestCase):
    ...
    def test_gp_matches_finite_kernel_ridge(self):
        ...
        y = np.sin(x * 2)
        gp = fit_gp_bank(x, y, arc=None, anchor_indices=np.array([0, 4, 8, 14]),
                         length_multiplier=1, noises=[0.1], row_weight=1)[0.1]
        kxu = matern_material_kernel((x - gp.location) / gp.scale, gp.anchors, gp.length)
    ...
    def test_query_batch_does_not_change_fit(self):
        ...
        gp = fit_gp_bank(x, x ** 2, arc=None, anchor_indices=np.arange(0, 20, 2),
                         length_multiplier=1, noises=[0.1], row_weight=0.1)[0.1]
    ...
    def test_boundary_and_exact_fallback(self):
        ...
        np.testing.assert_array_equal(add_residual(baseline, correction, frames, 0), baseline)
        changed = add_residual(baseline, correction, frames, 0.5)
        np.testing.assert_array_equal(changed[:, :, [0, 1, -2, -1]], baseline[:, :, [0, 1, -2, -1]])
    ...
    def test_balanced_anchors(self):
        ...
        np.testing.assert_array_equal(np.bincount(indices // (498 * 9)), np.repeat(6, 40))
