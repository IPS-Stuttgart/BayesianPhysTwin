...
from bayesian_phystwin_experiments.dlolab_wrapping_risk_guard_v8 import (
    continuous_worlds as calibration_v8_worlds,
)
...
def test_worlds_are_fresh_deterministic_and_in_registered_stress_region() -> None:
    ...
    assert len(roster) == study.WORLD_COUNT == 288
    assert len(_material_keys(roster)) == 288
    previous = (
        ...
        | _material_keys(calibration_v8_worlds())
    )
...
def test_pre_future_gate_and_score_capture_downside_tradeoff(
    ...
    decisions[:, :, 1] = np.where(
        np.arange(study.WORLD_COUNT)[:, None] < 12, 2, 1
    )
    ...
    assert result["continuous_harmed_worlds"] == 12
    assert result["guard_one_sided_95pct_harm_risk_upper"] < 0.05
...
def test_protocol_freezes_development_use_and_public_only_boundaries() -> None:
    ...
    assert value[
        "worlds_disjoint_from_source_v1_v2_v3_terminal_v4_and_calibration_v8"
    ]
    assert value["terminal_v4_retried_or_scored"] is False
    assert value["runtime_v7_qualification_passed"] is True
    assert value["method_class_changed_from_v8"] is True
    assert value["action_policy_changed_from_v8"] is False
    assert value["calibration_v8_strict_source_gate_passed"] is False
    assert value["calibration_certificate"]["passed"] is True
    for field in (
        "retry_authorized",
...
def test_v9_preserves_the_v4_action_policy_exactly() -> None:
    unchanged_functions = (
        "_base_native_qa",
        "prefix_native_qa",
        "future_native_qa",
    ...
    v9_protocol = study.protocol()
    assert v9_protocol["method"] == (
        "finite_sample_certify_or_exact_fallback_posterior_guard"
    )
    assert v9_protocol["posterior_improvement_probability_threshold"] == v4_protocol[
        "posterior_improvement_probability_threshold"
    ]
    assert v9_protocol["posterior_improvement_margin"] == v4_protocol[
        "posterior_improvement_margin"
    ]
    assert v9_protocol["registered_fixed_action_index"] == v4_protocol[
        "registered_fixed_action_index"
    ]
    assert v9_protocol["arms"] == v4_protocol["arms"]
