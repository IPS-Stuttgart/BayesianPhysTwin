...
SPEC = importlib.util.spec_from_file_location(
    "slingshot_policy_certificate_source_v1_runner",
    ROOT / "scripts/remote/run_dlolab_slingshot_policy_certificate_source_v1.py",
)
...
def test_compact_calibration_rejects_changed_realized_gain(
    ...
    candidate = {
        "candidate_actions": np.zeros(96, dtype=np.int64),
        "predicted_gain": np.zeros(96, dtype=np.float64),
        "mean_raw_upper": np.zeros((96, 7), dtype=np.float64),
    }
    ...
    future_ids = [f"future-{index:03d}" for index in range(96)]
    seal = {
...
def test_complete_rosters_have_no_replacement_slots() -> None:
    assert runner.COUNTS == {"calibration": 96, "evaluation": 288}
    assert runner.prefix_batch_count("calibration") == 12
...
def test_runner_binds_reproduced_authorization_and_frozen_sources() -> None:
    ...
    assert "tests/test_dlolab_slingshot_policy_certificate_source_v1_custody.py" in (
        runner.SOURCES
    )
    assert "scripts/remote/verify_dlolab_slingshot_policy_certificate_source_v1.py" in (
        runner.SOURCES
    )
