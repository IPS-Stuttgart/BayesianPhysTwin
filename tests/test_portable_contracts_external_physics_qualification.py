...
def _qualification(**changes: Any) -> PhysicsBackendQualificationV1:
    values: dict[str, Any] = {
        "backend_profile": BUILTIN_BACKEND_PROFILES[0],
        "runtime_id": DIGEST_A,
    ...
    return PhysicsBackendQualificationV1(**values)


def test_passing_qualification_is_content_addressed_roundtrippable_and_bound(
    tmp_path: Any,
) -> None:
    ...
    assert qualification.source_group_ids == ("object-a", "object-b", "object-c")
    assert qualification.artifact_id == content_id(qualification.descriptor())
    payload = qualification.to_payload()
    assert payload["schema"] == PHYSICS_BACKEND_QUALIFICATION_SCHEMA
    assert payload["schema_version"] == PHYSICS_BACKEND_QUALIFICATION_VERSION
    assert payload["claim_boundary"] == PHYSICS_BACKEND_QUALIFICATION_CLAIM_BOUNDARY
    assert payload["qualified"] is True
    with pytest.raises(TypeError):
        qualification.metadata["evidence_role"] = "tampered"  # type: ignore[index]

    path = tmp_path / "qualification.json"
    save_physics_backend_qualification_v1(qualification, path)
    loaded = load_physics_backend_qualification_v1(path)
    assert loaded == qualification
    assert loaded.to_payload() == payload
    assert (
        require_qualified_backend_runtime(
            qualification.backend_profile,
            qualification.runtime_id,
            qualification,
        )
        is qualification
    )
    with pytest.raises(FileExistsError):
        save_physics_backend_qualification_v1(qualification, path)
    save_physics_backend_qualification_v1(qualification, path, overwrite=True)


@pytest.mark.parametrize(
    ("changes", "reason"),
    [
        ({"units_coordinate_entity_order_valid": False}, "units-coordinate"),
        ({"deterministic_replay_valid": False}, "deterministic-replay"),
        ({"maximum_zero_action_drift_m": 0.002}, "zero-action-equilibrium"),
        (
        ...
        ({"source_query_parity_rmse_m": 0.006}, "source-query-parity"),
...
def test_threshold_equality_passes_and_multiple_failures_are_preserved() -> None:
...
def test_invalid_inputs_and_artifact_identity_fail_closed(tmp_path: Any) -> None:
    with pytest.raises(TypeError, match="backend_profile"):
        _qualification(backend_profile=cast(Any, object()))
...
def test_payload_tampering_and_runtime_mismatches_are_rejected() -> None:
    ...
    with pytest.raises(ValueError, match="does not replay"):
        PhysicsBackendQualificationV1.from_payload(tampered)
