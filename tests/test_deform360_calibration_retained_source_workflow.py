...
def test_reusable_workflow_binds_the_authoritative_source_execution() -> None:
    ...
    assert f'SOURCE_RUN_ID: "{SUPERSEDED_RUN_ID}"' not in source
...
def test_inventory_guide_records_the_artifact_digest_and_execution_chain() -> None:
    source = _source(INVENTORY_GUIDE)

    assert AUTHORITATIVE_ARTIFACT_DIGEST in source
    assert "prepared-source inventory" in source
    assert "frozen all-camera visual-production plan" in source
    assert "inventory-bound visual-execution admission" in source
