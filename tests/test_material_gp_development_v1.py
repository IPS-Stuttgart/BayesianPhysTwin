...
PATH = Path(__file__).resolve().parents[1] / 'scripts/experiments/material_gp_development_v1.py'
...
def test_full_inducing_matches_exact_gp():
    ...
    x, y, q = rng.normal(size=(22, 5)), rng.normal(size=(22, 3)), rng.normal(size=(5, 5))
    ...
    expected_var = gp.kernel_diag(q) - np.einsum('ij,ji->i', kqx, np.linalg.solve(k, kqx.T)) + 0.2
...
def test_candidate_exact_boundary_and_zero_shrinkage():
    ...
    np.testing.assert_array_equal(altered[:, :, [0, 1, -2, -1]], baseline[:, :, [0, 1, -2, -1]])
