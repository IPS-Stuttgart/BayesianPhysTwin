...
def test_ridge_mean_matches_direct_normal_equations():
    ...
    for node in range(2):
        ...
        coef = np.linalg.solve(
            design.T @ design + penalty, design.T @ y[:, :, node].reshape(-1, 3)
        )
        np.testing.assert_allclose(
            pred[:, :, node].reshape(-1, 3), design @ coef, atol=1e-13
        )
...
def test_query_batch_order_invariance(kind):
    ...
    y = rng.normal(size=(8, 12, 2, 3)) * 0.001
    model = ResidualExperts(
        Spec(kind, 2), feature_rank=3, response_rank=2, gate_stride=2
    ).fit(x[:6], y[:6])
    predictions = model.predict(x[6:])
    np.testing.assert_allclose(
        model.predict(x[6:][::-1])[::-1], predictions, atol=1e-13
    )
    np.testing.assert_allclose(
        np.concatenate([model.predict(x[6:7]), model.predict(x[7:8])]),
        predictions,
        atol=1e-13,
    )
    np.testing.assert_allclose(model.weights(x[6:]).sum(-1), 1.0, atol=1e-13)
