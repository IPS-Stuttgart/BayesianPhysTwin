...
def test_runtime_allows_only_the_exact_inherited_conflict() -> None:
    payload = json.loads(AMENDMENT.read_text(encoding="utf-8"))
    ...
    assert 'test "${pip_check_status}" -eq 1' in workflow
    assert 'test "${pip_check_output}" = "${EXPECTED_PIP_CHECK_CONFLICT}"' in workflow
    assert 'version("numpy") != "1.26.4"' in workflow
    assert 'version("pyrecest") != "2.4.3"' in workflow
