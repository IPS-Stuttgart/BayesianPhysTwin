...
def _predictor():
    ...
    return fit_local_policy_gain_predictor(
        reference_ids=tuple(f"ref-{index:02d}" for index in range(7)),
        reference_features=rng.normal(size=(7, 51)),
        reference_action_gains=rng.normal(0.01, 0.01, size=(7, 7)),
        neighbor_count=5,
    )
...
def test_rosters_are_fresh_disjoint_and_tasks_are_complete() -> None:
    ...
    assert prefix_batch_count("calibration") == 12
    assert prefix_batch_count("evaluation") == 36
    assert prefix_task("calibration", 11)["world_indices"] == list(range(88, 96))
...
def test_protocol_freezes_policy_level_information_order() -> None:
    frozen = protocol()
    assert frozen["reference"]["world_count"] == 51
    assert frozen["calibration"]["rank"] == 88
    assert frozen["local_predictor"]["neighbor_count"] == 5
    ...
    assert frozen["closed_288_world_panel_used"] is False
...
def test_candidate_predictions_preserve_fixed_posterior_policy() -> None:
    ...
    assert candidate["candidate_actions"].shape == (96,)
    assert candidate["predicted_gain"].shape == (96,)
    assert candidate["mean_raw_upper"].shape == (96, 7)
    assert candidate["neighbor_indices"].shape == (96, 5)
    assert candidate["features"].shape == (96, 51)


def test_guarded_decisions_have_exact_fallback() -> None:
    candidate = {
        "candidate_actions": np.full(288, 4, dtype=np.int64),
        "predicted_gain": np.r_[np.full(30, 0.1), np.zeros(258)],
    ...
    calibration = calibrate_policy_gain_lower_bound(
        predicted_gain=np.linspace(0.0, 0.095, 96),
        realized_gain=np.zeros(96),
        miscoverage=0.10,
    )
    simultaneous = RegretCalibration(coverage=0.9, count=96, rank=88, offset=1.0)
...
def test_score_preserves_incumbent_under_complete_fallback() -> None:
    ...
    result = score(
        ...
        rewards,
        calibration,
        RegretCalibration(coverage=0.9, count=96, rank=88, offset=1.0),
        all_native_qa=True,
...
def test_simultaneous_comparator_uses_the_same_96_calibration_futures() -> None:
    candidate = {"mean_raw_upper": np.zeros((96, 7), dtype=np.float64)}
    rewards = np.zeros((96, 7), dtype=np.float64)
    calibration = calibrate_simultaneous_guard(candidate, rewards)
    assert (calibration.coverage, calibration.count, calibration.rank) == (0.9, 96, 88)
