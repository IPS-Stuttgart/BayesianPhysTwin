...
def test_load_prob4d_api_v2_reports_missing_optional_dependency(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    with pytest.raises(ImportError, match="requires a compatible installation"):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("PROVIDER_API_VERSION", "2", "must be an integer"),
    ],
)
...
@pytest.mark.parametrize(
    "changes",
    [
        {"prob4d_project_identity": None},
        {"validate_prob4d_project_identity": None},
    ],
)
def test_inspect_prob4d_api_v2_requires_identity_functions(
    monkeypatch: pytest.MonkeyPatch,
    ...
    monkeypatch.setattr(
        bridge.importlib,
        "import_module",
        lambda name: _api(**changes),
    )

    with pytest.raises(ImportError, match="lacks project-identity validation"):
        bridge.inspect_prob4d_api_v2()


def test_inspect_prob4d_api_v2_requires_identity_mapping(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(
        bridge.importlib,
        "import_module",
        lambda name: _api(validate_prob4d_project_identity=lambda value: (value,)),
    )

    with pytest.raises(ImportError, match="identity must be a dictionary"):
        bridge.inspect_prob4d_api_v2()
...
@pytest.mark.parametrize(
    ("field", "value"),
    [
        ("project_id", 1295794737),
        ("canonical_repository", ""),
    ],
)
def test_inspect_prob4d_api_v2_requires_literal_nonempty_identity_strings(
    monkeypatch: pytest.MonkeyPatch,
    field: str,
    value: object,
) -> None:
    ...
    with pytest.raises(ImportError, match="must be a nonempty string"):
        bridge.inspect_prob4d_api_v2()
...
def test_tree_sparse_loader_requires_stable_callable(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(
        bridge.importlib,
        "import_module",
        lambda name: _api(load_claim_bearing_tree_sparse_observation=None),
    )

    with pytest.raises(ImportError, match="lacks the claim-bearing tree-sparse loader"):
        bridge.load_claim_bearing_tree_sparse_prob4d("claim.json")
