...
from bayesian_phystwin_experiments import dlolab_wrapping_risk_guard_v4 as study
...
def test_worlds_are_fresh_deterministic_and_in_registered_stress_region() -> None:
    ...
    assert len(roster) == study.WORLD_COUNT == 72
    assert len(_material_keys(roster)) == 72
    previous = (
...
def test_protocol_freezes_development_use_and_public_only_boundaries() -> None:
    ...
    assert value["worlds_disjoint_from_source_failed_v1_development_v2_and_v3"]
    for field in (
        "retry_authorized",
