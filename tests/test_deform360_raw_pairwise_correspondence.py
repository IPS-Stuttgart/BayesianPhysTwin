...
def test_raw_insufficient_support_uses_persistence_and_preserves_legacy_ablation() -> (
    ...
    np.testing.assert_array_equal(
        trajectories[PHYSICAL_CLIQUE_RBF_ARM][post_update], prior[post_update]
    )
    np.testing.assert_array_equal(
        trajectories[PERSISTENCE_CLIQUE_RBF_ARM][post_update],
        persistence[post_update],
    )
    assert all(
        all(
            gate["bit_exact_raw_fallback"]
            for gate in update["backbone_pairwise_gates"].values()
        )
        for update in report["updates"]
    )


def test_persistence_clique_arm_is_independent_of_selected_physical_backbone() -> None:
    arrays = list(_arrays(seed=10))
    prior, persistence, target = arrays[:3]
    measurement = arrays[5]
    measurement_visibility = arrays[6]
    measurement_validity = arrays[7]
    ...
    assert all(
        update["selected_backbone"] == "physical_prior" for update in report["updates"]
    )
    assert all(
        update["backbone_pairwise_gates"]["persistence"]["accepted"]
        for update in report["updates"]
    )
    baseline_error = np.linalg.norm(persistence[20] - target[20], axis=1).mean()
    corrected_error = np.linalg.norm(
        trajectories[PERSISTENCE_CLIQUE_RBF_ARM][20] - target[20], axis=1
    ).mean()
    assert corrected_error < baseline_error
    np.testing.assert_array_equal(trajectories[PHYSICAL_CLIQUE_RBF_ARM], prior)
