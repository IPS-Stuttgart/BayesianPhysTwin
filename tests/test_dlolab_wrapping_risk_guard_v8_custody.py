...
SPEC = importlib.util.spec_from_file_location(
    "wrapping_risk_guard_v8_runner",
    ROOT / "scripts/remote/run_dlolab_wrapping_risk_guard_v8.py",
)
...
def test_complete_roster_has_no_padding_or_replacement_slots() -> None:
    assert runner.PREFIX_BATCH_COUNT == 16
    assert runner.WORLD_COUNT == 144
    assert runner.prefix_task(15)["world_indices"] == list(range(135, 144))
    assert runner.prefix_task(15)["native_world_indices"] == list(range(135, 144))
...
def test_source_manifest_reads_only_compact_v4_and_v7_summaries() -> None:
...
def test_attempt_boundary_fields_are_required_by_validator_source() -> None:
    source = (ROOT / "scripts/remote/run_dlolab_wrapping_risk_guard_v8.py").read_text()
    for expression in (
