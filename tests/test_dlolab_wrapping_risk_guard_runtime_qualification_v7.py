...
SPEC = importlib.util.spec_from_file_location(
    "wrapping_runtime_qualification_v7",
...
def test_plan_freezes_native_linux_host_and_runtime() -> None:
    value = runner.plan()
    assert value["host"] == {
        "architecture": "x86_64",
        ...
        "native_linux": True,
        "os_release_sha256": runner.EXPECTED_OS_RELEASE_SHA256,
        "wsl": False,
    }
    assert value["runtime"]["native_library_tree_id"] == (
        runner.EXPECTED_NATIVE_LIBRARY_TREE_ID
    )
...
def test_source_lock_includes_plan_and_complete_failure_lineage() -> None:
    assert (
