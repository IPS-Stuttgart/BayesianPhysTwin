...
def test_v2_preserves_v1_scientific_design_exactly() -> None:
    expected = protocol_v1()
    actual = runner.protocol()
    assert actual.pop("parent_launch_failure")["native_worlds_started"] == 0
    assert actual.pop("runtime_preflight_before_attempt_consumption") is True
    assert actual.pop("external_write_once_attempt_ledger") is True
