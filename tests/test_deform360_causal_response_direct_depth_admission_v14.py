...
def test_v14_source_aggregate_is_order_invariant() -> None:
    left = aggregate_source_sha256(
        "tactile",
        {"right": "2" * 64, "left": "1" * 64},
    )
    right = aggregate_source_sha256(
        "tactile",
        {"left": "1" * 64, "right": "2" * 64},
    )
...
def test_v14_admission_artifacts_round_trip_without_plaintext(
    ...
    loaded = validate_v14_admission_report(root)

    assert loaded == report
    assert "fresh-unit-object" not in json.dumps(report, sort_keys=True)
