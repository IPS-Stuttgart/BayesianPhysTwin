...
def test_learned_twin_backend_is_a_registered_experiment() -> None:
    command = find_command_metadata("materialize-learned-twin-backend")
    assert command is not None
    assert command.status is CommandStatus.EXPERIMENT
    assert command.canonical_command == (
        "bpt experiment run materialize-learned-twin-backend"
    )
    assert command.owner == "learned-twin-backend-v1"
    assert command.optional_dependencies == ()
