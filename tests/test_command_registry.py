...
def test_registry_is_complete_and_unambiguous() -> None:
    ...
    assert len(COMMANDS_BY_LEGACY_ALIAS) == 89
...
def test_genesis_mpm_backend_is_a_registered_experiment() -> None:
    command = find_command_metadata("materialize-genesis-mpm-backend")
    assert command is not None
    assert command.status is CommandStatus.EXPERIMENT
    assert command.canonical_command == (
        "bpt experiment run materialize-genesis-mpm-backend"
    )
    assert command.owner == "genesis-mpm-backend-v1"
    assert command.optional_dependencies == ("genesis-mpm",)
