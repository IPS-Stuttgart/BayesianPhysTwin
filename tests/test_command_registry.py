...
def test_grouped_only_stable_command_does_not_extend_console_scripts() -> None:
    command = COMMANDS_BY_ID["decisive-evidence"]
    assert command.status is CommandStatus.STABLE
    assert command.route == ("evidence", "summarize")
    assert command.legacy_alias is None
    assert "bpt-decisive-evidence" not in _project_scripts()
