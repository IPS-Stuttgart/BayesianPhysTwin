...
def test_crossrepo_binding_is_content_addressed_and_target_closed() -> None:
    ...
    assert binding["schema"] == (
        "bayesian-phystwin.deform360-covariance-only-"
        "crossrepo-preregistration-binding"
    )
...
def test_code_and_paper_protocol_roles_are_explicit_and_fail_closed() -> None:
    ...
    assert paper == {
        ...
        "path": (
            "preregistrations/"
            "deform360_covariance_only_confirmation_v1.json"
        ),
...
def test_binding_matches_the_local_code_protocol_and_frozen_values() -> None:
    ...
    assert frozen["covariance_scales"] == candidate[
        "early_middle_late_covariance_scales"
    ]
    assert frozen["observation_std_m"] == candidate["observation_std_m"]
    assert frozen["development_object_session_count"] == cohort[
        "development_object_session_count"
    ]
    assert frozen["confirmation_object_session_count"] == cohort[
        "target_object_session_count"
    ]
    assert frozen["source_prediction_record_count"] == barrier[
        "source_prediction_seal_count_required"
    ]
    ...
    assert frozen["negative_result_complete"] == decision[
        "negative_or_inconclusive_result_is_complete"
    ]
