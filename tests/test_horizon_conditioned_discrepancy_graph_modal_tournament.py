...
def test_bundle_identity_is_order_invariant_and_binds_candidate_spec() -> None:
    ...
    assert first.candidate.covariance_bytes == sum(
        prediction.source_joint_covariance_m2.nbytes for prediction in first.predictions
    )
...
def test_graph_and_nested_reference_emit_a_valid_matched_tournament() -> None:
    ...
    physical_candidate = CandidateSpec(
        ...
        prediction_artifact_sha256=(graph_bundle.physical_fallback_artifact_sha256),
    )
    payload = {
        ...
        "records": [
            ...
            *[tournament_record_dict(record) for record in reference.candidate_records],
            *[tournament_record_dict(record) for record in graph.candidate_records],
        ],
    }
