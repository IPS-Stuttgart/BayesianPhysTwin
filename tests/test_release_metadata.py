...
def test_citation_matches_installed_distribution() -> None:
    assert _cff_scalar("version") == importlib.metadata.version(DISTRIBUTION_NAME)
...
def test_distribution_uses_canonical_project_urls() -> None:
    package_metadata = importlib.metadata.metadata(DISTRIBUTION_NAME)
...
def test_distribution_contains_pep561_typing_marker() -> None:
    ...
    marker = importlib.resources.files("bayesian_phystwin").joinpath("py.typed")
