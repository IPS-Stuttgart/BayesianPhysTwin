...
def kernel(x, z, length, material_length=None):
    ...
    dim = a.shape[1]
    dot = a @ b.T / dim
    d2 = np.maximum(np.sum(a*a, 1)[:, None]/dim + np.sum(b*b, 1)[None, :]/dim - 2*dot, 0)
...
class SparseGP:
    ...
    def fit(self, x, y):
        ...
        indices = np.random.default_rng(SEED).choice(len(x), min(self.inducing, len(x)), replace=False)
        ...
        phi = solve_triangular(self.lm, kernel(x, self.z, self.length, self.material_length).T, lower=True).T
    ...
    def predict(self, x, variance=False):
        ...
        for offset in range(0, len(x), 1024):
            ...
            phi = solve_triangular(self.lm, kernel(batch, self.z, self.length, self.material_length).T, lower=True).T
            means.append(phi @ self.weights)
            if variance:
                p = solve_triangular(self.la, phi.T, lower=True)
                latent = kernel_diag(batch) - np.sum(phi*phi, axis=1) + np.sum(p*p, axis=0)
...
class ResidualGP:
    ...
    def _inputs(self, features):
        ...
        material = np.broadcast_to(np.linspace(0, 1, nodes)[None, None, :, None], (n, t, nodes, 1))
    ...
    def predict(self, features, variance=False):
        ...
        for j, (model, sy) in enumerate(zip(self.models, self.y_scales)):
            xx = x.reshape(-1, x.shape[-1]) if self.shared else x[:, :, j].reshape(-1, x.shape[-1])
...
def group_rows(data, indices):
    ...
    result = {k: np.stack([data[k][rows[0]] for rows in groups.values()]) for k in ('initial', 'action', 'baseline')}
    result['target'] = np.stack([data['target'][rows].mean(axis=0) for rows in groups.values()])
    result['names'] = ['|'.join(str(data['names'][i]) for i in rows) for rows in groups.values()]
...
def subset(grouped, indices):
    return {k: value[indices] for k, value in grouped.items() if isinstance(value, np.ndarray)}
...
def metrics(prediction, target, baseline, names, variance=None):
    ...
    result = dict(l1_mm=float(case.mean()), rmse_mm=float(np.sqrt(np.mean(error**2))*1000),
                  wins_vs_hybrid=int(np.sum(case < base-1e-9)), worst_ratio_vs_hybrid=float(np.max(case/base)),
                  per_trajectory_l1_mm=dict(zip(names, case.tolist())))
    cuts = np.array_split(np.arange(error.shape[1]), 3)
    result['early_middle_late_l1_mm'] = [float(np.mean(np.abs(error[:, cut]))*1000) for cut in cuts]
    if variance is not None:
        e = error[:, :, 2:-2]
        v = np.maximum(variance, 1e-12)
        result.update(coordinate_coverage90=float(np.mean(np.abs(e) <= 1.6448536269514722*np.sqrt(v))),
                      normalized_coordinate_nees=float(np.mean(e*e/v)),
                      mean_full_width90_mm=float(2*1.6448536269514722*np.mean(np.sqrt(v))*1000),
                      coordinate_gaussian_nll=float(np.mean(0.5*(np.log(2*np.pi*v) + e*e/v))))
    return result
...
def paired_difference(left, right):
    ...
    return dict(mean_delta_mm=float(delta.mean()), bootstrap95_mm=np.quantile(boots, [0.025, 0.975]).tolist(),
                wins=int(np.sum(delta < -1e-9)), ties=int(np.sum(np.abs(delta) <= 1e-9)), n=len(delta))
...
def main():
    ...
    from bayesian_phystwin_experiments.deform_dlo_local_residual import (
        build_deform_local_residual_features, fit_deform_local_residual, predict_deform_local_residual)

    manifest_path = args.data.with_name('export.json')
    export = json.loads(manifest_path.read_text())
    if export['npz_sha256'] != checksum(args.data) or export['official_eval_read'] or export['source_test_read']:
    ...
    if set(train['keys']) & set(validation['keys']):
        raise ValueError('Exact causal-query overlap between fit and development-validation')
    fit_x, fit_frames = build_deform_local_residual_features(train['initial'], train['action'], train['baseline'])
    val_x, val_frames = build_deform_local_residual_features(validation['initial'], validation['action'], validation['baseline'])
    fit_y = np.einsum('ntvi,nij->ntvj', (train['target']-train['baseline'])[:, :, 2:-2], fit_frames)
    ...
    selection = dict(scope='development-only; reused historical validation, not new confirmation', seed=SEED,
                     original_fit_count=40, original_validation_count=8,
                     fit_clusters=len(fit_x), validation_clusters=len(val_x),
                     fit_cluster_sizes=train['cluster_sizes'], validation_cluster_sizes=validation['cluster_sizes'],
                     inner_train_names=[train['names'][i] for i in learn], inner_tune_names=[train['names'][i] for i in tune],
                     time_samples=TIME_SAMPLES, total_inducing_budget=TOTAL_INDUCING, gp_grid=GP_GRID,
                     shrinkages=SHRINKAGES, all_candidates=[], selected={})

    def ridge_fit(grouped, names, ridge):
        return fit_deform_local_residual(grouped['initial'], grouped['action'], grouped['baseline'], grouped['target'],
                                         names, ridge=ridge, variance_floor_m2=1e-6)

    def ridge_output(model, grouped, shrinkage):
        p = predict_deform_local_residual(model, grouped['initial'], grouped['action'], grouped['baseline'], shrinkage=shrinkage)
    ...
    for ridge in (0.1, 1.0, 10.0):
        ...
        for shrinkage in SHRINKAGES:
            ...
            ridge_candidates.append(dict(family='ridge', ridge=ridge, shrinkage=shrinkage, inner_l1_mm=loss))
    selection['all_candidates'].extend(ridge_candidates)
    selection['selected']['ridge_tuned'] = min(ridge_candidates, key=lambda r: r['inner_l1_mm'])
    selection['selected']['ridge_existing'] = dict(family='ridge', ridge=1.0, shrinkage=0.25)
    for shared in (False, True):
        ...
        for length, noise in GP_GRID:
            ...
            for shrinkage in SHRINKAGES:
                ...
                loss = float(np.mean(np.abs(pred-tune_data['target']))*1000)
                bank.append(dict(family=family, length=length, noise=noise, shrinkage=shrinkage, inner_l1_mm=loss))
            print('TUNING', family, length, noise, 'best-so-far', min(b['inner_l1_mm'] for b in bank), flush=True)
    ...
    print('FROZEN_SELECTION', json.dumps(selection['selected'], sort_keys=True), flush=True)
    ...
    for name, config in selection['selected'].items():
        ...
        if config['family'] == 'ridge':
            ...
            inner_model = ResidualGP(shared, config['length'], config['noise']).fit(fit_x[learn], fit_y[learn])
            inner_pred, inner_var = gp_output(inner_model, fit_x[tune], tune_frames, tune_data['baseline'], shrinkage)
            full_model = ResidualGP(shared, config['length'], config['noise']).fit(fit_x, fit_y)
            pred, var = gp_output(full_model, val_x, val_frames, validation['baseline'], shrinkage)
        if not np.array_equal(pred[:, :, [0, 1, -2, -1]], validation['baseline'][:, :, [0, 1, -2, -1]]):
            raise AssertionError('Clamped boundary was changed')
        scale = float(np.mean((inner_pred-tune_data['target'])[:, :, 2:-2]**2/np.maximum(inner_var, 1e-12)))
        calibrations[name] = max(scale, 1e-6)
        predictions[name], variances[name] = pred, var
        print('FITTED', name, 'inner-only covariance scale', calibrations[name], flush=True)
    np.savez_compressed(out/'predictions.npz', **predictions, **{k+'_variance': v for k, v in variances.items()})
    seal = dict(selection_sha256=checksum(out/'selection.json'), predictions_sha256=checksum(out/'predictions.npz'),
                input_sha256=checksum(args.data), calibration_scales=calibrations,
                validation_targets_used_for_residual_selection_or_fit=False,
                baseline_previously_selected_on_validation=True)
    (out/'prediction_seal.json').write_text(json.dumps(seal, indent=2, sort_keys=True)+'\n')
    records = {}
    for name, pred in predictions.items():
        var = variances.get(name)
        records[name] = metrics(pred, validation['target'], validation['baseline'], validation['names'], var)
        if var is not None:
            records[name]['inner_scaled_uncertainty'] = metrics(pred, validation['target'], validation['baseline'], validation['names'], var*calibrations[name])
    pairs = {}
    for gp in ('gp_independent', 'gp_material'):
        for base in ('hybrid', 'ridge_existing', 'ridge_tuned'):
            pairs[gp+'-minus-'+base] = paired_difference(list(records[gp]['per_trajectory_l1_mm'].values()), list(records[base]['per_trajectory_l1_mm'].values()))
    result = dict(schema='material-gp-development-result-v1', scope=selection['scope'],
                  implementation_sha=os.environ.get('GITHUB_SHA', 'local'), script_sha256=checksum(Path(__file__)),
                  run_id=os.environ.get('GITHUB_RUN_ID'), export=export, selected=selection['selected'],
                  fit_clusters=len(fit_x), validation_clusters=len(val_x), results=records, paired=pairs,
                  calibration_scales=calibrations, official_eval_read=False, source_test_read=False,
                  elapsed_seconds=time.perf_counter()-started,
                  limitations=['historical validation already used for baseline selection',
