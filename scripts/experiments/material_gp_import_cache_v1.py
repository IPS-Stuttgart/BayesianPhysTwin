...
TRAINING_ROOT = Path('/home/florianpfaff/source-only/deform-dlo2-local-residual-v5/train-8cc85de7/training_run')
...
def main():
    ...
    for split, count in [('fit', 40), ('validation', 8)]:
        ...
        if (receipt['checkpoint_sha256'] != CHECKPOINT or receipt['manifest_sha256'] != MANIFEST
                or receipt['sha256'] != sha(path) or receipt['official_eval_read'] is not False):
            raise ValueError('Sibling export receipt failed verification')
        with np.load(path, allow_pickle=False) as archive:
            data = {key: archive[key].copy() for key in
                    ('initial', 'action', 'predictions', 'targets', 'persistence', 'names')}
        names = data['names'].tolist()
        if names != manifest['split'][split] or len(names) != count or receipt['names'] != names:
        ...
        if data['predictions'].shape != data['targets'].shape or data['predictions'].shape != (count, 498, 12, 3):
            raise ValueError('Unexpected cached forecast shape')
        if not all(np.isfinite(value).all() for key, value in data.items() if key != 'names'):
            raise ValueError('Nonfinite cached numeric array')
        loss = float(np.mean(np.abs(data['predictions'] - data['targets']), dtype=np.float64))
        ...
        receipts[split] = dict(receipt=receipt, receipt_sha256=sha(receipt_path), path=str(path))
        arrays.append(data)
    combined = {key: np.concatenate([part[key] for part in arrays]) for key in arrays[0]}
    np.savez_compressed(out / 'development.npz', initial=combined['initial'], action=combined['action'],
                        baseline=combined['predictions'], target=combined['targets'],
                        persistence=combined['persistence'], names=combined['names'], fit_count=np.asarray([40]))
    meta = dict(schema='material-gp-development-cache-import-v1',
                scope='retrospective development only; imported identical permitted fit/validation partitions',
                npz_sha256=sha(out / 'development.npz'), checkpoint_sha256=CHECKPOINT,
                manifest_sha256=MANIFEST, source_test_read=False, official_eval_read=False,
                import_sha256=sha(__file__), implementation_sha=os.environ.get('GITHUB_SHA'),
                ...
                import_only_no_raw_trajectory_reads=True, upstream_receipts=receipts,
                baseline_validation_l1_m=loss,
                baseline_reference_l1_m=0.007912029745057225,
                fit_names=arrays[0]['names'].tolist(), validation_names=arrays[1]['names'].tolist())
