...
def main() -> int:
    ...
    from bayesian_phystwin_experiments.deform_dlo_local_residual import load_deform_dlo2_local_residual_protocol
    ...
    print('Loading 40 fit + 8 development-validation trajectories; no source-test/eval', flush=True)
    ...
    np.savez_compressed(out / 'development.npz', initial=initial, action=action,
                        baseline=prediction, target=target, names=np.asarray(names),
                        fit_count=np.asarray([40]), persistence=np.asarray(rollout['persistence']))
    meta = dict(schema='material-gp-development-export-v1',
                scope='retrospective development only; no independent confirmation',
                npz_sha256=sha(out / 'development.npz'), checkpoint_sha256=sha(checkpoint),
                training_sha256=sha(training_path), manifest_sha256=sha(manifest_path),
                implementation_sha=os.environ.get('GITHUB_SHA', 'local'),
                exporter_sha256=sha(Path(__file__)),
                runtime={k: sha(ROOT / 'scripts/remote' / k) for k in (
                    'run_deform_dlo2_local_residual.py', 'run_deform_dlo_longrun_posterior.py',
                    'run_deform_dlo_local_residual.py', 'run_deform_dlo_source.py')},
                python=sys.version, numpy=np.__version__, torch=torch.__version__,
                fit_names=fit_names, validation_names=validation_names,
                trajectory_reads=sorted(reads), official_eval_read=False,
                source_test_read=False, baseline_validation_l1_m=observed,
