...
SOURCE_ONLY_STATUS = (
    "source-only-lock; no target roster, payload, prediction, outcome, or claim "
    "authorization"
)
TARGET_CUSTODY_OWNER = "separately reviewed target protocol"
...
def load_locked_policy(
    ...
    """Load the source-only dry-run lock and construct its exact policy."""
    ...
    _require(
        protocol.get("status") == SOURCE_ONLY_STATUS,
        "dry-run protocol is not source-only",
    )
    ...
    _require(
        "target_binding" not in protocol,
        "source-only dry-run protocol must not bind a target roster",
    )
    target = _mapping(protocol.get("target_boundary"), name="target_boundary")
    _require(
        target.get("roster_and_custody_owned_by") == TARGET_CUSTODY_OWNER,
        "target custody ownership changed",
    )
    for key in (
        "target_roster_identity_bound_here",
        "target_payload_access_authorized",
        "target_prediction_authorized",
        "target_outcome_access_authorized",
    ):
        _require(target.get(key) is False, f"target boundary changed: {key}")
    _require(
    ...
    for key in (
        ...
        "fresh_target_roster_identity_bound_by_this_protocol",
    ...
    *,
    ...
    result = run_source_only_residual_history_dry_run(
        ...
        metadata={
            ...
            "source_only_lock_sha256": protocol["lock_sha256"],
        },
    )
