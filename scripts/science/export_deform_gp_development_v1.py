...
def main() -> int:
    ...
    common._verify_identity(
        args.longrun_result, protocol["longrun_result"], label="parent longrun result"
    )
    common._verify_identity(
        args.source_manifest, protocol["source_manifest"], label="DLO1 source manifest"
    )
    ...
    if len(validation) < 4:
        raise ValueError(
            "not enough validation recordings for separate selection and calibration"
        )
    ...
    preflight = {
        ...
        "roles": dict(
            zip(("fit", "select", "calibrate", "score"), groups, strict=True)
        ),
    ...
    rollout = common._rollout(
        state, trajectories, modules=modules, torch=torch, device=args.device
    )
    ...
    initial, action = deform_causal_inputs(
        common._stack_trajectories(trajectories, names)
    )
    fit_count, select_count = len(fit_names), len(select_names)
    fit_slice, select_slice = (
        slice(0, fit_count),
        slice(fit_count, fit_count + select_count),
    )
    choices = []
    for ridge in protocol["candidate_bank"]["ridges"]:
        fitted = fit_deform_local_residual(
            ...
            variance_floor_m2=float(
                protocol["posterior"]["coordinate_variance_floor_m2"]
            ),
        )
        for shrinkage in protocol["candidate_bank"]["shrinkages"]:
            candidate = predict_deform_local_residual(
                fitted,
                initial[select_slice],
                action[select_slice],
                baseline[select_slice],
                shrinkage=float(shrinkage),
            )["predictions"]
            choices.append(
                (
                    float(np.mean(np.abs(candidate - truth[select_slice]))),
                    float(ridge),
                    float(shrinkage),
                    fitted,
                )
    ...
    ridge_predictions = predict_deform_local_residual(
        best[3], initial, action, baseline, shrinkage=best[2]
    )["predictions"]
    ...
    role_labels = np.concatenate(
        [
            np.repeat(role, len(group))
            for role, group in zip(
                ("fit", "select", "calibrate", "score"), groups, strict=True
            )
        ]
    )
    ...
    np.savez_compressed(
        archive,
        features=features[:, times].reshape(len(names), len(times), -1),
        times=times / (baseline.shape[1] - 1),
        ids=np.asarray(names),
        split=role_labels,
        baseline=baseline[:, times],
        ridge=ridge_predictions[:, times],
        truth=truth[:, times],
    ...
    metadata = {
        **preflight,
        "schema": "gp-discrepancy-development-archive-v1",
        "archive_sha256": sha256_file(archive),
        "forecast_time_indices": times.tolist(),
        "native_ridge_selection": {
            "ridge": best[1],
            "shrinkage": best[2],
            "selection_l1_m": best[0],
        },
        "native_ridge_candidates": [
            {"selection_l1_m": item[0], "ridge": item[1], "shrinkage": item[2]}
            for item in choices
        ],
        "python": sys.version,
