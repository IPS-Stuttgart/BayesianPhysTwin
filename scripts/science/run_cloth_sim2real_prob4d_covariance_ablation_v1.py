...
METHODS: Final = {name: "cloth-prob4d-" + name.replace("_", "-") for name in TREATMENTS}
PARAMETERS: Final = {
...
def fit_variant(
    ...
    *,
    ...
    reference = float(
        np.median(statistics.local_mean_variance_m2 + config.shared_bias_std_m**2)
    )
    ...
    best_maximum = float(best["maximum"])
    best_win_fraction = float(best["win_fraction"])
    baseline_mean = float(np.mean(baseline_scores))
    improvement = (baseline_mean - best_mean) / max(baseline_mean, 1e-15)
    worst_ratio = best_maximum / max(float(np.max(baseline_scores)), 1e-15)
    accepted = bool(
        best_name != "baseline"
        and improvement >= config.minimum_validation_improvement
        and best_win_fraction >= config.minimum_validation_win_fraction
    ...
    if not accepted:
    ...
    elif selected.startswith("graph_l"):
        parts = selected.split("_")
        strength = float(parts[1][1:])
        scale = float(parts[2][1:])
        _, graph_variance, solve = _posterior_solution(
            information,
            rhs,
            laplacian_square,
            reference_variance_m2=reference,
        ...
        if graph_variance is None:
            raise AssertionError("graph covariance was not returned")
        variance = scale**2 * graph_variance
    ...
    return {
        ...
        "risk_score": float(best_mean / max(baseline_mean, 1e-15)),
        "validation_improvement": float(improvement),
        "validation_win_fraction": best_win_fraction,
...
def _seal_case(
    ...
    *,
    ...
    legacy_seal_path, _legacy_result_path = _legacy_case(
        legacy_root,
        split,
        case_id,
    )
    ...
    common = {
        ...
        "residual_mean_rows_sha256": _array_hash(statistics.residual_mean_rows_m),
    ...
    *,
    ...
    for treatment, prediction in predictions.items():
        ...
        for horizon, (base, estimate, observation) in enumerate(
            ...
            physical_symmetric.append(api["symmetric_l1_chamfer_m"](base, observation))
            candidate_symmetric.append(
                api["symmetric_l1_chamfer_m"](estimate, observation)
            )
            physical_directed.append(api["directed_l1_chamfer_m"](base, observation))
            ...
            reported_coverage.append(float(np.mean(np.abs(residual) <= reported_half)))
        ...
        metrics = {
            "physical_symmetric_l1_chamfer_m": float(np.mean(physical_symmetric_array)),
            ...
            "physical_directed_l1_chamfer_m": float(np.mean(physical_directed_array)),
            "candidate_directed_l1_chamfer_m": float(np.mean(candidate_directed_array)),
            ...
            "reported_90_coordinate_coverage": float(np.mean(reported_coverage)),
...
def _task_summary(rows: Sequence[Mapping[str, Any]]) -> Mapping[str, object]:
    ...
    return {
        ...
        "physical_symmetric_l1_chamfer_m": mean("physical_symmetric_l1_chamfer_m"),
        "candidate_symmetric_l1_chamfer_m": mean("candidate_symmetric_l1_chamfer_m"),
        "object_balanced_symmetric_relative_improvement": float(np.mean(improvements)),
        ...
        "reported_90_coordinate_coverage": mean("reported_90_coordinate_coverage"),
...
def run(args: argparse.Namespace) -> int:
    ...
    cases = [
        case for case in manifest["cases"] if case["split"] in {"calibration", "target"}
    ]
    _require(len(cases) == 12, "expected twelve calibration/target cases")
    common: dict[str, Mapping[str, object]] = {}
    sealed: dict[str, dict[str, Mapping[str, Mapping[str, Any]]]] = {
        "calibration": {},
        "target": {},
    }
    future: dict[str, dict[str, list[np.ndarray]]] = {
        "calibration": {},
        "target": {},
    }
    ...
    calibration: dict[str, Mapping[str, Mapping[str, Any]]] = {}
    ...
    for treatment in TREATMENTS:
        requirements = [
            max(
                1.0,
                rows[treatment]["metrics"]["trial_coordinate_abs_standardized_q90"]
                / NORMAL_90,
            )
            for rows in calibration.values()
        ]
        multipliers[treatment] = float(max(requirements))
    target: dict[str, Mapping[str, Mapping[str, Any]]] = {}
    for case in sorted(cases, key=lambda row: row["case_id"]):
    ...
    for treatment in TREATMENTS:
        ...
        covariance = {
            case_id: _read_json(
                output / "target" / _safe(case_id) / treatment / "prediction_seal.json"
            )["covariance_artifact_sha256"]
            for case_id in target_ids
        }
        covariance_hash = _json_hash({"policy": asdict(policy), "cases": covariance})
        run_manifest = {
            "ablation_id": ABLATION_ID,
            "treatment": treatment,
        ...
        summaries[treatment] = {
            ...
            "quasi_static_secondary": _task_summary(
                [row for row in rows if row["case_id"].endswith("/quasi_static")]
            ),
