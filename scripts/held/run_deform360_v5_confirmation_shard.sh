...
if [[ "${BPT_HELD_V5_CONFIRMATION_SHARD_ENV_NORMALIZED:-}" != "1" ]]; then
  exec env -i \
    ...
    BPT_HELD_V5_CONFIRMATION_SHARD_ENV_NORMALIZED=1 \
    /bin/bash "$0" "$@"
fi
...
while IFS='=' read -r environment_name _value; do
  case "$environment_name" in
    ...
    BPT_HELD_V5_CODE|BPT_HELD_V5_CONFIRMATION_SHARD_ENV_NORMALIZED)
      ;;
...
if [[ "$#" -lt 1 || "$#" -gt 2 ]]; then
  die "usage: run_deform360_v5_confirmation_shard.sh SHARD_INDEX [CUDA_DEVICE]"
fi
...
readonly CASE_RUNNER="$SCRIPT_DIR/run_deform360_v5_confirmation_case.sh"
...
readonly OUTCOME_DRIVER="$SCRIPT_DIR/run_deform360_v5_confirmation_outcomes.py"
...
[[ -f "$CONFIRMATION_LOCK" && ! -L "$CONFIRMATION_LOCK" ]] || die "held-v5 confirmation lock is absent or a symlink"
[[ "$(readlink -f -- "$CONFIRMATION_LOCK")" == "$CONFIRMATION_LOCK" ]] || \
  die "held-v5 confirmation lock is not canonical"
[[ "$(stat -c '%a' -- "$CONFIRMATION_LOCK")" == "400" ]] || die "held-v5 confirmation lock mode is not 0400"
...
# Mandatory, non-mutating verification recomputes all 112 bindings and the
...
if ! env -i \
  ...
  PYTHONHASHSEED=0 \
  PYTHONPYCACHEPREFIX="$PYCACHE_PREFIX" \
  "$PY" -B -X "pycache_prefix=$PYCACHE_PREFIX" -I - "$BPT_HELD_V5_CODE/src" "$CONFIRMATION_LOCK" "$CALIBRATION_LOCK" "$CALIBRATION_DECISION" \
  >>"$VERIFY_LOG" 2>&1 <<'PY'
import json
...
readonly BPT_HELD_V5_CONFIRMATION_LOCK_VERIFIED_SHA256="$(sha256sum -- "$CONFIRMATION_LOCK" | awk '{print $1}')"
[[ "$BPT_HELD_V5_CONFIRMATION_LOCK_VERIFIED_SHA256" =~ ^[0-9a-f]{64}$ ]] || die "invalid verified confirmation-lock digest"
export BPT_HELD_V5_CODE BPT_HELD_V5_CONFIRMATION_LOCK_VERIFIED_SHA256
...
echo "SHARD_START shard=$SHARD_INDEX cuda_device=$CUDA_DEVICE case_count=${#CASE_SPECS[@]} lock_sha256=$BPT_HELD_V5_CONFIRMATION_LOCK_VERIFIED_SHA256"
for spec in "${CASE_SPECS[@]}"; do
  IFS=: read -r case_name object_id episode_id <<<"$spec"
  env -i \
    HOME=/home/florianpfaff \
