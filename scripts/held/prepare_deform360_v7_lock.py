#!/usr/bin/env python3
"""Create the prospective Deform360 held-v7 lock from audited lineage.
...
v7 contracts, preserves unchanged external identities from v1, binds the exact
failed-closed v4, v5, and v6 execution reports, and admits only the dedicated
frozen Python runtime plus its sealed v7 CUDA supplement.
...
EXPECTED_V6_LOCK_FILE_SHA256 = (
    "0436d6467d14e1caa42a0895c5cede13e97d96bd9c01fdea6b40557d6f539f8c"
)
EXPECTED_V6_LOCK_ARTIFACT_SHA256 = (
    "b334f7fcb28753ab4526465e5ca6e69397fa4cc65d1db6b8d9a68c090c6bb3db"
)
EXPECTED_V6_LOCK_BINDING_COUNT = 113
EXPECTED_V6_REPORT_FILE_SHA256 = (
    "8a428535708057ff1c944b8ab81c93b3309539ae9d3dffb469ddc2b9f79de504"
)
EXPECTED_V6_REPORT_ARTIFACT_SHA256 = (
    "383d2d72ba148703482df76cdbf89ad8d43c6a5026b89325984a5d786748c843"
)
EXPECTED_V6_REPORT_SIZE_BYTES = 16_780
EXPECTED_V7_BINDING_COUNT = 118
EXPECTED_V7_MIGRATION_KEY_COUNT = 27
...
_CANONICAL_HELD_V7_ROOT = _CANONICAL_HELD_BASE / "held-v7"
...
_CANONICAL_V6_OUTCOME_WITHDRAWAL_REPORT = (
    _CANONICAL_HELD_V6_ROOT / "v6-outcome-withdrawal-report.json"
)
_CANONICAL_V7_LOCK = _CANONICAL_HELD_V7_ROOT / "calibration-lock.json"
_CANONICAL_GSPLAT_RUNTIME_SUPPLEMENT_MANIFEST = Path(
...
_CANONICAL_GSPLAT_RUNTIME_SMOKE_EVIDENCE = (
    _CANONICAL_HELD_V7_ROOT / "gsplat-runtime-smoke-evidence.json"
)
_CANONICAL_GSPLAT_CUDA_EXTENSION = (
    _CANONICAL_GSPLAT_RUNTIME_SUPPLEMENT_MANIFEST.parent / "gsplat_cuda.so"
)
_EXPECTED_GSPLAT_CUDA_EXTENSION_SHA256 = (
    "2dd5e0c2a349619e1afc3dd041086eca900b387602bc76627b7f54264fffec64"
)
_EXPECTED_GSPLAT_CUDA_EXTENSION_SIZE_BYTES = 6_982_312
...
LOCAL_FILE_BINDINGS: Mapping[str, str] = {
    ...
    "held_calibration_case_runner_source": (
        "scripts/held/run_deform360_v7_calibration_case.sh"
    ),
    "held_calibration_outcome_driver_source": (
        "scripts/held/run_deform360_v7_calibration_outcomes.py"
    ),
    "held_calibration_shard_runner_source": (
        "scripts/held/run_deform360_v7_calibration_shard.sh"
    ),
    "held_confirmation_case_runner_source": (
        "scripts/held/run_deform360_v7_confirmation_case.sh"
    ),
    "held_confirmation_outcome_driver_source": (
        "scripts/held/run_deform360_v7_confirmation_outcomes.py"
    ),
    "held_confirmation_shard_runner_source": (
        "scripts/held/run_deform360_v7_confirmation_shard.sh"
    ),
    ...
    "held_protocol_lock_operator_source": ("scripts/held/prepare_deform360_v7_lock.py"),
...
# These are the only v1 values that v7 may inherit.  They identify runtimes,
...
# These v7 external identities are not inherited from the v1 lock.  They bind
...
V7_PINNED_EXTERNAL_BINDING_VALUES: Mapping[str, str] = {
...
V7_PINNED_EXTERNAL_BINDING_KEYS = frozenset(V7_PINNED_EXTERNAL_BINDING_VALUES)
V7_PINNED_SEMANTIC_GATE_CONTRACT_SHA256 = (
...
def _require_deployed_semantic_binding_exports(
    *,
    ...
    _require(
        {
            key: V7_PINNED_EXTERNAL_BINDING_VALUES[key]
    ...
    _require(
        V7_PINNED_SEMANTIC_GATE_CONTRACT_SHA256 == semantic_gate_contract_sha256,
...
V7_ONLY_BINDING_KEYS = frozenset(
    {
        "deform360_dataset_containment_source",
        "deform360_robot_kinematics_source",
        "frame_zero_semantic_gate_contract",
...
def _validate_v6_lock(lock_path: Path) -> dict[str, Any]:
    path = _absolute(lock_path)
    _require(path == _CANONICAL_V6_LOCK, "v6 lock path changed")
    _require(path.resolve() == path, "v6 lock path is not canonical")
    _require_exact_mode_0400(path, "v6 calibration lock")
    lock, file_sha256 = _load_json(path, "v6 calibration lock")
    _require(
        file_sha256 == EXPECTED_V6_LOCK_FILE_SHA256,
        "v6 calibration-lock file checksum changed",
    )
    _require(
        lock.get("artifact_sha256") == EXPECTED_V6_LOCK_ARTIFACT_SHA256
        and _artifact_sha256(lock) == EXPECTED_V6_LOCK_ARTIFACT_SHA256,
        "v6 calibration-lock semantic checksum changed",
    )
    _require(
        lock.get("protocol_id") == "deform360-held-online-belief-v6"
        and isinstance(lock.get("immutable_bindings"), Mapping)
        and len(lock["immutable_bindings"]) == EXPECTED_V6_LOCK_BINDING_COUNT,
        "v6 calibration-lock identity or binding count changed",
    )
    return lock


def _validate_v6_outcome_withdrawal_report(report_path: Path) -> str:
    """Bind the exact metadata-only terminal v6 withdrawal report."""

    path = _absolute(report_path)
    _require(
        path == _CANONICAL_V6_OUTCOME_WITHDRAWAL_REPORT,
        "v6 outcome-withdrawal report path changed",
    )
    _require(path.resolve() == path, "v6 withdrawal-report path is not canonical")
    _require_exact_mode_0400(path, "v6 outcome-withdrawal report")
    payload = _read_regular_bytes(path, "v6 outcome-withdrawal report")
    _require(
        len(payload) == EXPECTED_V6_REPORT_SIZE_BYTES
        and hashlib.sha256(payload).hexdigest() == EXPECTED_V6_REPORT_FILE_SHA256,
        "v6 outcome-withdrawal report file checksum changed",
    )
    report, file_sha256 = _load_json(path, "v6 outcome-withdrawal report")
    canonical = (
        json.dumps(
            report,
            indent=2,
    ...
    _require(payload == canonical, "v6 withdrawal report is not canonical JSON")
    _require(
        report.get("artifact_sha256") == EXPECTED_V6_REPORT_ARTIFACT_SHA256
        and _artifact_sha256(report) == EXPECTED_V6_REPORT_ARTIFACT_SHA256,
        "v6 outcome-withdrawal report semantic checksum changed",
    )
    _require(
        report.get("artifact_kind") == "Deform360HeldProtocolExecutionWithdrawalReport"
        and report.get("schema_version") == 1
        and report.get("protocol_id") == "deform360-held-online-belief-v6"
        and report.get("replacement_protocol_id") == "deform360-held-online-belief-v7"
        and report.get("disposition")
        == "WITHDRAWN_DURING_FIRST_TARGET_OPERATION_BEFORE_ANY_COMPLETED_OUTCOME",
        "v6 outcome-withdrawal report identity or disposition changed",
    )
    cause = report.get("cause")
    _require(
        isinstance(cause, Mapping)
        and cause.get("classification") == "GSPLAT_CUDA_BACKEND_UNAVAILABLE"
        and cause.get("failed_case") == "002-rope-silk-ep0003",
        "v6 outcome-withdrawal cause changed",
    )
    counts = report.get("execution_counts")
    _require(
        isinstance(counts, Mapping)
        and counts.get("calibration_case_execution_count") == 15
        and counts.get("formal_online_prediction_count") == 15
        and counts.get("formal_physical_prediction_count") == 15
        and counts.get("sam2_camera_propagation_completed_count") == 8
        and counts.get("sam2_mask_archive_count") == 8
        and counts.get("target_operation_started_count") == 1
        and counts.get("target_operation_completed_count") == 0
        and counts.get("target_reconstruction_artifact_count") == 0
        and counts.get("calibration_score_evidence_count") == 0
        and counts.get("calibration_decision_count") == 0,
        "v6 outcome-withdrawal execution census changed",
    )
    _require(
        report.get("reuse")
        == {
        ...
        "v6-to-v7 no-reuse declaration changed",
    )
    _require(
        ...
        and os.lstat(_CANONICAL_HELD_V6_ROOT).st_mode & 0o222 == 0,
        "sealed held-v6 root is absent, aliased, or writable",
    )
    return file_sha256


def _validate_runtime_supplement_manifest(
    manifest_path: Path,
    *,
    expected_extension_contract: Mapping[str, Any],
    expected_extension_contract_sha256: str,
) -> tuple[str, dict[str, Any]]:
    path = _absolute(manifest_path)
    _require(
        path == _CANONICAL_GSPLAT_RUNTIME_SUPPLEMENT_MANIFEST,
        "gsplat runtime supplement manifest path changed",
    )
    _require(path.resolve() == path, "gsplat supplement manifest is non-canonical")
    _require_exact_mode_0400(path, "gsplat runtime supplement manifest")
    manifest, file_sha256 = _load_json(path, "gsplat runtime supplement manifest")
    payload = _read_regular_bytes(path, "gsplat runtime supplement manifest")
    canonical = (
        json.dumps(manifest, indent=2, sort_keys=True, allow_nan=False) + "\n"
    ).encode("utf-8")
    _require(payload == canonical, "gsplat supplement manifest is not canonical JSON")
    _require(
        set(manifest)
        == {
            "artifact_kind",
        ...
        and manifest.get("extension_contract") == dict(expected_extension_contract)
        and manifest.get("extension_contract_sha256")
        == expected_extension_contract_sha256
        and manifest.get("artifact_sha256") == _artifact_sha256(manifest),
        "gsplat supplement manifest identity or semantic checksum changed",
    )
    supplement_root = path.parent
    _require(
        supplement_root.is_dir()
        and not supplement_root.is_symlink()
        and supplement_root.resolve() == supplement_root
        and stat.S_IMODE(os.lstat(supplement_root).st_mode) == 0o555,
        "gsplat runtime supplement root is absent, linked, or writable",
    )
    _require(
        _CANONICAL_GSPLAT_CUDA_EXTENSION.is_file()
        and not _CANONICAL_GSPLAT_CUDA_EXTENSION.is_symlink()
        and _CANONICAL_GSPLAT_CUDA_EXTENSION.resolve()
        == _CANONICAL_GSPLAT_CUDA_EXTENSION
        and stat.S_IMODE(os.lstat(_CANONICAL_GSPLAT_CUDA_EXTENSION).st_mode) == 0o444
        and os.lstat(_CANONICAL_GSPLAT_CUDA_EXTENSION).st_size
        == _EXPECTED_GSPLAT_CUDA_EXTENSION_SIZE_BYTES
        and _sha256_file(
            _CANONICAL_GSPLAT_CUDA_EXTENSION, "frozen gsplat CUDA extension"
        )
        == _EXPECTED_GSPLAT_CUDA_EXTENSION_SHA256,
        "frozen gsplat CUDA extension identity changed",
    )
    return file_sha256, manifest


def _validate_runtime_smoke_evidence(
    evidence_path: Path,
    *,
    expected_contract_sha256: str,
    supplement_manifest_sha256: str,
    deployed_method_head: str,
    operator_source_sha256: str,
) -> tuple[str, dict[str, Any]]:
    path = _absolute(evidence_path)
    _require(
        path == _CANONICAL_GSPLAT_RUNTIME_SMOKE_EVIDENCE,
        "gsplat runtime smoke evidence path changed",
    )
    _require(path.resolve() == path, "gsplat smoke evidence is non-canonical")
    _require_exact_mode_0400(path, "gsplat runtime smoke evidence")
    evidence, file_sha256 = _load_json(path, "gsplat runtime smoke evidence")
    payload = _read_regular_bytes(path, "gsplat runtime smoke evidence")
    canonical = (
        json.dumps(evidence, indent=2, sort_keys=True, allow_nan=False) + "\n"
    ).encode("utf-8")
    _require(payload == canonical, "gsplat smoke evidence is not canonical JSON")
    _require(
        set(evidence)
        == {
            "artifact_kind",
            "artifact_sha256",
            "contract_sha256",
            "deployed_method_head",
            "operator_source_sha256",
            "runtime_supplement_manifest_sha256",
            "schema_version",
            "smokes",
        }
        and evidence.get("artifact_kind") == "Deform360HeldGsplatRuntimeSmokeEvidenceV1"
        and evidence.get("schema_version") == 1
        and evidence.get("contract_sha256") == expected_contract_sha256
        and evidence.get("deployed_method_head") == deployed_method_head
        and evidence.get("operator_source_sha256") == operator_source_sha256
        and evidence.get("runtime_supplement_manifest_sha256")
        == supplement_manifest_sha256
        and evidence.get("artifact_sha256") == _artifact_sha256(evidence),
        "gsplat runtime smoke evidence identity or checksum changed",
    )
    ...
    _require(
        isinstance(smokes, list)
        and len(smokes) == 2
        and all(isinstance(smoke, Mapping) for smoke in smokes)
        and [smoke.get("physical_gpu_index") for smoke in smokes] == [0, 1]
        and all(
            set(smoke) == expected_smoke_keys
            and smoke.get("schema_version") == 1
            and smoke.get("artifact_kind") == "Deform360HeldGsplatRuntimeSmokeV1"
            and smoke.get("contract_sha256") == expected_contract_sha256
            and smoke.get("artifact_sha256") == _artifact_sha256(smoke)
            ...
            and smoke.get("extension_path")
            == os.fspath(_CANONICAL_GSPLAT_CUDA_EXTENSION)
            and smoke.get("extension_sha256") == _EXPECTED_GSPLAT_CUDA_EXTENSION_SHA256
            ...
            and smoke.get("predicates") == expected_predicates
            for smoke in smokes
        ),
        "gsplat runtime smoke evidence does not attest exact GPUs 0 and 1",
    )
    return file_sha256, evidence
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--v6-lock", type=Path, required=True)
    parser.add_argument("--v6-outcome-withdrawal-report", type=Path, required=True)
    parser.add_argument(
        "--gsplat-runtime-supplement-manifest", type=Path, required=True
    )
    parser.add_argument("--gsplat-runtime-smoke-evidence", type=Path, required=True)
...
def main() -> int:
    ...
    _require(
        ...
        "reserved held-v7 pycache prefix is no longer unavailable",
    )
    _require(
    ...
    _require(
        ...
        "deployed code is outside the canonical fresh held-v7 root",
    )
    _require(
        _CANONICAL_HELD_V7_ROOT.is_dir()
        and not _CANONICAL_HELD_V7_ROOT.is_symlink()
        and _CANONICAL_HELD_V7_ROOT.resolve() == _CANONICAL_HELD_V7_ROOT,
        "canonical held-v7 root is absent, a symlink, or aliased",
    )
    _require(
        _absolute(arguments.output_lock) == _CANONICAL_V7_LOCK,
        "v7 lock path is outside the canonical fresh held-v7 root",
    )
    if not arguments.verify_existing_lock:
        with os.scandir(_CANONICAL_HELD_V7_ROOT) as entries:
            root_entries = sorted(entries, key=lambda entry: entry.name)
        _require(
            [entry.name for entry in root_entries]
            == [code.name, _CANONICAL_GSPLAT_RUNTIME_SMOKE_EVIDENCE.name]
            and root_entries[0].is_dir(follow_symlinks=False)
            and root_entries[1].is_file(follow_symlinks=False),
            "fresh held-v7 root must contain only deployed code and smoke evidence",
        )
    expected_operator = code / "scripts" / "held" / "prepare_deform360_v7_lock.py"
    operator = _absolute(Path(__file__))
    _require(
        not Path(__file__).is_symlink() and operator == expected_operator,
        "lock operator is not the tracked deployed v7 operator",
    )
    ...
    v6_lock = _validate_v6_lock(arguments.v6_lock)
    v6_report_file_sha256 = _validate_v6_outcome_withdrawal_report(
        arguments.v6_outcome_withdrawal_report
    )
    ...
    from bayesian_phystwin.deform360_held_gsplat_runtime import (  # noqa: PLC0415
    ...
    _require(PROTOCOL_ID == "deform360-held-online-belief-v7", "not v7 code")
    supplement_manifest_file_sha256, _supplement_manifest = (
        _validate_runtime_supplement_manifest(
            arguments.gsplat_runtime_supplement_manifest,
            expected_extension_contract=GSPLAT_CUDA_EXTENSION_CONTRACT,
            expected_extension_contract_sha256=(GSPLAT_CUDA_EXTENSION_CONTRACT_SHA256),
        )
    )
    smoke_evidence_file_sha256, _smoke_evidence = _validate_runtime_smoke_evidence(
        arguments.gsplat_runtime_smoke_evidence,
        expected_contract_sha256=GSPLAT_RUNTIME_SMOKE_CONTRACT_SHA256,
        supplement_manifest_sha256=supplement_manifest_file_sha256,
        deployed_method_head=str(provenance_before["method_head"]),
        operator_source_sha256=_sha256_file(
            code / LOCAL_FILE_BINDINGS["held_outcome_cuda_smoke_operator_source"],
            "deployed gsplat runtime smoke operator",
        ),
    )
    ...
    binding_groups = (
        ...
        set(V7_PINNED_EXTERNAL_BINDING_KEYS),
        local_files,
    ...
    expected_v1_keys = required - set(V7_ONLY_BINDING_KEYS)
    _require(set(old_bindings) == expected_v1_keys, "exact v1 binding keys changed")
    _require(
        set(V7_ONLY_BINDING_KEYS) == required - set(old_bindings)
        and len(V7_ONLY_BINDING_KEYS) == EXPECTED_V7_MIGRATION_KEY_COUNT,
        "exact v7 migration-key delta changed",
    )
    _require(
        len(required) == EXPECTED_V7_BINDING_COUNT,
        "exact v7 immutable-binding count changed",
    )
    bindings = {
        key: old_bindings[key] for key in sorted(INHERITED_EXTERNAL_BINDING_KEYS)
    }
    bindings.update(V7_PINNED_EXTERNAL_BINDING_VALUES)
    bindings.update(
        {
            "held_gsplat_runtime_supplement_manifest": (
                supplement_manifest_file_sha256
            ),
            "held_outcome_cuda_smoke_evidence": smoke_evidence_file_sha256,
        }
    )
    ...
    bindings.update(
        {
            ...
            "held_outcome_cuda_smoke_contract": (GSPLAT_RUNTIME_SMOKE_CONTRACT_SHA256),
            ...
            "v6_outcome_withdrawal_report": v6_report_file_sha256,
            **{
    ...
    _require(set(bindings) == required, "prospective v7 binding key set differs")
    _require(
        all(_SHA256.fullmatch(value) for value in bindings.values()),
        "prospective v7 contains an invalid digest",
    )
    ...
    _require(
        ...
        "v4 execution-withdrawal evidence changed during v7 preparation",
    )
    _require(
        _validate_v5_lock(arguments.v5_lock) == v5_lock,
        "v5 calibration lock changed during v7 preparation",
    )
    _require(
        _validate_v5_outcome_withdrawal_report(arguments.v5_outcome_withdrawal_report)
        == v5_report_file_sha256,
        "v5 outcome-withdrawal evidence changed during v7 preparation",
    )
    _require(
        _validate_v6_lock(arguments.v6_lock) == v6_lock,
        "v6 calibration lock changed during v7 preparation",
    )
    _require(
        _validate_v6_outcome_withdrawal_report(arguments.v6_outcome_withdrawal_report)
        == v6_report_file_sha256,
        "v6 outcome-withdrawal evidence changed during v7 preparation",
    )
    _require(
        _validate_runtime_supplement_manifest(
            arguments.gsplat_runtime_supplement_manifest,
        ...
        == (supplement_manifest_file_sha256, _supplement_manifest),
        "gsplat runtime supplement changed during v7 preparation",
    )
    _require(
        _validate_runtime_smoke_evidence(
            arguments.gsplat_runtime_smoke_evidence,
            expected_contract_sha256=GSPLAT_RUNTIME_SMOKE_CONTRACT_SHA256,
            supplement_manifest_sha256=supplement_manifest_file_sha256,
            deployed_method_head=str(provenance_before["method_head"]),
            operator_source_sha256=bindings["held_outcome_cuda_smoke_operator_source"],
        )
        == (smoke_evidence_file_sha256, _smoke_evidence),
        "gsplat runtime smoke evidence changed during v7 preparation",
    )
    if arguments.verify_existing_lock:
        output = _absolute(arguments.output_lock)
        _ensure_unprotected_path(output, "existing v7 lock")
        _require(output.name == "calibration-lock.json", "v7 lock filename changed")
        _require(code not in output.parents, "v7 lock may not be inside deployed code")
        raw_lock = _read_regular_bytes(output, "existing v7 lock")
        existing_mode = os.lstat(output).st_mode
        _require(
            stat.S_IMODE(existing_mode) == 0o400,
            "existing v7 lock mode is not exactly 0400",
        )
        lock = load_held_protocol_lock(output)
        _require(
            lock.get("immutable_bindings") == dict(sorted(bindings.items())),
            "existing v7 lock bindings disagree with recomputed bindings",
        )
        canonical_lock = (
            json.dumps(lock, indent=2, sort_keys=True, allow_nan=False) + "\n"
        ).encode("utf-8")
        _require(raw_lock == canonical_lock, "existing v7 lock file is not canonical")
        operation = "verified_existing_lock"
    else:
        output = _absolute(arguments.output_lock)
        _ensure_unprotected_path(output, "prospective v7 lock output")
        _require(output.name == "calibration-lock.json", "v7 lock filename changed")
        _require(code not in output.parents, "v7 lock may not be inside deployed code")
        _require(not os.path.lexists(output), "prospective v7 lock already exists")
        _require(
            output.parent.is_dir() and not output.parent.is_symlink(),
            "v7 lock parent must already be a real directory",
        )
        ...
        json.dumps(
            {
                ...
                "lock_file_sha256": _sha256_file(output, "v7 lock"),
                ...
                "v6_outcome_withdrawal_report_file_sha256": bindings[
                    "v6_outcome_withdrawal_report"
                ],
                "gsplat_runtime_supplement_manifest_file_sha256": bindings[
                    "held_gsplat_runtime_supplement_manifest"
                ],
                "gsplat_runtime_smoke_evidence_file_sha256": bindings[
                    "held_outcome_cuda_smoke_evidence"
                ],
            },
            sort_keys=True,
