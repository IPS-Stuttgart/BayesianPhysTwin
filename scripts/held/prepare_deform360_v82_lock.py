#!/usr/bin/env python3
"""Create the fresh Deform360 v8.2 process-isolated calibration lock."""
...
SOURCE_RELATIVE = Path("scripts/held/prepare_deform360_v82_lock.py")
...
PROCESS_SOURCE_BINDINGS: Mapping[str, str] = {
    "held_v82_lock_preparer_source": SOURCE_RELATIVE.as_posix(),
...
def _support(code: Path) -> ModuleType:
    return _load_module(code / SUPPORT_RELATIVE, "_deform360_v82_lock_support")
...
def _validate_qualification(
    *,
    ...
    _require(
        ...
        "v8.2 lock preparer is outside the exact source tree",
    )
...
def prospective_bindings(
    ...
    _require(
        protocol.PROTOCOL_ID == "deform360-held-online-belief-v8.2"
        and protocol.EXECUTION_ATTEMPT == 1,
        "source protocol is not fresh v8.2 attempt 1",
    )
    ...
    _require(
        ...
        "prospective v8.2 binding is not SHA-256",
    )
    return (
...
def create_lock_and_deployment(source_code: str | Path) -> dict[str, Any]:
    ...
    _require(not os.path.lexists(HELD_ROOT), "formal held-v82 root is not fresh")
    ...
    stage = HELD_BASE / f".held-v82-code-stage-{head}"
...
def main() -> int:
    ...
    if arguments.preflight:
        ...
        result = {
            "operation": "validated_held_v82_source_and_qualification",
