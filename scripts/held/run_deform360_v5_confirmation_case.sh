...
if [[ "${BPT_HELD_V5_CONFIRMATION_CASE_ENV_NORMALIZED:-}" != "1" ]]; then
  exec env -i \
    ...
    BPT_HELD_V5_CONFIRMATION_LOCK_VERIFIED_SHA256="${BPT_HELD_V5_CONFIRMATION_LOCK_VERIFIED_SHA256:-}" \
    BPT_HELD_V5_CONFIRMATION_CASE_ENV_NORMALIZED=1 \
    /bin/bash "$0" "$@"
fi
...
while IFS='=' read -r environment_name _value; do
  case "$environment_name" in
    ...
    BPT_HELD_V5_CODE|BPT_HELD_V5_CONFIRMATION_LOCK_VERIFIED_SHA256|\
    BPT_HELD_V5_CONFIRMATION_CASE_ENV_NORMALIZED)
      ;;
...
if [[ "$#" -ne 4 ]]; then
  die "usage: run_deform360_v5_confirmation_case.sh CUDA_DEVICE CASE OBJECT EPISODE"
fi
...
case "$CASE_NAME:$OBJECT:$EPISODE" in
  ...
  170-spider-ep0006:170-spider:0006)
    ;;
  *) die "case tuple is outside the exact six-case v5 confirmation cohort" ;;
esac
...
[[ "$CASE_RUNNER" == "$CODE/scripts/held/run_deform360_v5_confirmation_case.sh" ]] || \
...
[[ -f "$CALIBRATION_LOCK" && ! -L "$CALIBRATION_LOCK" && "$(stat -c '%a' -- "$CALIBRATION_LOCK")" == "400" ]] || \
  die "held-v5 calibration parent lock is absent, mutable, or a symlink"
[[ -f "$CALIBRATION_DECISION" && ! -L "$CALIBRATION_DECISION" ]] || \
  die "held-v5 calibration GO decision is absent or a symlink"
...
readonly VERIFIED_LOCK_SHA256="${BPT_HELD_V5_CONFIRMATION_LOCK_VERIFIED_SHA256:?run cases through the verified v5 confirmation shard runner}"
...
# The preparer validates the immutable 112-binding calibration parent.  The
...
if lock.get("stage") != "confirmation":
    raise RuntimeError("held-v5 confirmation lock has another stage")
