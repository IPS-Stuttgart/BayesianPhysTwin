#!/usr/bin/env python3
"""Run the permit-gated held-v7 Deform360 calibration outcome phase.

This operator driver has one deliberately narrow job:

1. load, attest, and smoke the sealed AOT gsplat CUDA runtime;
2. validate the exact 15-case online-prediction cohort and obtain the live
   calibration outcome capability;
3. only after that barrier, create or revalidate each locked official target
   through the pinned reconstruction adapter;
4. score the fixed primary method and comparator; and
5. write the immutable score evidence and frozen GO/NO-GO decision.

The driver contains no confirmation promotion or confirmation read path.  A
NO-GO exits non-zero with the confirmation payload still sealed.  The
``--dry-run-barrier-only`` mode smokes the runtime, then stops after step 2 and does not
...
EXPECTED_PROTOCOL_ID = "deform360-held-online-belief-v7"
EXPECTED_V7_BINDING_COUNT = 118
NO_GO_EXIT_CODE = 3
_CANONICAL_HELD_BASE = Path("/mnt/corsair/florianpfaff/bpt-online-belief-v1")
_CANONICAL_HELD_ROOT = _CANONICAL_HELD_BASE / "held-v7"
...
_CANONICAL_V5_LOCK = _CANONICAL_HELD_BASE / "held-v5" / "calibration-lock.json"
_CANONICAL_V5_OUTCOME_WITHDRAWAL_REPORT = (
    _CANONICAL_HELD_BASE / "held-v5" / "v5-outcome-withdrawal-report.json"
)
_CANONICAL_V6_LOCK = _CANONICAL_HELD_BASE / "held-v6" / "calibration-lock.json"
_CANONICAL_V6_OUTCOME_WITHDRAWAL_REPORT = (
    _CANONICAL_HELD_BASE / "held-v6" / "v6-outcome-withdrawal-report.json"
)
...
_CANONICAL_GSPLAT_RUNTIME_SMOKE_EVIDENCE = (
    _CANONICAL_HELD_ROOT / "gsplat-runtime-smoke-evidence.json"
)
...
EXPECTED_LOCK_OPERATOR_SHA256 = (
    "3d042d1cbfcaefa550446b8161a3d2dd5939dfc8d6725498a3faeac11e93e65e"
)
...
def _verify_locked_deployment(arguments: "DriverArguments") -> Path:
    ...
    _require(
        ...
        "held root is not the canonical fresh held-v7 root",
    )
    ...
    _require(
        ...
        "deployed v7 code is absent, linked, or non-canonical",
    )
    _require(
        ...
        "deployed code is outside canonical held-v7 code-$HEAD",
    )
    lock = Path(arguments.lock)
    _require(
        lock == _CANONICAL_HELD_ROOT / "calibration-lock.json"
        and not lock.is_symlink(),
        "calibration lock is outside the canonical held-v7 root",
    )
    _require_mode_0400(lock, "held-v7 calibration lock")
    for path, role in (
        ...
        (_CANONICAL_V5_LOCK, "sealed v5 calibration lock"),
        (
            _CANONICAL_V5_OUTCOME_WITHDRAWAL_REPORT,
            "sealed v5 outcome-withdrawal report",
        ),
        (_CANONICAL_V6_LOCK, "sealed v6 calibration lock"),
        (
            _CANONICAL_V6_OUTCOME_WITHDRAWAL_REPORT,
            "sealed v6 outcome-withdrawal report",
        ),
        (
            _CANONICAL_GSPLAT_RUNTIME_SUPPLEMENT_MANIFEST,
            "sealed gsplat runtime supplement manifest",
        ),
        (
            _CANONICAL_GSPLAT_RUNTIME_SMOKE_EVIDENCE,
            "sealed pre-lock gsplat runtime smoke evidence",
        ),
    ):
        _require(
    ...
    lock_operator = operator_dir / "prepare_deform360_v7_lock.py"
    expected_driver = operator_dir / "run_deform360_v7_calibration_outcomes.py"
    observed_driver = Path(__file__)
    _require(
        not observed_driver.is_symlink()
        and observed_driver.resolve() == expected_driver,
        "outcome driver is not the tracked deployed v7 source",
    )
    _require(
        lock_operator.is_file() and not lock_operator.is_symlink(),
        "tracked v7 lock operator is absent or linked",
    )
    ...
    _require(
        _sha256_regular_file(lock_operator, "tracked v7 lock operator")
        == EXPECTED_LOCK_OPERATOR_SHA256,
        "lock operator checksum differs from the audited v7 verifier",
    )
    ...
    command = [
        ...
        "--v5-lock",
        os.fspath(_CANONICAL_V5_LOCK),
        "--v5-outcome-withdrawal-report",
        os.fspath(_CANONICAL_V5_OUTCOME_WITHDRAWAL_REPORT),
        "--v6-lock",
        os.fspath(_CANONICAL_V6_LOCK),
        "--v6-outcome-withdrawal-report",
        os.fspath(_CANONICAL_V6_OUTCOME_WITHDRAWAL_REPORT),
        "--gsplat-runtime-supplement-manifest",
        os.fspath(_CANONICAL_GSPLAT_RUNTIME_SUPPLEMENT_MANIFEST),
        "--gsplat-runtime-smoke-evidence",
        os.fspath(_CANONICAL_GSPLAT_RUNTIME_SMOKE_EVIDENCE),
        "--deployed-code",
    ...
    _require(
        ...
        "source-only v7 deployment verification failed: "
        + completed.stderr.decode("utf-8", "replace").strip(),
    )
    try:
        verification = json.loads(completed.stdout.decode("utf-8"))
    except (UnicodeDecodeError, json.JSONDecodeError) as error:
        raise ValueError("v7 deployment verifier returned malformed JSON") from error
    _require(
        isinstance(verification, Mapping)
        and verification.get("operation") == "verified_existing_lock"
        and verification.get("protocol_id") == EXPECTED_PROTOCOL_ID
        and verification.get("binding_count") == EXPECTED_V7_BINDING_COUNT
        and verification.get("method_head") == code_argument.name.removeprefix("code-")
        and verification.get("lock_file_sha256")
        == _sha256_regular_file(lock, "held-v7 calibration lock")
        and verification.get("gsplat_runtime_supplement_manifest_file_sha256")
        == _sha256_regular_file(
            _CANONICAL_GSPLAT_RUNTIME_SUPPLEMENT_MANIFEST,
            "gsplat runtime supplement manifest",
        )
        and verification.get("gsplat_runtime_smoke_evidence_file_sha256")
        == _sha256_regular_file(
            _CANONICAL_GSPLAT_RUNTIME_SMOKE_EVIDENCE,
            "pre-lock gsplat runtime smoke evidence",
        ),
        "v7 deployment-verifier identity disagrees with requested deployment",
    )
...
class _ProgressCallback:
    ...
    def __call__(self) -> Any:
        _require_directory_identity(
            ...
            "calibration outcomes root",
        )
        _emit(
            "CALIBRATION_TARGET_OPERATION_START",
        ...
        _emit(
            "CALIBRATION_TARGET_OPERATION_COMPLETE",
            case_name=self.case_name,
            operation=self.operation,
        )
        return value
...
def _parse_seal_assignments(
    ...
    _require(
        ...
        "online seals must be the exact locked calibration cohort; "
...
def _validate_held_layout_after_barrier(
    ...
    _require(
        ...
        "held-v7 root must be an existing non-symlink directory",
    )
    ...
    for case_name in expected_cases:
        ...
        _require(
            path == canonical,
            f"online seal is outside the canonical held-v7 case path: {case_name}",
        )
    ...
    return HeldLayout(
        ...
        outcomes_root=calibration_root / "outcomes",
        evidence_path=calibration_root / "calibration-score-evidence.json",
        decision_path=calibration_root / "calibration-gate-decision.json",
    )
...
def _prepare_fresh_gate_outputs_after_barrier(layout: HeldLayout) -> None:
    calibration_root = layout.outcomes_root.parent
    _require(
        calibration_root.is_dir()
        and not calibration_root.is_symlink()
        and calibration_root.resolve() == calibration_root,
        "calibration output root is absent, linked, or non-canonical",
    )
    _require(
    ...
    _require(
        ...
        f"outcome root already exists; exact v7 outcome resume is forbidden: {layout.outcomes_root}",
    )
    claim = calibration_root / ".outcome-phase.claim"
    _require(
        not os.path.lexists(claim),
        "calibration outcome phase was already claimed",
    )
...
def _execute_driver(
    ...
    _require(
        ...
        "deployed protocol is not held-v7",
    )
    expected_cases = tuple(protocol.CALIBRATION_CASE_NAMES)
    _require(len(expected_cases) == 15, "held-v7 calibration cohort is not 15 cases")
    ...
    # This runtime-only smoke must succeed in the outcome process before the
    # first artifact capability is requested.  It loads the frozen AOT gsplat
    # extension and retains that backend module state for later Splatfacto use.
    smoke_evidence = smoke_gsplat_runtime()
    smoke_artifact_sha256 = (
        str(smoke_evidence.get("artifact_sha256", ""))
        if isinstance(smoke_evidence, Mapping)
        else ""
    )
    _require(
        isinstance(smoke_evidence, Mapping)
        and isinstance(smoke_evidence.get("artifact_sha256"), str)
        and len(smoke_artifact_sha256) == 64
        and all(character in "0123456789abcdef" for character in smoke_artifact_sha256),
        "gsplat runtime smoke returned invalid evidence",
    )
    _emit(
        "GSPLAT_RUNTIME_SMOKE_VALIDATED",
    ...
    _emit(
        "CALIBRATION_COHORT_BARRIER_VALIDATED",
    ...
    for case_name in expected_cases:
        ...
        _emit(
            "CALIBRATION_TARGET_OPERATION_PLANNED",
            case_name=case_name,
            operation=planned.operation,
        )
    _require(
        tuple(operations) == expected_cases,
        "TargetOperations are not in exact locked calibration order",
    )
    ...
    _require(
        decision.get("decision") in {"GO", "NO_GO"},
        "frozen calibration gate returned an invalid decision",
    )
    _emit(
        "CALIBRATION_SCORE_EVIDENCE_WRITTEN",
        path=str(layout.evidence_path),
        artifact_sha256=evidence.get("artifact_sha256"),
    )
    _emit(
        "CALIBRATION_GATE_DECISION_WRITTEN",
    ...
    if decision["decision"] == "NO_GO":
        _emit(
            "STOPPED_ON_NO_GO_CONFIRMATION_REMAINS_SEALED",
            exit_code=NO_GO_EXIT_CODE,
        )
        return NO_GO_EXIT_CODE
    _emit(
        "CALIBRATION_GO_CONFIRMATION_STILL_SEALED",
        note=(
            "This driver never creates a confirmation lock and never reads "
...
def _load_protocol(deployed_code: str) -> tuple[Any, Path]:
    ...
    _require(
        ...
        "deployed v7 code must be an existing non-symlink directory",
    )
    ...
    _require(
        ...
        "held protocol imported from outside the deployed v7 snapshot",
    )
...
def _load_post_barrier_api(source: Path) -> PostBarrierApi:
    ...
    return PostBarrierApi(
        ...
        score_and_create_gate=scoring.score_and_create_calibration_gate,
    )


def _load_gsplat_runtime_smoke(
    source: Path,
) -> Callable[[], Mapping[str, Any]]:
    from bayesian_phystwin import (  # noqa: PLC0415
        deform360_held_gsplat_runtime as gsplat_runtime,
    )

    expected = source / "bayesian_phystwin" / "deform360_held_gsplat_runtime.py"
    _require(
        Path(gsplat_runtime.__file__).resolve() == expected.resolve(),
        "gsplat runtime operator imported outside deployed snapshot",
    )
    _require_package_provenance(source)
    smoke = getattr(gsplat_runtime, "load_and_smoke_gsplat_runtime", None)
    _require(callable(smoke), "deployed gsplat runtime smoke callable is absent")
    expected_artifact_sha256 = _locked_gpu0_smoke_artifact_sha256()

    def smoke_and_match_locked_evidence() -> Mapping[str, Any]:
        observed = smoke()
        _require(
            isinstance(observed, Mapping)
            and observed.get("artifact_sha256") == expected_artifact_sha256,
            "live gsplat smoke differs from locked physical-GPU-0 evidence",
        )
        return observed

    return smoke_and_match_locked_evidence


def _locked_gpu0_smoke_artifact_sha256() -> str:
    path = _CANONICAL_GSPLAT_RUNTIME_SMOKE_EVIDENCE
    _require_mode_0400(path, "locked gsplat runtime smoke evidence")
    ...
    try:
        ...
        _require(
            ...
            "locked gsplat smoke evidence changed while opening",
        )
        with os.fdopen(descriptor, "rb", closefd=False) as stream:
        ...
        _require(
            ...
            "locked gsplat smoke evidence changed while reading",
        )
    finally:
        os.close(descriptor)
    smokes = evidence.get("smokes") if isinstance(evidence, Mapping) else None
    _require(
        isinstance(smokes, list)
        and len(smokes) == 2
        and isinstance(smokes[0], Mapping)
        and smokes[0].get("physical_gpu_index") == 0,
        "locked gsplat smoke evidence lacks physical GPU 0",
    )
    artifact_sha256 = str(smokes[0].get("artifact_sha256", ""))
    _require(
        len(artifact_sha256) == 64
        and all(character in "0123456789abcdef" for character in artifact_sha256),
        "locked physical-GPU-0 smoke artifact digest is invalid",
    )
...
def _normalized_environment_or_reexec() -> None:
    if (
        os.environ.get("BPT_HELD_OUTCOME_ENV_NORMALIZED") == "1"
        and sys.flags.isolated == 1
    ):
        expected = _minimal_environment(include_outcome_runtime=True)
        expected["BPT_HELD_OUTCOME_ENV_NORMALIZED"] = "1"
        _require(
    ...
    environment["BPT_HELD_OUTCOME_ENV_NORMALIZED"] = "1"
    os.execve(
        _PINNED_PYTHON,
        [
            _PINNED_PYTHON,
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "exact held-v7 15-case calibration cohort."
        )
    )
    ...
    parser.add_argument(
        ...
        default=[],
        metavar="CASE=PATH",
        help="repeat exactly once for each of the 15 locked calibration cases",
    )
    ...
    if not namespace.self_check:
        missing = [
            name
            for name in ("deployed_code", "held_root", "lock")
...
def _mock_arguments(root: Path, *, dry: bool) -> DriverArguments:
    cases = tuple(f"mock-object-{index:02d}-ep{index:04d}" for index in range(15))
    seals: list[str] = []
...
def _self_check() -> int:
    """Mock the orchestration to prove barrier-first and NO-GO stopping."""
    ...
    class FakeProtocol:
        ...
        def __init__(self, cases: tuple[str, ...]) -> None:
            self.CALIBRATION_CASE_NAMES = cases
        ...
        def authorize_outcome_phase(
            ...
            _require(role == "calibration" and len(seals) == 15, "bad mock permit")
    ...
    with tempfile.TemporaryDirectory(prefix="bpt-outcome-driver-self-check-") as temp:
        ...
        dry_code = _execute_driver(
            dry_args,
            FakeProtocol(cases),
            lambda: events.append("smoke") or {"artifact_sha256": "a" * 64},
            forbidden_post_import,
        )
        _require(
            dry_code == 0 and events == ["smoke", "authorize"],
            "dry smoke/barrier order failed",
        )

        events = []
        go_args = _mock_arguments(base / "go", dry=False)
        go_code = _execute_driver(
            go_args,
            FakeProtocol(cases),
            lambda: events.append("smoke") or {"artifact_sha256": "a" * 64},
            lambda: post_api("GO"),
        ...
        _require(
            go_code == 0 and events[:2] == ["smoke", "authorize"],
            "GO did not smoke before the barrier",
        )
        _require(
            events.count("score") == 1 and events[-1] == "score",
            "GO orchestration continued after scoring",
        )

        events = []
        no_go_args = _mock_arguments(base / "no-go", dry=False)
        no_go_code = _execute_driver(
            no_go_args,
            FakeProtocol(cases),
            lambda: events.append("smoke") or {"artifact_sha256": "a" * 64},
            lambda: post_api("NO_GO"),
            lambda _arguments: None,
        )
        _require(
            no_go_code == NO_GO_EXIT_CODE and events[:2] == ["smoke", "authorize"],
            "NO-GO did not smoke before the barrier and stop non-zero",
        )
        _require(
            events.count("score") == 1 and events[-1] == "score",
            "NO-GO orchestration did not stop at the gate",
        )
    _emit(
        "SELF_CHECK_PASSED",
        checks=[
            "dry-run smokes then authorizes and imports no post-barrier API",
            "full runs smoke before authorization and target planning",
            "exactly 15 TargetOperations are planned in locked order",
            "NO-GO exits non-zero immediately after immutable gate writing",
            "driver has no confirmation promotion or confirmation read API",
        ],
    )
    return 0
...
def main() -> int:
    ...
    smoke_gsplat_runtime = _load_gsplat_runtime_smoke(source)
    return _execute_driver(
        arguments,
        protocol,
        smoke_gsplat_runtime,
