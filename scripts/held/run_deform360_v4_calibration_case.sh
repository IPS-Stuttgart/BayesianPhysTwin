...
if [[ "${BPT_HELD_V4_CALIBRATION_CASE_ENV_NORMALIZED:-}" != "1" ]]; then
  exec env -i \
    ...
    BPT_HELD_V4_LOCK_VERIFIED_SHA256="${BPT_HELD_V4_LOCK_VERIFIED_SHA256:-}" \
    BPT_HELD_V4_CALIBRATION_CASE_ENV_NORMALIZED=1 \
    /bin/bash "$0" "$@"
fi
...
while IFS='=' read -r environment_name _value; do
  case "$environment_name" in
    ...
    BPT_HELD_V4_CODE|BPT_HELD_V4_LOCK_VERIFIED_SHA256|\
    BPT_HELD_V4_CALIBRATION_CASE_ENV_NORMALIZED)
      ;;
...
if [[ "$#" -ne 4 ]]; then
  die "usage: run_deform360_v4_calibration_case.sh CUDA_DEVICE CASE OBJECT EPISODE"
fi
...
case "$CASE_NAME:$OBJECT:$EPISODE" in
  ...
  170-spider-ep0007:170-spider:0007)
    ;;
  *) die "case tuple is outside the exact 15-case v4 calibration cohort" ;;
esac

readonly HELD="/mnt/corsair/florianpfaff/bpt-online-belief-v1/held-v4"
readonly CODE="${BPT_HELD_V4_CODE:?set BPT_HELD_V4_CODE to the immutable v4 deployment}"
...
readonly UPSTREAM="/mnt/corsair/florianpfaff/bpt-held-v4-runtimes/Bayesian-PhysTwin-upstream-58ab4808e59d"
readonly OFFICIAL="/mnt/corsair/florianpfaff/bpt-held-v4-runtimes/PhysTwin-upstream-2b6630528141"
...
[[ -d "$HELD" && ! -L "$HELD" ]] || die "held-v4 root is absent or a symlink"
[[ "$(readlink -f -- "$HELD")" == "$HELD" ]] || die "held-v4 root is not canonical"
[[ -d "$CODE" && ! -L "$CODE" ]] || die "v4 deployed code is absent or a symlink"
[[ "$(readlink -f -- "$CODE")" == "$CODE" ]] || die "v4 deployed code is not canonical"
...
[[ "$CASE_RUNNER" == "$CODE/scripts/held/run_deform360_v4_calibration_case.sh" ]] || \
...
[[ -z "$(find "$CASE_RUNNER" "$LOCK_OPERATOR" -maxdepth 0 -perm /222 -print -quit)" ]] || \
  die "tracked v4 operators are writable"
...
[[ "$ACTUAL_LOCK_OPERATOR_SHA256" == "$EXPECTED_LOCK_OPERATOR_SHA256" ]] || \
  die "lock operator checksum differs from the audited v4 verifier"
[[ -f "$LOCK" && ! -L "$LOCK" ]] || die "held-v4 lock is absent or a symlink"
[[ "$(readlink -f -- "$LOCK")" == "$LOCK" ]] || die "held-v4 lock is not canonical"
[[ "$(stat -c '%a' -- "$LOCK")" == "400" ]] || die "held-v4 lock mode is not 0400"
...
readonly VERIFIED_LOCK_SHA256="${BPT_HELD_V4_LOCK_VERIFIED_SHA256:?run cases through the verified v4 shard runner}"
...
[[ "$ACTUAL_LOCK_SHA256" == "$VERIFIED_LOCK_SHA256" ]] || \
  die "held-v4 lock changed after startup verification"

# Recompute the full deployed Git snapshot, all 110 immutable bindings, and
# exact v1/v2/v3 ancestry immediately before this case accesses episode payloads.
env -i \
  HOME=/home/florianpfaff \
  USER=florianpfaff \
  LOGNAME=florianpfaff \
...
[[ "$REVERIFIED_LOCK_SHA256" == "$VERIFIED_LOCK_SHA256" ]] || \
  die "held-v4 lock changed during case-level deployment verification"
...
# Refuse symlinks in every output parent.  This prevents a nominal held-v4
