...
if [[ "${BPT_HELD_V4_CONFIRMATION_CASE_ENV_NORMALIZED:-}" != "1" ]]; then
  exec env -i \
    ...
    BPT_HELD_V4_CONFIRMATION_CASE_ENV_NORMALIZED=1 \
    /bin/bash "$0" "$@"
fi
...
while IFS='=' read -r environment_name _value; do
  case "$environment_name" in
    ...
    BPT_HELD_V4_CODE|BPT_HELD_V4_CONFIRMATION_LOCK_VERIFIED_SHA256|\
    BPT_HELD_V4_CONFIRMATION_CASE_ENV_NORMALIZED)
      ;;
...
if [[ "$#" -ne 4 ]]; then
  die "usage: run_deform360_v4_confirmation_case.sh CUDA_DEVICE CASE OBJECT EPISODE"
fi
...
case "$CASE_NAME:$OBJECT:$EPISODE" in
  ...
  170-spider-ep0006:170-spider:0006)
    ;;
  *) die "case tuple is outside the exact six-case v4 confirmation cohort" ;;
esac
...
[[ "$CASE_RUNNER" == "$CODE/scripts/held/run_deform360_v4_confirmation_case.sh" ]] || \
...
readonly VERIFIED_LOCK_SHA256="${BPT_HELD_V4_CONFIRMATION_LOCK_VERIFIED_SHA256:?run cases through the verified v4 confirmation shard runner}"
...
# The preparer validates the immutable 110-binding calibration parent.  The
