#!/usr/bin/env python3
"""Run the permit-gated held-v7 Deform360 confirmation outcome phase.
...
2. otherwise load, attest, and smoke the sealed AOT gsplat CUDA runtime;
3. validate the exact six-case online-prediction cohort and obtain
   the live confirmation outcome capability;
4. only after that barrier, create each locked official target
   through the pinned reconstruction adapter;
5. score the fixed primary method and comparator; and
6. write immutable score evidence and the frozen final confirmation decision.

The promotion path accepts no prediction seal or target-bearing runtime path.
The ``--dry-run-barrier-only`` mode smokes the runtime, then stops immediately
after the six-seal barrier and does not load the post-barrier reconstruction or
scoring API into the outcome process.
...
def _verify_locked_deployment(arguments: "DriverArguments") -> Path:
    ...
    _require(
        lock == _CANONICAL_CALIBRATION_LOCK and not lock.is_symlink(),
    ...
    expected_driver = operator_dir / "run_deform360_v7_confirmation_outcomes.py"
...
def _promote_confirmation_lock(arguments: DriverArguments, protocol: Any) -> int:
    ...
    _require_mode_0400(destination, "held-v7 confirmation lock")
...
def _validate_confirmation_lock_lineage(
    ...
    _require_mode_0400(confirmation_lock, "held-v7 confirmation lock")
...
def _validate_held_layout_after_barrier(
    ...
    _require(
        ...
        "confirmation lock is outside the canonical held-v7 root",
    )
...
def _execute_driver(
    ...
    _require(len(expected_cases) == 6, "held-v7 confirmation cohort is not six cases")
    ...
    # Attest and load the frozen AOT gsplat backend in this exact process
    # before requesting the first artifact capability.  The loaded backend
    # remains installed for the later official Splatfacto reconstruction.
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "exact held-v7 six-case confirmation cohort."
        )
    )
...
def _self_check() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="bpt-outcome-driver-self-check-") as temp:
        ...
        _require(
            confirmed_code == 0 and events[:2] == ["smoke", "authorize"],
            "confirmation pass did not smoke before the barrier",
        )
        _require(
        ...
        _require(
            failed_code == NOT_CONFIRMED_EXIT_CODE
            and events[:2] == ["smoke", "authorize"],
            "failed confirmation did not smoke before barrier and stop non-zero",
        )
        _require(
            events.count("score") == 1 and events[-1] == "score",
