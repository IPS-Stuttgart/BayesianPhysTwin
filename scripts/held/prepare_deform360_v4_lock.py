#!/usr/bin/env python3
"""Create the prospective Deform360 held-v4 lock from audited v1 provenance.
...
v4 contracts, and preserves only unchanged external-runtime bindings from v1.
...
EXPECTED_V4_BINDING_COUNT = 110
EXPECTED_V4_MIGRATION_KEY_COUNT = 19
_PYCACHE_PREFIX = "/nonexistent/bpt-held-v4-pycache"
_PINNED_OPERATOR_ENVIRONMENT = {
    ...
    "PATH": "/home/florianpfaff/.venvs/bpt-gpu/bin:/usr/local/bin:/usr/bin:/bin",
...
LOCAL_FILE_BINDINGS: Mapping[str, str] = {
    ...
    "held_calibration_case_runner_source": (
        "scripts/held/run_deform360_v4_calibration_case.sh"
    ),
    "held_calibration_outcome_driver_source": (
        "scripts/held/run_deform360_v4_calibration_outcomes.py"
    ),
    "held_calibration_shard_runner_source": (
        "scripts/held/run_deform360_v4_calibration_shard.sh"
    ),
    "held_confirmation_case_runner_source": (
        "scripts/held/run_deform360_v4_confirmation_case.sh"
    ),
    "held_confirmation_outcome_driver_source": (
        "scripts/held/run_deform360_v4_confirmation_outcomes.py"
    ),
    "held_confirmation_shard_runner_source": (
        "scripts/held/run_deform360_v4_confirmation_shard.sh"
    ),
    ...
    "held_protocol_lock_operator_source": ("scripts/held/prepare_deform360_v4_lock.py"),
...
# These are the only v1 values that v4 may inherit.  They identify runtimes,
...
# These v4-only external identities are not inherited from the v1 lock.  They
# are the exact exports used by the sealed source-only production census.
V4_PINNED_EXTERNAL_BINDING_VALUES: Mapping[str, str] = {
...
V4_PINNED_EXTERNAL_BINDING_KEYS = frozenset(V4_PINNED_EXTERNAL_BINDING_VALUES)
V4_PINNED_SEMANTIC_GATE_CONTRACT_SHA256 = (
...
def _require_deployed_semantic_binding_exports(
    *,
    ...
    _require(
        dict(V4_PINNED_EXTERNAL_BINDING_VALUES) == expected_external_bindings,
        "pinned SigLIP2 bindings diverge from deployed semantic exports",
    )
    _require(
        V4_PINNED_SEMANTIC_GATE_CONTRACT_SHA256
        == semantic_gate_contract_sha256,
...
V4_ONLY_BINDING_KEYS = frozenset(
    {
        "deform360_dataset_containment_source",
        "deform360_robot_kinematics_source",
        "frame_zero_semantic_gate_contract",
...
def _expected_v3_boundary_incident_report() -> tuple[
...
def _validate_v2_withdrawal_report(report_path: Path) -> str:
    ...
    _require(
        ...
        "v2 withdrawal report is outside the canonical held-v4 audit path",
    )
...
def _create_v2_withdrawal_report(report_path: Path, lock_path: Path) -> None:
    path = _absolute(report_path)
    output_lock = _absolute(lock_path)
    _require(
        path == _CANONICAL_V2_WITHDRAWAL_REPORT and output_lock == _CANONICAL_V4_LOCK,
        "withdrawal report or lock is outside the canonical held-v4 root",
    )
    _require(not os.path.lexists(path), "v2 withdrawal report already exists")
    _require(
        path.parent.is_dir() and not path.parent.is_symlink(),
        "v2 withdrawal-report parent must already be a real directory",
    )
    _require(
        not os.path.lexists(_CANONICAL_HELD_V2_ROOT),
        "held-v2 root exists; cannot attest withdrawal before deployment",
    )
    _report, encoded, _file_sha256, _artifact_sha256_value = (
        _expected_v2_withdrawal_report()
    )
    descriptor = os.open(
        path,
    ...
    try:
        ...
        os.close(descriptor)
    _require_exact_mode_0400(path, "created v2 withdrawal report")
...
def _validate_v3_boundary_incident_report(report_path: Path) -> str:
    ...
    _require(
        ...
        "v3 boundary-incident report is outside the canonical held-v4 audit path",
    )
    _require(
        path.resolve() == path,
...
def _create_v3_boundary_incident_report(report_path: Path, lock_path: Path) -> None:
    path = _absolute(report_path)
    output_lock = _absolute(lock_path)
    _require(
        path == _CANONICAL_V3_BOUNDARY_INCIDENT_REPORT
        and output_lock == _CANONICAL_V4_LOCK,
        "v3 boundary-incident report or lock is outside the canonical held-v4 root",
    )
    _require(
        not os.path.lexists(path), "v3 boundary-incident report already exists"
    )
    _require(
        path.parent.is_dir() and not path.parent.is_symlink(),
        "v3 boundary-incident report parent must already be a real directory",
    )
    _require(
        not os.path.lexists(_CANONICAL_HELD_V3_ROOT),
        "held-v3 root exists; cannot attest withdrawal before lock or prediction",
    )
    _report, encoded, _file_sha256, _artifact_sha256_value = (
        _expected_v3_boundary_incident_report()
    )
    ...
    try:
        ...
        os.close(descriptor)
    _require_exact_mode_0400(path, "created v3 boundary-incident report")
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--create-v2-withdrawal-report",
        action="store_true",
        help=(
            "atomically create the exact source-only withdrawal report before "
            "creating the v4 lock"
        ),
    )
    parser.add_argument(
        "--create-v3-boundary-incident-report",
        action="store_true",
        help=(
            "atomically create the exact pre-lock boundary-incident report "
            "before creating the v4 lock"
        ),
    )
...
def main() -> int:
    ...
    _require(
        ...
        "reserved held-v4 pycache prefix is no longer unavailable",
    )
    _require(
    ...
    _require(
        code.parent == _CANONICAL_HELD_V4_ROOT,
        "deployed code is outside the canonical fresh held-v4 root",
    )
    _require(
        _CANONICAL_HELD_V4_ROOT.is_dir()
        and not _CANONICAL_HELD_V4_ROOT.is_symlink()
        and _CANONICAL_HELD_V4_ROOT.resolve() == _CANONICAL_HELD_V4_ROOT,
        "canonical held-v4 root is absent, a symlink, or aliased",
    )
    _require(
        _absolute(arguments.output_lock) == _CANONICAL_V4_LOCK,
        "v4 lock path is outside the canonical fresh held-v4 root",
    )
    expected_operator = code / "scripts" / "held" / "prepare_deform360_v4_lock.py"
    operator = _absolute(Path(__file__))
    _require(
        not Path(__file__).is_symlink() and operator == expected_operator,
        "lock operator is not the tracked deployed v4 operator",
    )
    ...
    _require(
        not (
            (
                arguments.create_v2_withdrawal_report
                or arguments.create_v3_boundary_incident_report
            )
            and arguments.verify_existing_lock
        ),
        "lineage-report creation is forbidden during non-mutating verification",
    )
    if arguments.create_v2_withdrawal_report:
        _create_v2_withdrawal_report(
            arguments.v2_withdrawal_report,
            arguments.output_lock,
        )
    v2_report_file_sha256 = _validate_v2_withdrawal_report(
        arguments.v2_withdrawal_report
    )
    if arguments.create_v3_boundary_incident_report:
        _create_v3_boundary_incident_report(
    ...
    _require(PROTOCOL_ID == "deform360-held-online-belief-v4", "not v4 code")
    ...
    binding_groups = (
        ...
        set(V4_PINNED_EXTERNAL_BINDING_KEYS),
        local_files,
    ...
    expected_v1_keys = required - set(V4_ONLY_BINDING_KEYS)
    _require(set(old_bindings) == expected_v1_keys, "exact v1 binding keys changed")
    _require(
        set(V4_ONLY_BINDING_KEYS) == required - set(old_bindings)
        and len(V4_ONLY_BINDING_KEYS) == EXPECTED_V4_MIGRATION_KEY_COUNT,
        "exact v4 migration-key delta changed",
    )
    _require(
        len(required) == EXPECTED_V4_BINDING_COUNT,
        "exact v4 immutable-binding count changed",
    )
    bindings = {
        key: old_bindings[key] for key in sorted(INHERITED_EXTERNAL_BINDING_KEYS)
    }
    bindings.update(V4_PINNED_EXTERNAL_BINDING_VALUES)
    ...
    _require(set(bindings) == required, "prospective v4 binding key set differs")
    _require(
        all(_SHA256.fullmatch(value) for value in bindings.values()),
        "prospective v4 contains an invalid digest",
    )
    ...
    _require(
        ...
        "v1 parent lock changed during v4 preparation",
    )
    _require(
        _validate_v1_report(arguments.v1_report) == v1_report_file_sha256,
        "v1 feasibility report changed during v4 preparation",
    )
    _require(
        _validate_v2_withdrawal_report(arguments.v2_withdrawal_report)
        == v2_report_file_sha256,
        "v2 withdrawal evidence or absence changed during v4 preparation",
    )
    _require(
        _validate_v3_boundary_incident_report(arguments.v3_boundary_incident_report)
        == v3_report_file_sha256,
        "v3 boundary-incident evidence or absence changed during v4 preparation",
    )
    if arguments.verify_existing_lock:
        output = _absolute(arguments.output_lock)
        _ensure_unprotected_path(output, "existing v4 lock")
        _require(output.name == "calibration-lock.json", "v4 lock filename changed")
        _require(code not in output.parents, "v4 lock may not be inside deployed code")
        raw_lock = _read_regular_bytes(output, "existing v4 lock")
        existing_mode = os.lstat(output).st_mode
        _require(
            stat.S_IMODE(existing_mode) == 0o400,
            "existing v4 lock mode is not exactly 0400",
        )
        lock = load_held_protocol_lock(output)
        _require(
            lock.get("immutable_bindings") == dict(sorted(bindings.items())),
            "existing v4 lock bindings disagree with recomputed bindings",
        )
        canonical_lock = (
            json.dumps(lock, indent=2, sort_keys=True, allow_nan=False) + "\n"
        ).encode("utf-8")
        _require(raw_lock == canonical_lock, "existing v4 lock file is not canonical")
        operation = "verified_existing_lock"
    else:
        output = _absolute(arguments.output_lock)
        _ensure_unprotected_path(output, "prospective v4 lock output")
        _require(output.name == "calibration-lock.json", "v4 lock filename changed")
        _require(code not in output.parents, "v4 lock may not be inside deployed code")
        _require(not os.path.lexists(output), "prospective v4 lock already exists")
        _require(
            output.parent.is_dir() and not output.parent.is_symlink(),
            "v4 lock parent must already be a real directory",
        )
        ...
        json.dumps(
            {
                ...
                "lock_file_sha256": _sha256_file(output, "v4 lock"),
