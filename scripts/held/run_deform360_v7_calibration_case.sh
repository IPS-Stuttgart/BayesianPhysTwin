...
# Normalize before inspecting the deployment or dataset.  The formal shard
# invokes this file through an absolute /bin/bash under env -i; this re-exec
# also makes direct diagnostic invocation fail closed to the same environment.
if [[ "${BPT_HELD_V7_CALIBRATION_CASE_ENV_NORMALIZED:-}" != "1" ]]; then
  exec env -i \
    ...
    BPT_HELD_V7_CALIBRATION_CASE_ENV_NORMALIZED=1 \
    /bin/bash "$0" "$@"
fi
...
# This is a calibration-prefix runner only.  It deliberately accepts no path
...
while IFS='=' read -r environment_name _value; do
  case "$environment_name" in
    ...
    BPT_HELD_V7_CODE|BPT_HELD_V7_LOCK_VERIFIED_SHA256|\
    BPT_HELD_V7_CALIBRATION_CASE_ENV_NORMALIZED)
      ;;
    *) die "normalized calibration-case environment contains $environment_name" ;;
  esac
done < <(env)

if [[ "$#" -ne 4 ]]; then
  die "usage: run_deform360_v7_calibration_case.sh CUDA_DEVICE CASE OBJECT EPISODE"
fi
...
# object, and episode arguments.  These are the exact 15 calibration cases.
case "$CASE_NAME:$OBJECT:$EPISODE" in
  ...
  170-spider-ep0007:170-spider:0007)
    ;;
  *) die "case tuple is outside the exact 15-case v7 calibration cohort" ;;
esac

readonly HELD="/mnt/corsair/florianpfaff/bpt-online-belief-v1/held-v7"
readonly CODE="${BPT_HELD_V7_CODE:?set BPT_HELD_V7_CODE to the immutable v7 deployment}"
...
[[ -d "$HELD" && ! -L "$HELD" ]] || die "held-v7 root is absent or a symlink"
[[ "$(readlink -f -- "$HELD")" == "$HELD" ]] || die "held-v7 root is not canonical"
[[ -d "$CODE" && ! -L "$CODE" ]] || die "v7 deployed code is absent or a symlink"
[[ "$(readlink -f -- "$CODE")" == "$CODE" ]] || die "v7 deployed code is not canonical"
...
[[ "$CASE_RUNNER" == "$CODE/scripts/held/run_deform360_v7_calibration_case.sh" ]] || \
...
[[ -z "$(find "$CASE_RUNNER" "$LOCK_OPERATOR" -maxdepth 0 -perm /222 -print -quit)" ]] || \
  die "tracked v7 operators are writable"
...
[[ "$ACTUAL_LOCK_OPERATOR_SHA256" == "$EXPECTED_LOCK_OPERATOR_SHA256" ]] || \
  die "lock operator checksum differs from the audited v7 verifier"
[[ -f "$LOCK" && ! -L "$LOCK" ]] || die "held-v7 lock is absent or a symlink"
[[ "$(readlink -f -- "$LOCK")" == "$LOCK" ]] || die "held-v7 lock is not canonical"
[[ "$(stat -c '%a' -- "$LOCK")" == "400" ]] || die "held-v7 lock mode is not 0400"
...
readonly VERIFIED_LOCK_SHA256="${BPT_HELD_V7_LOCK_VERIFIED_SHA256:?run cases through the verified v7 shard runner}"
...
[[ "$ACTUAL_LOCK_SHA256" == "$VERIFIED_LOCK_SHA256" ]] || \
  die "held-v7 lock changed after startup verification"

# Recompute the full deployed Git snapshot, all 118 immutable bindings, and
# exact v1-through-v6 ancestry immediately before this case accesses payloads.
env -i \
  HOME=/home/florianpfaff \
  USER=florianpfaff \
  LOGNAME=florianpfaff \
...
[[ "$REVERIFIED_LOCK_SHA256" == "$VERIFIED_LOCK_SHA256" ]] || \
  die "held-v7 lock changed during case-level deployment verification"
...
# Refuse symlinks in every output parent.  This prevents a nominal held-v7
...
if result["case_name"] != sys.argv[3] or result["role"] != "calibration":
