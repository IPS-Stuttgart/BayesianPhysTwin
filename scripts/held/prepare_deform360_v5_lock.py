#!/usr/bin/env python3
"""Create the prospective Deform360 held-v5 lock from audited lineage.
...
v5 contracts, preserves unchanged external identities from v1, binds the exact
failed-closed v4 execution report, and admits only the dedicated frozen v5
Python runtime.
...
EXPECTED_V5_MIGRATION_KEY_COUNT = 21
...
LOCAL_FILE_BINDINGS: Mapping[str, str] = {
    ...
    "held_protocol_lock_operator_source": ("scripts/held/prepare_deform360_v5_lock.py"),
...
# These are the only v1 values that v5 may inherit.  They identify runtimes,
...
# These v5 external identities are not inherited from the v1 lock.  They bind
...
V5_PINNED_EXTERNAL_BINDING_VALUES: Mapping[str, str] = {
...
V5_PINNED_EXTERNAL_BINDING_KEYS = frozenset(V5_PINNED_EXTERNAL_BINDING_VALUES)
...
def _require_deployed_semantic_binding_exports(
    *,
    ...
    _require(
        {
            key: V5_PINNED_EXTERNAL_BINDING_VALUES[key]
    ...
    _require(
        V5_PINNED_SEMANTIC_GATE_CONTRACT_SHA256 == semantic_gate_contract_sha256,
...
V5_ONLY_BINDING_KEYS = frozenset(
    {
        "deform360_dataset_containment_source",
        "deform360_robot_kinematics_source",
        "frame_zero_semantic_gate_contract",
...
def main() -> int:
    ...
    _require(
        code.parent == _CANONICAL_HELD_V5_ROOT,
        "deployed code is outside the canonical fresh held-v5 root",
    )
    _require(
        ...
        "canonical held-v5 root is absent, a symlink, or aliased",
    )
    _require(
        _absolute(arguments.output_lock) == _CANONICAL_V5_LOCK,
        "v5 lock path is outside the canonical fresh held-v5 root",
    )
    if not arguments.verify_existing_lock:
        with os.scandir(_CANONICAL_HELD_V5_ROOT) as entries:
    ...
    expected_operator = code / "scripts" / "held" / "prepare_deform360_v5_lock.py"
    operator = _absolute(Path(__file__))
    _require(
        not Path(__file__).is_symlink() and operator == expected_operator,
        "lock operator is not the tracked deployed v5 operator",
    )
    ...
    _require(PROTOCOL_ID == "deform360-held-online-belief-v5", "not v5 code")
    ...
    binding_groups = (
        ...
        set(V5_PINNED_EXTERNAL_BINDING_KEYS),
        local_files,
    ...
    expected_v1_keys = required - set(V5_ONLY_BINDING_KEYS)
    _require(set(old_bindings) == expected_v1_keys, "exact v1 binding keys changed")
    _require(
        set(V5_ONLY_BINDING_KEYS) == required - set(old_bindings)
        and len(V5_ONLY_BINDING_KEYS) == EXPECTED_V5_MIGRATION_KEY_COUNT,
        "exact v5 migration-key delta changed",
    )
    _require(
        len(required) == EXPECTED_V5_BINDING_COUNT,
        "exact v5 immutable-binding count changed",
    )
    bindings = {
        key: old_bindings[key] for key in sorted(INHERITED_EXTERNAL_BINDING_KEYS)
    }
    bindings.update(V5_PINNED_EXTERNAL_BINDING_VALUES)
    ...
    _require(set(bindings) == required, "prospective v5 binding key set differs")
    _require(
        all(_SHA256.fullmatch(value) for value in bindings.values()),
        "prospective v5 contains an invalid digest",
    )
    ...
    _require(
        ...
        "v4 execution-withdrawal evidence changed during v5 preparation",
    )
    if arguments.verify_existing_lock:
        output = _absolute(arguments.output_lock)
        _ensure_unprotected_path(output, "existing v5 lock")
        _require(output.name == "calibration-lock.json", "v5 lock filename changed")
        _require(code not in output.parents, "v5 lock may not be inside deployed code")
        raw_lock = _read_regular_bytes(output, "existing v5 lock")
        existing_mode = os.lstat(output).st_mode
        _require(
            stat.S_IMODE(existing_mode) == 0o400,
            "existing v5 lock mode is not exactly 0400",
        )
        lock = load_held_protocol_lock(output)
        _require(
            lock.get("immutable_bindings") == dict(sorted(bindings.items())),
            "existing v5 lock bindings disagree with recomputed bindings",
        )
        canonical_lock = (
            json.dumps(lock, indent=2, sort_keys=True, allow_nan=False) + "\n"
        ).encode("utf-8")
        _require(raw_lock == canonical_lock, "existing v5 lock file is not canonical")
        operation = "verified_existing_lock"
    else:
        output = _absolute(arguments.output_lock)
        _ensure_unprotected_path(output, "prospective v5 lock output")
        _require(output.name == "calibration-lock.json", "v5 lock filename changed")
        _require(code not in output.parents, "v5 lock may not be inside deployed code")
        _require(not os.path.lexists(output), "prospective v5 lock already exists")
        _require(
            output.parent.is_dir() and not output.parent.is_symlink(),
            "v5 lock parent must already be a real directory",
        )
        ...
        json.dumps(
            {
                ...
                "lock_file_sha256": _sha256_file(output, "v5 lock"),
