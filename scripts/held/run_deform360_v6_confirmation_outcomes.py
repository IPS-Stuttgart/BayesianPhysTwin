#!/usr/bin/env python3
"""Run the permit-gated held-v6 Deform360 confirmation outcome phase.
...
def _verify_locked_deployment(arguments: "DriverArguments") -> Path:
    ...
    expected_driver = operator_dir / "run_deform360_v6_confirmation_outcomes.py"
...
def _promote_confirmation_lock(arguments: DriverArguments, protocol: Any) -> int:
    ...
    _require_mode_0400(destination, "held-v6 confirmation lock")
...
def _validate_confirmation_lock_lineage(
    ...
    _require_mode_0400(confirmation_lock, "held-v6 confirmation lock")
...
def _validate_held_layout_after_barrier(
    ...
    _require(
        ...
        "confirmation lock is outside the canonical held-v6 root",
    )
...
def _execute_driver(
    ...
    _require(len(expected_cases) == 6, "held-v6 confirmation cohort is not six cases")
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "exact held-v6 six-case confirmation cohort."
        )
    )
