#!/usr/bin/env python3
"""Run the permit-gated held-v4 Deform360 calibration outcome phase.

This operator driver has one deliberately narrow job:

1. validate the exact 15-case online-prediction cohort and obtain the live
   calibration outcome capability;
2. only after that barrier, create or revalidate each locked official target
   through the pinned reconstruction adapter;
3. score the fixed primary method and comparator; and
4. write the immutable score evidence and frozen GO/NO-GO decision.
...
``--dry-run-barrier-only`` mode stops immediately after step 1 and does not
...
EXPECTED_PROTOCOL_ID = "deform360-held-online-belief-v4"
EXPECTED_V4_BINDING_COUNT = 110
NO_GO_EXIT_CODE = 3
_CANONICAL_HELD_BASE = Path("/mnt/corsair/florianpfaff/bpt-online-belief-v1")
_CANONICAL_HELD_ROOT = _CANONICAL_HELD_BASE / "held-v4"
...
_CANONICAL_V2_WITHDRAWAL_REPORT = (
    _CANONICAL_HELD_ROOT / "v2-design-withdrawal-report.json"
)
_CANONICAL_V3_BOUNDARY_INCIDENT_REPORT = (
    _CANONICAL_HELD_ROOT / "v3-prelock-boundary-incident-report.json"
)
_CANONICAL_ALIGNED_ROOT = Path(
    "/mnt/lexar4tb/datasets/deform360/data-7fea8e2/replication-v1/aligned"
)
_PINNED_PYTHON = "/home/florianpfaff/.venvs/bpt-gpu/bin/python"
_PINNED_PATH = "/home/florianpfaff/.venvs/bpt-gpu/bin:/usr/local/bin:/usr/bin:/bin"
...
EXPECTED_LOCK_OPERATOR_SHA256 = (
    "b5cfa123a322ff1753227b232cd079dd259e8e5b6621dcd6bdc03211d15e7b3f"
)
...
def _verify_locked_deployment(arguments: "DriverArguments") -> Path:
    ...
    _require(
        ...
        "held root is not the canonical fresh held-v4 root",
    )
    ...
    _require(
        ...
        "deployed v4 code is absent, linked, or non-canonical",
    )
    _require(
        ...
        "deployed code is outside canonical held-v4 code-$HEAD",
    )
    lock = Path(arguments.lock)
    _require(
        lock == _CANONICAL_HELD_ROOT / "calibration-lock.json"
        and not lock.is_symlink(),
        "calibration lock is outside the canonical held-v4 root",
    )
    _require_mode_0400(lock, "held-v4 calibration lock")
    for path, role in (
    ...
    lock_operator = operator_dir / "prepare_deform360_v4_lock.py"
    expected_driver = operator_dir / "run_deform360_v4_calibration_outcomes.py"
    observed_driver = Path(__file__)
    _require(
        not observed_driver.is_symlink()
        and observed_driver.resolve() == expected_driver,
        "outcome driver is not the tracked deployed v4 source",
    )
    _require(
        lock_operator.is_file() and not lock_operator.is_symlink(),
        "tracked v4 lock operator is absent or linked",
    )
    ...
    _require(
        _sha256_regular_file(lock_operator, "tracked v4 lock operator")
        == EXPECTED_LOCK_OPERATOR_SHA256,
        "lock operator checksum differs from the audited v4 verifier",
    )
    ...
    _require(
        ...
        "source-only v4 deployment verification failed: "
        + completed.stderr.decode("utf-8", "replace").strip(),
    )
    try:
        verification = json.loads(completed.stdout.decode("utf-8"))
    except (UnicodeDecodeError, json.JSONDecodeError) as error:
        raise ValueError("v4 deployment verifier returned malformed JSON") from error
    _require(
        isinstance(verification, Mapping)
        and verification.get("operation") == "verified_existing_lock"
        and verification.get("protocol_id") == EXPECTED_PROTOCOL_ID
        and verification.get("binding_count") == EXPECTED_V4_BINDING_COUNT
        and verification.get("method_head") == code_argument.name.removeprefix("code-")
        and verification.get("lock_file_sha256")
        == _sha256_regular_file(lock, "held-v4 calibration lock"),
        "v4 deployment-verifier identity disagrees with requested deployment",
    )
...
def _validate_held_layout_after_barrier(
    ...
    _require(
        ...
        "held-v4 root must be an existing non-symlink directory",
    )
    ...
    for case_name in expected_cases:
        ...
        _require(
            path == canonical,
            f"online seal is outside the canonical held-v4 case path: {case_name}",
        )
...
def _prepare_fresh_gate_outputs_after_barrier(layout: HeldLayout) -> None:
    ...
    _require(
        ...
        f"outcome root already exists; exact v4 outcome resume is forbidden: {layout.outcomes_root}",
    )
...
def _execute_driver(
    ...
    _require(
        ...
        "deployed protocol is not held-v4",
    )
    expected_cases = tuple(protocol.CALIBRATION_CASE_NAMES)
    _require(len(expected_cases) == 15, "held-v4 calibration cohort is not 15 cases")
...
def _load_protocol(deployed_code: str) -> tuple[Any, Path]:
    ...
    _require(
        ...
        "deployed v4 code must be an existing non-symlink directory",
    )
    ...
    _require(
        ...
        "held protocol imported from outside the deployed v4 snapshot",
    )
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "exact held-v4 15-case calibration cohort."
        )
    )
...
def _self_check() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="bpt-outcome-driver-self-check-") as temp:
        ...
        _require(go_code == 0 and events[0] == "authorize", "GO was not barrier-first")
        _require(
        ...
        _require(
            no_go_code == NO_GO_EXIT_CODE and events[0] == "authorize",
            "NO-GO was not barrier-first and non-zero",
        )
        _require(
            events.count("score") == 1 and events[-1] == "score",
