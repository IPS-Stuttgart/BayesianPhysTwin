...
if [[ "${BPT_HELD_V6_CALIBRATION_SHARD_ENV_NORMALIZED:-}" != "1" ]]; then
  exec env -i \
    ...
    BPT_HELD_V6_CALIBRATION_SHARD_ENV_NORMALIZED=1 \
    /bin/bash "$0" "$@"
fi
...
while IFS='=' read -r environment_name _value; do
  case "$environment_name" in
    ...
    BPT_HELD_V6_CODE|BPT_HELD_V6_CALIBRATION_SHARD_ENV_NORMALIZED)
      ;;
...
if [[ "$#" -lt 1 || "$#" -gt 2 ]]; then
  die "usage: run_deform360_v6_calibration_shard.sh SHARD_INDEX [CUDA_DEVICE]"
fi
...
readonly CASE_RUNNER="$SCRIPT_DIR/run_deform360_v6_calibration_case.sh"
...
readonly BPT_HELD_V6_LOCK="$HELD/calibration-lock.json"
...
readonly OUTCOME_DRIVER="$SCRIPT_DIR/run_deform360_v6_calibration_outcomes.py"
...
[[ -f "$BPT_HELD_V6_LOCK" && ! -L "$BPT_HELD_V6_LOCK" ]] || die "held-v6 lock is absent or a symlink"
[[ "$(readlink -f -- "$BPT_HELD_V6_LOCK")" == "$BPT_HELD_V6_LOCK" ]] || \
  die "held-v6 lock is not canonical"
[[ "$(stat -c '%a' -- "$BPT_HELD_V6_LOCK")" == "400" ]] || die "held-v6 lock mode is not 0400"
...
readonly BPT_HELD_V6_LOCK_VERIFIED_SHA256="$(sha256sum -- "$BPT_HELD_V6_LOCK" | awk '{print $1}')"
[[ "$BPT_HELD_V6_LOCK_VERIFIED_SHA256" =~ ^[0-9a-f]{64}$ ]] || die "invalid verified lock digest"
export BPT_HELD_V6_CODE BPT_HELD_V6_LOCK BPT_HELD_V6_LOCK_VERIFIED_SHA256
...
echo "SHARD_START shard=$SHARD_INDEX cuda_device=$CUDA_DEVICE case_count=${#CASE_SPECS[@]} lock_sha256=$BPT_HELD_V6_LOCK_VERIFIED_SHA256"
for spec in "${CASE_SPECS[@]}"; do
  IFS=: read -r case_name object_id episode_id <<<"$spec"
  env -i \
    HOME=/home/florianpfaff \
    ...
    PYTHONPYCACHEPREFIX="$PYCACHE_PREFIX" \
    BPT_HELD_V6_CODE="$BPT_HELD_V6_CODE" \
    BPT_HELD_V6_LOCK_VERIFIED_SHA256="$BPT_HELD_V6_LOCK_VERIFIED_SHA256" \
