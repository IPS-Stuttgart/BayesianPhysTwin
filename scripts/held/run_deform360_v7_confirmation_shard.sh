...
if [[ "${BPT_HELD_V7_CONFIRMATION_SHARD_ENV_NORMALIZED:-}" != "1" ]]; then
  exec env -i \
    ...
    LANG=C.UTF-8 \
    LC_ALL=C.UTF-8 \
    BPT_HELD_V7_CODE="${BPT_HELD_V7_CODE:-}" \
    BPT_HELD_V7_CONFIRMATION_SHARD_ENV_NORMALIZED=1 \
    /bin/bash "$0" "$@"
fi
...
while IFS='=' read -r environment_name _value; do
  case "$environment_name" in
    ...
    BPT_HELD_V7_CODE|BPT_HELD_V7_CONFIRMATION_SHARD_ENV_NORMALIZED)
      ;;
...
if [[ "$#" -lt 1 || "$#" -gt 2 ]]; then
  die "usage: run_deform360_v7_confirmation_shard.sh SHARD_INDEX [CUDA_DEVICE]"
fi
...
readonly CASE_RUNNER="$SCRIPT_DIR/run_deform360_v7_confirmation_case.sh"
...
[[ "$(hostname)" == "workstation2" ]] || \
  die "formal held-v7 shards must run together on gpuserver6000/workstation2"
...
readonly V6_HELD="/mnt/corsair/florianpfaff/bpt-online-belief-v1/held-v6"
readonly V6_LOCK="$V6_HELD/calibration-lock.json"
readonly V6_OUTCOME_WITHDRAWAL_REPORT="$V6_HELD/v6-outcome-withdrawal-report.json"
readonly GSPLAT_RUNTIME_SUPPLEMENT_MANIFEST="/mnt/corsair/florianpfaff/bpt-held-v7-runtimes/gsplat-cuda-py312-cu121-2dd5e0c2a349619e1afc3dd041086eca900b387602bc76627b7f54264fffec64/runtime-supplement-manifest.json"
readonly GSPLAT_RUNTIME_SMOKE_EVIDENCE="$HELD/gsplat-runtime-smoke-evidence.json"
...
readonly BPT_HELD_V7_CODE="${BPT_HELD_V7_CODE:?set BPT_HELD_V7_CODE to the immutable v7 deployment}"
readonly LOCK_OPERATOR="$SCRIPT_DIR/prepare_deform360_v7_lock.py"
readonly OUTCOME_DRIVER="$SCRIPT_DIR/run_deform360_v7_confirmation_outcomes.py"
# Updated mechanically after the final lock-operator source is frozen.
readonly EXPECTED_LOCK_OPERATOR_SHA256="3d042d1cbfcaefa550446b8161a3d2dd5939dfc8d6725498a3faeac11e93e65e"
...
readonly PYCACHE_PREFIX="/nonexistent/bpt-held-v7-pycache"
readonly RUN="$HELD/confirmation"
readonly LOG_DIR="$RUN/logs"

cd -- "$BPT_HELD_V7_CODE"
[[ "$PWD" == "$BPT_HELD_V7_CODE" ]] || die "failed to enter immutable deployed code root"
...
[[ -d "$HELD" && ! -L "$HELD" && "$(readlink -f -- "$HELD")" == "$HELD" ]] || \
  die "held-v7 root is absent, a symlink, or non-canonical"
[[ -d "$BPT_HELD_V7_CODE" && ! -L "$BPT_HELD_V7_CODE" ]] || \
  die "v7 deployed code is absent or a symlink"
[[ "$(readlink -f -- "$BPT_HELD_V7_CODE")" == "$BPT_HELD_V7_CODE" ]] || \
  die "v7 deployed code is not canonical"
[[ "$BPT_HELD_V7_CODE" =~ ^${HELD}/code-([0-9a-f]{40}|[0-9a-f]{64})$ ]] || \
  die "v7 deployed code is outside the canonical immutable snapshot path"
[[ "$SCRIPT_DIR" == "$BPT_HELD_V7_CODE/scripts/held" ]] || \
  die "operator bundle is outside the deployed v7 snapshot"
[[ -z "$(find "$BPT_HELD_V7_CODE" -xdev -perm /222 -print -quit)" ]] || \
  die "v7 deployed code tree is writable"
[[ -f "$CALIBRATION_LOCK" && ! -L "$CALIBRATION_LOCK" && "$(stat -c '%a' -- "$CALIBRATION_LOCK")" == "400" ]] || \
  die "held-v7 calibration parent lock is absent, mutable, or a symlink"
[[ -f "$CALIBRATION_DECISION" && ! -L "$CALIBRATION_DECISION" ]] || \
  die "held-v7 calibration GO decision is absent or a symlink"
[[ -f "$CONFIRMATION_LOCK" && ! -L "$CONFIRMATION_LOCK" ]] || die "held-v7 confirmation lock is absent or a symlink"
[[ "$(readlink -f -- "$CONFIRMATION_LOCK")" == "$CONFIRMATION_LOCK" ]] || \
  die "held-v7 confirmation lock is not canonical"
[[ "$(stat -c '%a' -- "$CONFIRMATION_LOCK")" == "400" ]] || die "held-v7 confirmation lock mode is not 0400"
...
[[ -f "$V6_LOCK" && ! -L "$V6_LOCK" && "$(stat -c '%a' -- "$V6_LOCK")" == "400" ]] || \
  die "sealed v6 calibration lock is absent, mutable, or a symlink"
[[ -f "$V6_OUTCOME_WITHDRAWAL_REPORT" && ! -L "$V6_OUTCOME_WITHDRAWAL_REPORT" && "$(stat -c '%a' -- "$V6_OUTCOME_WITHDRAWAL_REPORT")" == "400" ]] || \
  die "sealed v6 outcome-withdrawal report is absent, mutable, or a symlink"
[[ -f "$GSPLAT_RUNTIME_SUPPLEMENT_MANIFEST" && ! -L "$GSPLAT_RUNTIME_SUPPLEMENT_MANIFEST" && "$(stat -c '%a' -- "$GSPLAT_RUNTIME_SUPPLEMENT_MANIFEST")" == "400" ]] || \
  die "gsplat runtime supplement manifest is absent, mutable, or a symlink"
[[ -f "$GSPLAT_RUNTIME_SMOKE_EVIDENCE" && ! -L "$GSPLAT_RUNTIME_SMOKE_EVIDENCE" && "$(stat -c '%a' -- "$GSPLAT_RUNTIME_SMOKE_EVIDENCE")" == "400" ]] || \
  die "pre-lock gsplat runtime smoke evidence is absent, mutable, or a symlink"
...
[[ ! -e /nonexistent && ! -L /nonexistent && ! -e "$PYCACHE_PREFIX" && ! -L "$PYCACHE_PREFIX" ]] || \
  die "reserved held-v7 pycache prefix is no longer unavailable"

# Refuse a pre-existing symlink at either output-parent level before mkdir -p
# can follow it outside held-v7.
...
# Mandatory, non-mutating verification recomputes all 118 bindings and the
# canonical lock bytes from the immutable deployment and audited v1-v6 ancestry.
if env -i \
  HOME=/home/florianpfaff \
  USER=florianpfaff \
  LOGNAME=florianpfaff \
  ...
  --v6-lock "$V6_LOCK" \
  --v6-outcome-withdrawal-report "$V6_OUTCOME_WITHDRAWAL_REPORT" \
  --gsplat-runtime-supplement-manifest "$GSPLAT_RUNTIME_SUPPLEMENT_MANIFEST" \
  --gsplat-runtime-smoke-evidence "$GSPLAT_RUNTIME_SMOKE_EVIDENCE" \
  --deployed-code "$BPT_HELD_V7_CODE" \
...
if ! env -i \
  ...
  PYTHONHASHSEED=0 \
  PYTHONPYCACHEPREFIX="$PYCACHE_PREFIX" \
  "$PY" -B -X "pycache_prefix=$PYCACHE_PREFIX" -I - "$BPT_HELD_V7_CODE/src" "$CONFIRMATION_LOCK" "$CALIBRATION_LOCK" "$CALIBRATION_DECISION" \
  >>"$VERIFY_LOG" 2>&1 <<'PY'
import json
...
if lock.get("stage") != "confirmation":
    raise RuntimeError("held-v7 confirmation lock has another stage")
...
readonly BPT_HELD_V7_CONFIRMATION_LOCK_VERIFIED_SHA256="$(sha256sum -- "$CONFIRMATION_LOCK" | awk '{print $1}')"
[[ "$BPT_HELD_V7_CONFIRMATION_LOCK_VERIFIED_SHA256" =~ ^[0-9a-f]{64}$ ]] || die "invalid verified confirmation-lock digest"
export BPT_HELD_V7_CODE BPT_HELD_V7_CONFIRMATION_LOCK_VERIFIED_SHA256

# Refuse a partial/reused v7 shard up front.  No v1, v2, or v3 prediction path is
...
for spec in "${CASE_SPECS[@]}"; do
  ...
  [[ ! -e "$RUN/cases/$case_name" && ! -L "$RUN/cases/$case_name" ]] || \
    die "v7 case output already exists: $case_name"
  if compgen -G "$LOG_DIR/$case_name.*" >/dev/null; then
    die "v7 case log already exists: $case_name"
  fi
done
...
mkdir -- "$SHARD_CLAIM" 2>/dev/null || die "this v7 shard was already claimed"

echo "SHARD_START shard=$SHARD_INDEX cuda_device=$CUDA_DEVICE case_count=${#CASE_SPECS[@]} lock_sha256=$BPT_HELD_V7_CONFIRMATION_LOCK_VERIFIED_SHA256"
for spec in "${CASE_SPECS[@]}"; do
  IFS=: read -r case_name object_id episode_id <<<"$spec"
  env -i \
    HOME=/home/florianpfaff \
    ...
    PYTHONPYCACHEPREFIX="$PYCACHE_PREFIX" \
    BPT_HELD_V7_CODE="$BPT_HELD_V7_CODE" \
    BPT_HELD_V7_CONFIRMATION_LOCK_VERIFIED_SHA256="$BPT_HELD_V7_CONFIRMATION_LOCK_VERIFIED_SHA256" \
