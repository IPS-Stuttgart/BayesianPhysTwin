...
if [[ "${BPT_HELD_V5_CALIBRATION_CASE_ENV_NORMALIZED:-}" != "1" ]]; then
  exec env -i \
    ...
    BPT_HELD_V5_CALIBRATION_CASE_ENV_NORMALIZED=1 \
    /bin/bash "$0" "$@"
fi
...
while IFS='=' read -r environment_name _value; do
  case "$environment_name" in
    ...
    BPT_HELD_V5_CODE|BPT_HELD_V5_LOCK_VERIFIED_SHA256|\
    BPT_HELD_V5_CALIBRATION_CASE_ENV_NORMALIZED)
      ;;
...
if [[ "$#" -ne 4 ]]; then
  die "usage: run_deform360_v5_calibration_case.sh CUDA_DEVICE CASE OBJECT EPISODE"
fi
...
case "$CASE_NAME:$OBJECT:$EPISODE" in
  ...
  170-spider-ep0007:170-spider:0007)
    ;;
  *) die "case tuple is outside the exact 15-case v5 calibration cohort" ;;
esac

readonly HELD="/mnt/corsair/florianpfaff/bpt-online-belief-v1/held-v5"
readonly CODE="${BPT_HELD_V5_CODE:?set BPT_HELD_V5_CODE to the immutable v5 deployment}"
...
[[ -d "$HELD" && ! -L "$HELD" ]] || die "held-v5 root is absent or a symlink"
[[ "$(readlink -f -- "$HELD")" == "$HELD" ]] || die "held-v5 root is not canonical"
[[ -d "$CODE" && ! -L "$CODE" ]] || die "v5 deployed code is absent or a symlink"
[[ "$(readlink -f -- "$CODE")" == "$CODE" ]] || die "v5 deployed code is not canonical"
...
[[ "$CASE_RUNNER" == "$CODE/scripts/held/run_deform360_v5_calibration_case.sh" ]] || \
...
[[ -z "$(find "$CASE_RUNNER" "$LOCK_OPERATOR" -maxdepth 0 -perm /222 -print -quit)" ]] || \
  die "tracked v5 operators are writable"
...
[[ "$ACTUAL_LOCK_OPERATOR_SHA256" == "$EXPECTED_LOCK_OPERATOR_SHA256" ]] || \
  die "lock operator checksum differs from the audited v5 verifier"
[[ -f "$LOCK" && ! -L "$LOCK" ]] || die "held-v5 lock is absent or a symlink"
[[ "$(readlink -f -- "$LOCK")" == "$LOCK" ]] || die "held-v5 lock is not canonical"
[[ "$(stat -c '%a' -- "$LOCK")" == "400" ]] || die "held-v5 lock mode is not 0400"
...
readonly VERIFIED_LOCK_SHA256="${BPT_HELD_V5_LOCK_VERIFIED_SHA256:?run cases through the verified v5 shard runner}"
...
[[ "$ACTUAL_LOCK_SHA256" == "$VERIFIED_LOCK_SHA256" ]] || \
  die "held-v5 lock changed after startup verification"

# Recompute the full deployed Git snapshot, all 112 immutable bindings, and
# exact v1-through-v4 ancestry immediately before this case accesses payloads.
env -i \
  HOME=/home/florianpfaff \
  USER=florianpfaff \
  LOGNAME=florianpfaff \
...
[[ "$REVERIFIED_LOCK_SHA256" == "$VERIFIED_LOCK_SHA256" ]] || \
  die "held-v5 lock changed during case-level deployment verification"
...
# Refuse symlinks in every output parent.  This prevents a nominal held-v5
