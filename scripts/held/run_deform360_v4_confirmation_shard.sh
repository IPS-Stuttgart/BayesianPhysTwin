...
if [[ "${BPT_HELD_V4_CONFIRMATION_SHARD_ENV_NORMALIZED:-}" != "1" ]]; then
  exec env -i \
    ...
    LANG=C.UTF-8 \
    LC_ALL=C.UTF-8 \
    BPT_HELD_V4_CODE="${BPT_HELD_V4_CODE:-}" \
    BPT_HELD_V4_CONFIRMATION_SHARD_ENV_NORMALIZED=1 \
    /bin/bash "$0" "$@"
fi

set -Eeuo pipefail
umask 077
export PATH=/home/florianpfaff/.venvs/bpt-gpu/bin:/usr/local/bin:/usr/bin:/bin
...
while IFS='=' read -r environment_name _value; do
  case "$environment_name" in
    ...
    BPT_HELD_V4_CODE|BPT_HELD_V4_CONFIRMATION_SHARD_ENV_NORMALIZED)
      ;;
...
if [[ "$#" -lt 1 || "$#" -gt 2 ]]; then
  die "usage: run_deform360_v4_confirmation_shard.sh SHARD_INDEX [CUDA_DEVICE]"
fi
...
readonly CASE_RUNNER="$SCRIPT_DIR/run_deform360_v4_confirmation_case.sh"
...
[[ "$(hostname)" == "workstation2" ]] || \
  die "formal held-v4 shards must run together on gpuserver6000/workstation2"
...
readonly HELD="/mnt/corsair/florianpfaff/bpt-online-belief-v1/held-v4"
...
readonly V2_WITHDRAWAL_REPORT="$HELD/v2-design-withdrawal-report.json"
readonly V3_BOUNDARY_INCIDENT_REPORT="$HELD/v3-prelock-boundary-incident-report.json"
...
readonly BPT_HELD_V4_CODE="${BPT_HELD_V4_CODE:?set BPT_HELD_V4_CODE to the immutable v4 deployment}"
readonly LOCK_OPERATOR="$SCRIPT_DIR/prepare_deform360_v4_lock.py"
readonly OUTCOME_DRIVER="$SCRIPT_DIR/run_deform360_v4_confirmation_outcomes.py"
# Updated mechanically after the final lock-operator source is frozen.
readonly EXPECTED_LOCK_OPERATOR_SHA256="b5cfa123a322ff1753227b232cd079dd259e8e5b6621dcd6bdc03211d15e7b3f"
readonly PY="/home/florianpfaff/.venvs/bpt-gpu/bin/python"
readonly PYCACHE_PREFIX="/nonexistent/bpt-held-v4-pycache"
readonly RUN="$HELD/confirmation"
readonly LOG_DIR="$RUN/logs"

cd -- "$BPT_HELD_V4_CODE"
[[ "$PWD" == "$BPT_HELD_V4_CODE" ]] || die "failed to enter immutable deployed code root"
...
[[ -d "$HELD" && ! -L "$HELD" && "$(readlink -f -- "$HELD")" == "$HELD" ]] || \
  die "held-v4 root is absent, a symlink, or non-canonical"
[[ -d "$BPT_HELD_V4_CODE" && ! -L "$BPT_HELD_V4_CODE" ]] || \
  die "v4 deployed code is absent or a symlink"
[[ "$(readlink -f -- "$BPT_HELD_V4_CODE")" == "$BPT_HELD_V4_CODE" ]] || \
  die "v4 deployed code is not canonical"
[[ "$BPT_HELD_V4_CODE" =~ ^${HELD}/code-([0-9a-f]{40}|[0-9a-f]{64})$ ]] || \
  die "v4 deployed code is outside the canonical immutable snapshot path"
[[ "$SCRIPT_DIR" == "$BPT_HELD_V4_CODE/scripts/held" ]] || \
  die "operator bundle is outside the deployed v4 snapshot"
[[ -z "$(find "$BPT_HELD_V4_CODE" -xdev -perm /222 -print -quit)" ]] || \
  die "v4 deployed code tree is writable"
[[ -f "$CALIBRATION_LOCK" && ! -L "$CALIBRATION_LOCK" && "$(stat -c '%a' -- "$CALIBRATION_LOCK")" == "400" ]] || \
  die "held-v4 calibration parent lock is absent, mutable, or a symlink"
[[ -f "$CALIBRATION_DECISION" && ! -L "$CALIBRATION_DECISION" ]] || \
  die "held-v4 calibration GO decision is absent or a symlink"
[[ -f "$CONFIRMATION_LOCK" && ! -L "$CONFIRMATION_LOCK" ]] || die "held-v4 confirmation lock is absent or a symlink"
[[ "$(readlink -f -- "$CONFIRMATION_LOCK")" == "$CONFIRMATION_LOCK" ]] || \
  die "held-v4 confirmation lock is not canonical"
[[ "$(stat -c '%a' -- "$CONFIRMATION_LOCK")" == "400" ]] || die "held-v4 confirmation lock mode is not 0400"
...
[[ ! -e /nonexistent && ! -L /nonexistent && ! -e "$PYCACHE_PREFIX" && ! -L "$PYCACHE_PREFIX" ]] || \
  die "reserved held-v4 pycache prefix is no longer unavailable"

# Refuse a pre-existing symlink at either output-parent level before mkdir -p
# can follow it outside held-v4.
...
# Mandatory, non-mutating verification recomputes all 110 bindings and the
# canonical lock bytes from the immutable deployment and audited v1/v2/v3 ancestry.
if env -i \
  HOME=/home/florianpfaff \
  USER=florianpfaff \
  LOGNAME=florianpfaff \
  ...
  --deployed-code "$BPT_HELD_V4_CODE" \
...
if ! env -i \
  ...
  PYTHONHASHSEED=0 \
  PYTHONPYCACHEPREFIX="$PYCACHE_PREFIX" \
  "$PY" -B -X "pycache_prefix=$PYCACHE_PREFIX" -I - "$BPT_HELD_V4_CODE/src" "$CONFIRMATION_LOCK" "$CALIBRATION_LOCK" "$CALIBRATION_DECISION" \
  >>"$VERIFY_LOG" 2>&1 <<'PY'
import json
...
if lock.get("stage") != "confirmation":
    raise RuntimeError("held-v4 confirmation lock has another stage")
...
readonly BPT_HELD_V4_CONFIRMATION_LOCK_VERIFIED_SHA256="$(sha256sum -- "$CONFIRMATION_LOCK" | awk '{print $1}')"
[[ "$BPT_HELD_V4_CONFIRMATION_LOCK_VERIFIED_SHA256" =~ ^[0-9a-f]{64}$ ]] || die "invalid verified confirmation-lock digest"
export BPT_HELD_V4_CODE BPT_HELD_V4_CONFIRMATION_LOCK_VERIFIED_SHA256

# Refuse a partial/reused v4 shard up front.  No v1, v2, or v3 prediction path is
...
for spec in "${CASE_SPECS[@]}"; do
  ...
  [[ ! -e "$RUN/cases/$case_name" && ! -L "$RUN/cases/$case_name" ]] || \
    die "v4 case output already exists: $case_name"
  if compgen -G "$LOG_DIR/$case_name.*" >/dev/null; then
    die "v4 case log already exists: $case_name"
  fi
done
...
mkdir -- "$SHARD_CLAIM" 2>/dev/null || die "this v4 shard was already claimed"

echo "SHARD_START shard=$SHARD_INDEX cuda_device=$CUDA_DEVICE case_count=${#CASE_SPECS[@]} lock_sha256=$BPT_HELD_V4_CONFIRMATION_LOCK_VERIFIED_SHA256"
for spec in "${CASE_SPECS[@]}"; do
  IFS=: read -r case_name object_id episode_id <<<"$spec"
  env -i \
    HOME=/home/florianpfaff \
    ...
    PYTHONPYCACHEPREFIX="$PYCACHE_PREFIX" \
    BPT_HELD_V4_CODE="$BPT_HELD_V4_CODE" \
    BPT_HELD_V4_CONFIRMATION_LOCK_VERIFIED_SHA256="$BPT_HELD_V4_CONFIRMATION_LOCK_VERIFIED_SHA256" \
