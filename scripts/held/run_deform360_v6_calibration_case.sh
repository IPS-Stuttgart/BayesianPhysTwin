...
if [[ "${BPT_HELD_V6_CALIBRATION_CASE_ENV_NORMALIZED:-}" != "1" ]]; then
  exec env -i \
    ...
    BPT_HELD_V6_CALIBRATION_CASE_ENV_NORMALIZED=1 \
    /bin/bash "$0" "$@"
fi
...
while IFS='=' read -r environment_name _value; do
  case "$environment_name" in
    ...
    BPT_HELD_V6_CODE|BPT_HELD_V6_LOCK_VERIFIED_SHA256|\
    BPT_HELD_V6_CALIBRATION_CASE_ENV_NORMALIZED)
      ;;
...
if [[ "$#" -ne 4 ]]; then
  die "usage: run_deform360_v6_calibration_case.sh CUDA_DEVICE CASE OBJECT EPISODE"
fi
...
case "$CASE_NAME:$OBJECT:$EPISODE" in
  ...
  170-spider-ep0007:170-spider:0007)
    ;;
  *) die "case tuple is outside the exact 15-case v6 calibration cohort" ;;
esac
...
[[ "$CASE_RUNNER" == "$CODE/scripts/held/run_deform360_v6_calibration_case.sh" ]] || \
...
readonly VERIFIED_LOCK_SHA256="${BPT_HELD_V6_LOCK_VERIFIED_SHA256:?run cases through the verified v6 shard runner}"
