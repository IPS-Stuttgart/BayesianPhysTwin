#!/usr/bin/env python3
"""Run the permit-gated held-v6 Deform360 calibration outcome phase.
...
EXPECTED_PROTOCOL_ID = "deform360-held-online-belief-v6"
EXPECTED_V6_BINDING_COUNT = 113
NO_GO_EXIT_CODE = 3
_CANONICAL_HELD_BASE = Path("/mnt/corsair/florianpfaff/bpt-online-belief-v1")
_CANONICAL_HELD_ROOT = _CANONICAL_HELD_BASE / "held-v6"
...
def _verify_locked_deployment(arguments: "DriverArguments") -> Path:
    ...
    _require(
        ...
        "held root is not the canonical fresh held-v6 root",
    )
    ...
    _require(
        ...
        "deployed v6 code is absent, linked, or non-canonical",
    )
    _require(
        ...
        "deployed code is outside canonical held-v6 code-$HEAD",
    )
    lock = Path(arguments.lock)
    _require(
        lock == _CANONICAL_HELD_ROOT / "calibration-lock.json"
        and not lock.is_symlink(),
        "calibration lock is outside the canonical held-v6 root",
    )
    _require_mode_0400(lock, "held-v6 calibration lock")
    for path, role in (
    ...
    lock_operator = operator_dir / "prepare_deform360_v6_lock.py"
    expected_driver = operator_dir / "run_deform360_v6_calibration_outcomes.py"
    observed_driver = Path(__file__)
    _require(
        not observed_driver.is_symlink()
        and observed_driver.resolve() == expected_driver,
        "outcome driver is not the tracked deployed v6 source",
    )
    _require(
        lock_operator.is_file() and not lock_operator.is_symlink(),
        "tracked v6 lock operator is absent or linked",
    )
    ...
    _require(
        _sha256_regular_file(lock_operator, "tracked v6 lock operator")
        == EXPECTED_LOCK_OPERATOR_SHA256,
        "lock operator checksum differs from the audited v6 verifier",
    )
    ...
    _require(
        ...
        "source-only v6 deployment verification failed: "
        + completed.stderr.decode("utf-8", "replace").strip(),
    )
    try:
        verification = json.loads(completed.stdout.decode("utf-8"))
    except (UnicodeDecodeError, json.JSONDecodeError) as error:
        raise ValueError("v6 deployment verifier returned malformed JSON") from error
    _require(
        isinstance(verification, Mapping)
        and verification.get("operation") == "verified_existing_lock"
        and verification.get("protocol_id") == EXPECTED_PROTOCOL_ID
        and verification.get("binding_count") == EXPECTED_V6_BINDING_COUNT
        and verification.get("method_head") == code_argument.name.removeprefix("code-")
        and verification.get("lock_file_sha256")
        == _sha256_regular_file(lock, "held-v6 calibration lock"),
        "v6 deployment-verifier identity disagrees with requested deployment",
    )
...
def _validate_held_layout_after_barrier(
    ...
    _require(
        ...
        "held-v6 root must be an existing non-symlink directory",
    )
    ...
    for case_name in expected_cases:
        ...
        _require(
            path == canonical,
            f"online seal is outside the canonical held-v6 case path: {case_name}",
        )
...
def _prepare_fresh_gate_outputs_after_barrier(layout: HeldLayout) -> None:
    ...
    _require(
        ...
        f"outcome root already exists; exact v6 outcome resume is forbidden: {layout.outcomes_root}",
    )
...
def _execute_driver(
    ...
    _require(
        ...
        "deployed protocol is not held-v6",
    )
    expected_cases = tuple(protocol.CALIBRATION_CASE_NAMES)
    _require(len(expected_cases) == 15, "held-v6 calibration cohort is not 15 cases")
...
def _load_protocol(deployed_code: str) -> tuple[Any, Path]:
    ...
    _require(
        ...
        "deployed v6 code must be an existing non-symlink directory",
    )
    ...
    _require(
        ...
        "held protocol imported from outside the deployed v6 snapshot",
    )
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "exact held-v6 15-case calibration cohort."
        )
    )
