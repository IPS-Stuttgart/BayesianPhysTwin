#!/usr/bin/env python3
"""Verify the continuous-interpolation wrapping source result."""
...
OUTPUT = Path("/home/fpfaff/source-only/dlolab-wrapping-continuous-interp-source-v2")
ATTEMPT = Path(
...
SUMMARY = ROOT / "results/sota/dlolab_wrapping_continuous_interp_source_v2/summary.json"
FROZEN_REVISION = "7683e29dcc1b2525ba5f8d35a83e0c0f66c1e201"
PREFLIGHT_ID = "e5aeac42cd8a4bd0b402650e3f28ffc5bc470f4b8a16760ce4748706054f16ab"
LOCK_ID = "c787cd3000f6f64d5478d800ceed5209ff1490cfd68aadf19fdb543c1cfb101b"
DECISION_ID = "e50b7f86a256a08d0781a9bd2fbc1ea0bc65a3c2379c2b20b341b8d2ef01e34b"
BARRIER_ID = "5b9ffad42827d61a12a5d16379ab5644e85479b736035e7db22e26b690f9de50"
GENERATION_ID = "28119ec7e65eb0fbfdac0b99613a51ecefe4b038b01d97e12f67941e83f3a1ca"
RESULT_ID = "2f95e41f51753881cdfe8ca77774cdd60c5d1f92a0552d5570e7299a3244b5bf"
...
def _independent_arithmetic(decisions: Array, rewards: Array) -> dict[str, Any]:
    ...
    paired: dict[str, dict[str, Any]] = {
        name: {
            ...
            "ci95": interval(
                world_reward[:, primary_index] - world_reward[:, index]
            ),
        }
        for index, name in enumerate(ARM_NAMES)
    ...
    fixed_gain = float(
        (world_reward[:, primary_index] - world_reward[:, 0]).mean()
    )
    return {
        "arms": arms,
        "paired": paired,
        ...
        "oracle_fraction": fixed_gain / headroom,
        "distinct_oracle_actions": int(len(np.unique(oracle_action))),
    }
...
def verify(output: Path) -> dict[str, Any]:
    ...
    if (
        attempt.get("schema") != "dlolab-wrapping-continuous-interp-attempt-v2"
        or attempt.get("revision") != FROZEN_REVISION
        or attempt.get("protocol") != protocol()
        or attempt.get("output_root") != str(OUTPUT)
        or lock.get("schema") != "dlolab-wrapping-continuous-interp-lock-v2"
    ...
    if (
        decision_seal.get("artifact_id") != DECISION_ID
    ...
    if (
        generation.get("artifact_id") != GENERATION_ID
    ...
    expected_score = score(
        decision["decisions"], reward, all_native_qa=all(prefix_qa)
    )
    ...
    if (
        ...
        or arithmetic["paired"] != result["paired_continuous_bayes_gain"]
        ...
        or arithmetic["oracle_fraction"]
        != result["oracle_headroom_fraction_captured"]
    ...
    if (
        summary.get("schema") != "dlolab-wrapping-continuous-interp-summary-v2"
        or summary.get("result_id") != RESULT_ID
        or summary.get("source_gate_passed") is not False
        or summary.get("continuous_bayes_gain_over_fixed")
        != result["paired_continuous_bayes_gain"][
            "continuous_prior_best_fixed"
        ]["mean_gain"]
        or summary.get("continuous_bayes_gain_over_finite_bayes")
        != result["paired_continuous_bayes_gain"]["finite_particle_bayes"][
            "mean_gain"
        ]
    ):
        raise ValueError("compact summary changed")
    return {
        "schema": "dlolab-wrapping-continuous-interp-verification-v2",
