#!/usr/bin/env python3
"""Generate the frozen candidate bank for one PokeFlex calibration object."""
...
def main() -> None:
    ...
    calibration = load_pokeflex_regret_guard_calibration_protocol(
        args.calibration_protocol.resolve()
    )
    if take_root.name not in calibration["take_ids"]:
        raise ValueError("take is outside the calibration cohort")
    candidate_path = (
        _repository_root()
        / "scripts"
        / "remote"
    ...
    expected_hash = calibration["payload"]["frozen_deployment"][
    ...
    calibration_objects = sorted(
        {take_id.rsplit("_T", 1)[0] for take_id in calibration["take_ids"]}
    )
    authorized_registration["payload"]["cohort"]["development_objects"] = (
        calibration_objects
    )

    original_loader = candidate_runner.load_pokeflex_registration_protocol

    def calibration_authorization_adapter(path: str | Path) -> dict[str, object]:
    ...
    candidate_runner.load_pokeflex_registration_protocol = (
        calibration_authorization_adapter
    )
    try:
    ...
    result["independent_object_calibration"] = {
        "protocol_id": calibration["payload"]["protocol_id"],
        "protocol_sha256": calibration["protocol_sha256"],
        "candidate_runner_sha256": expected_hash,
        "authorization_adapter_changed_only_allowed_object_identity": True,
    ...
    if output.exists() and output.read_text(encoding="utf-8") != rendered:
        raise ValueError(f"existing calibration artifact differs: {output}")
