#!/usr/bin/env python3
"""Project one frozen missing-five PokeFlex source ZIP for remote scoring."""
...
def main() -> None:
    ...
    if file_sha256(Path(__file__)) != expected_runner_hash:
    ...
    if take_id not in validation["source_take_ids"]:
        raise ValueError("archive is outside the frozen source cohort")
    expected = validation["archive_inventory"][take_id]
    ...
    payload: dict[str, object] = {
        "schema_version": 1,
        "artifact_kind": "PokeFlexMissingFiveScaleSourceProjection",
        "protocol_sha256": validation["protocol_sha256"],
        "projection_runner_file_sha256": expected_runner_hash,
        "take_id": take_id,
