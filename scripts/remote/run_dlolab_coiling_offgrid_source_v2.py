#!/usr/bin/env python3
"""Run the frozen public DLO-Lab off-grid coiling source screen once."""
...
from bayesian_phystwin_experiments.dlolab_coiling_native_v2 import run_world
...
OUTPUT = Path("/home/fpfaff/source-only/dlolab-coiling-offgrid-source-v2")
PARENT_SUMMARY = (
    ROOT
    / "results/source/dlolab_coiling_query_competence_development_v1_1/summary.json"
)
SOURCES = (
    "src/bayesian_phystwin_experiments/dlolab_coiling_offgrid_source_v2.py",
    "src/bayesian_phystwin_experiments/dlolab_coiling_native_v2.py",
    "scripts/remote/run_dlolab_coiling_offgrid_source_v2.py",
    "tests/test_dlolab_coiling_offgrid_source_v2.py",
    "tests/test_dlolab_coiling_offgrid_runner_v2.py",
    "docs/dlolab_coiling_offgrid_source_v2.md",
    "results/source/dlolab_coiling_query_competence_development_v1_1/summary.json",
...
def parent_result() -> dict[str, Any]:
    value = dict(
        load_strict_json_object(PARENT_SUMMARY, label="coiling development result")
    )
    if (
        file_digest(PARENT_SUMMARY) != PARENT_DEVELOPMENT_SUMMARY_SHA256
        or value.get("status") != "terminal_post_world_qa_serialization_failure"
        or value.get("completed_worlds") != 1
        or value.get("completed_world_rederived_qa_passed") is not True
        or value.get("source_bank_generated") is not False
        or value.get("value_analysis_executed") is not False
        or value.get("development_gate_evaluated") is not False
        or value.get("further_replacement_authorized") is not False
        or value.get("retry_authorized") is not False
        or value.get("protected_data_read") is not False
    ):
        raise ValueError("exact terminal coiling development result required")
    return value
...
def validate_lock(output: Path) -> dict[str, Any]:
    if output.resolve() != OUTPUT or output.is_symlink():
        raise ValueError("only the registered write-once off-grid root is permitted")
    if (output / "failure.json").exists() or (output / "result.json").exists():
        raise ValueError("terminal off-grid coiling source study; no retry")
    lock = read_record(output / "lock.json")
    expected_source = {path: file_digest(ROOT / path) for path in SOURCES}
    if (
        lock.get("schema") != "dlolab-coiling-offgrid-source-lock-v2"
        or lock.get("revision") != clean_revision(ROOT)
        or lock.get("protocol") != protocol()
        or lock.get("parent_result") != parent_result()
        ...
        raise ValueError("clean frozen off-grid implementation/runtime/source required")
...
def load_task(
    ...
    if (
        ...
        raise ValueError("native off-grid coiling task custody changed")
    arrays = load_native_bundle(directory, seal["bundle"])
    qa = rederive_native_qa(arrays, seal["native"], spec["world"])
...
def worker(output: Path, index: int) -> None:
    ...
    for previous in range(index):
        ...
        if (
            ...
            raise ValueError("all earlier off-grid coiling worlds must requalify")
    directory = output / spec["name"]
...
def run(output: Path) -> None:
    if output.resolve() != OUTPUT or output.exists() or output.is_symlink():
        raise ValueError("the registered off-grid coiling source root must be fresh")
    ...
    completed: list[str] = []
    attempted = 0
    stage = "native_source_worlds"

    def terminal(status: str, **extra: Any) -> dict[str, Any]:
        result = write_record(
            output / "result.json",
            {
                "schema": "dlolab-coiling-offgrid-source-result-v2",
                "lock_id": lock["artifact_id"],
                "status": status,
                "completed_worlds": len(completed),
                "attempted_worlds": attempted,
                "unrun_worlds": len(source_worlds()) - attempted,
        ...
        print(
            f"terminal={status}; gate={result['source_gate_passed']}; "
    ...
    try:
        for index in range(len(source_worlds())):
            stage = f"source_world_{index:02d}"
            attempted += 1
            print(
                f"starting frozen off-grid coiling world "
                f"{index + 1}/{len(source_worlds())}",
                flush=True,
            )
            ...
            if not qa["passed"]:
                ...
                return
            print(
                f"sealed and qualified world {index + 1}/{len(source_worlds())}",
                flush=True,
            )
        stage = "source_bank_seal"
        rows = [load_task(output, lock, index) for index in range(len(source_worlds()))]
        bank = {
        ...
        stage = "source_crossfit"
        metrics = source_crossfit(bank["prefix"], bank["reward"])
        terminal(
            "complete",
...
if __name__ == "__main__":
    ...
    parser.add_argument("--worker", type=int, choices=range(len(source_worlds())))
