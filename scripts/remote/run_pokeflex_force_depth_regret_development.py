#!/usr/bin/env python3
"""Generate D405-scored force-reachable candidates on opened PokeFlex takes."""
...
def main() -> None:
    ...
    scales = tuple(
        map(float, independent["payload"]["method_lock"]["correction_scales"])
    )
    ...
    try:
        result = candidate_runner.run_smoke(
            ...
            correction_fields=FORCE_FIELDS,
            ...
            independent_anchor_maximum_template_distance_m=(
                float(
                    independent["payload"]["method_lock"][
                        "static_template_support_radius_mm"
                    ]
                )
                / 1000.0
            ),
        ...
        json.dumps(
            {
                ...
                "candidate_field_count": len(FORCE_FIELDS),
            },
            indent=2,
            sort_keys=True,
        )
