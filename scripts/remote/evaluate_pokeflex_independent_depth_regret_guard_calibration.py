#!/usr/bin/env python3
"""Evaluate the no-refit PokeFlex regret guard on calibration objects."""
...
from bayesian_phystwin.pokeflex_independent_depth_regret_guard_calibration_protocol import (  # noqa: E402
    load_pokeflex_regret_guard_calibration_protocol,
)
...
def main() -> None:
    ...
    protocol = load_pokeflex_regret_guard_calibration_protocol(
        args.protocol.resolve()
    )
    source_path = args.source_evaluation.resolve()
    expected_source_hash = protocol["payload"]["frozen_deployment"][
    ...
    for payload in payloads:
        registration = payload.get("independent_object_calibration", {})
        if registration.get("protocol_sha256") != protocol["protocol_sha256"]:
            raise ValueError("candidate artifact calibration protocol changed")
        if registration.get("candidate_bank_refit") is not False:
            raise ValueError("candidate bank was refit")
    result = evaluate_pokeflex_regret_guard_prospective(
        payloads,
        source,
        expected_take_ids=protocol["take_ids"],
    )
    result["claim_status"] = "no-refit independent-object calibration evaluation"
    ...
    false_safe_rate = (
        result["accepted_frame_losses"] / result["accepted_frame_count"]
        if result["accepted_frame_count"]
        else 0.0
    )
    result["false_safe_rate"] = false_safe_rate
    result["maximum_object_regression"] = object_regression
    result["below_published_kinect_reference"] = (
        result["selected_object_mean_CD_UL1_mm"]
        < float(gates["published_kinect_reference_CD_UL1_mm"])
    )
    result["gate_checks"] = {
        ...
        "false_safe_rate": false_safe_rate <= float(gates["maximum_false_safe_rate"]),
        "nonempty_acceptance": (
            bool(result["accepted_frame_count"])
            if gates["require_nonempty_acceptance"]
            else True
        ),
    }
    result["gate_passed"] = all(result["gate_checks"].values())
    result["target_protocol_drafting_permitted"] = result["gate_passed"]
    ...
    if output.exists() and output.read_text(encoding="utf-8") != rendered:
        raise ValueError(f"existing calibration evaluation differs: {output}")
        ...
        json.dumps(
            {
                ...
                "below_published_kinect_reference": result[
                    "below_published_kinect_reference"
                ],
            },
            indent=2,
            sort_keys=True,
