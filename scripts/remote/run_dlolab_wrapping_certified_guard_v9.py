#!/usr/bin/env python3
"""Run the frozen wrapping posterior certified-guard study."""
...
from bayesian_phystwin_experiments.dlolab_wrapping_certified_guard_native_v9 import (
    run_worlds,
)
...
ATTEMPT = Path(
    "/home/florianpfaff/source-only/dlolab-wrapping-risk-certified-guard-source-v9.attempt.json"
)
...
CALIBRATION_V8_SUMMARY_SHA256 = (
    "885bfdbef28d272e51bf855828a9131101dc852b8258c2e1951066d5218fa4bd"
)
CALIBRATION_CERTIFICATE_SHA256 = (
    "1175f3f8fd0a57f2b7362dc8275bc67acc5aae977e876d2795a3dedafaf8ddd4"
)
NEW_SOURCES = (
    "src/bayesian_phystwin_experiments/dlolab_wrapping_certified_guard_v9.py",
    "src/bayesian_phystwin_experiments/dlolab_wrapping_certified_guard_native_v9.py",
    "scripts/remote/run_dlolab_wrapping_certified_guard_v9.py",
    "tests/test_dlolab_wrapping_certified_guard_v9.py",
    "tests/test_dlolab_wrapping_certified_guard_v9_custody.py",
    "docs/dlolab_wrapping_certified_guard_source_v9.md",
    "docs/dlolab_wrapping_certified_guard_calibration_v9_result.md",
    "scripts/audit_dlolab_wrapping_certified_guard_calibration_v9.py",
    "results/sota/dlolab_wrapping_certified_guard_calibration_v9/summary.json",
    "tests/test_dlolab_wrapping_certified_guard_calibration_v9.py",
...
def _development_v4_diagnostic() -> dict[str, Any]:
    if (
        ...
        raise ValueError("registered risk-certified-guard development diagnostic required")
    ...
    if (
        ...
        raise ValueError("risk-certified-guard development diagnostic changed")
    return result
...
def _calibration_v8() -> dict[str, Any]:
    result = _summary(
        CALIBRATION_V8_SUMMARY,
        CALIBRATION_V8_SUMMARY_SHA256,
    ...
    if (
        result.get("schema") != "dlolab-wrapping-risk-guard-source-v8-summary-v1"
        or result.get("status") != "complete_source_gate_failed"
        or result.get("result_id") != CALIBRATION_V8_RESULT_ID
        or result.get("ordinary_worlds") != 144
        or result.get("guard_harmed_worlds") != 2
        or result.get("source_gate_passed") is not False
        or result.get("strict_gate_reclassified") is not False
        or result.get("post_outcome_tuning") is not False
        or result.get("retry_authorized") is not False
        or result.get("replacement_authorized") is not False
        or result.get("protected_data_read") is not False
    ):
        raise ValueError("registered v8 calibration summary changed")
    return result


def _calibration_certificate() -> dict[str, Any]:
    result = _summary(
        CALIBRATION_CERTIFICATE,
        CALIBRATION_CERTIFICATE_SHA256,
    ...
    if (
        result.get("schema")
        != "dlolab-wrapping-certified-guard-calibration-v9"
        or result.get("artifact_id") != CALIBRATION_CERTIFICATE_ID
        or result.get("status") != "post_open_calibration_certificate_passed"
        or result.get("v8_result_id") != CALIBRATION_V8_RESULT_ID
        or result.get("v8_strict_source_gate_passed") is not False
        or result.get("v8_strict_gate_reclassified") is not False
        or result.get("candidate_threshold_selected_from_v8_outcomes") is not False
        or result.get("world_count") != 144
        or result.get("harm_count") != 2
        or result.get("confidence") != 0.95
        or result.get("risk_budget") != 0.05
        or result.get("one_sided_exact_clopper_pearson_upper")
        != 0.04307319681566585
        or result.get("certificate_passed") is not True
        or result.get("lead_is_not_v9_evidence") is not True
        or result.get("v9_fresh_replication_automatically_authorized") is not False
        or result.get("protected_data_read") is not False
    ):
        raise ValueError("registered v8 calibration certificate changed")
    return result
...
def _validate(output: Path) -> tuple[dict[str, Any], dict[str, Any], dict[str, Array]]:
    ...
    calibration_v8 = _calibration_v8()
    certificate = _calibration_certificate()
    if (
        lock.get("schema") != "dlolab-wrapping-risk-certified-guard-lock-v9"
        ...
        or attempt.get("schema") != "dlolab-wrapping-risk-certified-guard-attempt-v9"
        ...
        or attempt.get("calibration_v8_raw_arrays_read") is not False
        ...
        or lock.get("calibration_v8_summary_id")
        != calibration_v8.get("artifact_id")
        or lock.get("calibration_v8_result_id") != CALIBRATION_V8_RESULT_ID
        or lock.get("calibration_v8_summary_sha256")
        != CALIBRATION_V8_SUMMARY_SHA256
        or lock.get("calibration_v8_raw_arrays_read") is not False
        or lock.get("calibration_certificate_id") != certificate.get("artifact_id")
...
def _load_task(
    ...
    if (
        claim.get("schema") != "dlolab-wrapping-risk-certified-guard-claim-v9"
        ...
        or seal.get("schema") != "dlolab-wrapping-risk-certified-guard-seal-v9"
...
def _load_decisions(
    ...
    data = load_native_bundle(directory, seal["bundle"])
    if (
        seal.get("schema") != "dlolab-wrapping-risk-certified-guard-decision-seal-v9"
        or seal.get("lock_id") != lock["artifact_id"]
    ...
    gate = pre_future_checks(
        ...
        calibration_certificate_valid=(
            _calibration_certificate().get("certificate_passed") is True
        ),
    )
...
def _worker(output: Path, kind: str, index: int) -> None:
    ...
    try:
        ...
        write_record(
            directory / "seal.json",
            {
                "schema": "dlolab-wrapping-risk-certified-guard-seal-v9",
                "lock_id": lock["artifact_id"],
        ...
        write_record(
            ...
            {
                "schema": "dlolab-wrapping-risk-certified-guard-failure-v9",
                "lock_id": lock["artifact_id"],
                "claim_id": claim["artifact_id"],
                "task": task,
    ...
    if run.returncode:
        ...
        if (directory / "claim.json").is_file() and not (
            ...
            write_record(
                ...
                {
                    "schema": "dlolab-wrapping-risk-certified-guard-process-failure-v9",
...
def _run(output: Path) -> None:
    ...
    attempt = write_record(
        ATTEMPT,
        {
            "schema": "dlolab-wrapping-risk-certified-guard-attempt-v9",
            "revision": revision,
            ...
            "calibration_v8_raw_arrays_read": False,
            "protected_data_read": False,
        },
    )
    output.mkdir()
    lock = write_record(
        output / "lock.json",
        {
            "schema": "dlolab-wrapping-risk-certified-guard-lock-v9",
            "revision": revision,
            "source_sha256": sources,
            "protocol": protocol(),
            "output_root": str(OUTPUT),
            ...
            "runtime_v7_arrays_read": False,
            "calibration_v8_summary_id": calibration_v8["artifact_id"],
            "calibration_v8_result_id": CALIBRATION_V8_RESULT_ID,
            "calibration_v8_summary_sha256": CALIBRATION_V8_SUMMARY_SHA256,
            "calibration_v8_raw_arrays_read": False,
            "calibration_certificate_id": certificate["artifact_id"],
            "calibration_certificate_sha256": CALIBRATION_CERTIFICATE_SHA256,
            "calibration_certificate_passed": True,
    ...
    try:
        ...
        decision_seal = write_record(
            directory / "seal.json",
            {
                "schema": "dlolab-wrapping-risk-certified-guard-decision-seal-v9",
                "lock_id": lock["artifact_id"],
        ...
        if not gate["pre_future_gate_passed"]:
            write_record(
                output / "result.json",
                {
                    "schema": "dlolab-wrapping-risk-certified-guard-result-v9",
        ...
        generation = write_record(
            ...
            {
                "schema": "dlolab-wrapping-risk-certified-guard-generation-v9",
                "lock_id": lock["artifact_id"],
                "barrier_id": barrier["artifact_id"],
