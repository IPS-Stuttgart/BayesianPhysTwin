#!/usr/bin/env python3
"""Apply the source-frozen frame-zero fallback to known post-open failures."""
...
def _load_sealed_json(path: Path, *, digest_key: str) -> dict[str, Any]:
    payload = json.loads(path.read_text(encoding="utf-8"))
    observed = payload.get(digest_key)
...
def _load_initializer_config(repository: Path, config: dict[str, Any]) -> FrameZeroInitializerConfig:
    source_path = (
        repository
        / "configs"
        / "sota"
        / "deform360_frame_zero_initializer_source_v1.json"
    )
    source_payload = json.loads(source_path.read_text(encoding="utf-8"))
    expected = config["source_candidate"]["source_config_sha256"]
    _require(source_payload.get("config_sha256") == expected, "source config changed")
    method = source_payload["config"]["method"]
    fields = FrameZeroInitializerConfig.__dataclass_fields__
    return FrameZeroInitializerConfig(**{name: method[name] for name in fields})
...
def _evaluate_case(
    ...
    *,
    ...
    failure = _load_sealed_json(failure_path, digest_key="result_sha256")
    _require(
    ...
    masks, images, intrinsics, extrinsics, inputs = _load_frame_zero_views(episode_dir)
    ...
    attempt = selected.diagnostics["fallback"]["attempts"][-1]
    required_votes = int(attempt["required_vote_count"])
    ...
    largest_fraction = float(attempt["components"]["largest_component_fraction"])
    passed = bool(
        selected.method == "strict-multiview-visual-hull-surface"
        and len(selected.points_m) >= initializer_config.minimum_fallback_point_count
        and np.all(np.isfinite(selected.points_m))
        and np.all(np.isfinite(selected.colors))
        and np.all(votes >= required_votes)
    ...
    archive = case_output / "frame_zero_fallback.npz"
    np.savez_compressed(
        archive,
    ...
    return {
        **record,
        ...
        "selected_mask_dilation_radius_pixels": int(
            fallback.diagnostics["selected_mask_dilation_radius_pixels"]
        ),
...
def main() -> int:
    ...
    source_result = _load_sealed_json(
        ...
        / "results"
        / "sota"
        / "deform360_frame_zero_initializer_source_v1"
        / "source_audit.json",
    ...
    for record in records:
        ...
        try:
            result = _evaluate_case(
                ...
                minimum_component_fraction=float(
                    config["gate"]["minimum_largest_component_fraction"]
                ),
            )
    ...
    output: dict[str, Any] = {
        "schema_version": 1,
        "artifact_kind": "Deform360FrameZeroPostOpenFailureAudit",
