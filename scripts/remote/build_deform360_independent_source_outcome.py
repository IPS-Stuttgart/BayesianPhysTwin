#!/usr/bin/env python3
"""Build one source outcome only after its prediction has been sealed."""
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Reuse depth/tracking/PCD only when release provenance validates.",
    )
...
def main() -> int:
    ...
    authorize_independent_source_episode(
        lock, str(seal["object_id"]), int(seal["episode_id"])
    )
    if seal.get("lock_sha256") != sha256_file(args.lock):
        raise ValueError("prediction seal uses another independent-source lock")
    ...
    if (
        ...
        or manifest.get("object_id") != seal["object_id"]
        or int(manifest.get("episode_index", -1)) != int(seal["episode_id"])
    ):
        raise ValueError("staged episode differs from the sealed source episode")
    ...
    if (
        ...
        or reconstruction.get("prediction_seal", {}).get("result_sha256")
        != seal["result_sha256"]
        ...
        raise ValueError("full reconstruction was not opened after this seal")
    ...
    if _git_revision(args.deform360_repo) != parent["deform360_processing_revision"]:
        raise ValueError("Deform360 processing revision changed")
    if sha256_file(args.checkpoint) != parent["tracking"]["checkpoint_sha256"]:
    ...
    consensus = lock["frozen_predictor"]["observation_consensus"]
    ...
    if len(pcd_files) != 76:
        raise ValueError("independent source outcome must contain 76 point frames")
    ...
    if points.shape[0] != 76 or colors.shape != points.shape:
        raise ValueError("outcome point identities are inconsistent")

    with np.load(seal["prediction_archive"]["path"], allow_pickle=False) as stored:
        sealed_initial = np.asarray(stored["frame_zero_points_m"], dtype=np.float32)
    if not np.array_equal(points[0], sealed_initial):
        raise ValueError("outcome frame-zero identities differ from the prediction")
    ...
    payload = {
        ...
        "input_sha256": {
            ...
            "prediction_seal": sha256_file(args.prediction_seal),
        ...
        "information_boundary": {
            "prediction_seal_verified_before_future_open": True,
            "source_future_opened_for_outcome_construction": True,
        ...
        "claim_boundary": (
            "public independent-source outcome construction after prediction "
            "sealing; no metric, calibration, or official target was opened"
        ),
    }
