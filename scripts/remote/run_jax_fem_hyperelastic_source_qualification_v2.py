#!/usr/bin/env python3
"""Run the frozen source-only JAX-FEM hyperelastic v2 qualification."""
...
def _group_root(value: str) -> tuple[str, Path]:
    ...
    if not separator or not group_id or not root:
        raise argparse.ArgumentTypeError("group roots must use GROUP_ID=/absolute/path")
    path = Path(root)
