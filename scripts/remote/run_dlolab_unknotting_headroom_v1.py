#!/usr/bin/env python3
"""Run the frozen public DLO-Lab unknotting headroom screen once."""
...
from bayesian_phystwin_experiments.dlolab_unknotting_native_v1 import run_world

ROOT = Path(__file__).resolve().parents[2]
ASSETS = Path("/home/fpfaff/source-only/dlolab-benchmark-source-v1-assets")
ATTEMPT = Path(
    "/home/fpfaff/source-only/dlolab-unknotting-headroom-development-v1.attempt.json"
)
OUTPUT = Path("/home/fpfaff/source-only/dlolab-unknotting-headroom-development-v1")
OSMESA_DIRECTORY = Path(
    "/home/fpfaff/source-only/dlo-lab-decision-v1-assets/"
    "native-libs/root/usr/lib/x86_64-linux-gnu"
)
SOURCES = (
    "src/bayesian_phystwin_experiments/dlolab_unknotting_headroom_v1.py",
    "src/bayesian_phystwin_experiments/dlolab_unknotting_native_v1.py",
    "scripts/remote/run_dlolab_unknotting_headroom_v1.py",
    "tests/test_dlolab_unknotting_headroom_v1.py",
    "tests/test_dlolab_unknotting_headroom_runner_v1.py",
    "docs/dlolab_unknotting_headroom_development_v1.md",
...
def runtime() -> dict[str, Any]:
    ...
    if {name: os.environ.get(name) for name in expected_environment} != (
        expected_environment
    ):
...
def validate_lock(output: Path) -> dict[str, Any]:
    if output.resolve() != OUTPUT or output.is_symlink():
        raise ValueError("only the registered write-once unknotting root is permitted")
    if (output / "failure.json").exists() or (output / "result.json").exists():
        raise ValueError("terminal unknotting development study; no retry")
    ...
    if (
        attempt.get("schema") != "dlolab-unknotting-headroom-attempt-v1"
        ...
        or lock.get("schema") != "dlolab-unknotting-headroom-lock-v1"
        or lock.get("attempt_id") != attempt["artifact_id"]
        or any(attempt.get(key) != lock.get(key) for key in launch_keys)
        ...
        raise ValueError(
            "clean frozen unknotting implementation/runtime/source required"
        )
    return cast(dict[str, Any], lock)


def prepare_launch(output: Path) -> dict[str, Any]:
    """Validate all pure launch inputs before consuming the attempt."""
    if (
        ...
        raise ValueError(
            "the registered unknotting attempt and output root must be fresh"
        )
    return {
...
def load_task(
    ...
    if (
        ...
        raise ValueError("native unknotting task custody changed")
...
def worker(output: Path, index: int) -> None:
    ...
    for previous in range(index):
        ...
        if (
            ...
            raise ValueError("all earlier unknotting worlds must requalify")
    directory = output / spec["name"]
...
def run(output: Path) -> None:
    ...
    def terminal(status: str, **extra: Any) -> dict[str, Any]:
        result = write_record(
            output / "result.json",
            {
                "schema": "dlolab-unknotting-headroom-result-v1",
                "lock_id": lock["artifact_id"],
                "status": status,
    ...
    try:
        for index in range(len(worlds())):
            ...
            attempted += 1
            print(
                f"starting frozen unknotting world {index + 1}/{len(worlds())}",
                flush=True,
            )
        ...
        reward_bank = np.asarray(
            [row[2]["final_rewards"][:UNIQUE_ACTION_COUNT] for row in rows],
            dtype=np.float64,
        )
