#!/usr/bin/env python3
"""Run the frozen target-free V13 adaptive-query audit on one source case."""
...
def _require_bound_sources(repo: Path, protocol: dict[str, Any]) -> None:
    for relative, expected in protocol["implementation_file_sha256"].items():
        _require(
...
def _load_protocol(
    ...
    _require(
        ...
        == "deform360-causal-response-adaptive-query-v13-source",
        "protocol ID changed",
    )
    _require(
    ...
    _require_bound_sources(repo, payload)
    query = AdaptiveCausalResponseQueryConfig(**payload["query"])
    _require(
        query == AdaptiveCausalResponseQueryConfig(),
        "query settings differ from the frozen V13 carrier",
    )
...
def main() -> None:
    ...
    _require(
        ...
        == "",
        "implementation commit is not an ancestor of the execution revision",
    )
    ...
    schedule = build_adaptive_causal_response_query_schedule(
        ...
        config=AdaptiveCausalResponseQueryConfig(**protocol["query"]),
    )
        ...
        json.dumps(
            {
                ...
                "strict_eligible_entity_count": (schedule.panels.strict_eligible_count),
                "fallback_eligible_entity_count": (
                    schedule.panels.fallback_eligible_count
                ),
