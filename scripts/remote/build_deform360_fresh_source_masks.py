#!/usr/bin/env python3
"""Create frozen generic-SAM2 masks for one sealed fresh source window."""
...
def main() -> int:
    ...
    window_protocol, queue, _ = validate_window_sources(
        args.window_protocol.resolve(),
        args.queue.resolve(),
        args.download_manifest.resolve(),
    )
    mask_protocol = load_fresh_source_mask_protocol(args.mask_protocol.resolve())
    _require(
    ...
    status = "ready_for_source_processing" if successful >= minimum else "technical_failure"
