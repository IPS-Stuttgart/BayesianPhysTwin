...
def context_summary(features):
    ...
    return np.concatenate((features[:, indices].reshape(n, -1),
                           features.mean(1).reshape(n, -1),
                           features.std(1).reshape(n, -1)), axis=1)
...
class Gate:
    ...
    def weights(self, features):
        ...
        d = x.shape[1]
        logweights = []
        for weight, mean, covariance in zip(self.mixture.weights_,
                                            self.mixture.means_,
                                            self.mixture.covariances_):
            # Marginal p(context | component): no query residual is supplied.
            logweights.append(np.log(max(weight, 1e-300)) +
                              multivariate_normal.logpdf(
                                  x, mean=mean[:d], cov=covariance[:d, :d]))
...
def fit_gate(features, residual, family, k, concentration):
    ...
    joint = np.concatenate((cx.transform(context_summary(features)),
                            ry.transform(residual_summary(residual))), axis=1)
    ...
    model = BayesianGaussianMixture(
        ...
        mean_precision_prior=1.0, covariance_prior=np.eye(joint.shape[1]),
        reg_covar=1e-5, n_init=5, max_iter=1000, tol=1e-5,
        random_state=SEED)
    with warnings.catch_warnings(record=True) as caught:
        warnings.simplefilter('always')
        model.fit(joint)
    info = dict(family=family, cap=k, concentration=concentration,
                converged=bool(model.converged_), iterations=int(model.n_iter_),
                component_weights=model.weights_.tolist(),
                active_components_above_0p05=int(np.sum(model.weights_ > .05)),
...
def sufficient_statistics(design, features, residual):
    ...
    for node in range(features.shape[2]):
        ...
        y = residual[:, :, node]
        out.append((x.swapaxes(1, 2) @ x,
                    x.swapaxes(1, 2) @ y, y, x))
    return out
...
def fit_experts(stats, weights, ridge):
    ...
    for component in range(k):
        ...
        if fallback:
            w = np.ones(n)
        support.append(dict(expected_trajectories=mass, effective_trajectories=ess,
        ...
        for gram, cross, y, x in stats:
            ...
            variances.append(np.einsum('n,ntc->c', w, errors**2)/(w.sum()*y.shape[1]))
        fits.append((np.stack(coefficients), np.stack(covariances),
                     np.stack(variances)))
    return fits, support
...
def component_predictions(design, fits, features, frames, uncertainty=False):
    ...
    for beta, covariance, noise in fits:
        ...
        for node in range(features.shape[2]):
            ...
            if uncertainty:
                epistemic = np.stack([
                    np.sum((x @ covariance[node, c]) * x, axis=-1)
        ...
        if uncertainty:
            variances.append(np.einsum('ntvj,nij->ntvi',
                                      np.stack(va, axis=2), frames**2))
...
def point_metrics(prediction, target):
    ...
    return dict(mean_l1_mm=float(absolute.mean()*1000),
                trajectory_l1_mm=(absolute.mean((1, 2, 3))*1000).tolist(),
                last_quarter_l1_mm=float(absolute[:, -max(1, absolute.shape[1]//4):].mean()*1000),
...
def distribution_metrics(baseline, target, corrections, variances, weights, shrinkage):
    ...
    logpdf = -.5*(np.log(2*np.pi*variance) + (y-means)**2/variance)
    ...
    return dict(marginal_gaussian_mixture_nll_per_coordinate_m=float(nll.mean()),
                ...
                mean_between_component_variance_mm2=float(between.mean()*1e6)), mean, within+between
...
def paired_comparison(candidate, reference):
    ...
    return dict(relative_improvement_percent=float(100*(1-c.mean()/r.mean())),
                wins=int(np.sum(c < r-1e-9)), ties=int(np.sum(np.abs(c-r) <= 1e-9)),
                n_trajectories=len(c),
                paired_trajectory_bootstrap_95_percent=np.quantile(improvement, [.025, .975]).tolist())
...
def save_fitted(path, design, fits, gate, support):
    ...
    for k, (beta, covariance, variance) in enumerate(fits):
        arrays.update({f'beta_{k}': beta, f'covariance_{k}': covariance,
                       f'variance_{k}': variance})
    if gate is not None:
        for name, projection in [('context', gate.context), ('residual', gate.residual)]:
            arrays.update({f'{name}_location': projection.location,
        ...
        arrays.update(gate_weights=gate.mixture.weights_, gate_means=gate.mixture.means_,
...
def self_test():
    ...
    r = rng.normal(size=(12, 10, 2, 3))*.001
    design = Design.fit(f)
    stats = sufficient_statistics(design, f, r)
    fits, _ = fit_experts(stats, np.ones((12, 1)), 1.)
    ...
    penalty = np.eye(6); penalty[0, 0] = 0
    ...
    pred = point_prediction(baseline, correction, np.ones((12, 1)), .5)
    assert np.array_equal(pred[:, :, [0, 1, -2, -1]], baseline[:, :, [0, 1, -2, -1]])
    assert np.all(variance >= 0)
    for family in ('dp', 'finite'):
        gate = fit_gate(f, r, family, 3, 1.)
        weight = gate.weights(f)
        np.testing.assert_allclose(weight.sum(1), 1., atol=1e-12)
    ...
    print('SELF_TEST_PASS: weighted ridge equivalence, finite covariance, '
          'exact clamps, normalized DP/finite marginal gates', flush=True)
...
def main():
    ...
    from bayesian_phystwin_experiments.deform_dlo_local_residual import (
        ...
        fit_deform_local_residual, predict_deform_local_residual)
    import sklearn
    import scipy
    import torch
    ...
    common._verify_identity(manifest_path, protocol['source_manifest'], label='source manifest')
    common._verify_identity(longrun_path, protocol['longrun_result'], label='frozen baseline result')
    manifest = json.loads(manifest_path.read_text())
    longrun = json.loads(longrun_path.read_text())
    assert manifest['dlo_type'] == 'DLO1' and manifest['partition'] == 'train'
    all_names = sum((manifest['split'][s] for s in ('fit','validation','source_test')), [])
    ...
    for forbidden in (upstream/'data_set/DLO1/eval', *(upstream/f'data_set/DLO{i}' for i in range(2,6))):
        source._install_eval_read_guard(forbidden)
    longrun_protocol = json.loads((repo/protocol['longrun_protocol']['repository_path']).read_text())
    os.environ['CUBLAS_WORKSPACE_CONFIG'] = longrun_protocol['training']['cublas_workspace_config']
    ...
    if longrun['selected_checkpoint']['checkpoint']['sha256'] != protocol['baseline']['checkpoint_sha256']:
        raise ValueError('Unexpected baseline checkpoint')
    design_spec = dict(contract='dp-residual-dlo1-retrospective-pilot-v1',
                       observation_contract='two-states-known-clamped-action-baseline-only',
                       protocol_sha256=sha(protocol_path), source_manifest_sha256=sha(manifest_path),
                       baseline_checkpoint_sha256=protocol['baseline']['checkpoint_sha256'],
                       splits=manifest['split'], ridges=RIDGES, shrinkages=SHRINKAGES,
                       finite_k=[2,3,4,6], dp_cap=6, dp_concentrations=[.1,1.,10.],
                       context_pca=4, residual_pca=3, seed=SEED,
                       nonlinear_rff_features=64, nonlinear_bandwidths=[1.,3.],
                       prior_scope='two-stage-summary-mixture-not-joint-regression-posterior',
                       fresh_confirmatory_evidence=False, official_eval_read=False,
                       other_dlo_read=False, source_test_read=False,
                       git_sha=os.environ.get('GITHUB_SHA'), python=platform.python_version(),
    ...
    def load_split(names):
        trajectories = source._load_named_trajectories(manifest, names, frame_count=500, node_count=13)
        ...
        initial, action = deform_causal_inputs(np.stack([trajectories[name] for name in names]))
        ...
        return dict(names=names, initial=initial, action=action, baseline=baseline,
                    target=target, features=features, frames=frames, residual=residual)
    ...
    common._require_baseline_reproduction(float(np.abs(val['baseline']-val['target']).mean()),
        expected=protocol['baseline']['validation_l1_m'], tolerance=1e-7, stage='validation')
    print('VALIDATION_BASELINE_REPRODUCED', flush=True)
    original = fit_deform_local_residual(fit['initial'],fit['action'],fit['baseline'],fit['target'],
                                         fit['names'],ridge=1.,variance_floor_m2=FLOOR)
    reference = predict_deform_local_residual(original,val['initial'],val['action'],val['baseline'],shrinkage=.5)
    design = Design.fit(fit['features'])
    stats = sufficient_statistics(design, fit['features'], fit['residual'])
    ref_fits, ref_support = fit_experts(stats, np.ones((len(fit['names']),1)), 1.)
    ref_c, ref_v = component_predictions(design, ref_fits, val['features'], val['frames'], True)
    replicated = point_prediction(val['baseline'],ref_c,np.ones((len(val['names']),1)),.5)
    np.testing.assert_allclose(replicated,reference['predictions'],rtol=1e-7,atol=1e-9)
    np.testing.assert_allclose(ref_v[0]+(.5*ref_c[0])**2+FLOOR,
                               reference['coordinate_variance_m2'][:,:,2:-2],rtol=1e-6,atol=1e-10)
    print('PRODUCTION_RESIDUAL_MEAN_AND_VARIANCE_REPRODUCED',flush=True)

    finalists = {'fixed_current': dict(design=design,fits=ref_fits,gate=None,support=ref_support,
                   spec=dict(family='fixed_current',ridge=1.,shrinkage=.5),
                   validation=point_metrics(replicated,val['target']))}
    table=[]

    def consider(family, dg, statistics, gate, details):
        train_w = np.ones((len(fit['names']),1)) if gate is None else gate.train_weights
        val_w = np.ones((len(val['names']),1)) if gate is None else gate.weights(val['features'])
        for ridge in RIDGES:
            models,support = fit_experts(statistics,train_w,ridge)
            corrections,_ = component_predictions(dg,models,val['features'],val['frames'])
            for shrink in SHRINKAGES:
                predicted = point_prediction(val['baseline'],corrections,val_w,shrink)
                metric = point_metrics(predicted,val['target'])
                spec={**details, 'family': family, 'ridge': ridge, 'shrinkage': shrink}
                table.append(dict(spec=spec,**metric))
                if family not in finalists or metric['mean_l1_mm'] < finalists[family]['validation']['mean_l1_mm']-1e-12:
                    finalists[family] = dict(design=dg,fits=models,gate=gate,support=support,
                                              spec=spec,validation=metric)
        print('VALIDATED',family,details,finalists[family]['validation']['mean_l1_mm'],flush=True)

    consider('single_tuned',design,stats,None,{})
    for k in (2,3,4,6):
        gate=fit_gate(fit['features'],fit['residual'],'finite',k,1./k)
    ...
    del stats
    for bandwidth in (1.,3.):
        nonlinear=Design.fit(fit['features'],bandwidth)
        nonlinear_stats=sufficient_statistics(nonlinear,fit['features'],fit['residual'])
        consider('nonlinear_tuned',nonlinear,nonlinear_stats,None,dict(bandwidth=bandwidth))
        del nonlinear_stats

    selection={}
    for family,record in finalists.items():
        path=output/f'{family}_fitted.npz'
        save_fitted(path,record['design'],record['fits'],record['gate'],record['support'])
        selection[family]=dict(spec=record['spec'],validation=record['validation'],
                               model_sha256=sha(path),support=record['support'])
    write_json(output/'validation_candidates.json',table)
    write_json(output/'selection.json',selection)
    write_json(output/'source_opening.json',dict(selection_sha256=sha(output/'selection.json'),
                validation_candidates_sha256=sha(output/'validation_candidates.json'),
                protocol_sha256=sha(output/'protocol.json'),source_test_read=False,
                official_eval_read=False,other_dlo_read=False))
    print('SOURCE_SELECTION_SEALED',json.dumps(selection),flush=True)
    test=load_split(manifest['split']['source_test'])
    common._require_baseline_reproduction(float(np.abs(test['baseline']-test['target']).mean()),
        expected=protocol['baseline']['source_test_l1_m'],tolerance=1e-7,stage='source-test')
    results={'baseline':point_metrics(test['baseline'],test['target'])}
    arrays=dict(names=np.asarray(test['names']),baseline=test['baseline'],target=test['target'])
    for family,record in finalists.items():
        gate=record['gate']
        weights=np.ones((len(test['names']),1)) if gate is None else gate.weights(test['features'])
        corrections,variances=component_predictions(record['design'],record['fits'],
                                       test['features'],test['frames'],True)
        shrink=record['spec']['shrinkage']
        predicted=point_prediction(test['baseline'],corrections,weights,shrink)
        distribution,_,variance=distribution_metrics(test['baseline'],test['target'],
                                                  corrections,variances,weights,shrink)
        results[family]=dict(**point_metrics(predicted,test['target']),**distribution,
                            spec=record['spec'],test_weights=weights.tolist())
        arrays[family+'_prediction']=predicted
        arrays[family+'_coordinate_variance']=variance
        assert np.array_equal(predicted[:,:,[0,1,-2,-1]],test['baseline'][:,:,[0,1,-2,-1]])
    ...
    comparisons={reference:paired_comparison(results['dp_tuned'],results[reference])
                 for reference in ('baseline','fixed_current','single_tuned','finite_tuned','nonlinear_tuned')}
    np.savez_compressed(output/'source_predictions.npz',**arrays)
    result=dict(contract='dp-residual-dlo1-retrospective-pilot-result-v1',
                results=results,dp_comparisons=comparisons,selection=selection,
                source_names=test['names'],source_test_read=True,official_eval_read=False,
                other_dlo_read=False,source_selection_sha256=sha(output/'selection.json'),
                prediction_sha256=sha(output/'source_predictions.npz'),
                independent_trajectory_units=len(test['names']),
                fresh_confirmatory_evidence=False,elapsed_seconds=time.time()-start,
                limitations=['DLO1 source split has been used in earlier method development',
