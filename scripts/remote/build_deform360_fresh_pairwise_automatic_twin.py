#!/usr/bin/env python3
"""Build the exact frame-zero automatic twin for one fresh locked case."""
...
def main() -> int:
    args = _parse_args()
    protocol = load_fresh_pairwise_protocol(args.protocol)
    cohort = load_bound_cohort(args.cohort_lock, protocol)
    authorization = fresh_case_record(
        cohort,
    ...
    _require(canonical_node_count == 384, "fresh physical node count changed")
    ...
    candidate_reliability = np.asarray(
        visibility[0] & validity[0], dtype=np.float64
    )
    ...
    summary: dict[str, Any] = {
        "schema_version": 1,
        "artifact_kind": "Deform360FreshPairwiseAutomaticEpisodeTwin",
        "protocol_id": PROTOCOL_ID,
        "protocol_config_sha256": file_sha256(args.protocol),
        ...
        **authorization,
        ...
        "passed": bool(state.metrics["passed"]),
        "claim_boundary": (
            "automatic 384-node frame-zero episode twin under the exact "
            "open-27 numerical contract"
        ),
    }
