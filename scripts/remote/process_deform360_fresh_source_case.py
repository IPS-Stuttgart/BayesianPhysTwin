#!/usr/bin/env python3
"""Run the frozen official source pipeline for one fresh Deform360 case."""
...
MASK_MANIFEST_FILENAME = "fresh_source_masks.json"
PROCESSING_MANIFEST_FILENAME = "fresh_source_processing.json"
ADMISSION_FILENAME = "fresh_source_admission.json"
...
def main() -> int:
    ...
    processing_protocol = load_fresh_source_processing_protocol(
        processing_protocol_path
    )
    window_protocol, queue, download = validate_window_sources(
        args.window_protocol.resolve(),
        args.queue.resolve(),
        args.download_manifest.resolve(),
    )
    mask_protocol_path = args.mask_protocol.resolve()
    mask_protocol = load_fresh_source_mask_protocol(mask_protocol_path)
    ...
    stage = validate_fresh_source_window_stage(
        stage_path,
    ...
    mask_manifest, cameras = validate_fresh_source_mask_artifact(
        mask_manifest_path,
    ...
    try:
        ...
        try:
            splats = reconstruct_stage.process_reconstruction_episode(
                ...
                args.episode_id,
                cameras=cameras,
                first_frame_iterations=int(
                    reconstruction["first_frame_iterations"]
                ),
                warm_start_iterations=int(
                    reconstruction["warm_start_iterations"]
                ),
