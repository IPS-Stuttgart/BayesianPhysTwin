#!/usr/bin/env python3
"""Run the frozen JAX-FEM v2 prediction and outcome-order gates."""
...
def _binding(value: str) -> tuple[str, Path]:
    group_id, separator, raw = value.partition("=")
    path = Path(raw)
    if (
        not separator
        or not group_id
...
def main() -> int:
    ...
    if args.command == "predict":
        result = generate_jax_fem_hyperelastic_source_value_predictions_v2(
