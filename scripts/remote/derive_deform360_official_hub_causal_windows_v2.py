#!/usr/bin/env python3
"""Derive the separately locked v2 Deform360 calibration windows."""
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--schedule-recovery-lock", type=Path, required=True)
...
def main() -> None:
    ...
    schedule_lock = load_strict_json_object(
        args.schedule_recovery_lock,
        label="v2 causal schedule recovery lock",
    )
    ...
    save_deform360_official_hub_causal_window_manifest_v2(args.output, manifest)
    print(
        json.dumps(
            {
