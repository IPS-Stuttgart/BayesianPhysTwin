...
from bayesian_phystwin_experiments.dlolab_wrapping_risk_guard_native_v4 import (
    run_worlds,
)
...
PREFLIGHT_ATTEMPT = Path(
    ...
    "dlolab-wrapping-risk-guard-runtime-preflight-v4.attempt.json"
)
...
DEVELOPMENT_V3_FILE_SHA256 = {
    "attempt.json": "454be9eefa350f4e6d3917dea14daab935d3bba22979505ed39ae323f1c21a99",
    "lock.json": "074a60dd00b07dcbcaf23526822d933646ac1cc6a50d16d1f1a457e546c1e3cb",
    "decisions/arrays.npz": (
        "f812fc6e7249d5b29a1a1327ee29cb751299829f5489878cdffbf63075ef96f7"
    ),
    "decisions/seal.json": (
        "ae3a5c651b5d0c6d8605799a6d8f616a76d31c52c8b3794c14e89b9fdb294cd0"
    ),
    "decision-barrier.json": (
        "c655e860f804b8589881b5440194a3ec52422a372bf0ee5b0625b3aa6125b40d"
    ),
    "generation/arrays.npz": (
        "9922b5eb64d0b9fab00874c48f65612146897140b594a590b2799170dd3459b7"
    ),
    "generation/seal.json": (
        "4c28f119add07e26949545dc150d35cc77377874216d60c44472101bcb9701a0"
    ),
    "result.json": "bfb02757a9879a0263388635e5fc5173cf806371af78511c1f3b50007539bc08",
}
...
NEW_SOURCES = (
    ...
    "src/bayesian_phystwin_experiments/dlolab_wrapping_risk_guard_native_v4.py",
    "scripts/remote/run_dlolab_wrapping_risk_guard_v4.py",
    "tests/test_dlolab_wrapping_risk_guard_v4.py",
    "tests/test_dlolab_wrapping_risk_guard_v4_custody.py",
    "docs/dlolab_wrapping_risk_guard_source_v4.md",
...
def _development_v3() -> tuple[dict[str, Any], dict[str, str]]:
    paths = {
        "attempt.json": DEVELOPMENT_V3_ATTEMPT,
        "lock.json": DEVELOPMENT_V3 / "lock.json",
        "decisions/arrays.npz": DEVELOPMENT_V3 / "decisions" / "arrays.npz",
        "decisions/seal.json": DEVELOPMENT_V3 / "decisions" / "seal.json",
        "decision-barrier.json": DEVELOPMENT_V3 / "decision-barrier.json",
        "generation/arrays.npz": DEVELOPMENT_V3 / "generation" / "arrays.npz",
        "generation/seal.json": DEVELOPMENT_V3 / "generation" / "seal.json",
        "result.json": DEVELOPMENT_V3 / "result.json",
    }
    if any(not path.is_file() or path.is_symlink() for path in paths.values()):
        raise ValueError("complete wrapping development v3 custody required")
    hashes = {name: file_digest(path) for name, path in paths.items()}
    result = read_record(paths["result.json"])
    if (
        hashes != DEVELOPMENT_V3_FILE_SHA256
        or result.get("artifact_id") != DEVELOPMENT_V3_RESULT_ID
        or result.get("status") != "complete"
        or result.get("source_gate_passed") is not False
        ...
        or result.get("retry_authorized") is not False
        or result.get("protected_data_read") is not False
        or (DEVELOPMENT_V3 / "failure.json").exists()
    ):
        raise ValueError("wrapping development v3 evidence changed")
    return result, hashes
...
def _load_preflight() -> tuple[dict[str, Any], dict[str, str]]:
    ...
    if (
        attempt.get("schema") != "dlolab-wrapping-risk-guard-preflight-attempt-v4"
        ...
        or lock.get("schema") != "dlolab-wrapping-risk-guard-preflight-lock-v4"
        ...
        or lock.get("development_v3_result_id") != development_v3.get("artifact_id")
        or lock.get("development_v3_file_sha256") != development_v3_hashes
...
def _validate(output: Path) -> tuple[dict[str, Any], dict[str, Any], dict[str, Array]]:
    ...
    development_v4 = _development_v4_diagnostic()
    if (
        lock.get("schema") != "dlolab-wrapping-risk-guard-lock-v4"
        or lock.get("revision") != clean_revision(ROOT)
        or lock.get("source_sha256") != _source_hashes()
        or lock.get("protocol") != protocol()
        or lock.get("output_root") != str(OUTPUT)
        or lock.get("attempt_id") != attempt.get("artifact_id")
        or attempt.get("schema") != "dlolab-wrapping-risk-guard-attempt-v4"
        or attempt.get("revision") != lock.get("revision")
        or attempt.get("source_sha256") != lock.get("source_sha256")
...
def _load_decisions(
    ...
    data = load_native_bundle(directory, seal["bundle"])
    if (
        seal.get("schema") != "dlolab-wrapping-risk-guard-decision-seal-v4"
        or seal.get("lock_id") != lock["artifact_id"]
...
def _worker(output: Path, kind: str, index: int) -> None:
    ...
    claim = write_record(
        ...
        {
            "schema": "dlolab-wrapping-risk-guard-claim-v4",
            "lock_id": lock["artifact_id"],
            "task": task,
    ...
    try:
        ...
        write_record(
            directory / "seal.json",
            {
                "schema": "dlolab-wrapping-risk-guard-seal-v4",
                "lock_id": lock["artifact_id"],
        ...
        write_record(
            ...
            {
                "schema": "dlolab-wrapping-risk-guard-failure-v4",
                "lock_id": lock["artifact_id"],
                "claim_id": claim["artifact_id"],
                "task": task,
...
def _run_preflight() -> None:
    ...
    attempt = write_record(
        PREFLIGHT_ATTEMPT,
        {
            "schema": "dlolab-wrapping-risk-guard-preflight-attempt-v4",
            "revision": revision,
    ...
    lock = write_record(
        PREFLIGHT / "lock.json",
        {
            "schema": "dlolab-wrapping-risk-guard-preflight-lock-v4",
            "revision": revision,
            "source_sha256": sources,
            "output_root": str(PREFLIGHT),
            ...
            "development_v3_result_id": development_v3["artifact_id"],
            "development_v3_file_sha256": development_v3_hashes,
    ...
    claim = write_record(
        ...
        {
            "schema": "dlolab-wrapping-risk-guard-preflight-claim-v4",
            "lock_id": lock["artifact_id"],
            "prefix_only": True,
            "retry_authorized": False,
        },
    ...
    try:
        ...
        seal = write_record(
            PREFLIGHT / "seal.json",
            {
                "schema": "dlolab-wrapping-risk-guard-preflight-seal-v4",
                "lock_id": lock["artifact_id"],
                "claim_id": claim["artifact_id"],
        ...
        result = write_record(
            ...
            {
                "schema": "dlolab-wrapping-risk-guard-preflight-result-v4",
                "lock_id": lock["artifact_id"],
        ...
        write_record(
            ...
            {
                "schema": "dlolab-wrapping-risk-guard-preflight-failure-v4",
                "lock_id": lock["artifact_id"],
                "error_type": type(error).__name__,
                "message": str(error),
                "retry_authorized": False,
...
def _run(output: Path) -> None:
    ...
    attempt = write_record(
        ATTEMPT,
        {
            "schema": "dlolab-wrapping-risk-guard-attempt-v4",
            "revision": revision,
            "source_sha256": sources,
            "protocol": protocol(),
    ...
    lock = write_record(
        output / "lock.json",
        {
            "schema": "dlolab-wrapping-risk-guard-lock-v4",
            "revision": revision,
            "source_sha256": sources,
            "protocol": protocol(),
            "output_root": str(OUTPUT),
    ...
    try:
        ...
        decision_seal = write_record(
            directory / "seal.json",
            {
                "schema": "dlolab-wrapping-risk-guard-decision-seal-v4",
                "lock_id": lock["artifact_id"],
        ...
        if not gate["pre_future_gate_passed"]:
            write_record(
                output / "result.json",
                {
                    "schema": "dlolab-wrapping-risk-guard-result-v4",
        ...
        generation = write_record(
            ...
            {
                "schema": "dlolab-wrapping-risk-guard-generation-v4",
                "lock_id": lock["artifact_id"],
                "barrier_id": barrier["artifact_id"],
