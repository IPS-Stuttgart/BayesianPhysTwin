...
def main() -> int:
    ...
    parser.add_argument(
        "--protocol",
        type=Path,
        default=Path("protocols/deform_gp_residual_development_v1.json"),
    )
    ...
    if (
        protocol["contract"] != "deform-gp-residual-development-v1"
        or protocol["allowed_dataset"] != "DLO1/train"
    ):
    ...
    if (
        manifest["dlo_type"] != "DLO1"
        or manifest["partition"] != "train"
        or manifest["official_eval_read"]
    ):
        raise ValueError("forbidden dataset")
    split = manifest["split"]
    names = split["fit"] + split["validation"] + split["source_test"]
    if [len(split[k]) for k in ("fit", "validation", "source_test")] != [
        40,
        8,
        8,
    ] or len(set(names)) != 56:
    ...
    for stage in ("fit", "validation", "source_test"):
        for name in split[stage]:
            ...
            if (
                p.name != name
                or p.parent.name != "train"
                or p.parent.parent.name != "DLO1"
            ):
    ...
    def audit(event: str, arguments: tuple) -> None:
        ...
        low = p.lower()
        if ("/data_set/dlo" in low or "/datasets/deform/" in low) and low.endswith(
            ".pkl"
        ):
            stage = allowed.get(p)
    ...
    common._verify_identity(
        longrun_path, parent["longrun_result"], label="longrun result"
    )
    ...
    common._require_baseline_reproduction(
        float(errors(vb, vy).mean()),
        expected=parent["baseline"]["validation_l1_m"],
        tolerance=parent["baseline"]["reproduction_tolerance_m"],
        stage="validation",
    )
    ...
    for ridge in protocol["ridge_ridges"]:
        model = fit_deform_local_residual(
            fi, fa, fb, fy, fit_names, ridge=ridge, variance_floor_m2=floor
        )
        ridge_models[ridge] = model
        for shrinkage in protocol["ridge_shrinkages"]:
            key = f"ridge-r{ridge:g}-s{shrinkage:g}"
            p = predict_deform_local_residual(model, vi, va, vb, shrinkage=shrinkage)[
                "predictions"
            ]
    ...
    fixed_fit = predict_deform_local_residual(
        fixed_model, fi, fa, fb, shrinkage=protocol["fixed_shrinkage"]
    )["predictions"]
    fixed_val = predict_deform_local_residual(
        fixed_model, vi, va, vb, shrinkage=protocol["fixed_shrinkage"]
    )["predictions"]
    ...
    for coupled in (False, True):
        ...
        for multiplier in protocol["length_multipliers"]:
            ...
            for node in [None] if coupled else range(nodes):
                x = (
                    features.reshape(-1, dim)
                    if coupled
                    else features[:, :, node].reshape(-1, dim)
                )
                y = (
                    response.reshape(-1, 6)
                    if coupled
                    else response[:, :, node].reshape(-1, 6)
                )
                a = (
                    np.broadcast_to(arc, (n, horizon, nodes)).reshape(-1)
                    if coupled
                    else None
                )
                count = (
                    protocol["spatial_anchors"]
                    if coupled
                    else protocol["independent_anchors"]
                )
                indices = balanced_anchor_indices(
                    n, horizon, nodes if coupled else 1, count
                )
                bank = fit_gp_bank(
                    x,
                    y,
                    ...
                    noises=protocol["noise_levels"],
                    row_weight=1.0 / horizon,
                    nugget=protocol["explicit_nystrom_nugget"],
                    material_length=protocol["material_length"],
                )
                banks.append(bank)
            for noise in protocol["noise_levels"]:
                ...
                if coupled:
                    ...
                    pred = (
                        models[0]
                        .predict(vf.reshape(-1, dim), a)
                        .reshape(*vf.shape[:3], 6)
                    )
                else:
                    pred = np.stack(
                        [
                            m.predict(vf[:, :, j].reshape(-1, dim)).reshape(
                                *vf.shape[:2], 6
                            )
    ...
    families = [
        "independent",
        "spatial",
        "ridge_plus_independent",
        "ridge_plus_spatial",
    ]
    selected = {
        family: min(
            (k for k in specs if specs[k]["family"] == family),
            key=lambda k: (validation[k], k),
        )
    ...
    selection = {
        "selected": selected,
        "champion": champion,
        "tuned_ridge": tuned_ridge,
        "validation_mean_l1_m": validation,
        "specifications": specs,
        "fit_causal_query_clusters": [list(g) for g in groups],
        "source_test_opened": False,
    }
    ...
    print(
        "GP_SELECTION "
        + json.dumps(
            {"champion": champion, "selected": selected, "tuned_ridge": tuned_ridge}
        ),
        flush=True,
    )
    ...
    common._require_baseline_reproduction(
        float(errors(sb, sy).mean()),
        expected=parent["baseline"]["source_test_l1_m"],
        tolerance=parent["baseline"]["reproduction_tolerance_m"],
        stage="historical-source",
    )
    sf, sframes = build_deform_local_residual_features(si, sa, sb)
    fixed_source = predict_deform_local_residual(
        fixed_model, si, sa, sb, shrinkage=protocol["fixed_shrinkage"]
    )["predictions"]
    spec = specs[tuned_ridge]
    tuned_source = predict_deform_local_residual(
        ridge_models[spec["ridge"]], si, sa, sb, shrinkage=spec["shrinkage"]
    )["predictions"]
    ...
    for family, key in selected.items():
        ...
        if spec["coupled"]:
            ...
            pred = np.stack(
                [
                    m.predict(sf[:, :, j].reshape(-1, dim)).reshape(*sf.shape[:2], 6)
                    for j, m in enumerate(models)
                ],
                axis=2,
            )
        residual = pred[..., 3:] if spec["augmented"] else pred[..., :3]
        predictions[family] = add_residual(
            fixed_source if spec["augmented"] else sb,
            residual,
            sframes,
            spec["shrinkage"],
        )
    ...
    for key, p in predictions.items():
        ...
        metrics[key] = {
            ...
            "relative_gain_vs_baseline": float(
                1 - case_errors.mean() / baseline_errors.mean()
            ),
            "relative_gain_vs_tuned_ridge": float(
                1 - case_errors.mean() / ridge_errors.mean()
            ),
            "wins_vs_tuned_ridge": int(np.sum(case_errors < ridge_errors)),
            "maximum_case_ratio_vs_tuned_ridge": float(
                np.max(case_errors / ridge_errors)
            ),
            "case_l1_mm": dict(
                zip(split["source_test"], (1000 * case_errors).tolist(), strict=True)
            ),
            "horizon_bin_l1_mm": [
                float(1000 * np.abs(p[:, indices] - sy[:, indices]).mean())
                for indices in np.array_split(np.arange(horizon), 5)
            ],
        }
    best = metrics[specs[champion]["family"]]
    worthwhile = (
        best["relative_gain_vs_tuned_ridge"]
        >= protocol["continuation_minimum_relative_gain_vs_tuned_ridge"]
        and best["wins_vs_tuned_ridge"] >= protocol["continuation_minimum_paired_wins"]
        and best["maximum_case_ratio_vs_tuned_ridge"]
        <= protocol["continuation_maximum_case_ratio"]
    )
    np.savez_compressed(
        out / "source_predictions.npz",
        names=np.asarray(split["source_test"]),
        targets=sy,
        **predictions,
    )
    result = {
        ...
        "validation_selected_gp": champion,
        "historical_source_metrics": metrics,
        "continuation_gate_passed": bool(worthwhile),
        "source_trajectories": len(sb),
        ...
        "raw_data_files_read": sorted(accessed),
        "dlo2_read": False,
        "official_eval_read": False,
        "pokeflex_read": False,
