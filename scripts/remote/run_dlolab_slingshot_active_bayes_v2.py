...
PREFLIGHT_LOCK_ID = "3364143e1721ca7ec538b70ca33392f080058bf60426cf6ae505ba54790ff81d"
PREFLIGHT_SEAL_ID = "2fd6b9af647b684eef4b5f85eccfa89ed0afed2a80666564152e104dbeaef318"
...
NEW_SOURCES = (
    "src/bayesian_phystwin_experiments/dlolab_slingshot_active_bayes_v2.py",
    "scripts/remote/run_dlolab_slingshot_active_bayes_v2.py",
    "tests/test_dlolab_slingshot_active_bayes_v2.py",
    "tests/test_dlolab_slingshot_active_bayes_v2_custody.py",
    "docs/dlolab_slingshot_active_bayes_source_v2.md",
)
...
def _source() -> tuple[
    ...
    if any(
        ...
        raise ValueError("registered parent, preflight, or v1 evidence changed")
    ...
    preflight_lock = read_record(PREFLIGHT / "lock.json")
    preflight_seal = read_record(PREFLIGHT / "seal.json")
    preflight_result = read_record(PREFLIGHT / "result.json")
    v1_summary = read_record(
        ROOT / "results/sota/dlolab_slingshot_active_bayes_source_v1/summary.json"
    )
    ...
    if (
        ...
        or preflight_lock.get("artifact_id") != PREFLIGHT_LOCK_ID
        or preflight_seal.get("artifact_id") != PREFLIGHT_SEAL_ID
        or preflight_result.get("artifact_id") != PREFLIGHT_RESULT_ID
        or preflight_seal.get("lock_id") != PREFLIGHT_LOCK_ID
        or preflight_result.get("lock_id") != PREFLIGHT_LOCK_ID
        or preflight_result.get("seal_id") != PREFLIGHT_SEAL_ID
        or preflight_result.get("runtime_preflight_passed") is not True
        or preflight_result.get("study_attempt_consumed") is not False
        or preflight_result.get("retry_authorized") is not False
        or any(not value for value in preflight_result.get("checks", {}).values())
        or any(
            file_digest(ROOT / name) != digest
            for name, digest in preflight_lock.get("source_sha256", {}).items()
        )
        or v1_summary.get("artifact_id") != V1_RESULT_ID
        or v1_summary.get("status") != "terminal_pre_science_runtime_failure"
        or v1_summary.get("scientific_hypothesis_resolved") is not False
        or v1_summary.get("retry_authorized") is not False
...
def _validate(
    ...
    if (
        lock.get("schema") != "dlolab-slingshot-active-bayes-lock-v2"
        ...
        or attempt.get("schema") != "dlolab-slingshot-active-bayes-attempt-v2"
        ...
        or lock.get("runtime_preflight_lock_id") != PREFLIGHT_LOCK_ID
        or lock.get("runtime_preflight_seal_id") != PREFLIGHT_SEAL_ID
        or lock.get("runtime_preflight_result_id") != PREFLIGHT_RESULT_ID
        or lock.get("runtime_preflight_file_sha256") != PREFLIGHT_FILE_SHA256
        or lock.get("terminal_v1_result_id") != V1_RESULT_ID
        or lock.get("terminal_v1_file_sha256") != V1_FILE_SHA256
        or lock.get("expected_python") != str(EXPECTED_PYTHON)
...
def _load_decisions(
    ...
    data = load_native_bundle(directory, seal["bundle"])
    if (
        seal.get("schema") != "dlolab-slingshot-active-bayes-decision-seal-v2"
        or seal.get("lock_id") != lock["artifact_id"]
...
def _worker(output: Path, kind: str, index: int, probe: int | None) -> None:
    ...
    claim = write_record(
        ...
        {
            "schema": "dlolab-slingshot-active-bayes-claim-v2",
            "lock_id": lock["artifact_id"],
            "task": task,
    ...
    try:
        ...
        write_record(
            directory / "seal.json",
            {
                "schema": "dlolab-slingshot-active-bayes-seal-v2",
                "lock_id": lock["artifact_id"],
        ...
        write_record(
            ...
            {
                "schema": "dlolab-slingshot-active-bayes-failure-v2",
                "lock_id": lock["artifact_id"],
                "claim_id": claim["artifact_id"],
                "task": task,
...
def _run(output: Path) -> None:
    ...
    attempt = write_record(
        ATTEMPT,
        {
            "schema": "dlolab-slingshot-active-bayes-attempt-v2",
            "revision": revision,
    ...
    lock = write_record(
        output / "lock.json",
        {
            "schema": "dlolab-slingshot-active-bayes-lock-v2",
            "revision": revision,
            "source_sha256": sources,
            "protocol": protocol(),
            "output_root": str(OUTPUT),
            ...
            "runtime_preflight_lock_id": PREFLIGHT_LOCK_ID,
            "runtime_preflight_seal_id": PREFLIGHT_SEAL_ID,
            "runtime_preflight_result_id": PREFLIGHT_RESULT_ID,
            "runtime_preflight_file_sha256": PREFLIGHT_FILE_SHA256,
            "terminal_v1_result_id": V1_RESULT_ID,
            "terminal_v1_file_sha256": V1_FILE_SHA256,
    ...
    try:
        ...
        decision_seal = write_record(
            directory / "seal.json",
            {
                "schema": "dlolab-slingshot-active-bayes-decision-seal-v2",
                "lock_id": lock["artifact_id"],
        ...
        if not gate["pre_future_gate_passed"]:
            write_record(
                output / "result.json",
                {
                    "schema": "dlolab-slingshot-active-bayes-result-v2",
        ...
        generation = write_record(
            ...
            {
                "schema": "dlolab-slingshot-active-bayes-generation-v2",
                "lock_id": lock["artifact_id"],
                "barrier_id": barrier["artifact_id"],
