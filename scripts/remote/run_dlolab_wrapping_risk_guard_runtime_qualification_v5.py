...
from bayesian_phystwin_experiments.dlolab_wrapping_risk_guard_native_v5 import (
...
PLAN_PATH = (
    ROOT / "configs/sota/dlolab_wrapping_risk_guard_runtime_qualification_v5.json"
)
...
SOURCE_PATHS = (
    "configs/sota/dlolab_wrapping_risk_guard_runtime_qualification_v5.json",
    "src/bayesian_phystwin_experiments/dlolab_wrapping_risk_guard_native_v5.py",
    "scripts/remote/run_dlolab_wrapping_risk_guard_runtime_qualification_v5.py",
    "tests/test_dlolab_wrapping_risk_guard_runtime_qualification_v5.py",
    "docs/dlolab_wrapping_risk_guard_runtime_qualification_v5.md",
...
def plan() -> dict[str, Any]:
    value = dict(load_strict_json_object(PLAN_PATH, label="runtime v5 plan"))
    ...
    if (
        value.get("schema") != "dlolab-wrapping-risk-guard-runtime-qualification-v5"
        ...
        raise ValueError("registered runtime v5 qualification plan changed")
    return cast(dict[str, Any], value)


def runtime_identity_v5() -> dict[str, Any]:
...
def _source_hashes() -> dict[str, str]:
    if any(not (ROOT / name).is_file() for name in SOURCE_PATHS):
        raise ValueError("complete runtime v5 qualification source required")
...
def _validate_lock(output: Path) -> dict[str, Any]:
    ...
    current_plan = plan()
    current_runtime = runtime_identity_v5()
    current_native = native_source()
    lineage = _v4_failure_lineage()
    expected_plan_id = content_id(current_plan)
    if (
        lock.get("schema") != "dlolab-wrapping-risk-guard-runtime-qualification-lock-v5"
        ...
        or lock.get("v4_summary_id") != lineage["artifact_id"]
        or lock.get("v4_failure_id") != lineage["failure_id"]
        ...
        != "dlolab-wrapping-risk-guard-runtime-qualification-attempt-v5"
...
def _expected_claim(lock: dict[str, Any], task: dict[str, Any]) -> dict[str, Any]:
    return {
        "schema": "dlolab-wrapping-risk-guard-runtime-qualification-claim-v5",
...
def _load_success(
    ...
    if (
        ...
        or seal.get("schema")
        != "dlolab-wrapping-risk-guard-runtime-qualification-seal-v5"
...
def _worker(output: Path, kind: str, index: int) -> None:
    ...
    try:
        ...
        write_record(
            directory / "seal.json",
            {
                "schema": "dlolab-wrapping-risk-guard-runtime-qualification-seal-v5",
                "lock_id": lock["artifact_id"],
        ...
        write_record(
            ...
            {
                "schema": "dlolab-wrapping-risk-guard-runtime-qualification-task-failure-v5",
                "lock_id": lock["artifact_id"],
                "claim_id": claim["artifact_id"],
                "task": task,
    ...
    if run.returncode:
        if not (directory / "failure.json").exists():
            write_record(
                ...
                {
                    "schema": "dlolab-wrapping-risk-guard-runtime-process-failure-v5",
                    "lock_id": lock["artifact_id"],
                    "claim_id": claim["artifact_id"],
                    "task": task,
...
def _run(output: Path) -> None:
    ...
    attempt = write_record(
        ATTEMPT,
        {
            "schema": "dlolab-wrapping-risk-guard-runtime-qualification-attempt-v5",
            "revision": revision,
    ...
    lock = write_record(
        output / "lock.json",
        {
            "schema": "dlolab-wrapping-risk-guard-runtime-qualification-lock-v5",
            "revision": revision,
            "source_sha256": sources,
            "plan": current_plan,
            "plan_id": plan_id,
            ...
            "v4_summary_id": lineage["artifact_id"],
            "v4_failure_id": lineage["failure_id"],
            "output_root": str(OUTPUT),
    ...
    try:
        ...
        result = write_record(
            output / "result.json",
            {
                "schema": "dlolab-wrapping-risk-guard-runtime-qualification-result-v5",
                "status": "complete",
                "lock_id": lock["artifact_id"],
        ...
        write_record(
            output / "failure.json",
            {
                "schema": "dlolab-wrapping-risk-guard-runtime-qualification-failure-v5",
