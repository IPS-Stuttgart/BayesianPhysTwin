#!/usr/bin/env python3
"""Build one hash-only V14 frame-zero automatic twin before source locking."""
...
def main() -> int:
    ...
    _require(canonical_node_count == 384, "V14 physical node count changed")
    ...
    validation = validate_prediction_only_bundle(episode)
    marker = episode.get("prediction_only_input", {})
    _require(
        isinstance(marker, dict)
        and marker.get("protocol_id") == PRELOCK_PROTOCOL_ID
        and marker.get("queue_rank") == record["queue_rank"]
        and marker.get("object_hash") == record["object_hash"]
        and marker.get("case_hash") == record["case_hash"]
        and marker.get("plaintext_object_or_episode_identity_present") is False,
        "V14 automatic twin input identity changed",
    )
    ...
    summary: dict[str, Any] = {
        "schema_version": 1,
        "artifact_kind": ("Deform360CausalResponseDirectDepthAutomaticEpisodeTwinV14"),
        ...
        **record,
        ...
        "input_sha256": {
            ...
            "physical_prelock_protocol": file_sha256(args.prelock_protocol),
            "staging_queue": file_sha256(args.queue),
        },
        "output_sha256": {
        ...
        "passed": bool(state.metrics["passed"]),
        "claim_boundary": (
            "pre-lock automatic frame-zero twin; topology and state are built "
            "without prefix response, future object evidence, or outcomes"
        ),
    }
    summary["artifact_sha256"] = _canonical_sha256(summary)
