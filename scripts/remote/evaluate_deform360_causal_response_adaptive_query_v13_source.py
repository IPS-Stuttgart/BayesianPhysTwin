#!/usr/bin/env python3
"""Summarize the frozen target-free V13 adaptive-query source audit."""
...
CONFIG_RELATIVE_PATH = Path(
    "configs/sota/deform360_causal_response_adaptive_query_v13.json"
)
...
def main() -> None:
            ...
            rows.append(
                {
                    ...
                    "admitted": False,
                    "arm": None,
                    "strict_eligible_entity_count": None,
                    "fallback_eligible_entity_count": None,
                    "selected_entity_count": None,
                    "result_sha256": None,
                }
            )
            continue
        report, arrays = validate_adaptive_causal_response_query_artifacts(case_dir)
        _require(report["case"] == case_id, "adaptive report case changed")
        _require(
            report["inputs_sha256"]["protocol"] == file_sha256(config_path),
            "adaptive report used a different protocol file",
        )
        ...
        rows.append(
            {
                ...
                "arm": schedule["arm"],
                "strict_eligible_entity_count": int(
                    schedule["panel_score"]["strict_eligible_count"]
                ),
                "fallback_eligible_entity_count": int(
                    schedule["panel_score"]["fallback_eligible_count"]
                ),
    ...
    strict_admitted_count = sum(
        row["admitted"] and row["arm"] == STRICT_ARM for row in rows
    )
    fallback_admitted_count = admitted_count - strict_admitted_count
    technical_failure_count = sum(row["status"] == "technical_failure" for row in rows)
    gate = protocol["source_feasibility_gate"]
    gate_passed = (
        admitted_count >= int(gate["minimum_admitted_case_count"])
        and strict_admitted_count >= int(gate["minimum_strict_admitted_case_count"])
        and technical_failure_count <= int(gate["maximum_technical_failure_count"])
    )
    result: dict[str, Any] = {
        "schema_version": 1,
        "artifact_kind": (
            "Deform360CausalResponseAdaptiveQueryV13SourceFeasibilityResult"
        ),
        ...
        "strict_admitted_case_count": strict_admitted_count,
        "fallback_admitted_case_count": fallback_admitted_count,
        "technical_failure_count": technical_failure_count,
        "gate": {**gate, "passed": gate_passed},
        "cases": rows,
    ...
    _require(not output.exists(), "adaptive source summary already exists")
