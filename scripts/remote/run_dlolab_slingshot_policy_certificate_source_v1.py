...
from bayesian_phystwin_experiments.dlolab_slingshot_policy_certificate_source_v1 import (
    COUNTS,
    calibrate,
...
DEVELOPMENT_SUMMARY = (
    ROOT / "results/source/dlolab_slingshot_policy_certificate_development_v1/summary.json"
)
DEVELOPMENT_ARTIFACT_ID = (
    "ecf2a3375c38f31e2a371236ab6643083b9a12ffb9c48dfb292041ecad5e3bc0"
)
DEVELOPMENT_FILE_SHA256 = (
    "914a5f1978eac4a088a0f67c187e2969acc6c158cb95fff5e542363f9be5135f"
)
PARENT_FILE_SHA256 = {
...
SOURCES = (
    ...
    "src/bayesian_phystwin_experiments/dlolab_slingshot_policy_certificate_source_v1.py",
    "scripts/remote/run_dlolab_slingshot_policy_certificate_source_v1.py",
    "scripts/remote/verify_dlolab_slingshot_policy_certificate_source_v1.py",
    "tests/test_policy_gain_certificate.py",
    "tests/test_dlolab_slingshot_policy_certificate_v1.py",
    "tests/test_dlolab_slingshot_policy_certificate_source_v1.py",
    "tests/test_dlolab_slingshot_policy_certificate_source_v1_custody.py",
    "docs/dlolab_slingshot_policy_certificate_development_v1.md",
    "docs/dlolab_slingshot_policy_certificate_source_v1.md",
...
def load_parent() -> tuple[
    ...
    if (
        ...
        or development.get("closed_288_world_panel_used") is not False
    ...
    features = bias_invariant_features(np.stack(observations))
...
def freeze(output: Path) -> dict[str, Any]:
    ...
    return cast(
        ...
        write_record(
            output / "lock.json",
            {
                "schema": "dlolab-slingshot-policy-certificate-lock-v1",
...
def validate_lock(output: Path) -> dict[str, Any]:
    ...
    parent, _, _ = load_parent()
    if (
        lock.get("schema") != "dlolab-slingshot-policy-certificate-lock-v1"
...
def _candidate_seal(output: Path, lock: dict[str, Any], role: str) -> dict[str, Any]:
    ...
    if (
        seal.get("schema") != "dlolab-slingshot-policy-candidates-v1"
...
def _calibration_seal(output: Path, lock: dict[str, Any]) -> dict[str, Any]:
    ...
    if (
        seal.get("schema") != "dlolab-slingshot-policy-calibration-v1"
        or seal.get("lock_id") != lock["artifact_id"]
        or seal.get("policy_calibration", {}).get("calibration_count") != 96
        or seal.get("policy_calibration", {}).get("rank") != 88
        or seal.get("simultaneous_calibration", {}).get("count") != 96
        or seal.get("simultaneous_calibration", {}).get("rank") != 88
    ):
...
def _evaluation_barrier(
    ...
    if (
        decision.get("schema") != "dlolab-slingshot-policy-decisions-v1"
        or decision.get("lock_id") != lock["artifact_id"]
        or barrier.get("schema") != "dlolab-slingshot-policy-barrier-v1"
...
def worker(output: Path, role: str, kind: str, index: int) -> None:
    ...
    claim = write_record(
        ...
        {
            "schema": "dlolab-slingshot-policy-task-claim-v1",
            "lock_id": lock["artifact_id"],
            "task": spec,
    ...
    try:
        ...
        write_record(
            directory / "seal.json",
            {
                "schema": "dlolab-slingshot-policy-task-seal-v1",
                "lock_id": lock["artifact_id"],
        ...
        write_record(
            ...
            {
                "schema": "dlolab-slingshot-policy-task-failure-v1",
                "lock_id": lock["artifact_id"],
                "claim_id": claim["artifact_id"],
                "task": spec,
...
def _task_records(
    ...
    if (
        claim.get("schema") != "dlolab-slingshot-policy-task-claim-v1"
        or seal.get("schema") != "dlolab-slingshot-policy-task-seal-v1"
...
def _candidate_artifact(
    ...
    metadata = {
        "schema": "dlolab-slingshot-policy-candidates-v1",
        "lock_id": lock["artifact_id"],
        "role": role,
...
def _calibration_artifact(
    ...
    metadata = {
        "schema": "dlolab-slingshot-policy-calibration-v1",
        "lock_id": lock["artifact_id"],
...
def seal_evaluation_decisions(
    ...
    decision = write_record(
        directory / "seal.json",
        {
            "schema": "dlolab-slingshot-policy-decisions-v1",
            "lock_id": lock["artifact_id"],
            "candidate_seal_id": candidate_seal["artifact_id"],
    ...
    barrier = write_record(
        ...
        {
            "schema": "dlolab-slingshot-policy-barrier-v1",
            "lock_id": lock["artifact_id"],
            "decision_seal_id": decision["artifact_id"],
            "calibration_seal_id": calibration_seal["artifact_id"],
            "pre_future": preflight,
    ...
    try:
        ...
        if output.is_dir():
            write_record(
                output / "failure.json",
                {
                    "schema": "dlolab-slingshot-policy-run-failure-v1",
