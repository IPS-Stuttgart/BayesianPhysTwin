#!/usr/bin/env python3
"""Build the frozen automatic episode twin for one fresh authorized case."""
...
def _load_frozen_upstream(repo: Path) -> dict[str, Any]:
    """Load numerical modules from the separately checksummed runtime tree."""

    import bayesian_phystwin
    import causal4d_public

    upstream_root = repo.resolve() / "src"
    causal_package_path = upstream_root / "causal4d_public"
    bayesian_package_path = upstream_root / "bayesian_phystwin"
    _require(causal_package_path.is_dir(), "frozen causal4d_public package is missing")
    _require(
        bayesian_package_path.is_dir(), "frozen bayesian_phystwin package is missing"
    )
    _require(
        "bayesian_phystwin.phystwin_graph" not in sys.modules,
        "local PhysTwin graph loaded before frozen runtime binding",
    )
    if str(causal_package_path) not in causal4d_public.__path__:
        causal4d_public.__path__.insert(0, str(causal_package_path))
    if str(bayesian_package_path) not in bayesian_phystwin.__path__:
        bayesian_phystwin.__path__.insert(0, str(bayesian_package_path))
    dense = importlib.import_module(
        "causal4d_public.deform360_dense_reusable_panel"
    )
...
def main() -> int:
    args = _parse_args()
    upstream = _load_frozen_upstream(args.repo)
    load_dense_reusable_panel_config = upstream[
    ...
    build_canonical_deform360_graph = upstream["build_canonical_deform360_graph"]
    write_canonical_deform360_graph = upstream["write_canonical_deform360_graph"]
    PhysTwinSpringGraphConfig = upstream["PhysTwinSpringGraphConfig"]
    protocol = load_bias_aware_prospective_protocol(args.protocol)
    authorization = prospective_case_record(
    ...
    dense_config_path = (
        ...
        / "configs"
        / "causal4d_public"
        / "deform360_dense_reusable_panel_v1.json"
    )
    dense_protocol = load_dense_reusable_panel_config(dense_config_path)
    ...
    summary: dict[str, Any] = {
        "schema_version": 1,
        "artifact_kind": "Deform360BiasAwareAutomaticEpisodeTwin",
        ...
        **authorization,
        ...
        "input_sha256": {
            ...
            "dense_numeric_config": _sha256_file(dense_config_path),
        },
        "output_sha256": {
