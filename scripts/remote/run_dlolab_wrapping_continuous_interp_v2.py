#!/usr/bin/env python3
"""Run the frozen continuous-material wrapping interpolation study."""
...
from bayesian_phystwin_experiments.dlolab_wrapping_continuous_interp_native_v2 import (
    run_worlds,
)
...
PREFLIGHT = Path(
    "/home/fpfaff/source-only/dlolab-wrapping-continuous-interp-runtime-preflight-v2"
)
PREFLIGHT_ATTEMPT = Path(
    "/home/fpfaff/source-only/"
    "dlolab-wrapping-continuous-interp-runtime-preflight-v2.attempt.json"
)
...
NEW_SOURCES = (
    ...
    "src/bayesian_phystwin_experiments/dlolab_wrapping_continuous_interp_native_v2.py",
    "scripts/remote/run_dlolab_wrapping_continuous_interp_v2.py",
    "tests/test_dlolab_wrapping_continuous_interp_v2.py",
    "tests/test_dlolab_wrapping_continuous_interp_v2_custody.py",
    "docs/dlolab_wrapping_continuous_interp_source_v2.md",
...
def _load_preflight() -> tuple[dict[str, Any], dict[str, str]]:
    ...
    if (
        attempt.get("schema") != "dlolab-wrapping-continuous-interp-preflight-attempt-v2"
        ...
        or lock.get("schema") != "dlolab-wrapping-continuous-interp-preflight-lock-v2"
...
def _validate(output: Path) -> tuple[dict[str, Any], dict[str, Any], dict[str, Array]]:
    ...
    terminal_v1, terminal_hashes = _terminal_v1()
    if (
        lock.get("schema") != "dlolab-wrapping-continuous-interp-lock-v2"
        or lock.get("revision") != clean_revision(ROOT)
        or lock.get("source_sha256") != _source_hashes()
        or lock.get("protocol") != protocol()
        or lock.get("output_root") != str(OUTPUT)
        or lock.get("attempt_id") != attempt.get("artifact_id")
        or attempt.get("schema") != "dlolab-wrapping-continuous-interp-attempt-v2"
        or attempt.get("revision") != lock.get("revision")
        or attempt.get("source_sha256") != lock.get("source_sha256")
...
def _load_task(
    ...
    if (
        claim.get("schema") != "dlolab-wrapping-continuous-interp-claim-v2"
        ...
        or seal.get("schema") != "dlolab-wrapping-continuous-interp-seal-v2"
...
def _load_decisions(
    ...
    data = load_native_bundle(directory, seal["bundle"])
    if (
        seal.get("schema") != "dlolab-wrapping-continuous-interp-decision-seal-v2"
        or seal.get("lock_id") != lock["artifact_id"]
...
def _barrier_contents(
    ...
    return {
        "schema": "dlolab-wrapping-continuous-interp-decision-barrier-v2",
...
def _worker(output: Path, kind: str, index: int) -> None:
    ...
    claim = write_record(
        ...
        {
            "schema": "dlolab-wrapping-continuous-interp-claim-v2",
            "lock_id": lock["artifact_id"],
            "task": task,
    ...
    try:
        ...
        write_record(
            directory / "seal.json",
            {
                "schema": "dlolab-wrapping-continuous-interp-seal-v2",
                "lock_id": lock["artifact_id"],
        ...
        write_record(
            ...
            {
                "schema": "dlolab-wrapping-continuous-interp-failure-v2",
                "lock_id": lock["artifact_id"],
                "claim_id": claim["artifact_id"],
                "task": task,
...
def _run_preflight() -> None:
    ...
    attempt = write_record(
        PREFLIGHT_ATTEMPT,
        {
            "schema": "dlolab-wrapping-continuous-interp-preflight-attempt-v2",
            "revision": revision,
    ...
    lock = write_record(
        PREFLIGHT / "lock.json",
        {
            "schema": "dlolab-wrapping-continuous-interp-preflight-lock-v2",
            "revision": revision,
            "source_sha256": sources,
            "output_root": str(PREFLIGHT),
    ...
    claim = write_record(
        ...
        {
            "schema": "dlolab-wrapping-continuous-interp-preflight-claim-v2",
            "lock_id": lock["artifact_id"],
            "prefix_only": True,
            "retry_authorized": False,
        },
    ...
    try:
        ...
        seal = write_record(
            PREFLIGHT / "seal.json",
            {
                "schema": "dlolab-wrapping-continuous-interp-preflight-seal-v2",
                "lock_id": lock["artifact_id"],
                "claim_id": claim["artifact_id"],
        ...
        result = write_record(
            ...
            {
                "schema": "dlolab-wrapping-continuous-interp-preflight-result-v2",
                "lock_id": lock["artifact_id"],
        ...
        write_record(
            ...
            {
                "schema": "dlolab-wrapping-continuous-interp-preflight-failure-v2",
                "lock_id": lock["artifact_id"],
                "error_type": type(error).__name__,
                "message": str(error),
                "retry_authorized": False,
...
def _run(output: Path) -> None:
    ...
    attempt = write_record(
        ATTEMPT,
        {
            "schema": "dlolab-wrapping-continuous-interp-attempt-v2",
            "revision": revision,
            "source_sha256": sources,
            "protocol": protocol(),
    ...
    lock = write_record(
        output / "lock.json",
        {
            "schema": "dlolab-wrapping-continuous-interp-lock-v2",
            "revision": revision,
            "source_sha256": sources,
            "protocol": protocol(),
            "output_root": str(OUTPUT),
    ...
    try:
        ...
        decision_seal = write_record(
            directory / "seal.json",
            {
                "schema": "dlolab-wrapping-continuous-interp-decision-seal-v2",
                "lock_id": lock["artifact_id"],
        ...
        if not gate["pre_future_gate_passed"]:
            write_record(
                output / "result.json",
                {
                    "schema": "dlolab-wrapping-continuous-interp-result-v2",
        ...
        generation = write_record(
            ...
            {
                "schema": "dlolab-wrapping-continuous-interp-generation-v2",
                "lock_id": lock["artifact_id"],
                "barrier_id": barrier["artifact_id"],
