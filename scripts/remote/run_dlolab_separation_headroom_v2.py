#!/usr/bin/env python3
"""Run the frozen public DLO-Lab separation headroom v2 screen once."""
...
from bayesian_phystwin_experiments.dlolab_separation_headroom_v2 import (
    UNIQUE_ACTION_COUNT,
    development_metrics,
    native_qa,
    protocol,
...
ATTEMPT = Path(
    "/home/fpfaff/source-only/dlolab-separation-headroom-development-v2.attempt.json"
)
OUTPUT = Path("/home/fpfaff/source-only/dlolab-separation-headroom-development-v2")
...
SOURCES = (
    ...
    "src/bayesian_phystwin_experiments/dlolab_separation_headroom_v2.py",
    "src/bayesian_phystwin_experiments/dlolab_separation_native_v1.py",
    "scripts/remote/run_dlolab_separation_headroom_v2.py",
    "tests/test_dlolab_separation_headroom_v1.py",
    "tests/test_dlolab_separation_headroom_runner_v2.py",
    "docs/dlolab_separation_headroom_development_v2.md",
...
def runtime() -> dict[str, Any]:
    ...
    if {name: os.environ.get(name) for name in expected_environment} != (
        ...
        raise ValueError("registered v2 CPU/software-rendering environment required")
...
def validate_lock(output: Path) -> dict[str, Any]:
    ...
    if (
        attempt.get("schema") != "dlolab-separation-headroom-attempt-v2"
        ...
        or lock.get("schema") != "dlolab-separation-headroom-lock-v2"
...
def prepare_launch(output: Path) -> dict[str, Any]:
    """Validate all pure launch inputs before consuming the v2 attempt."""
    if (
        ...
        raise ValueError("the registered v2 attempt and output root must be fresh")
    return {
...
def run(output: Path) -> None:
    ...
    attempt = write_record(
        ATTEMPT,
        {
            "schema": "dlolab-separation-headroom-attempt-v2",
            **launch,
            "retry_authorized": False,
            "protected_data_read": False,
        },
    ...
    lock = write_record(
        output / "lock.json",
        {
            "schema": "dlolab-separation-headroom-lock-v2",
    ...
    def terminal(status: str, **extra: Any) -> dict[str, Any]:
        result = write_record(
            output / "result.json",
            {
                "schema": "dlolab-separation-headroom-result-v2",
                "lock_id": lock["artifact_id"],
                "status": status,
