#!/usr/bin/env python3
"""Run one post-open Dynamic TAPNext++ V2 source-development case."""
...
def main() -> int:
    ...
    active = (
        np.arange(58)[:, None] >= schedule.birth_frames[None]
    ) & (
        np.arange(58)[:, None] <= schedule.update_frames[None]
    )
    endpoint_supported = (
    ...
    report: dict[str, Any] = {
        ...
        "support": {
            ...
            "claim_bearing_fraction": float(
                np.sum(result.accepted_support & active)
                / max(1, np.sum(active))
            ),
