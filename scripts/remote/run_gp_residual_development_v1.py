...
def install_payload_guard(allowed_paths: set[Path]) -> None:
    ...
    def audit(event: str, arguments: tuple) -> None:
        ...
        if "eval" in path.parts or (
            path.suffix == ".pkl" and str(path) not in allowed
        ):
...
def metrics(prediction: np.ndarray, truth: np.ndarray) -> dict:
    ...
    return {
        ...
        "horizon_quartile_l1_mm": [
            float(block.mean() * 1000)
            for block in np.array_split(error, 4, axis=1)
        ],
    }
...
def mlp_correction(
    ...
    *,
    ...
    optimizer = torch.optim.Adam(
        [w1, b1, w2, b2], lr=0.003, weight_decay=weight_decay
    )

    def forward(value):
        hidden_values = torch.tanh(
            torch.einsum("bnf,nfh->bnh", value, w1) + b1
        )
...
def compare(protocol: dict, arrays: tuple, output: Path) -> dict:
    ...
    anchor_model = fit_deform_local_residual(
        initial[:40], action[:40], baseline[:40], targets[:40], names[:40],
        ridge=1.0, variance_floor_m2=1e-6,
    )
    anchor = predict_deform_local_residual(
        anchor_model, initial[40:], action[40:], baseline[40:], shrinkage=0.25,
    )["predictions"]
    experiments = []
    for seed in protocol["seeds"]:
        for budget in protocol["budgets"]:
            ...
            ridge = fit_deform_local_residual(
                ...
                [names[i] for i in train], ridge=1.0, variance_floor_m2=1e-6,
            )
            ...
            for strength in protocol["strengths"]:
                pred = predict_deform_local_residual(
                    ridge, initial[query], action[query], baseline[query],
                    shrinkage=strength,
                )["predictions"]
                candidates["ridge"].append(({"strength": strength}, pred))
            fixed_ridge = predict_deform_local_residual(
                ridge, initial[query], action[query], baseline[query], shrinkage=0.25,
            )["predictions"]
            ...
            for length in protocol["length_scales"]:
                for noise in protocol["noise_variances"]:
                    ...
                    for node in range(features.shape[2]):
                        ...
                        corrections.append(value.reshape(len(query), features.shape[1], 3))
                    correction = np.stack(corrections, axis=2)
                    for strength in protocol["strengths"]:
                        pred = apply_canonical_correction(
                            baseline[query], correction, frames[query], strength,
                        )
            ...
            for width in protocol["mlp_widths"]:
                correction = mlp_correction(
                    features[train], residual[train], features[query],
                    hidden=width, weight_decay=0.001, seed=seed,
                    per_group=protocol["support_per_recording"],
                )
                for strength in protocol["strengths"]:
                    pred = apply_canonical_correction(
                    ...
                    candidates["mlp"].append(({
                        "hidden": width, "strength": strength,
                    }, pred))
            selection = {}
            ...
            for family, bank in candidates.items():
                index = select_strength([value[:8] for _, value in bank], targets[inner])
                parameters, prediction = bank[index]
                selection[family] = {
                    "parameters": parameters,
                    "inner_l1_mm": float(np.mean(np.abs(prediction[:8] - targets[inner])) * 1000),
                    "all_inner_candidates": [
                        {**spec, "l1_mm": float(np.mean(np.abs(value[:8] - targets[inner])) * 1000)}
            ...
            write_json(subdir / "selection.json", {
            ...
            record = {
                "seed": seed, "budget": budget, "scores": scores,
                "selection": selection, "outer_names": [names[i] for i in outer],
                "prediction_sha256": digest(prediction_path),
            }
            write_json(subdir / "result.json", record)
            print(json.dumps({
                "seed": seed, "budget": budget,
                "l1_mm": {key: value["coordinate_l1_mm"] for key, value in scores.items()},
            }, sort_keys=True), flush=True)
            experiments.append(record)
    return {
        "contract": "gp-residual-development-v1", "experiments": experiments,
        "source_test_read": False, "official_eval_read": False,
        "upstream_training": False, "fresh_confirmation": False,
