#!/usr/bin/env python3
"""Run one frozen CPU-only wiring-post source screen; never retry a task."""
...
from bayesian_phystwin_experiments.dlolab_wiring_native import run_world
...
OUTPUT = Path("/home/fpfaff/source-only/dlolab-wiring-belief-source-v1")
SOURCES = (
    "src/bayesian_phystwin_experiments/dlolab_wiring_source.py",
    "src/bayesian_phystwin_experiments/dlolab_wiring_native.py",
    "scripts/remote/run_dlolab_wiring_source.py",
    "tests/test_dlolab_wiring_source.py",
    "tests/test_dlolab_wiring_runner.py",
    "docs/dlolab_wiring_belief_source_v1.md",
...
def validate_lock(output: Path) -> dict:
    ...
    if (output / "failure.json").exists() or (output / "result.json").exists():
        raise ValueError("terminal wiring source study; no retry")
    lock = read_record(output / "lock.json")
    if (
        lock["schema"] != "dlolab-wiring-source-lock-v1"
...
def load_task(output: Path, lock: dict, index: int):
    ...
    target = np.load(
        ASSETS / "upstream/genesis/assets/dlo-lab/target_pos/wiring_post_finalpos.npy",
        allow_pickle=False,
    )
    if array_digest(data["target_pos_m"]) != array_digest(target):
        raise ValueError("registered public target geometry changed")
    qa = native_qa(data, seal["native"], spec["world"])
    return seal, data, qa
...
def run(output: Path) -> None:
    ...
    def terminal(status: str, **extra: Any) -> dict:
        value = write_record(
            output / "result.json",
            {
                "schema": "dlolab-wiring-source-result-v1",
                "lock_id": lock["artifact_id"],
                "status": status,
                "completed_batches": len(completed),
                "admitted_batches": len(admitted),
                "ordinary_trajectories": 8 * len(completed),
    ...
    try:
        for index in range(11):
            ...
            attempted += 1
            print(f"starting frozen wiring batch {index + 1}/11", flush=True)
        ...
        bank = {
            ...
            "reward": np.asarray(
                [row[0]["native"]["native_final_reward"][:7] for row in rows]
            ),
        }
