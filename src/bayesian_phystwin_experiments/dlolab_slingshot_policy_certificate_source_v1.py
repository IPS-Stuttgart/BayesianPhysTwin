...
COUNTS = {"calibration": 96, "evaluation": 288}
WORLD_SEEDS = {"calibration": 261940, "evaluation": 261941}
SENSOR_SEEDS = {"calibration": 261942, "evaluation": 261943}
BOOTSTRAP_SEED = 261944
...
def protocol() -> dict[str, Any]:
    validate_rosters()
    return {
        "schema": "dlolab-slingshot-policy-certificate-source-v1",
        "role": "prospective_public_simulator_policy_level_gain_certificate",
        "reference": {
            "world_count": 51,
            "status": "previously_opened_development_only",
        ...
        "feature": "shared_bias_invariant_relative_geometry_and_temporal_increments",
        "local_predictor": {
            "kind": "standardized_nearest_neighbor_mean_gain",
...
def candidate_predictions(
    ...
    features = bias_invariant_features(observation)
...
def calibrate(
    ...
    if (result.calibration_count, result.rank) != (COUNTS["calibration"], 88):
...
def calibrate_simultaneous_guard(
    ...
    if (result.count, result.rank, result.offset is None) != (96, 88, False):
...
def score(
    ...
    *,
    ...
    return {
        "schema": "dlolab-slingshot-policy-certificate-result-v1",
        "arms": arms,
