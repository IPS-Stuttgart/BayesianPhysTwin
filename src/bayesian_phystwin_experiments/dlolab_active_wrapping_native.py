"""CPU-only native execution for the frozen active wrapping source study."""
...
from .dlolab_active_wrapping_source import (
    N_ENVS,
...
def _observe(env: Any) -> dict[str, np.ndarray]:
    def host(value: Any) -> np.ndarray:
        if hasattr(value, "detach"):
            value = value.detach().cpu().numpy()
        result = np.array(value, order="C", copy=True)
        if not np.isfinite(result).all():
            raise RuntimeError("nonfinite native active-wrapping observable")
        return result
    ...
    return {
        ...
        "robot_qpos": np.concatenate(
            [host(robot.get_qpos()) for robot in (env.franka1, env.franka2)], axis=1
        ),
    }
...
def run_world(
    ...
    if stage == "probe":
        if probe_index is not None:
            raise ValueError("probe-bank execution cannot select a probe")
        controls = probe_bank_controls()
    elif stage in {"baseline", "active"}:
        if type(probe_index) is not int:
            raise ValueError("full execution requires a selected probe")
        controls = full_action_controls(probe_index)
    else:
        raise ValueError("unregistered active-wrapping stage")
    native_steps = controls.shape[1] * 200
    ...
    try:
        ...
        def step(*args: Any, **kwargs: Any) -> Any:
            if len(trace) >= native_steps:
                raise RuntimeError("native active-wrapping horizon exceeded")
            if not trace:
        ...
        if (
            ...
            raise RuntimeError("incomplete native active-wrapping execution")
