...
class StudyData:
    ...
    def validate(self) -> None:
        if self.kind not in {"controlled-synthetic", "retrospective-development"}:
            raise ValueError(
                "this study cannot authorize confirmatory or target evidence"
            )
        ...
        if (
            self.baseline.shape[:2] != (recordings, horizon)
            or self.baseline.shape[-1] != 3
        ):
            raise ValueError("features and point predictions do not align")
        if (
            self.ridge.shape != self.baseline.shape
            or self.truth.shape != self.baseline.shape
        ):
            raise ValueError("point arrays do not align")
        if (
            self.times.shape != (horizon,)
            or horizon < 2
        ...
        if (
            self.ids.shape != (recordings,)
            or self.ids.dtype.kind != "U"
            or len(set(self.ids.tolist())) != recordings
        ):
        ...
        if not np.allclose(
            np.swapaxes(self.frames, -1, -2) @ self.frames, np.eye(3), atol=1e-10
        ) or not np.allclose(np.linalg.det(self.frames), 1.0, atol=1e-10):
        ...
        if not np.allclose(
            self.basis.T @ self.basis, np.eye(self.basis.shape[1]), atol=1e-12
        ):
...
def _modes(points: Array, basis: Array, frames: Array | None = None) -> Array:
    if frames is not None:
        points = (
            points @ frames
            if frames.ndim == 2
            else np.einsum("stvi,sij->stvj", points, frames)
        )
...
def _points(coefficients: Array, basis: Array, frames: Array | None = None) -> Array:
    ...
    if frames is not None:
        points = (
            points @ frames.T
            if frames.ndim == 2
            else np.einsum("stvj,sij->stvi", points, frames)
        )
    return points
...
def run_study(data: StudyData, protocol: dict) -> tuple[dict, dict[str, Array]]:
    ...
    selected_times = np.unique(
        np.linspace(0, len(data.times) - 1, protocol["train_times"], dtype=int)
    )
    train_features = data.features[fit][:, selected_times].reshape(
        len(fit) * len(selected_times), -1
    )
    train_response = _modes(
        (data.truth[fit] - data.ridge[fit])[:, selected_times],
        data.basis,
        data.frames[fit],
    ...
    for item in protocol["candidates"]:
        config = GPConfig(
            **item,
            ...
            output_scale_floor=protocol["output_scale_floor_m"],
        )
        model = RecordingDiscrepancyGP.fit(
            train_features, train_time, train_ids, train_response, config
        )
        errors = []
        for index in select:
            prediction = model.predict(
                data.features[index],
                data.times,
                np.repeat(data.ids[index], len(data.times)),
            )
            candidate = data.ridge[index] + _points(
                prediction.mean, data.basis, data.frames[index]
            )
            errors.append(float(np.mean(np.abs(candidate - data.truth[index]))))
        selection_records.append(
            {"config": asdict(config), "select_coordinate_l1_m": float(np.mean(errors))}
        )
        fitted.append(model)
    selected = min(
        range(len(fitted)), key=lambda i: selection_records[i]["select_coordinate_l1_m"]
    )
    model = fitted[selected]
    ridge_select_l1 = float(np.mean(np.abs(data.ridge[select] - data.truth[select])))
    accept_mean = (
        selection_records[selected]["select_coordinate_l1_m"] < ridge_select_l1
    )
    ...
    for index in np.r_[calibrate, score]:
        ...
        gp_means[int(index)] = data.ridge[index] + _points(
            prediction.mean, data.basis, data.frames[index]
        )
        means[int(index)] = (
            gp_means[int(index)] if accept_mean else data.ridge[index].copy()
        )
        raw_covariances[int(index)] = prediction.joint_covariance()
    calibration_error = np.stack(
        [
            _modes(data.truth[i] - means[int(i)], data.basis, data.frames[i]).ravel()
            for i in calibrate
        ]
    )
    scale = float(
        np.mean(
            [
                gaussian_metrics(error, raw_covariances[int(i)])["normalized_nees"]
    ...
    lowrank = empirical_lowrank(
        calibration_error,
        min(protocol["lowrank_rank"], *calibration_error.shape),
        protocol["lowrank_diagonal_fraction"],
        floor,
    )
    modes_per_frame = data.basis.shape[1] * 3
    calibration_truth = _modes(
        data.truth[calibrate], data.basis, data.frames[calibrate]
    )
    threshold = float(
        np.quantile(calibration_truth[:, -1, 0] - calibration_truth[:, 0, 0], 0.75)
    )
    ...
    for position, index in enumerate(score):
        ...
        point_records.append(
            {
                "recording_id": str(data.ids[index]),
                "baseline_l1_m": float(np.mean(np.abs(data.baseline[index] - truth))),
                "existing_ridge_l1_m": float(
                    np.mean(np.abs(data.ridge[index] - truth))
                ),
                "ridge_plus_gp_l1_m": float(
                    np.mean(np.abs(score_gp_means[position] - truth))
                ),
                "selected_mean_l1_m": float(
                    np.mean(np.abs(score_means[position] - truth))
                ),
            }
        )
        mean_modes = _modes(
            score_means[position], data.basis, data.frames[index]
        ).ravel()
        ...
        for name, covariance in covariances.items():
            ...
            probability = 0.5 * math.erfc(
                (threshold - float(query @ mean_modes)) / math.sqrt(2 * query_variance)
            )
            covariance_records.append(
                {
                    ...
                    "method": name,
                    **metrics,
                    "contrast_brier": (probability - event) ** 2,
                }
            )
    point_keys = (
        "baseline_l1_m",
    ...
    point_summary = {
        key: float(np.mean([item[key] for item in point_records])) for key in point_keys
    }
    covariance_summary = {}
    metric_keys = (
        "nll_per_dimension",
    ...
    for method in sorted({item["method"] for item in covariance_records}):
        ...
        covariance_summary[method] = {
            key: float(np.mean([item[key] for item in records])) for key in metric_keys
        }
    paired = {}
    gp_nll = np.array(
        [
            item["nll_per_dimension"]
            for item in covariance_records
            if item["method"] == "gp_scaled"
        ]
    )
    ...
    result = {
        ...
        "splits": {
            role: data.ids[data.split == role].tolist()
            for role in ("fit", "select", "calibrate", "score")
        },
        "train_rows": len(train_features),
        "fit_time_indices": selected_times.tolist(),
        "selection": selection_records,
        "selected_config": asdict(model.config),
        "gp_mean_accepted_on_select": bool(accept_mean),
        "existing_ridge_select_l1_m": ridge_select_l1,
    ...
    arrays = {
        "ids": data.ids[score],
        "times": data.times,
        "selected_predictions": score_means,
        "ridge_plus_gp_predictions": score_gp_means,
        "existing_ridge_predictions": data.ridge[score],
...
def synthetic_data(seed: int, adverse: bool = False) -> StudyData:
    ...
    features = np.stack(
        [np.tile(time, (recordings, 1)), np.repeat(context[:, None], horizon, axis=1)],
        axis=-1,
    )
    ...
    deterministic = (
        deterministic[:, :, None] * np.linspace(0.7, 1.3, modes)[None, None, :]
    )
    ou = np.exp(-np.abs(time[:, None] - time[None, :]) / 0.22)
    root = np.linalg.cholesky(ou + np.eye(horizon) * 0.15)
    execution = (
        np.stack(
            [rng.standard_normal((recordings, horizon)) @ root.T for _ in range(modes)],
            axis=-1,
        )
        * 0.0015
    )
    execution += (
        rng.normal(0, 0.0007, (recordings, 1, modes))
        * np.sin(2 * np.pi * time)[None, :, None]
    )
    residual = deterministic + execution
    if adverse:
        residual[split == "score"] += (
            0.009
            * np.sign(context[split == "score", None, None])
            * (time[None, :, None] > 0.45)
        )
    ...
    return StudyData(
        features,
        time,
        np.array([f"synthetic-{seed}-{i:02d}" for i in range(recordings)]),
        split,
        baseline,
        ridge,
        truth,
        basis,
        np.tile(np.eye(3), (recordings, 1, 1)),
