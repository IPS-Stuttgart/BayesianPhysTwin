"""CPU execution of registered off-grid DLO-Lab coiling worlds."""
...
from .dlolab_coiling_offgrid_source_v2 import source_worlds
...
def run_world(
    ...
    if world not in worlds or world != worlds[int(world["index"])]:
        raise ValueError("unregistered off-grid coiling world")
    ...
    state_realization: dict[str, list[float]] = {}
    state_measurements = {"maximum_offset_realization_error_m": float("inf")}
    ...
    class RegisteredWorld(Train_Env_Coiling):  # type: ignore[misc]
        ...
        def _randomize_positions(
            ...
            if envs_idx is not None or objects != [self.rope]:
                raise ValueError("full native coiling state reset required")
            before = _host(self.rope.get_all_verts())
            if before.shape != (8, 60, 3):
                raise ValueError("native coiling reset geometry changed")
            offset = torch.tensor(
                [world["offset_x_m"], world["offset_y_m"], 0.0],
                dtype=gs.tc_float,
            )
            updated = self.rope.get_all_verts_tc().clone()
            updated += offset[None, None]
            self.rope.set_pos(0, updated)
            actual = _host(self.rope.get_all_verts())
            expected = np.asarray(
                [world["offset_x_m"], world["offset_y_m"], 0.0],
                dtype=np.float64,
            )
            error = float(np.max(np.abs(actual - before - expected[None, None])))
            if error > 1e-12:
                raise ValueError("native coiling state offset changed")
            state_realization["offset_m"] = expected.tolist()
            state_measurements["maximum_offset_realization_error_m"] = error
    ...
    try:
        ...
        if (
            ...
            or set(state_realization) != {"offset_m"}
        ):
            raise RuntimeError("incomplete native off-grid coiling execution")
        ...
        return arrays, {
            ...
            "state_realization": state_realization,
