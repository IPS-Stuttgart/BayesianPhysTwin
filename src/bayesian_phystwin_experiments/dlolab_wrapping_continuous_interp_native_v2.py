"""CPU-only native execution for fresh continuous wrapping worlds."""
...
from .dlolab_wrapping_continuous_interp_v2 import validate_continuous_world
...
def run_worlds(
    ...
    *,
    ...
    class RegisteredWorld(Train_Env_Wrapping):  # type: ignore[misc]
        def _set_parameter(
            self,
            objects: list[Any],
            envs_idx: Any | None,
    ...
    try:
        ...
        def step(*args: Any, **kwargs: Any) -> Any:
            ...
            if not trace:
                initial.append(
                    np.array(env.rope.get_all_verts(), order="C", copy=True)
                )
