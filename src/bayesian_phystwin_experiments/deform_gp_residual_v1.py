...
class FiniteGP:
    ...
    def predict(self, x: np.ndarray, arc: np.ndarray | None = None) -> np.ndarray:
        ...
        for start in range(0, len(x), 2048):
            ...
            cross = matern_material_kernel(
                standardized, self.anchors, self.length, local_arc,
                self.anchor_arc, self.material_length,
            )
    ...
    def archive(self) -> dict[str, np.ndarray]:
        return {
            ...
            "factor": self.factor, "length": np.array(self.length),
            "material_length": np.array(self.material_length),
            "output_scale": self.output_scale, "weights": self.weights,
            "weight_covariance": self.weight_covariance, "noise": np.array(self.noise),
        }


def fit_gp_bank(
    x: np.ndarray, y: np.ndarray, *, arc: np.ndarray | None,
    anchor_indices: np.ndarray, length_multiplier: float,
    noises: list[float], row_weight: float, nugget: float = 1e-6,
    material_length: float = 0.35,
) -> dict[float, FiniteGP]:
    ...
    if indices.dtype.kind not in "iu" or np.any(indices < 0) or np.any(indices >= len(x)):
        raise ValueError("invalid anchor indices")
    if len(np.unique(indices)) != len(indices):
        raise ValueError("anchor indices repeat")
    if any(not np.isfinite(v) or v <= 0 for v in [row_weight, nugget, length_multiplier, *noises]):
    ...
    kuu = matern_material_kernel(anchors, anchors, length, anchor_arc, anchor_arc, material_length)
    ...
    for start in range(0, len(x), 2048):
        stop = start + 2048
        local_arc = None if arc is None else arc[start:stop]
        cross = matern_material_kernel((x[start:stop] - location) / scale, anchors, length,
                                       local_arc, anchor_arc, material_length)
    ...
    for noise in noises:
        ...
        covariance = noise * np.linalg.solve(chol.T, np.linalg.solve(chol, np.eye(len(indices))))
        result[noise] = FiniteGP(location, scale, anchors, anchor_arc, factor, length,
                                material_length, output_scale, weights, covariance, noise)
    return result


def balanced_anchor_indices(trajectories: int, horizon: int, nodes: int, count: int) -> np.ndarray:
...
def add_residual(baseline: np.ndarray, canonical: np.ndarray, frames: np.ndarray,
                 shrinkage: float) -> np.ndarray:
    baseline, canonical, frames = checked(baseline, 4), checked(canonical, 4), checked(frames, 3)
