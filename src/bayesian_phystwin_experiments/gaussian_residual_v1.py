...
def grouped_support(groups: np.ndarray, per_group: int) -> np.ndarray:
    ...
    if (
        isinstance(per_group, bool)
        or not isinstance(per_group, int)
        or per_group < 1
    ):
...
class GaussianResidual:
    ...
    def predict(
        ...
        *,
        ...
        if full_covariance:
            covariance = (
                matern32(query, query, self.length_scale) - solved.T @ solved
            )
...
def apply_canonical_correction(
    ...
    if (
        ...
        or not all(
            np.isfinite(value).all() for value in (baseline, correction, frames)
        )
    ):
        raise ValueError("correction inputs are invalid")
    if not np.allclose(
        np.swapaxes(frames, 1, 2) @ frames, np.eye(3), atol=1e-10
    ):
        raise ValueError("frames must be orthonormal")
    if strength == 0.0:
        return baseline
    candidate = baseline.copy()
    candidate[:, :, 2:-2] += strength * np.einsum(
        "ntvj,nij->ntvi", correction, frames
    )
    return candidate
