...
def _binomial_cdf(k: int, n: int, probability: float) -> float:
    ...
    The direct log-sum is sufficient for the small and medium confirmation
    panels used by the repository and avoids a SciPy dependency.
    """
...
def clopper_pearson_upper(
    ...
    *,
    ...
    """Exact one-sided Clopper--Pearson upper confidence endpoint.

    The returned value ``u`` satisfies the standard exact-binomial guarantee
    ``P_p(p <= u(K, n)) >= 1 - alpha`` for every Bernoulli probability ``p``.
    ...
    # For k < n, the one-sided endpoint is the p solving
    # P_{Bin(n,p)}(X <= k) = alpha.  The CDF is monotone decreasing in p.
    low, high = 0.0, 1.0
...
def hoeffding_upper(
    ...
    *,
    ...
    """Simultaneous one-sided Hoeffding upper endpoint for a finite family.

    If each group-level excess loss lies in ``[lower, upper]``, then a union
    bound over ``family_size`` fixed contexts gives simultaneous coverage at
    least ``1 - alpha``.
    """
...
class SelectiveCompetenceCertificateV1:
    """A compact certificate for one frozen selective policy."""
