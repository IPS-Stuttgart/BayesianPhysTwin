"""Outcome-gated source-value experiment for the qualified Genesis MPM runtime."""
...
from .genesis_mpm_source_qualification_v1 import (
...
PROTOCOL_SCHEMA: Final = "bayesian-phystwin.genesis-mpm-source-value-protocol"
GRID_SCHEMA: Final = "bayesian-phystwin.genesis-mpm-source-value-grid"
PREFIX_SCHEMA: Final = "bayesian-phystwin.genesis-mpm-source-value-prefix-result"
FUTURE_SCHEMA: Final = "bayesian-phystwin.genesis-mpm-source-value-future-result"
GRID_FILENAME: Final = "genesis-source-value-grid.json"
PREFIX_FILENAME: Final = "genesis-source-value-prefix-result.json"
FUTURE_FILENAME: Final = "genesis-source-value-future-result.json"
...
@dataclass(frozen=True, slots=True)
class GenesisSourceValueProtocolV1:
    ...
    groups: tuple[GenesisValueGroupV1, ...]
...
def load_genesis_source_value_protocol_v1(
    ...
    groups: list[GenesisValueGroupV1] = []
    ...
    return GenesisSourceValueProtocolV1(
        value=value,
...
def generate_genesis_source_value_predictions_v1(
    *,
    ...
    provenance = _git_provenance(
        ...
        source_paths=(
            ...
            "src/bayesian_phystwin/genesis_mpm_source_value_v1.py",
            "scripts/remote/run_genesis_mpm_source_value_v1.py",
        ),
    )
    ...
    for group in protocol.groups:
        ...
        arrays = load_genesis_source_inputs_v1(
            ...
            group=GenesisSourceGroupV1(
                ...
                source_inputs_relative_path=group.source_inputs_relative_path,
                source_inputs_sha256=group.source_inputs_sha256,
                incumbent_relative_path=group.incumbent_relative_path,
                incumbent_sha256=group.incumbent_sha256,
                frame_count=group.frame_count,
                material_particle_count=group.material_particle_count,
                controller_point_count=group.controller_point_count,
                attached_particle_count=group.attached_particle_count,
            ),
        )
        ...
        for index, modulus in enumerate(protocol.young_moduli_pa):
            replay = _run_native_replay(
                ...
                young_modulus_pa=modulus,
                substeps=int(protocol.candidate["substeps"]),
...
def _validation_ratios(metrics: Mapping[str, Any]) -> dict[str, float]:
    ...
    genesis = _mapping(validation["genesis"], name="Genesis validation metrics")
...
def _validation_checks(
    *,
    protocol: GenesisSourceValueProtocolV1,
...
def _validated_group_metrics(
    value: object,
    *,
    expected_group: GenesisValueGroupV1,
) -> dict[str, Any]:
    ...
    for split_name in ("fit", "validation"):
        ...
        _exact_fields(
            raw_split,
            {"genesis", "persistence", "incumbent", "matphys"},
        ...
        normalized_metrics[split_name] = {
            ...
            for comparator in ("genesis", "persistence", "incumbent", "matphys")
        }
...
def score_genesis_source_value_prefix_v1(
    *,
    protocol_path: str | Path,
    group_roots: Mapping[str, str | Path],
    matphys_paths: Mapping[str, str | Path],
...
def score_genesis_source_value_future_v1(
    *,
    protocol_path: str | Path,
    group_roots: Mapping[str, str | Path],
    matphys_paths: Mapping[str, str | Path],
