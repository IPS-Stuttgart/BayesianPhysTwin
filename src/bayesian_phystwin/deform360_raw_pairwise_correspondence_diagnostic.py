...
PHYSICAL_CLIQUE_RBF_ARM = "physical_full_blend_rbf_pairwise_clique"
PERSISTENCE_CLIQUE_RBF_ARM = "persistence_full_blend_rbf_pairwise_clique"
...
def evaluate_raw_pairwise_correspondence_arrays(
    ...
    *,
    ...
    trajectories[PERSISTENCE_CLIQUE_RBF_ARM] = persistence_input.copy()
    ...
    for update_index, update in enumerate(UPDATE_FRAMES):
        ...
        trajectories[PHYSICAL_CLIQUE_RBF_ARM][update + 1 : stop] = prior[
            update + 1 : stop
        ]
        trajectories[PERSISTENCE_CLIQUE_RBF_ARM][update + 1 : stop] = persistence[
            update + 1 : stop
        ]
        ...
        if selector_support:
            ...
            for backbone_name, arm in (
                (PHYSICAL_ARM, PHYSICAL_CLIQUE_RBF_ARM),
                (PERSISTENCE_ARM, PERSISTENCE_CLIQUE_RBF_ARM),
            ):
                backbone_gate = gates[backbone_name]
                backbone = backbones[backbone_name]
                if not backbone_gate.accepted:
                    continue
                for frame in range(update + 1, stop):
                    decoded = decode_recursive_rbf_belief(
                        clique_states[backbone_name],
                    ...
                    trajectories[arm][frame] = _corrected_frame(
                        backbone[frame], decoded.mean_m, dtype=output_dtype
                    )
        ...
        for backbone_name, arm in (
            ...
            if gates[backbone_name].accepted:
                continue
            backbone = backbones[backbone_name]
            if not np.array_equal(
                trajectories[arm][update + 1 : stop],
                backbone[update + 1 : stop],
            ):
                raise AssertionError(
                    f"{backbone_name} clique abstention did not preserve its backbone"
                )
        ...
        updates.append(
            {
                ...
                "backbone_pairwise_gates": {
                    backbone_name: {
                        "accepted": bool(selector_support and gate.accepted),
                        "decision": (
                            gate.decision
                            if selector_support
                        ...
                        "bit_exact_raw_fallback": bool(
                            not (selector_support and gate.accepted)
                            and np.array_equal(
                                trajectories[
                                    PHYSICAL_CLIQUE_RBF_ARM
                                    if backbone_name == PHYSICAL_ARM
                                    else PERSISTENCE_CLIQUE_RBF_ARM
                                ][update + 1 : stop],
                                backbones[backbone_name][update + 1 : stop],
                            )
                        ),
                    }
                    for backbone_name, gate in gates.items()
                },
...
def evaluate_raw_pairwise_correspondence_cohort(
    ...
    pairs = (
        ...
        (PERSISTENCE_CLIQUE_RBF_ARM, PERSISTENCE_ARM),
        (PERSISTENCE_CLIQUE_RBF_ARM, SELECTED_RAW_ARM),
        (PERSISTENCE_CLIQUE_RBF_ARM, CLIQUE_RBF_ARM),
        (PHYSICAL_CLIQUE_RBF_ARM, PHYSICAL_ARM),
        (CLIQUE_RBF_ARM, PHYSICAL_CLIQUE_RBF_ARM),
