...
def analyze_discrepancy_candidate_tournament(
    ...
    provisional_selected, summaries = _select_candidate(
        evidence,
        groups,
        seed_offset=0,
    )
    ...
    for fold_index, held_group in enumerate(sorted(groups)):
        ...
        stable_selection &= fold_selected == provisional_selected
    ...
    provisional_summary = summary_by_id[provisional_selected]
    source_gate_passed = (
        provisional_selected != evidence.reference_candidate
        and bool(provisional_summary["eligible"])
        and held_group_nonregression
        and (stable_selection or not evidence.config.require_crossfit_stability)
    )
    selected = (
        provisional_selected if source_gate_passed else evidence.reference_candidate
    )
    ...
    report: dict[str, Any] = {
        ...
        "provisional_selected_candidate": provisional_selected,
        "provisional_selected_candidate_summary": provisional_summary,
