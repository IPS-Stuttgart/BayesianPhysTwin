...
def _parse_target(value: object, *, index: int) -> PracticalEquivalenceTargetV1:
    ...
    _fields(
        target,
        required=frozenset({"metric", "stream", "margin", "unit", "margin_basis"}),
        name=name,
    )
...
def parse_practical_equivalence_policy(
    ...
    if root["contract"] != PRACTICAL_EQUIVALENCE_POLICY_CONTRACT:
        raise ValueError(f"contract must be {PRACTICAL_EQUIVALENCE_POLICY_CONTRACT!r}")
    ...
    return PracticalEquivalencePolicyV1(
        ...
        margins_frozen_before_outcomes=_boolean(
            ...
            name=("policy.information_boundary.margins_frozen_before_outcomes"),
        ),
        outcomes_used_for_margin_selection=_boolean(
            information["outcomes_used_for_margin_selection"],
            name=("policy.information_boundary.outcomes_used_for_margin_selection"),
        ),
...
def _policy_json(policy: PracticalEquivalencePolicyV1) -> dict[str, object]:
    return {
        ...
        "information_boundary": {
            "margins_frozen_before_outcomes": (policy.margins_frozen_before_outcomes),
            "outcomes_used_for_margin_selection": (
...
def _group_difference_summary(
    ...
    *,
    ...
    if tuple(candidate) != tuple(reference):
        raise ValueError(f"candidate/reference group sets differ for metric {metric!r}")
    differences = {
...
def assess_practical_equivalence(
    ...
    for target in policy.targets:
        ...
        candidate = _required_mapping(
            ...
            name=(
                f"bootstrap candidate {policy.candidate_method!r} for {target.metric!r}"
            ),
        )
        ...
        decision_authorized = prospective_policy and group_requirement_met and complete
        ...
        if not complete or interval is None:
            ...
            probability = comparison.get("bootstrap_probability_candidate_better")
            result.update(
                {
                    "confidence_interval": {
                        "confidence": policy.bootstrap_confidence,
    ...
    summary = {
        ...
        "all_practically_equivalent": all(
            decision["practical_equivalence_pass"] is True for decision in decisions
        ),
