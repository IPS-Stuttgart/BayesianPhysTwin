"""Adapt fixed-identity Newton MPM particles to physical rollout v1."""
...
NEWTON_MPM_RUNTIME_SCHEMA: Final = "bayesian-phystwin.newton-mpm-runtime"
NEWTON_MPM_ARTIFACT_SCHEMA: Final = "bayesian-phystwin.newton-mpm-backend"
NEWTON_MPM_SCHEMA_VERSION: Final = 1
NEWTON_MPM_BACKEND_KIND: Final = "newton-implicit-mpm-v1"
NEWTON_MPM_ENGINE_REPOSITORY: Final = "newton-physics/newton"
NEWTON_MPM_RAW_ARRAY_NAMES: Final = frozenset(
    {
...
RAW_ARCHIVE_FILENAME: Final = "newton-particle-rollout.npz"
RUNTIME_FILENAME: Final = "newton-runtime.json"
ARTIFACT_FILENAME: Final = "newton-mpm-backend.json"
CHECKSUMS_FILENAME: Final = "SHA256SUMS"

NEWTON_MPM_CLAIM_BOUNDARY: Final = (
    "A synthetic fixed-material-identity compatibility smoke for Newton's "
    "implicit MPM solver. It validates the simulator-to-belief artifact path; "
    "it does not reproduce PhysWorld, establish real-data transfer, calibrate "
    "an MPM twin, or support a state-of-the-art claim."
)
...
def validate_newton_mpm_runtime_manifest(
    value: Mapping[str, Any],
    *,
    raw_rollout_path: str | Path | None = None,
) -> dict[str, Any]:
    """Validate a content-addressed Newton runtime record."""

    require_exact_fields(value, expected=_RUNTIME_FIELDS, name="runtime manifest")
    _require(value.get("schema") == NEWTON_MPM_RUNTIME_SCHEMA, "runtime schema changed")
    _require(
        value.get("schema_version") == NEWTON_MPM_SCHEMA_VERSION,
        "runtime schema version changed",
    )
    _require(
        value.get("backend_kind") == NEWTON_MPM_BACKEND_KIND,
        "runtime backend kind changed",
    )
    _require(
        value.get("engine_repository") == NEWTON_MPM_ENGINE_REPOSITORY,
    ...
    _require(simulation.get("scene") == "kinematic-beam-bend-v1", "scene changed")
    ...
    action_displacement = _finite_vector(
        simulation.get("action_displacement_m"),
        name="action_displacement_m",
    )
    _require(
        np.linalg.norm(action_displacement) > 0.0,
        "action displacement must be nonzero",
    )
    ...
    damping = simulation.get("damping")
    _require(
        isinstance(damping, (int, float))
        and not isinstance(damping, bool)
        and np.isfinite(float(damping))
        and float(damping) >= 0.0,
        "damping must be finite and nonnegative",
    )
    _positive_integer(simulation.get("substeps"), name="substeps")
    _positive_integer(simulation.get("max_iterations"), name="max_iterations")
    nonempty_string(simulation.get("solver"), name="solver")
    raw_digest = sha256_digest(
        value.get("raw_rollout_sha256"),
...
def load_newton_particle_rollout(
    path: str | Path,
) -> tuple[Path, dict[str, npt.NDArray[Any]]]:
    """Load a fixed-identity Newton particle rollout without pickle."""

    source = _ordinary_file(path, name="raw rollout")
    try:
        with np.load(source, allow_pickle=False) as stored:
            _require(
                set(stored.files) == set(NEWTON_MPM_RAW_ARRAY_NAMES),
                "raw Newton array roster changed",
            )
            arrays = {
        ...
        raise ValueError("cannot load raw Newton rollout") from error
    ...
    _require(
        ...
        "Newton particle positions must have shape (T,P,3)",
    )
    ...
    _require(
        ...
        "Newton particle positions must share a floating dtype",
    )
    _require(
        np.all(np.isfinite(driven)) and np.all(np.isfinite(zero)),
        "Newton particle positions are non-finite",
    )
    _require(np.array_equal(driven[0], zero[0]), "Newton rollouts differ at frame zero")
    _require(
...
def physical_rollout_from_newton_particles(
...
def materialize_newton_mpm_backend(
    *,
    raw_rollout_path: str | Path,
    runtime_manifest_path: str | Path,
    output_dir: str | Path,
) -> dict[str, Any]:
    """Publish one self-contained Newton MPM compatibility bundle."""

    raw_source, raw = load_newton_particle_rollout(raw_rollout_path)
    runtime_source = _ordinary_file(runtime_manifest_path, name="runtime manifest")
    runtime = validate_newton_mpm_runtime_manifest(
        load_strict_json_object(runtime_source, label="Newton runtime manifest"),
    ...
    try:
        ...
        identity = {
            "schema": NEWTON_MPM_ARTIFACT_SCHEMA,
            ...
            "claim_boundary": NEWTON_MPM_CLAIM_BOUNDARY,
        }
        ...
        raise
    return validate_newton_mpm_backend(output)


def validate_newton_mpm_backend(output_dir: str | Path) -> dict[str, Any]:
    ...
    _require(
        artifact.get("claim_boundary") == NEWTON_MPM_CLAIM_BOUNDARY,
