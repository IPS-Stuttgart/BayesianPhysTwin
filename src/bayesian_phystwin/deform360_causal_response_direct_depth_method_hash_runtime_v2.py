"""Runtime-only repair for the frozen V14 method-protocol checksum."""

from __future__ import annotations

import hashlib
import importlib.util
import json
import sys
from collections.abc import Mapping
from pathlib import Path
from types import ModuleType
from typing import Any

from .deform360_object_exclusion import file_sha256

RUNTIME_KIND = "Deform360CausalResponseDirectDepthMethodHashRuntimeV14V2"
RUNTIME_CONTRACT = "deform360-causal-response-direct-depth-method-hash-runtime-v14-v2"
RUNTIME_PROTOCOL_ID = (
    "deform360-causal-response-direct-depth-v14-method-hash-runtime-v2"
)
METHOD_PROTOCOL_ID = "deform360-causal-response-direct-depth-v14-source"
METHOD_HASH_NAMESPACE = b"deform360-causal-response-direct-depth-v14-protocol\0"
AMENDMENT_ARGUMENT = "--method-hash-runtime-v2"
ROLE_PARENT_FILES = {
    "admission": (
        "scripts/remote/run_deform360_causal_response_direct_depth_v14_admission.py"
    ),
    "source_finalizer": (
        "scripts/remote/finalize_deform360_causal_response_direct_depth_v14_source.py"
    ),
}


def _require(condition: bool, message: str) -> None:
    if not condition:
        raise ValueError(message)


def _valid_digest(value: Any) -> bool:
    return (
        isinstance(value, str)
        and len(value) == 64
        and all(character in "0123456789abcdef" for character in value)
    )


def _read_json(path: str | Path) -> dict[str, Any]:
    source = Path(path)
    try:
        payload = json.loads(source.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as error:
        raise ValueError(f"cannot read V14 method-hash artifact: {source}") from error
    _require(isinstance(payload, dict), f"JSON artifact is not an object: {source}")
    return payload


def _canonical_sha256(payload: Mapping[str, Any]) -> str:
    canonical = dict(payload)
    canonical.pop("config_sha256", None)
    return hashlib.sha256(
        b"deform360-causal-response-direct-depth-method-hash-runtime-v14-v2\0"
        + json.dumps(
            canonical,
            sort_keys=True,
            separators=(",", ":"),
            allow_nan=False,
        ).encode("utf-8")
    ).hexdigest()


def correct_v14_method_config_sha256(payload: Mapping[str, Any]) -> str:
    """Return the checksum registered by the frozen V14 method protocol."""

    canonical = dict(payload)
    canonical.pop("config_sha256", None)
    return hashlib.sha256(
        METHOD_HASH_NAMESPACE
        + json.dumps(
            canonical,
            sort_keys=True,
            separators=(",", ":"),
            allow_nan=False,
        ).encode("utf-8")
    ).hexdigest()


def legacy_v14_method_config_sha256(payload: Mapping[str, Any]) -> str:
    """Return the incorrect unnamespaced checksum used by both parent runners."""

    canonical = dict(payload)
    canonical.pop("config_sha256", None)
    return hashlib.sha256(
        json.dumps(
            canonical,
            sort_keys=True,
            separators=(",", ":"),
            allow_nan=False,
        ).encode("utf-8")
    ).hexdigest()


def load_v14_method_hash_runtime_v2(
    path: str | Path,
    *,
    repository: str | Path,
    method_protocol_path: str | Path,
) -> dict[str, Any]:
    """Validate the checksum-only amendment and all frozen parent bytes."""

    payload = _read_json(path)
    _require(
        payload.get("schema_version") == 1
        and payload.get("artifact_kind") == RUNTIME_KIND
        and payload.get("contract") == RUNTIME_CONTRACT
        and payload.get("protocol_id") == RUNTIME_PROTOCOL_ID
        and payload.get("status")
        == "locked_after_checksum_defect_discovery_before_first_source_admission"
        and payload.get("config_sha256") == _canonical_sha256(payload),
        "V14 method-hash runtime identity or checksum changed",
    )
    root = Path(repository).resolve()
    method_path = Path(method_protocol_path).resolve()
    method = _read_json(method_path)
    parents = payload.get("parent_artifacts")
    _require(
        isinstance(parents, Mapping)
        and set(parents) == {"method_protocol", "admission_prelock", "source_finalizer"}
        and all(
            isinstance(record, Mapping)
            and _valid_digest(record.get("semantic_sha256"))
            and _valid_digest(record.get("file_sha256"))
            and isinstance(record.get("path"), str)
            for record in parents.values()
        ),
        "V14 method-hash parent ledger changed",
    )
    method_parent = parents["method_protocol"]
    _require(
        method_path == (root / method_parent["path"]).resolve()
        and method.get("protocol_id") == METHOD_PROTOCOL_ID
        and method.get("config_sha256") == method_parent["semantic_sha256"]
        and file_sha256(method_path) == method_parent["file_sha256"]
        and correct_v14_method_config_sha256(method) == method["config_sha256"],
        "V14 method protocol differs from the registered parent",
    )
    for role in ("admission_prelock", "source_finalizer"):
        record = parents[role]
        parent_path = (root / record["path"]).resolve()
        parent = _read_json(parent_path)
        _require(
            parent.get("config_sha256") == record["semantic_sha256"]
            and file_sha256(parent_path) == record["file_sha256"],
            f"V14 method-hash parent changed: {role}",
        )

    trigger = payload.get("trigger")
    _require(
        isinstance(trigger, Mapping)
        and trigger.get("registered_namespaced_sha256") == method["config_sha256"]
        and trigger.get("correct_namespaced_sha256")
        == correct_v14_method_config_sha256(method)
        and trigger.get("legacy_unnamespaced_sha256")
        == legacy_v14_method_config_sha256(method)
        and trigger.get("legacy_matches_registered") is False
        and trigger.get("source_admission_artifact_created") is False
        and trigger.get("source_lock_artifact_created") is False
        and trigger.get("prefix_scan_started") is False
        and trigger.get("source_outcome_read") is False,
        "V14 method-hash trigger changed",
    )
    amendment = payload.get("amendment")
    _require(
        isinstance(amendment, Mapping)
        and amendment.get("patched_symbol") == "_canonical_config_sha256"
        and amendment.get("patched_parent_roles") == ["admission", "source_finalizer"]
        and amendment.get("method_protocol_bytes_changed") is False
        and amendment.get("estimator_or_gate_changed") is False
        and amendment.get("only_adds_registered_hash_namespace") is True,
        "V14 method-hash amendment scope changed",
    )

    implementation = payload.get("implementation")
    files = implementation.get("files") if isinstance(implementation, Mapping) else None
    _require(
        isinstance(implementation, Mapping)
        and isinstance(implementation.get("parent_commit"), str)
        and len(implementation["parent_commit"]) == 40
        and isinstance(files, Mapping)
        and set(files)
        == {
            "runtime_module",
            "admission_wrapper",
            "source_finalizer_wrapper",
            "admission_parent",
            "source_finalizer_parent",
        }
        and all(
            isinstance(record, Mapping)
            and isinstance(record.get("path"), str)
            and _valid_digest(record.get("file_sha256"))
            for record in files.values()
        ),
        "V14 method-hash implementation ledger changed",
    )
    for name, record in files.items():
        implementation_path = (root / record["path"]).resolve()
        _require(
            implementation_path.is_file()
            and file_sha256(implementation_path) == record["file_sha256"],
            f"V14 method-hash implementation changed: {name}",
        )
    _require(
        files["admission_parent"]["path"] == ROLE_PARENT_FILES["admission"]
        and files["source_finalizer_parent"]["path"]
        == ROLE_PARENT_FILES["source_finalizer"],
        "V14 method-hash parent roles changed",
    )
    boundary = payload.get("information_boundary")
    _require(
        isinstance(boundary, Mapping)
        and boundary.get("object_observation_read") is False
        and boundary.get("tactile_or_robot_prefix_read") is False
        and boundary.get("future_identity_or_metric_read") is False
        and boundary.get("source_outcome_read") is False
        and boundary.get("target_object_or_outcome_read") is False
        and boundary.get("held_v8_artifact_or_process_access") is False,
        "V14 method-hash runtime crossed its information boundary",
    )
    return payload


def _argument_path(name: str, *, remove: bool) -> Path:
    indices = [index for index, value in enumerate(sys.argv) if value == name]
    _require(len(indices) == 1, f"{name} must appear exactly once")
    index = indices[0]
    _require(index + 1 < len(sys.argv), f"{name} lacks a value")
    path = Path(sys.argv[index + 1]).resolve()
    if remove:
        del sys.argv[index : index + 2]
    return path


def _load_parent(path: Path, role: str) -> ModuleType:
    spec = importlib.util.spec_from_file_location(
        f"bayesian_phystwin_v14_method_hash_parent_{role}",
        path,
    )
    _require(
        spec is not None and spec.loader is not None,
        f"cannot load V14 parent runner: {path}",
    )
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


def run_v14_method_hash_amended_parent(
    *,
    role: str,
    wrapper_path: str | Path,
) -> int:
    """Validate, patch, and execute one otherwise frozen V14 parent runner."""

    _require(role in ROLE_PARENT_FILES, "V14 method-hash parent role is invalid")
    amendment_path = _argument_path(AMENDMENT_ARGUMENT, remove=True)
    method_path = _argument_path("--method-protocol", remove=False)
    wrapper = Path(wrapper_path).resolve()
    repository = wrapper.parents[2]
    amendment = load_v14_method_hash_runtime_v2(
        amendment_path,
        repository=repository,
        method_protocol_path=method_path,
    )
    wrapper_role = f"{role}_wrapper"
    _require(
        file_sha256(wrapper)
        == amendment["implementation"]["files"][wrapper_role]["file_sha256"],
        "V14 method-hash wrapper changed",
    )
    parent_path = (repository / ROLE_PARENT_FILES[role]).resolve()
    parent = _load_parent(parent_path, role)
    method = _read_json(method_path)
    _require(
        parent._canonical_config_sha256(method)
        == amendment["trigger"]["legacy_unnamespaced_sha256"],
        "V14 parent no longer exhibits the registered checksum defect",
    )
    parent._canonical_config_sha256 = correct_v14_method_config_sha256
    _require(
        parent._canonical_config_sha256(method) == method["config_sha256"],
        "V14 method-hash runtime did not repair the registered checksum",
    )
    return int(parent.main())


__all__ = [
    "AMENDMENT_ARGUMENT",
    "METHOD_HASH_NAMESPACE",
    "METHOD_PROTOCOL_ID",
    "ROLE_PARENT_FILES",
    "RUNTIME_CONTRACT",
    "RUNTIME_KIND",
    "RUNTIME_PROTOCOL_ID",
    "correct_v14_method_config_sha256",
    "legacy_v14_method_config_sha256",
    "load_v14_method_hash_runtime_v2",
    "run_v14_method_hash_amended_parent",
]
