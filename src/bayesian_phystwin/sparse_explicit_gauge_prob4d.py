"""Claim-bearing Prob4D factors with native sparse gauge inference."""
...
class NativeSparseExplicitGaugeFactorAdapterResult:
    """Strict explicit-gauge inputs without a dense row-by-gauge tensor."""

    batch: GaugeAwareObservationBatch
    sparse_gauge_design: SparseGaugeDesignV1
    observation_artifact_id: str
    linearization_artifact_id: str
    provider_manifest_id: str
    calibration_artifact_ids: Mapping[str, str]
    runtime_revision_source: str
    dense_gauge_design_avoided_bytes: int
    gauge_ids: tuple[str, ...]
    view_ids: tuple[str, ...]
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.sparse_gauge_design, SparseGaugeDesignV1):
            raise TypeError("sparse_gauge_design must be a SparseGaugeDesignV1")
        object.__setattr__(
            self,
        ...
        object.__setattr__(
            self,
            "dense_gauge_design_avoided_bytes",
            _require_integer(
                self.dense_gauge_design_avoided_bytes,
                name="dense_gauge_design_avoided_bytes",
                minimum=0,
            ),
        )
        object.__setattr__(
        ...
        if self.sparse_gauge_design.gauge_ids != self.gauge_ids:
            raise ValueError("adapter gauge IDs differ from its sparse gauge design")
        if (
            self.sparse_gauge_design.equivalent_dense_design_bytes
            != self.dense_gauge_design_avoided_bytes
        ):
            raise ValueError("adapter dense-design accounting has changed")


def build_claim_bearing_native_sparse_explicit_gauge_batch(
    validated_bundle: Any,
    sparse_stack: Any,
    ...
    *,
    ...
    """Validate a strict factor bundle while retaining block-local gauges."""
    ...
    reserved_metadata: dict[str, Any] = {
        ...
        "prob4d_dense_gauge_design_avoided_bytes": (
            sparse_gauge_design.equivalent_dense_design_bytes
        ),
        "prob4d_sparse_gauge_block_size": sparse_gauge_design.block_size,
    ...
    if collisions:
        raise ValueError(
            "metadata overrides reserved native-sparse explicit-gauge fields: "
            f"{sorted(collisions)}"
        )
    extra_plain = cast(dict[str, Any], plain_json(extra_metadata))
    ...
    batch = GaugeAwareObservationBatch(
        ...
        gauge_jacobian=np.zeros(
            (selected_observation_count, 3, 0),
            dtype=np.float64,
        ),
    ...
    return NativeSparseExplicitGaugeFactorAdapterResult(
        batch=batch,
        sparse_gauge_design=sparse_gauge_design,
        observation_artifact_id=artifact_id,
        linearization_artifact_id=linearization.artifact_id,
...
def update_claim_bearing_native_sparse_explicit_gauge_from_artifacts(
    validated_bundle: Any,
    sparse_stack: Any,
    linearization: PhysicalLinearizationV1,
    *,
...
__all__ = [
    "NATIVE_SPARSE_EXPLICIT_GAUGE_BRIDGE_VERSION",
    "NativeSparseExplicitGaugeFactorAdapterResult",
