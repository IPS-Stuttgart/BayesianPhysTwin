...
class GroupedStudentTLikelihoodConfig:
    ...
    def __post_init__(self) -> None:
        if not np.isfinite(self.degrees_of_freedom) or (
        ...
        if not np.isfinite(self.model_discrepancy_variance_m2) or (
            ...
            raise ValueError(
                "model_discrepancy_variance_m2 must be nonnegative"
            )
...
class GroupedStudentTLikelihoodResult:
    ...
    def __post_init__(self) -> None:
        ...
        arrays = {
            ...
            "weighted_negative_log_likelihood": (
                self.weighted_negative_log_likelihood
            ),
            "posterior_nominal_probability": (
                self.posterior_nominal_probability
            ),
            ...
            "mean_association_probability": (
                self.mean_association_probability
            ),
            "covariance_log_determinant_m2": (
                self.covariance_log_determinant_m2
            ),
            "covariance_mahalanobis_squared": (
                self.covariance_mahalanobis_squared
            ),
        }
        count = len(group_ids)
    ...
    def mean_posterior_nominal_probability(self) -> float:
        ...
        return float(
            np.sum(weights * self.posterior_nominal_probability)
            / np.sum(weights)
        )
...
def _covariance_statistics(
    ...
    *,
    ...
    blocks += (
        model_discrepancy_variance_m2 + covariance_jitter_m2
    ) * identity
    inverse = np.linalg.inv(blocks)
    signs, log_determinants = np.linalg.slogdet(blocks)
    if np.any(signs <= 0.0):
        raise ValueError("local covariance lost positive definiteness")
    log_determinant = float(np.sum(log_determinants))
    inverse_residual = np.einsum("nij,nj->ni", inverse, residual)
    mahalanobis = float(np.sum(residual * inverse_residual))

    rank = low_rank_factor.shape[2]
    if rank == 0 or not np.any(low_rank_factor):
        return log_determinant, max(mahalanobis, 0.0)

    unique_factor_groups = np.unique(factor_group_ids)
    group_position = {
        int(group_id): position
        for position, group_id in enumerate(unique_factor_groups)
    }
    columns = rank * len(unique_factor_groups)
    factor_matrix = np.zeros((count * 3, columns), dtype=np.float64)
    for row, factor_group in enumerate(factor_group_ids):
        start = rank * group_position[int(factor_group)]
        factor_matrix[3 * row : 3 * row + 3, start : start + rank] = (
            low_rank_factor[row]
        )
    inverse_factor = np.einsum(
        "nij,njk->nik",
        inverse,
        factor_matrix.reshape(count, 3, columns),
    ).reshape(count * 3, columns)
    gram = np.eye(columns, dtype=np.float64) + (
        factor_matrix.T @ inverse_factor
    )
    sign, log_gram = np.linalg.slogdet(gram)
    if sign <= 0.0:
        raise ValueError("low-rank covariance update is not positive definite")
    flattened_inverse_residual = inverse_residual.reshape(count * 3)
    projection = factor_matrix.T @ flattened_inverse_residual
    correction = float(projection @ np.linalg.solve(gram, projection))
    return (
        log_determinant + float(log_gram),
...
def grouped_student_t_mixture_likelihood(
    ...
    *,
    ...
    if predicted.shape != belief.mean_xyz_m.shape:
        raise ValueError(
            "predicted_xyz_m must match the observation mean shape"
        )
    ...
    for position, group_id in enumerate(belief.group_ids):
        ...
        covariance_logdet[position], covariance_mahalanobis[position] = (
            _covariance_statistics(
                ...
                model_discrepancy_variance_m2=(
                    settings.model_discrepancy_variance_m2
                ),
        ...
        log_nominal[position] = _student_t_log_density_from_statistics(
            ...
            covariance_log_determinant=covariance_logdet[position],
            covariance_mahalanobis_squared=(
        ...
        log_outlier[position] = _student_t_log_density_from_statistics(
            ...
            covariance_multiplier=(
                settings.outlier_covariance_multiplier
            ),
        )
        log_nominal_component = math.log(prior[position]) + (
            log_nominal[position]
        )
        log_outlier_component = math.log1p(-prior[position]) + (
            log_outlier[position]
        )
        log_mixture = float(
            np.logaddexp(log_nominal_component, log_outlier_component)
        )
        nll[position] = -log_mixture
        weighted_nll[position] = (
            belief.group_composite_weight[position] * nll[position]
        )
        posterior_nominal[position] = math.exp(
            log_nominal_component - log_mixture
        )
