...
class QueryUpdateAdmissionPolicyV1:
    ...
    identifiable_fraction_tolerance: float = _DEFAULT_NUMERICAL_TOLERANCE
    regret_tolerance: float = _DEFAULT_NUMERICAL_TOLERANCE
    information_gain_tolerance: float = _DEFAULT_NUMERICAL_TOLERANCE
    ...
    def __post_init__(self) -> None:
        ...
        legacy_tolerance = self.numerical_tolerance
        if legacy_tolerance is not None:
            legacy_tolerance = _finite_real(
                legacy_tolerance,
                name="numerical_tolerance",
                minimum=0.0,
            )
            explicitly_split = any(
                getattr(self, name) != _DEFAULT_NUMERICAL_TOLERANCE
                for name in (
            ...
            if explicitly_split:
                raise ValueError(
                    "numerical_tolerance cannot be combined with split tolerances"
                )
            object.__setattr__(
                self,
                "identifiable_fraction_tolerance",
            ...
            object.__setattr__(self, "regret_tolerance", legacy_tolerance)
            object.__setattr__(
                self,
                "information_gain_tolerance",
                legacy_tolerance,
            )
            object.__setattr__(self, "numerical_tolerance", legacy_tolerance)
    ...
    def descriptor(self) -> dict[str, object]:
        return {
            ...
            "identifiable_fraction_tolerance": (
                self.identifiable_fraction_tolerance
            ),
