...
def build_spring_overlay_checkpoint(
    ...
    *,
    ...
    """Replace only ``spring_Y`` and bind the operation to hashed inputs."""
    ...
    overlaid["spring_Y"] = torch.as_tensor(
        candidate.reshape(tuple(source_tensor.shape)), dtype=source_tensor.dtype
    )
    output.parent.mkdir(parents=True, exist_ok=True)
    torch.save(overlaid, output)

    log_ratio = np.log(candidate) - np.log(source_values)
    summary = {
        "schema_version": 1,
        ...
        "identity_field": bool(np.array_equal(candidate, source_values)),
        "spring_ratio": {
            "minimum": float(np.min(candidate / source_values)),
            "median": float(np.median(candidate / source_values)),
            "maximum": float(np.max(candidate / source_values)),
