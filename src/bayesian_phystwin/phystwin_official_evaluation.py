...
def evaluate_official_phystwin_arrays(
    ...
    *,
    ...
    def evaluate_split(start_frame: int, end_frame: int) -> dict[str, object]:
        ...
        for frame_index in range(start_frame, end_frame):
            ...
            if np.any(current_mask):
                ...
                track_by_frame.append(
                    float(np.mean(np.linalg.norm(residual, axis=1)))
                )
            else:
    ...
    return {
        "train": evaluate_split(1, train_frame),
        "test": evaluate_split(train_frame, test_frame),
    }
