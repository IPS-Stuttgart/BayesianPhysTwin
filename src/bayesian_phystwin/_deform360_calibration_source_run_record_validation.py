...
def _validate_source_lock_summary(record: Mapping[str, Any]) -> None:
    ...
    if valid:
        ...
        for key in _SOURCE_LOCK_DIGEST_KEYS:
            sha256(record[key], name=key)
        return
    if any(record[key] is not None for key in _SOURCE_LOCK_DIGEST_KEYS):
        raise ValueError("invalid source-lock summary retained lock evidence")
    if not available:
        if error != "missing":
