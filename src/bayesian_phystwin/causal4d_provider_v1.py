"""Versioned Bayesian-PhysTwin provider API for downstream causal inference.

This module is the only supported execution and artifact boundary for Causal4D.
It deliberately hides experiment-specific and underscore-prefixed implementation
helpers behind content-addressed, non-pickled contracts.
"""
...
class ProviderManifestV1:
    ...
    capabilities: tuple[str, ...] = BASE_PROVIDER_CAPABILITIES
    ...
    def __post_init__(self) -> None:
        ...
        if not capabilities or any(not value for value in capabilities):
            raise ValueError("provider capabilities must contain nonempty values")
        ...
        if not versions or any(
            not name or version < 1 for name, version in versions.items()
        ):
            raise ValueError("artifact schema versions must be positive and named")
        object.__setattr__(self, "capabilities", capabilities)
        object.__setattr__(self, "artifact_schema_versions", versions)
        object.__setattr__(self, "metadata", _validated_json(self.metadata))

    @property
    def manifest_id(self) -> str:
        return hashlib.sha256(
            _canonical_json(self.as_dict(include_id=False))
        ).hexdigest()
    ...
    def as_dict(self, *, include_id: bool = True) -> dict[str, Any]:
        ...
        if include_id:
            values["manifest_id"] = self.manifest_id
        return values
...
class GuardDecisionV1:
    """Prospective candidate decision kept separate from candidate inference."""
    ...
    *,
    ...
    for key, expected in (
        ...
        if actual is not None and actual != expected:
            raise ValueError(
                f"candidate {key} does not match the guard decision"
            )
...
class BPTBeliefExportConfigV1:
    """Frozen settings for provider-side endpoint-belief construction."""
...
def build_physical_belief_from_replays(
    *,
    ...
    from .phystwin_residual_dynamics import _lift_map, _lift_residual
...
def target_validity(
    ...
    """Public wrapper for the released PhysTwin target-validity convention."""

    from .phystwin_residual_dynamics import _target_validity

    return np.asarray(_target_validity(visible, motion_valid), dtype=bool)


def released_self_collision_for_case(case_name: str) -> bool:
    """Public wrapper for the released case-specific collision convention."""

    from .phystwin_state_injection import _released_self_collision_for_case

    return bool(_released_self_collision_for_case(case_name))
...
def replay_official_phystwin_particles(
    *,
    ...
    """Replay selected grouped-spring particles through official Warp."""
    ...
    try:
        for particle in particles:
            ...
            position, velocity = _rollout_initial(
...
class PhysTwinReplayProviderV1(Protocol):
    """Minimal execution protocol for alternative PhysTwin replay providers."""
    ...
    @property
    def observed_positions_m(self) -> np.ndarray: ...

    @property
    def observed_valid(self) -> np.ndarray: ...
    ...
    @property
    def particle_ids(self) -> tuple[str, ...]: ...

    def replay_prefix(self) -> tuple[np.ndarray, np.ndarray]: ...


def build_physical_belief_from_provider(
    provider: PhysTwinReplayProviderV1,
    *,
    metadata: Mapping[str, Any] | None = None,
    config: BPTBeliefExportConfigV1 | None = None,
) -> PhysicalBeliefV1:
    """Build a belief from any provider satisfying the public replay protocol."""

    positions, velocities = provider.replay_prefix()
    return build_physical_belief_from_replays(
        provider_manifest_id=provider.provider_manifest_id,
        causal_frame_stop=provider.causal_frame_stop,
        replay_positions_m=positions,
        replay_velocities_mps=velocities,
        observed_positions_m=provider.observed_positions_m,
        observed_valid=provider.observed_valid,
        theta=provider.theta,
        theta_names=provider.theta_names,
        weights=provider.weights,
        particle_ids=provider.particle_ids,
        metadata=metadata,
        config=config,
    )
