...
DESCRIPTION_OVERRIDES: Final[dict[str, str]] = {
    ...
    "validate-observation-belief": "validate or score an ObservationBeliefV1 artifact",
...
def optional_dependencies(command_id: str) -> tuple[str, ...]:
    ...
    graph = (
        "graph", "spring", "topology", "structure", "structural",
        "state-injection", "state-decay", "state-modes", "pgrd",
        "bayesian-anchor", "online-belief", "hierarchical-residual",
        "combined", "calibration", "controller-sensitivity",
    )
    vision = (
        "raw-camera", "crossview-supplement", "raw-pairwise", "cotracker3",
        "raw-cues", "perception-cues", "motioncrafter",
    )
