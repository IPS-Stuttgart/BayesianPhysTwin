"""Listing, inspection, and lifecycle runners for registered commands."""
...
def registry_help() -> str:
    return (
        "usage: bpt commands <list|describe> [arguments]\n\n"
        "commands:\n"
        "  list           list entries [--status STATUS] [--json]\n"
        "  describe       show one command by id, route, or alias [--json]\n\n"
...
def _print_list(commands: Sequence[CommandSpec], *, json_output: bool) -> None:
    if json_output:
        print(
            json.dumps(
                [item.to_dict() for item in commands], indent=2, sort_keys=True
            )
        )
        return
    if not commands:
...
def _print_command(command: CommandSpec, *, json_output: bool) -> None:
    ...
    print(f"grouped route: {command.grouped_command}")
...
def commands_main(arguments: Sequence[str]) -> int:
    if not arguments or arguments[0] in HELP_FLAGS:
        ...
        return 0
    action = arguments[0]
    if len(arguments) == 2 and arguments[1] in HELP_FLAGS:
        print(registry_help(), end="")
        return 0
    if action == "list":
    ...
    if action == "describe":
        ...
        command = find_command(" ".join(selectors))
        if command is None:
...
def catalog_main(namespace: str, arguments: Sequence[str]) -> int:
    ...
    if action == "describe":
        ...
        if parsed is None or len(parsed[0]) != 1:
            print(catalog_help(namespace), file=sys.stderr, end="")
            return 2
        command = find_command(parsed[0][0], status=status)
        if command is None:
            print(f"unknown {status.value} command: {parsed[0][0]}", file=sys.stderr)
            return 2
        _print_command(command, json_output=parsed[1])
        return 0
    if action == "run" and len(arguments) >= 2:
        command = find_command(arguments[1], status=status)
        if command is None:
            print(f"unknown {status.value} command: {arguments[1]}", file=sys.stderr)
            return 2
        return invoke(command, list(arguments[2:]))
    print(catalog_help(namespace), file=sys.stderr, end="")
    return 2
