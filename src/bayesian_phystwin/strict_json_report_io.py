"""Strict JSON input and atomic publication for content-addressed reports."""
...
def _artifact_label(value: object) -> str:
    if type(value) is not str or not value or value != value.strip():
        raise ValueError("artifact_label must be a nonempty trimmed string")
    if any(character in value for character in "\r\n"):
        raise ValueError("artifact_label must be a single line")
    return value
...
def load_strict_json_mapping(
    ...
    *,
    ...
    label = _artifact_label(artifact_label)
    ...
    if not stat.S_ISREG(before.st_mode):
        raise ValueError(f"{label} input must be an ordinary file")
    if before.st_size > maximum_input_bytes:
        raise ValueError(f"{label} input exceeds its byte budget")
    ...
    if _ordinary_file_signature(before) != _ordinary_file_signature(after):
        raise ValueError(f"{label} input changed while it was read")
    if len(payload_bytes) != before.st_size:
        raise ValueError(f"{label} input size changed while it was read")
    try:
        text = payload_bytes.decode("utf-8")
    except UnicodeDecodeError as error:
        raise ValueError(f"{label} input must be UTF-8 JSON") from error
    try:
        value = json.loads(
            text,
        ...
        raise ValueError(f"{label} input is not valid JSON") from error
    if not isinstance(value, Mapping):
        raise ValueError(f"{label} input must contain a JSON object")
    artifact = {
        "path": str(input_path),
