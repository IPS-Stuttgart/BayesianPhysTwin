...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run one source-only temporal Deform360 residual fold."
    )
    ...
    parser.add_argument(
        "--deform360-code-root",
        type=Path,
        help="Optional Deform360 checkout providing official gripper geometry.",
    )
    ...
    parser.add_argument("--trust-diagnosis", type=Path, required=True)
    parser.add_argument("--closure-diagnosis", type=Path, required=True)
