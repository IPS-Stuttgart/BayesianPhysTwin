...
Each call represents one complete source object/session unit; callers may not
split a unit into frame-, track-, camera-, or coordinate-level pseudo-replicates.
...
The permanent change contains only the implementation, positive and adversarial
portable-contract tests, this documentation, and its source-distribution
manifest entry. The two test files exercise one implementation surface: one
covers registered behavior and the other covers fail-closed guard branches.
Neither creates an alternate execution route. The change contains no
write-enabled workflow or target execution helper.
