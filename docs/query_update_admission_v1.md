...
with different scales and semantics. The existing `numerical_tolerance` field
remains the common default for backward compatibility. Any split tolerance that
is supplied overrides that default only for its own quantity; omitted split
fields continue to inherit the common value. The canonical policy descriptor
records both the common default and all three effective tolerances.
