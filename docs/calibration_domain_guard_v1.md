...
`bayesian_phystwin.domain_covariance_calibration` uses this domain guard as the
prospective support layer for conservative scale-plus-floor covariance
calibration. It fits transforms only on calibration groups, evaluates support
with leave-one-group-out Gaussian log scores, and returns the exact raw
