...
the same selected candidate in every fold. If no challenger satisfies the rule,
the report retains the registered reference. That is a valid negative source
result.
...
trace, the selected candidate, and the decision. `claim_authorized` is always
false.
