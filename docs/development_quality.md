...
reinterpretation or hid numerical admission:
...
- `BPTQ002` rejects `config or SomeConfig()` when `config` is a function
  ...
  and
- `BPTQ003` rejects direct `numpy.linalg.inv` calls, including common import
  aliases, because an inverse hides the positive-definite or rank decision,
  condition admission, and any regularization. New code must use an explicit
  solve, Cholesky factorization, or admitted eigendecomposition and retain the
  relevant numerical diagnostics.

An intentional internal conversion or exact frozen reproduction can be
suppressed only next to the complete expression with an auditable marker such as
`# bpt-quality: allow BPTQ001 -- internal 0/1 sentinel` or
`# bpt-quality: allow BPTQ003 -- frozen tagged reproduction`. Suppressions are
not an alternative to validation at public observation, belief, provider, guard,
or prospective numerical boundaries.
...
New symmetric covariance and information systems should use the explicit
[`numerical_linear_algebra_v1`](numerical_linear_algebra_v1.md) policy. Its SPD
and PSD operations apply no implicit jitter or ridge term, expose solver and rank
semantics, gate conditioning, verify residual consistency, and return immutable
solutions with diagnostics. Historical evidence-bound source remains untouched
unless a separately reviewed reproduction repair requires a change.
