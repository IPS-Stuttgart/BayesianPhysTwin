# Prior-aware sparse-identity state update: source smoke v2
...
V2 changes only this replay contract. It preserves the identity split, graph
basis, response frames, perturbations, priors, gates, and metrics from v1.
Because v1 opened no outcomes, this is a pre-outcome technical replacement,
not a post-result method revision.
...
V2 additionally requires bit-exact replay parity and persists the complete
...
`configs/sota/phystwin_prior_aware_sparse_identity_source_v2.json`.
...
PYTHONPATH=/home/florianpfaff/bpt-prior-aware-sparse-identity-v1/runtime-warp111:src \
  ...
  --protocol configs/sota/phystwin_prior_aware_sparse_identity_source_v2.json \
  --output /home/florianpfaff/bpt-prior-aware-sparse-identity-v2/run/single_lift_cloth
```
