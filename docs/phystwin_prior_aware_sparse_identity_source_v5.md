# Prior-aware sparse-identity state update: source smoke v5
...
remains recorded in the hash-bound historical summary. It then reached the
simulator parity gate and persisted a diagnostic, but still did not fit a state
or score an outcome. The report exposed a second replay-contract mismatch: v3
enabled cloth self-collision through a generic case-name rule, while the
historical headless replay defining the selected baseline uses
`self_collision=False`.

V4 makes that historical setting explicit. It preserves the identity split,
graph basis, response frames, perturbations, priors, gates, and metrics from
v1-v3. It passed exact replay and wrote a checksummed position/velocity state
carrier, then stopped before state fitting: its frame-zero-only FPS sensors
`[0,8,1,7]` provide complete response-prefix observations for only one
identity, with a second identity available in three frames.

Five identities `[3,4,5,6,8]` are complete throughout the seven allowed
response frames. V5 applies the same deterministic frame-zero FPS rule within
that support-qualified set, selecting `[3,4,8,5]`. This is a pre-outcome
source-design change, not a mere implementation repair: it uses prefix
availability but no future visibility, displacement, error, or metric. V5
preserves the graph basis, perturbations, priors, gates, and metrics from
v1-v4. Every predecessor is archived and must not be rerun or scored.

## Method

Frame-zero farthest-point sampling fixes observed identities `[3, 4, 8, 5]`
among identities complete in all seven response frames. Identities
`[0, 1, 2, 6, 7]` are hidden from inference and used only by the
post-seal scorer. Frame-zero nearest-node association is fixed before the
response prefix and maps the four sensors to distinct nodes within `3.49` mm.
...
V5 requires bit-exact selected-baseline replay parity and persists the complete
...
`configs/sota/phystwin_prior_aware_sparse_identity_source_v5.json`.
...
PYTHONPATH=/home/florianpfaff/bpt-prior-aware-sparse-identity-v1/runtime-warp111:src \
  ...
  --protocol configs/sota/phystwin_prior_aware_sparse_identity_source_v5.json \
  --output /home/florianpfaff/bpt-prior-aware-sparse-identity-v5/run/single_lift_cloth
```
