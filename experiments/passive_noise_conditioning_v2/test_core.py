...
def sample_model():
    x = np.random.default_rng(1).normal(size=(160, 72)) * .01
    return m.fit(x)
...
def test_supplied_noise_preserves_shared_mode():
    ...
    assert actual[0, 3] == pytest.approx(.01**2)
    assert actual[0, 0] == pytest.approx(.01**2+.002**2)
...
def test_miscalibration_does_not_change_actual_observations():
    ...
    np.testing.assert_allclose(r, .25*actual)
...
def test_independent_noise_augmented_ridge_equivalence():
    ...
    a, v = m.gain(model, nodes, "adaptive_empirical", .01, r)
    b, w = m.gain(model, nodes, "direct_noise_aware_ridge", .01, r)
...
def test_noise_adaptation_changes_gain_and_large_noise_returns_prior():
    model = sample_model()
    nodes = np.array([0, 7])
    a, v = m.gain(model, nodes, "adaptive_empirical", .01, np.zeros((6, 6)))
    b, w = m.gain(model, nodes, "adaptive_empirical", .01, np.eye(6)*1e6)
...
def test_fixed_gain_is_constant_for_noise_inputs():
    ...
    for arm in ("fixed_ridge", "pooled_noise_ridge"):
        a, _ = m.gain(model, nodes, arm, .1, np.zeros((6, 6)))
        b, _ = m.gain(model, nodes, arm, .1, np.eye(6))
...
def test_exact_expected_noise_risk_against_monte_carlo():
    ...
    np.testing.assert_allclose(exact, simulation, rtol=.02)
...
def test_no_hidden_or_future_outcome_argument():
    ...
    model = sample_model()
    nodes = np.array([2, 7])
    g, _ = m.gain(model, nodes, "adaptive_empirical", .1, np.zeros((6, 6)))
...
def test_rod_covariance_same_marginals_psd():
    ...
    rod = m.fit(source, 2)
    np.testing.assert_allclose(np.diag(empirical["cov"]), np.diag(rod["cov"]), atol=1e-12)
...
def test_invalid_observations_and_covariance_rejected():
    ...
    for r in (np.eye(6)*-1, np.full((6, 6), np.nan), np.eye(3)):
        with pytest.raises(ValueError):
            m.gain(model, nodes, "adaptive_empirical", .1, r)
    with pytest.raises(ValueError):
...
def test_calibration_uses_only_source_and_complete_folds():
    ...
    source = rng.normal(size=(4, 3, 72))*.01
    original = source.copy()
    result = m.source_cv(source, 2, "adaptive_empirical", {"reg": .1}, True)
