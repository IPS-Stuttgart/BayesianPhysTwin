...
D = 24
ARMS = ("mean_only", "fixed_ridge", "pooled_noise_ridge", "adaptive_empirical",
        "adaptive_rod", "diagonal_R", "direct_noise_aware_ridge")
...
def hidden(nodes: np.ndarray) -> np.ndarray:
    return np.concatenate([h * D + coords(np.setdiff1d(np.arange(8), nodes))
...
def noise(label: str, budget: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    if label == "clean":
        ...
        sds = np.resize([.002, .010], budget)
        r = np.diag(np.repeat(sds**2, 3))
    elif label.startswith("shared10"):
        r = .002**2 * np.eye(size) + .010**2 * np.kron(np.ones((budget, budget)), np.eye(3))
    else:
        raise ValueError("Unknown noise condition")
    scale = .25 if label.endswith("Rquarter") else 4 if label.endswith("Rfour") else 1
    return r, scale * r
...
def fit(samples: np.ndarray, rank: int = 0) -> dict:
    ...
    if rank:
        ...
        cov = .95 * cov * np.outer(scale, scale) + .05 * np.diag(marginal)
    return {"mean": mean, "cov": cov, "centered": centered,
            "reg_scale": float(marginal[:D].mean())}


def gain(model: dict, nodes: np.ndarray, arm: str, reg: float,
         supplied_r: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    o = coords(nodes)
    c = model["cov"]
    ...
    if arm == "fixed_ridge":
        ...
        r = np.eye(len(o)) * ((0**2 + .005**2 + .015**2) / 3)
    elif arm == "diagonal_R":
        r = np.diag(np.diag(r))
    ...
    if arm == "direct_noise_aware_ridge":
        ...
        g = np.linalg.solve(x.T @ x / len(x) + r + regularizer,
                            x.T @ targets / len(x)).T
    else:
...
def predict(model: dict, g: np.ndarray, observed_residual: np.ndarray,
...
def expected_error2(predicted: np.ndarray, truth: np.ndarray,
                    g: np.ndarray, actual_r: np.ndarray) -> np.ndarray:
...
def source_cv(source: np.ndarray, budget: int, arm: str, param: dict,
              calibration: bool = False) -> float | dict:
    ...
    return {"static_variance": (static_sum / counts).tolist(),
...
def select(source: np.ndarray) -> dict:
    ...
    for budget in (2, 4):
        ...
        for arm in ARMS:
            ...
            options = [{"parameters": p, "source_expected_mse": source_cv(source, budget, arm, p)} for p in params]
            chosen = min(options, key=lambda v: v["source_expected_mse"])
            selections[budget][arm] = {"parameters": chosen["parameters"], "options": options,
                "calibration": source_cv(source, budget, arm, chosen["parameters"], True)}
            print("SOURCE", budget, arm, chosen, flush=True)
        # These arms are algebraically equivalent, not independent competitors.
        if selections[budget]["adaptive_empirical"]["parameters"] != selections[budget]["direct_noise_aware_ridge"]["parameters"]:
            raise ArithmeticError("Independent equivalent fits chose different regularizers")
    return selections
...
def load_panel(parent: Path, data: Path, dlo: str, stage: str, pins: dict) -> tuple:
    ...
    expected = manifest["partitions"]["source_test"] if stage == "source" else manifest["ordered_names"]
    ...
    samples = np.stack([residual[:, CUTS + h] for h in HORIZONS], axis=2).reshape(len(names), len(CUTS), 72)
    if not np.isfinite(samples).all():
        raise ValueError("Nonfinite residual")
    return names, samples, identities


def evaluate(dlo: str, names: list, target: np.ndarray, source: np.ndarray,
             choices: dict) -> tuple[list, float]:
    rows, max_parity = [], 0.0
    for budget in (2, 4):
        models = {arm: fit(source, choices[budget][arm]["parameters"].get("rank", 0)) for arm in ARMS}
        fixed_arm = min(("fixed_ridge", "pooled_noise_ridge"),
            ...
            for nodes in masks(budget, False):
                ...
                for arm in ARMS:
                    ...
                    for j, idx in enumerate(hid):
                        ...
                        accum[arm][:, j, 1] += .5 * (np.log(2*np.pi*adaptive_v) + e2[:, :, idx]/adaptive_v).mean((1, 2))
                        accum[arm][:, j, 2] += .5 * (np.log(2*np.pi*static_v) + e2[:, :, idx]/static_v).mean((1, 2))
                max_parity = max(max_parity, float(np.max(np.abs(preds["adaptive_empirical"] - preds["direct_noise_aware_ridge"]))))
            ...
            for arm, vals in accum.items():
                ...
                for k, name in enumerate(names):
                    for j, horizon in enumerate(HORIZONS):
                        rows.append({"dlo": dlo, "trajectory": name, "budget": budget,
                          "condition": condition, "arm": arm, "horizon": horizon,
                          "expected_mse_m2": float(vals[k, j, 0]),
                          "root_expected_mse_mm": float(1000*np.sqrt(vals[k, j, 0])),
                          "posterior_expected_nll": float(vals[k, j, 1]),
                          "static_expected_nll_same_mean": float(vals[k, j, 2]),
                          "mask_count": count, "cut_count": len(CUTS), "source_selected_fixed": fixed_arm})
    if max_parity > 1e-10:
        ...
        for horizon in HORIZONS:
            for condition in ("primary_mix",) + CONDITIONS:
                ...
                for arm in arms:
                    grouped = {}
                    for row in rows:
                        if row["budget"] == budget and row["horizon"] == horizon and row["condition"] in use and row["arm"] == arm:
                            grouped.setdefault((row["dlo"], row["trajectory"]), []).append(row)
                    if len(grouped) != 28 or any(len(v) != len(use) for v in grouped.values()):
                        raise ValueError("Incomplete evaluation panel")
                    mse = {key: np.mean([r["expected_mse_m2"] for r in v]) for key, v in grouped.items()}
                    vec = {dlo: np.array([1000*np.sqrt(mse[k]) for k in sorted(mse) if k[0] == dlo]) for dlo in ("DLO4", "DLO5")}
                    vectors[arm] = vec
                    summary.append({"budget": budget, "horizon": horizon, "condition": condition, "arm": arm,
                      "DLO4_mm": float(vec["DLO4"].mean()), "DLO5_mm": float(vec["DLO5"].mean()),
                      "equal_object_mm": float(np.mean([v.mean() for v in vec.values()])),
                      "posterior_expected_nll": float(np.mean([r["posterior_expected_nll"] for v in grouped.values() for r in v])),
                      "static_expected_nll_same_mean": float(np.mean([r["static_expected_nll_same_mean"] for v in grouped.values() for r in v]))})
                for a, b in (("adaptive_empirical", "source_selected_fixed"), ("adaptive_rod", "source_selected_fixed"),
                             ("adaptive_rod", "direct_noise_aware_ridge"), ("adaptive_empirical", "diagonal_R"),
                             ("adaptive_empirical", "mean_only")):
                    differences = {d: vectors[a][d] - vectors[b][d] for d in vectors[a]}
                    boot = sum(v[rng.integers(0, 14, (bootstrap_reps, 14))].mean(1) for v in differences.values()) / 2
                    delta = float(np.mean([v.mean() for v in differences.values()]))
                    contrasts.append({"budget": budget, "horizon": horizon, "condition": condition,
                      "candidate": a, "comparator": b, "difference_mm": delta,
                      "gain_pct": float(-100*delta/np.mean([v.mean() for v in vectors[b].values()])),
                      "ci95_mm": np.quantile(boot, [.025, .975]).tolist(),
                      "per_dlo_difference_mm": {d: float(v.mean()) for d, v in differences.items()},
                      "wins": sum(int((v < -1e-9).sum()) for v in differences.values())})
...
def main() -> None:
    ...
    try:
        if protocol["horizons"] != list(HORIZONS) or protocol["cutoffs"] != CUTS.tolist() or protocol["target_conditions"] != list(CONDITIONS):
            raise ValueError("Protocol/implementation schedule mismatch")
        sources, choices, source_ids = {}, {}, {}
        for dlo in ("DLO4", "DLO5"):
            _, sources[dlo], source_ids[dlo] = load_panel(args.parent, args.data, dlo, "source", protocol["data_pins"][dlo]["source"])
            choices[dlo] = select(sources[dlo])
        save(args.output / "source_choices.json", choices)
        save(args.output / "source_seal.json", {"source_sha256": sha(Path(__file__)),
          "protocol_sha256": sha(protocol_path), "source_choices_sha256": sha(args.output/"source_choices.json"),
          "source_identities": source_ids, "target_loaded": False, "git_sha": os.environ.get("GITHUB_SHA")})
        rows, parity, target_ids = [], [], {}
        for dlo in ("DLO4", "DLO5"):
            names, target, target_ids[dlo] = load_panel(args.parent, args.data, dlo, "target", protocol["data_pins"][dlo]["target"])
        ...
        primary = [c for c in contrasts if (c["budget"], c["horizon"], c["condition"]) == (2, 0, "primary_mix")]
        for c in primary:
            threshold = 1 if c["comparator"] == "direct_noise_aware_ridge" else 2
            c["passed"] = c["gain_pct"] >= threshold and c["ci95_mm"][1] < 0 and all(v < 0 for v in c["per_dlo_difference_mm"].values())
        result = {"contract": protocol["contract"], "status": "complete", "scope": protocol["scope"],
          "noise_assumption": protocol["noise"], "primary": primary, "summary": summary, "contrasts": contrasts,
          "max_independent_ridge_parity_m": max(parity), "target_identities": target_ids,
          "source_seal_sha256": sha(args.output/"source_seal.json"), "rows_sha256": sha(args.output/"per_trajectory.csv"),
          "runtime": {"run_id": os.environ.get("GITHUB_RUN_ID"), "git_sha": os.environ.get("GITHUB_SHA"),
                      "runner": os.environ.get("RUNNER_NAME"), "python": platform.python_version(),
                      "numpy": np.__version__, "seconds": time.monotonic()-start}}
        save(args.output/"result.json", result)
        lines = ["# Passive visibility and noise transfer", "", protocol["scope"], "", protocol["noise"], "",
        ...
        for row in summary:
            if (row["budget"], row["horizon"], row["condition"]) == (2, 0, "primary_mix"):
                lines.append(f"| {row['arm']} | {row['DLO4_mm']:.4f} | {row['DLO5_mm']:.4f} | {row['equal_object_mm']:.4f} |")
        lines += ["", "Primary contrasts (candidate minus comparator, mm):", "", json.dumps(primary, indent=2), "",
