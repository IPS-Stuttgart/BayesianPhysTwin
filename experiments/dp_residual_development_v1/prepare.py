...
def main() -> None:
    ...
    data = source._load_named_trajectories(manifest, names, frame_count=500, node_count=13)
    print("Preparing 40 fit and 8 validation trajectories; no source-test/eval access", flush=True)
    rollout = common._rollout(state, data, modules=modules, torch=torch, device="cuda:0")
    ...
    np.savez_compressed(bundle, names=np.asarray(names), initial=initial, action=action,
                        baseline=baseline, targets=targets, persistence=persistence,
    ...
    record = {
        ...
        "bundle_sha256": digest(bundle), "protocol_sha256": digest(protocol_path),
        ...
        "fit_count": 40, "validation_count": 8, "source_test_read": False,
        "official_eval_read": False, "other_dlos_read": False,
