...
class ResidualExperts:
    ...
    def _gate_features(self, features: np.ndarray, fit: bool) -> np.ndarray:
        ...
        reduced = self.gate_pca.transform(self.gate_scaler.transform(raw.reshape(-1, d)))
    ...
    def fit(self, features: np.ndarray, residual: np.ndarray) -> ResidualExperts:
        ...
        if k == 1:
            ...
            common = dict(n_components=k, covariance_type="full", reg_covar=1e-4,
                          max_iter=500, n_init=3, tol=1e-4, random_state=self.seed)
            if self.spec.family == "finite":
                self.gate = GaussianMixture(**common)
            else:
                prior_type = ("dirichlet_process" if self.spec.family == "dp"
                              else "dirichlet_distribution")
                concentration = (self.spec.concentration if self.spec.family == "dp"
                                 else self.spec.concentration / k)
                self.gate = BayesianGaussianMixture(
                    **common, weight_concentration_prior_type=prior_type,
        ...
        for node in range(self.nodes):
            ...
            global_coef = np.linalg.solve(design.T @ design + global_penalty,
                                          design.T @ response)
            ...
            for component in range(k):
                if k == 1:
                    ...
                    delta = np.linalg.solve(design.T @ weighted + penalty,
                                            weighted.T @ remaining)
    ...
    def predict(self, features: np.ndarray, *, hard: bool = False) -> np.ndarray:
        ...
        for node in range(self.nodes):
            design = self._design(x, node)
            component_mean = np.einsum("nd,kdc->nkc", design,
                                       self.coefficients[:, node], optimize=True)
            mean = np.einsum("nk,nkc->nc", probabilities.reshape(-1, self.spec.components),
                             component_mean, optimize=True)
    ...
    def diagnostics(self) -> dict:
        return {"family": self.spec.family, "max_components": self.spec.components,
                "concentration": self.spec.concentration, "seed": self.seed,
                "ridge": self.ridge, "weights": self.weights.tolist(),
