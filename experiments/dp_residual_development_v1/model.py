...
def marginal_weights(x: np.ndarray, means: np.ndarray, covs: np.ndarray,
    ...
    for mu, cov, weight in zip(means, covs, weights, strict=True):
        ...
        scores.append(np.log(max(float(weight), 1e-300))
...
class ResidualExperts:
    ...
    def __init__(self, spec: Spec, ridge: float = 1.0,
                 feature_rank: int = 6, response_rank: int = 4,
                 gate_stride: int = 10):
        if (spec.kind not in {"ridge", "rff", "finite", "dp", "finite_bayes"}
                or spec.components < 1 or spec.alpha <= 0 or ridge <= 0
                or gate_stride < 1):
    ...
    def fit(self, features: np.ndarray, residual: np.ndarray) -> ResidualExperts:
        ...
        if self.spec.kind in {"ridge", "rff"}:
            ...
            self.x_pca = PCA(n_components=min(self.feature_rank, gx.shape[1], len(sample)-1),
                             whiten=True, svd_solver="full")
            self.y_pca = PCA(n_components=min(self.response_rank, gy.shape[1], len(sample)-1),
                             whiten=True, svd_solver="full")
            ...
            common = dict(n_components=self.spec.components, covariance_type="full",
                          reg_covar=0.03, max_iter=300, tol=1e-4, n_init=2,
                          random_state=self.spec.seed)
            if self.spec.kind == "finite":
                self.gate = GaussianMixture(**common)
            else:
                prior = ("dirichlet_process" if self.spec.kind == "dp"
                         else "dirichlet_distribution")
                concentration = (self.spec.alpha if self.spec.kind == "dp"
                                 else self.spec.alpha/self.spec.components)
                self.gate = BayesianGaussianMixture(
                    **common, weight_concentration_prior_type=prior,
    ...
    def weights(self, features: np.ndarray) -> np.ndarray:
        ...
        ex = self.x_pca.transform((gx-self.gx_center)/self.gx_scale)
        return marginal_weights(ex, self.gate.means_, self.gate.covariances_,
                                self.gate.weights_).reshape(n, h, -1)
...
def self_test() -> dict:
    ...
    for spec in (Spec("ridge"), Spec("finite", 2), Spec("dp", 8)):
        model = ResidualExperts(spec, feature_rank=6, response_rank=2,
                                gate_stride=2).fit(x[train], y[train])
        pred = model.predict(x[test])
        assert np.allclose(model.weights(x[test]).sum(-1), 1)
        assert np.array_equal(pred, model.predict(x[test]))
        result[spec.kind] = {"mae_m": float(np.mean(np.abs(pred-y[test]))),
                             "active_components": model.active_components}
    assert result["dp"]["mae_m"] < result["ridge"]["mae_m"]
    z = np.zeros((2, 1))
    mu = np.array([[0., -100.], [0., 100.]])
    cov = np.broadcast_to(np.eye(2), (2, 2, 2)).copy()
    assert np.allclose(marginal_weights(z, mu, cov, np.array([.3, .7])), [.3, .7])
    try:
    ...
    return {"status": "passed", "synthetic_only": True, "metrics": result,
            "checks": ["finite_input", "deterministic_prediction", "normalized_gate",
                       "unknown_response_marginalization", "observable_regime_positive_control"]}
