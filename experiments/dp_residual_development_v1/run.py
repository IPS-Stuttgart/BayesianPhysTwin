...
LONGRUN = Path('/home/florianpfaff/source-only/deform-bayesian-v2/runs/longrun-v2-2060c71/longrun_result.json')
...
def save_model(path: Path, model: ResidualExperts) -> None:
    arrays = {key: getattr(model, key) for key in
              ('location', 'scale', 'coef', 'rff_w', 'rff_b', 'component_mass')}
    ...
    if model.gate is not None:
        ...
        for kind in ('x', 'y'):
            for key in ('components_', 'mean_', 'explained_variance_'):
                arrays[kind+'_pca_'+key] = getattr(getattr(model, kind+'_pca'), key)
...
def main() -> None:
    ...
    config = json.loads((ROOT/'configs/sota/deform_dlo_local_residual_v4.json').read_text())
    ...
    if longrun['selected_checkpoint']['checkpoint']['sha256'] != config['baseline']['checkpoint_sha256']:
    ...
    os.environ['CUBLAS_WORKSPACE_CONFIG'] = json.loads(lp.read_text())['training']['cublas_workspace_config']
    import torch
    ...
    blocked = {str(Path(manifest['trajectories'][name]['path']).resolve())
    ...
    assert len(fit_names) == 40 and len(val_names) == 8 and not set(fit_names) & set(val_names)
    ...
    protocol = {
        ...
        'shrinkages': list(SHRINKAGES), 'ridge': 1.0,
        'feature_rank': 6, 'response_rank': 4, 'gate_stride': 10,
        ...
        'other_dlo_read': False, 'prob4d_used': False, 'new_physical_data': False,
        'environment': {'python': sys.version, 'numpy': np.__version__,
                        'scipy': scipy.__version__, 'sklearn': sklearn.__version__,
                        'torch': torch.__version__},
        'limitations': ['backbone checkpoint previously selected on this development validation panel',
    ...
    rollout = posterior._evaluate_state(state, trajectories, modules=modules,
                                       torch=torch, device='cuda:0', dlo_type='DLO1', node_count=13)
    ...
    del modified, x2
    query_hashes = [hashlib.sha256(initial[i].tobytes()+action[i].tobytes()+base[i].tobytes()).hexdigest()
                    for i in range(len(names))]
    if len(set(query_hashes)) != len(names):
        raise ValueError('Duplicate causal queries: grouped split is required before fitting')
    np.savez_compressed(output/'development_arrays.npz', initial=initial, action=action,
                        base=base, target=target, names=np.asarray(names))
    ...
    for spec in SPECS:
        ...
        record = {'spec': asdict(spec), 'name': spec.name, 'inner_l1_m': error,
                  'shrinkage': shrink, 'active_components': model.active_components,
                  'fit_seconds': time.perf_counter()-t0, 'warnings': model.warnings,
                  'converged': True if model.gate is None else bool(model.gate.converged_)}
        tuning.append(record)
        old = selection.get(spec.kind)
        if old is None or (error, shrink, spec.name) < (old['inner_l1_m'], old['shrinkage'], old['name']):
    ...
    write(output/'selection_seal.json', {'selection': selection, 'all_candidates': tuning,
    ...
    for kind, choice in selection.items():
        ...
        predictions[kind] = corrected(base[40:], residual, frames[40:], choice['shrinkage'])
        save_model(output/f'model_{kind}.npz', model)
        details[kind] = {'selection': choice, 'active_components': model.active_components,
                         'refit_seconds': time.perf_counter()-t0, 'warnings': model.warnings,
                         'converged': True if model.gate is None else bool(model.gate.converged_),
                         'component_weights': [1.] if model.gate is None else model.gate.weights_.tolist()}
        if kind == 'ridge':
            predictions['ridge_fixed_s0p5'] = corrected(base[40:], residual, frames[40:], .5)
        print('REFIT', kind, 'components', model.active_components, flush=True)
    reference = fit_deform_local_residual(initial[:40], action[:40], base[:40], target[:40],
                                         fit_names, ridge=1.0, variance_floor_m2=1e-6)
    reference_pred = predict_deform_local_residual(reference, initial[40:], action[40:],
                                                   base[40:], shrinkage=.5)['predictions']
    ...
    write(output/'prediction_seal.json', {'prediction_sha256': digest(output/'sealed_predictions.npz'),
    ...
    errors = {kind: case_errors(pred, target[40:]) for kind, pred in predictions.items()}
    metrics = {}
    for kind, e in errors.items():
        pred = predictions[kind]
        metrics[kind] = {'mean_coordinate_l1_mm': float(e.mean()*1000),
                         'coordinate_rmse_mm': float(np.sqrt(np.mean((pred-target[40:])**2))*1000),
                         'per_trajectory_l1_mm': dict(zip(val_names, (e*1000).tolist())),
                         'wins_vs_fixed_ridge': int(np.sum(e < errors['ridge_fixed_s0p5']-1e-12)),
                         'per_horizon_third_l1_mm': [float(np.mean(np.abs(a-b))*1000)
                             for a, b in zip(np.array_split(pred, 3, axis=1),
                                             np.array_split(target[40:], 3, axis=1))]}
    rng = np.random.default_rng(991)
    draw = rng.integers(0, len(val_names), size=(10000, len(val_names)))
    contrasts = {}
    for kind in ('backbone', 'ridge_fixed_s0p5', 'ridge', 'rff', 'finite', 'finite_bayes'):
        delta = (errors['dp']-errors[kind])*1000
        contrasts[kind] = {'dp_minus_comparator_mm': float(delta.mean()),
                           'descriptive_trajectory_bootstrap_95': np.quantile(delta[draw].mean(1), [.025, .975]).tolist(),
                           'dp_wins': int(np.sum(delta < -1e-9)),
                           'relative_improvement_percent': float(100*(1-errors['dp'].mean()/errors[kind].mean()))}
    result = {'contract': protocol['contract'], 'status': 'completed',
              'evidence_status': 'retrospective-development-only', 'code_sha': protocol['code_sha'],
              'run_id': protocol['run_id'], 'source_test_read': False, 'official_eval_read': False,
              'n_fit': 40, 'n_validation': 8, 'n_forecast_frames': int(base.shape[1]),
              'metrics': metrics, 'selected_models': details, 'dp_contrasts': contrasts,
              'ridge_max_abs_parity_m': parity, 'backbone_reproduction_error_m': abs(actual-expected),
              'elapsed_seconds': time.perf_counter()-started,
              'paper_claim_authorized': False, 'limitations': protocol['limitations']}
