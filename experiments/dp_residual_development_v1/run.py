...
SPECS = (GateSpec("single", 1), *(GateSpec("finite", k) for k in (2, 4, 8)),
...
def bootstrap_difference(candidate, reference):
    ...
    samples = difference[rng.integers(0, len(difference), size=(5000, len(difference)))].mean(1)
    return {"mean_difference_mm": float(difference.mean()),
            "paired_bootstrap_95_percent_mm": np.quantile(samples, [.025, .975]).tolist(),
            "wins": int(np.sum(difference < -1e-10)), "ties": int(np.sum(abs(difference) <= 1e-10)),
            "cases": len(difference), "interpretation": "Descriptive pilot interval; old validation trajectories, one object"}
...
def main():
    ...
    with np.load(args.bundle, allow_pickle=False) as z:
        ...
        initial, action = z["initial"].astype(np.float64), z["action"].astype(np.float64)
    ...
    overlap = [v for v in groups.values() if any(i < 40 for i in v) and any(i >= 40 for i in v)]
    if overlap:
        write_json(out / "blocked.json", {"reason": "Exact causal queries cross historical fit/validation", "groups": overlap})
        raise RuntimeError("Cross-split duplicate causal queries; cannot score a held-trajectory pilot")
    ...
    protocol = {"contract": "dp-residual-dlo1-pilot-v1", "seeds": SEEDS,
                "ridges": RIDGES, "shrinkages": SHRINKAGES,
                ...
                "gate_covariance": "full", "gate_pca_dimension_cap": 8,
                ...
                "subfit_names": names[subfit_ids].tolist(), "tuning_names": names[tune_ids].tolist(),
                "refit_names": names[fit_ids].tolist(), "held_validation_names": names[held_ids].tolist(),
                "full_fit_count_before_deduplication": 40, "full_validation_count_before_deduplication": 8,
                "bundle_sha256": metadata["bundle_sha256"], "commit": os.environ.get("GITHUB_SHA"),
    ...
    for spec in SPECS:
        for ridge in RIDGES:
            pred, diagnostics, _ = fit_predict(spec, ridge, features[subfit_ids],
                                               residual[subfit_ids], features[tune_ids])
            for shrinkage in SHRINKAGES:
                prediction = correction_prediction(baseline[tune_ids], pred, frames[tune_ids], shrinkage)
                error = case_error(prediction, truth[tune_ids])
                bank.append({"spec": asdict(spec), "key": spec.key, "ridge": ridge,
                             "shrinkage": shrinkage, "tuning_mean_l1_mm": float(error.mean()),
                             "tuning_case_l1_mm": error.tolist(), "diagnostics": diagnostics})
    ...
    for family in ("single", "finite", "finite_bayes", "dp"):
        ...
        selection[family] = min(eligible, key=lambda a: (a["tuning_mean_l1_mm"],
                                a["spec"]["components"], a["shrinkage"], a["ridge"], a["key"]))
    write_json(out / "selection_seal.json", {"selection": selection, "bank": bank,
               "protocol_sha256": digest(out / "protocol.json"), "validation_scored": False})
    predictions = {"hybrid": baseline[held_ids].copy(), "action_persistence": persistence[held_ids].copy()}
    ...
    for family, chosen in selection.items():
        ...
        pred, diagnostics, individual = fit_predict(spec, chosen["ridge"], features[fit_ids],
                                                    residual[fit_ids], features[held_ids])
        predictions[family] = correction_prediction(baseline[held_ids], pred,
                                                    frames[held_ids], chosen["shrinkage"])
        fitted[family] = diagnostics
        seed_predictions[family] = [correction_prediction(baseline[held_ids], p, frames[held_ids],
                                                         chosen["shrinkage"]) for p in individual]
    old = fit_deform_local_residual(initial[fit_ids], action[fit_ids], baseline[fit_ids],
                                    truth[fit_ids], names[fit_ids].tolist(), ridge=1., variance_floor_m2=1e-6)
    predictions["existing_local_r1_s0p5"] = predict_deform_local_residual(
        old, initial[held_ids], action[held_ids], baseline[held_ids], shrinkage=.5)["predictions"]
    single_check = ResidualExperts(GateSpec("single", 1), 1).fit(features[fit_ids], residual[fit_ids])
    equivalent = correction_prediction(baseline[held_ids], single_check.predict(features[held_ids]), frames[held_ids], .5)
    ...
    write_json(out / "prediction_seal.json", {"sha256": digest(prediction_path),
               "selection_sha256": digest(out / "selection_seal.json"), "validation_scored": False})
    scores = {}
    for key, prediction in predictions.items():
        error = case_error(prediction, truth[held_ids])
        scores[key] = {"mean_l1_mm": float(error.mean()), "case_l1_mm": error.tolist(),
                       "prefix_horizon_mean_l1_mm": {str(h): float(case_error(prediction[:, :h], truth[held_ids, :h]).mean())
                                                   for h in (50, 150, 498)}}
    differences = {ref: bootstrap_difference(scores["dp"]["case_l1_mm"], scores[ref]["case_l1_mm"])
                   for ref in ("hybrid", "existing_local_r1_s0p5", "single", "finite", "finite_bayes")}
    references = [scores[k]["mean_l1_mm"] for k in ("existing_local_r1_s0p5", "single", "finite", "finite_bayes")]
    dp_better = scores["dp"]["mean_l1_mm"] < .99 * min(references)
    result = {"contract": "dp-residual-dlo1-pilot-result-v1", "preparation": metadata,
              "protocol": protocol, "selection": selection, "fitted_gate_diagnostics": fitted,
              "scores": scores, "dp_paired_differences": differences,
              "seed_case_errors_mm": {f: [case_error(p, truth[held_ids]).tolist() for p in pp]
              ...
              "interpretation": ("Promising exploratory DP accuracy result; not confirmation" if dp_better
                                 else "This pilot does not establish an incremental DP accuracy benefit"),
              "uncertainty_calibration_tested": False, "sticky_hdp_tested": False,
              "versions": {"python": sys.version, "numpy": np.__version__, "sklearn": sklearn.__version__},
              "elapsed_seconds": time.monotonic() - started}
    write_json(out / "result.json", result)
    lines = ["# DP residual DLO1 development pilot", "", protocol["claim_boundary"], "",
             "| Model | Mean coordinate L1 (mm) |", "|---|---:|"]
    lines += [f"| {key} | {value['mean_l1_mm']:.6f} |" for key, value in scores.items()]
    lines += ["", result["interpretation"], "", "Tuning used only whole trajectory groups inside the original fit split.",
    ...
    print("RESULT_JSON " + json.dumps({"scores": scores, "dp_paired_differences": differences,
                                      "dp_improves_all_strong_references_by_one_percent": dp_better}), flush=True)
