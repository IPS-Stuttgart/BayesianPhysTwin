...
def fit_gp_basis(x, y, rank, length, material):
    ...
    model = dict(
        keep=keep, location=location, scale=scale, inducing=inducing,
        chol=chol, length=length, material=material,
        response_scale=response_scale, inducing_indices=indices,
    )
...
def solve_gp(model, noise):
    ...
    return {**model, "noise": noise, "factor": factor,
...
def family_models(x, y, length, family):
    ...
    if family == "material_gp":
        return [fit_gp_basis(x.reshape(-1, x.shape[-1]), y.reshape(-1, 3),
                             RANK_PER_NODE * count, length, True)]
    return [fit_gp_basis(x[:, :, node].reshape(-1, x.shape[-1]),
                         y[:, :, node].reshape(-1, 3), RANK_PER_NODE, length, False)
...
def family_predict(models, x, noise, family, variance=False):
    ...
    predictions = [predict_gp(x[:, :, i].reshape(-1, x.shape[-1]), model, variance)
                   for i, model in enumerate(solved)]
    mean = np.stack([item[0].reshape(x.shape[:2] + (3,)) for item in predictions], axis=2)
    var = None
    if variance:
        var = np.stack([item[1].reshape(x.shape[:2] + (3,)) for item in predictions], axis=2)
...
def ridge_prediction(train, query, shrinkage):
    model = fit_deform_local_residual(
        train["initial"], train["action"], train["baseline"], train["targets"],
        train["names"].tolist(), ridge=1.0, variance_floor_m2=1e-6,
    )
...
def prepare(output):
    ...
    for path, key in [(training_path, "training_result"), (manifest_path, "source_manifest"),
    ...
    for label, names in [("fit", fit_names), ("validation", validation_names)]:
        ...
        rollout = runtime._rollout(state, trajectories, modules=modules, torch=torch,
                                   device="cuda:0")
        ...
        if label == "validation":
            ...
            if abs(actual - expected) > 1e-7:
                raise ValueError(f"baseline reproduction failed: {actual} vs {expected}")
        data = dict(initial=initial, action=action, baseline=baseline, names=np.asarray(names))
        if label == "fit":
        ...
        for index in range(len(names)):
            digest = hashlib.sha256(initial[index].tobytes() + action[index].tobytes()
                                     + baseline[index].tobytes()).hexdigest()
    ...
    write_json(output / "preparation.json", dict(
        training_result_sha256=sha(training_path), source_manifest_sha256=sha(manifest_path),
        checkpoint_sha256=sha(checkpoint_path), fit_names=fit_names,
        validation_names=validation_names, causal_query_digests=identities,
        source_test_read=False, official_eval_read=False,
        torch_version=torch.__version__, numpy_version=np.__version__,
        code_revision=os.environ.get("EXPERIMENT_REVISION"),
    ))
    print("PREPARATION_COMPLETE: 40 fit and 8 validation trajectories; baseline reproduced", flush=True)
...
def predict(output):
    ...
    initial, action, baseline, targets, groups = _collapse_duplicate_queries(
        data["initial"], data["action"], data["baseline"], data["targets"], data["names"]
    )
    train = dict(initial=initial, action=action, baseline=baseline, targets=targets,
                 names=np.asarray([group[0] for group in groups]))
    ordered = sorted(range(len(groups)), key=lambda i: hashlib.sha256(
        train["names"][i].encode()).hexdigest())
    ...
    response = np.einsum("ntvi,nij->ntvj", development["targets"] - development["baseline"],
                         fit_frames)[:, :, 2:-2]
    bank = []
    selected = {}
        ...
        for length in LENGTHS:
            ...
            for noise in NOISES:
                ...
                for shrinkage in SHRINKAGES:
                    prediction = world_prediction(selection["baseline"], local,
                                                  selection_frames, shrinkage)
                    loss = float(np.mean(np.abs(prediction - selection["targets"])))
                    entry = dict(family=family, length=length, noise_variance=noise,
                                 shrinkage=shrinkage, inner_l1_m=loss)
                    bank.append(entry)
                    print("INNER", json.dumps(entry), flush=True)
        selected[family] = min([row for row in bank if row["family"] == family],
                               key=lambda row: (row["inner_l1_m"], row["length"],
                                                row["noise_variance"], row["shrinkage"]))
    ridge_bank = []
    for shrinkage in (0.125, 0.25, 0.5, 1.0):
        prediction = ridge_prediction(development, selection, shrinkage)
        ridge_bank.append(dict(shrinkage=shrinkage, inner_l1_m=float(
            np.mean(np.abs(prediction - selection["targets"])))))
    selected_ridge = min(ridge_bank, key=lambda row: row["inner_l1_m"])
    write_json(output / "selection.json", dict(
        gp_bank=bank, selected=selected, ridge_bank=ridge_bank, selected_ridge=selected_ridge,
        inner_fit_names=development["names"].tolist(),
        inner_validation_names=selection["names"].tolist(), duplicate_groups=groups,
        outer_validation_truth_read=False,
    ))
    all_x, all_frames = canonical(train)
    query_x, query_frames = canonical(query)
    all_response = np.einsum("ntvi,nij->ntvj", train["targets"] - train["baseline"],
                             all_frames)[:, :, 2:-2]
    predictions = {"baseline": query["baseline"],
                   "ridge_fixed": ridge_prediction(train, query, 0.25),
                   "ridge_inner_selected": ridge_prediction(train, query, selected_ridge["shrinkage"])}
    for family, configuration in selected.items():
        models = family_models(all_x, all_response, configuration["length"], family)
        local, variance, solved = family_predict(models, query_x,
                                                 configuration["noise_variance"], family, True)
        shrinkage = configuration["shrinkage"]
        predictions[family] = world_prediction(query["baseline"], local, query_frames, shrinkage)
        ...
        predictions[family + "_raw_variance"] = np.maximum(shrinkage ** 2 * global_variance, 1e-6)
        arrays = {}
        for index, model in enumerate(solved):
            for key in ("keep", "location", "scale", "inducing", "chol", "response_scale",
                        "normal", "rhs", "weights", "inducing_indices"):
    ...
    write_json(output / "prediction_seal.json", dict(
        predictions_sha256=sha(output / "predictions.npz"),
        selection_sha256=sha(output / "selection.json"),
        outer_validation_truth_read=False, elapsed_seconds=time.monotonic() - started,
        model_family="finite-rank-Nystrom-Matern32-GP", rank_per_node=RANK_PER_NODE,
        all_fit_rows_used=True, covariance_calibrated=False,
    ))
...
def score(output):
    ...
    methods = ["baseline", "ridge_fixed", "ridge_inner_selected", "independent_gp", "material_gp"]
    per_case = {name: np.mean(np.abs(predictions[name] - targets), axis=(1, 2, 3))
                for name in methods}
    ...
    for name in methods:
        ...
        entry = dict(method=name, mean_coordinate_l1_mm=float(loss.mean() * 1000),
                     case_l1_mm=(loss * 1000).tolist(),
                     improvement_vs_ridge_percent=float((1 - loss.mean() / per_case["ridge_fixed"].mean()) * 100),
                     wins_vs_ridge=int(np.sum(difference < -1e-12)),
                     paired_difference_vs_ridge_mm=float(difference.mean() * 1000),
                     paired_bootstrap_95_mm=(np.quantile(difference[draws].mean(axis=1),
                                                         [0.025, 0.975]) * 1000).tolist())
        if name.endswith("gp"):
            variance = predictions[name + "_raw_variance"]
            errors = predictions[name][:, :, 2:-2] - targets[:, :, 2:-2]
            entry["raw_coordinate_coverage90"] = float(np.mean(
                np.abs(errors) <= 1.6448536269514722 * np.sqrt(variance)))
            entry["raw_coordinate_nll"] = float(np.mean(
                0.5 * (np.log(2 * np.pi * variance) + errors ** 2 / variance)))
        rows.append(entry)
    result = dict(
        status="completed", claim_boundary="Retrospective DLO2 development pilot, not fresh confirmation",
        methods=rows, validation_names=names, trajectory_count=len(names),
        statistical_unit="complete-trajectory", selection=json.loads((output / "selection.json").read_text()),
        preparation=json.loads((output / "preparation.json").read_text()), prediction_seal=seal,
        source_test_read=False, official_eval_read=False, robot_data_collected=False,
    ...
    lines = ["# DLO2 GP residual development pilot", "",
             "Retrospective development only. No source-test or official evaluation read.", "",
             "| Method | L1 (mm) | Improvement vs fixed ridge | Wins / 8 |",
             "| --- | ---: | ---: | ---: |"]
    for row in rows:
        lines.append(f"| {row['method']} | {row['mean_coordinate_l1_mm']:.6f} | "
                     f"{row['improvement_vs_ridge_percent']:.2f}% | {row['wins_vs_ridge']} |")
    lines += ["", "GP hyperparameters were selected using 8 trajectories inside the original 40-trajectory fitting set.",
