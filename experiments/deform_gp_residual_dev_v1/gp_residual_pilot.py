...
def gp_mean(x: np.ndarray, y: np.ndarray, q: np.ndarray, *, length: float,
            noise: float, inducing: int = 128, seed: int = 20260907) -> np.ndarray:
    ...
    for start in range(0, len(x), 2048):
        basis = solve_triangular(chol, kernel(x[start:start+2048], z, length).T,
                                 lower=True).T
    ...
    for start in range(0, len(q), 2048):
        basis = solve_triangular(chol, kernel(q[start:start+2048], z, length).T,
                                 lower=True).T
...
def unit_tests() -> dict[str, bool]:
    ...
    result = gp_mean(x, y, q, length=1.0, noise=.1, inducing=len(x))
    location, scale = x.mean(0), x.std(0)
    xx, qq = (x-location)/scale, (q-location)/scale
    direct = y.mean(0) + kernel(qq, xx, 1.) @ np.linalg.solve(
        kernel(xx, xx, 1.) + np.eye(len(x))*.1, y-y.mean(0))
    assert np.max(np.abs(result-direct)) < 2e-5
    assert np.array_equal(result, gp_mean(x, y, q, length=1., noise=.1,
                                        inducing=len(x)))
    assert np.allclose(gp_mean(x, np.ones((50, 2))*3., q,
                              length=1., noise=.1), 3.)
    return {'dense_gp_mean_parity': True, 'deterministic': True,
            'constant_response': True}


def load_partition(path: Path, names: list[str]) -> dict:
    """Read a single explicitly named development NPZ; no recursive discovery."""
    ...
    aliases = {'initial': ['initial', 'initial_states'],
    ...
    with np.load(path, allow_pickle=False) as data:
        print('DEVELOPMENT_CACHE_SCHEMA', path.name,
              {k: [list(data[k].shape), str(data[k].dtype)] for k in data.files},
              flush=True)
        ...
        result['names'] = data['names'].astype(str).tolist() if 'names' in data else names
...
def main() -> int:
    ...
    tests = unit_tests()
    if args.self_test:
        print(json.dumps(tests)); return 0
    ...
    from bayesian_phystwin_experiments.deform_dlo_local_residual import (
        ...
        predict_deform_local_residual, _collapse_duplicate_queries)
    ...
    fitted = fit_deform_local_residual(fit['initial'], fit['action'], fit['baseline'],
               fit['targets'], fit_names, ridge=1., variance_floor_m2=1e-10)
    ridge = predict_deform_local_residual(fitted, val['initial'], val['action'],
               val['baseline'], shrinkage=.25)['predictions']
    ...
    for shrink in [.1, .5, 1.]:
        pred = predict_deform_local_residual(fitted, val['initial'], val['action'],
                    val['baseline'], shrinkage=shrink)['predictions']
        ...
        for noise in [.1, 1.]:
            ...
            for node in range(xf.shape[2]):
                canonical[:, :, node] = gp_mean(
                    ...
                    length=length, noise=noise).reshape(len(val_names), xv.shape[1], 3)
            correction = np.einsum('ntvj,nij->ntvi', canonical, vframes)
            for shrink in [.1, .25, .5, 1.]:
                pred = val['baseline'].copy()
                pred[:, :, 2:-2] += shrink*correction
                score(f'gp_l{length}_n{noise}_s{shrink}', pred,
                      {'length': length, 'noise': noise, 'shrinkage': shrink,
                       'inducing_per_node': 128})
    report = {'schema_version': 1, 'contract': 'deform-gp-development-pilot-v1',
              ...
              'new_hardware_data': False, 'unique_bayesian_accuracy_claim': False,
              ...
              'tests': tests, 'elapsed_seconds': time.perf_counter()-started,
              'source_commit': os.environ.get('GITHUB_SHA'), 'arms': results}
