...
def export(cache: Path, partitions: list[str]) -> dict:
    ...
    metadata = {'checkpoint_sha256': CHECKPOINT_HASH, 'manifest_sha256': MANIFEST_HASH,
                'source_revision': os.environ.get('GITHUB_SHA'),
                'python': sys.version, 'torch': torch.__version__, 'numpy': np.__version__,
                'official_eval_read': False, 'partitions': {}}
    ...
    for partition in partitions:
        ...
        rollout = posterior._evaluate_state(state, trajectories, modules=modules, torch=torch,
                                             device='cuda:0', dlo_type='DLO2', node_count=12)
        ...
        values = {key: np.asarray(rollout[key]) for key in ('predictions', 'targets', 'persistence')}
        if not all(np.isfinite(value).all() for value in values.values()):
            raise ValueError('nonfinite frozen hybrid rollout')
        np.savez_compressed(output, names=np.asarray(names), initial=initial, action=action, **values)
        l1 = np.mean(np.abs(values['predictions'] - values['targets']), axis=(1, 2, 3))
        record = {'checkpoint_sha256': CHECKPOINT_HASH, 'manifest_sha256': MANIFEST_HASH,
                  'sha256': digest(output), 'names': names, 'shape': list(values['predictions'].shape),
                  'baseline_l1_m': float(l1.mean()), 'case_l1_m': l1.tolist(),
                  'elapsed_seconds': time.monotonic() - started, 'official_eval_read': False}
        if partition == 'validation':
            expected = 0.007912029745057225
            record['historical_baseline_l1_m'] = expected
            record['baseline_absolute_parity_error_m'] = abs(float(l1.mean()) - expected)
