...
def kernel(x: np.ndarray, z: np.ndarray, length: float, arc_length: float | None) -> np.ndarray:
...
class SparseGP:
    ...
    @classmethod
    def prepare(cls, x: np.ndarray, y: np.ndarray, *, length: float,
                arc_length: float | None = None, inducing_count: int = 128,
                seed: int = 20260907) -> SparseGP:
        ...
        for start in range(0, len(x), 2048):
            ...
            phi = solve_triangular(lower, kernel(x[start:stop], z, length, arc_length).T,
                                   lower=True, check_finite=False).T
            gram += phi.T @ phi
            cross += phi.T @ yy[start:stop]
        return cls(z, lower, gram, cross, location, scale, length,
                   arc_length, len(x))
    ...
    def coefficients(self, noise: float) -> tuple[np.ndarray, tuple[np.ndarray, bool]]:
        ...
        factor = cho_factor(self.gram + noise * np.eye(len(self.inducing)),
                            lower=True, check_finite=False)
        return cho_solve(factor, self.cross, check_finite=False), factor

    def predict(self, x: np.ndarray, *, noise: float,
                return_cov: bool = False) -> tuple[np.ndarray, np.ndarray | None]:
        x = matrix(x)
...
def self_test() -> dict[str, bool]:
    ...
    expected = kt @ np.linalg.solve(kd + 0.1 * np.eye(len(x)),
                                     (y - gp.location) / gp.scale)
    expected = expected * gp.scale + gp.location
    np.testing.assert_allclose(mean, expected, atol=1e-6)
    expected_cov = kernel(test, test, 0.8, None) - kt @ np.linalg.solve(
        kd + 0.1 * np.eye(len(x)), kt.T) + 0.1 * np.eye(len(test))
    ...
    try:
        ...
        pass
    return {"exact_gp_mean_parity": True, "exact_gp_covariance_parity": True,
            "predictive_covariance_psd": True, "zero_target_control": True,
            "material_product_kernel_psd": True, "invalid_noise_rejected": True}
