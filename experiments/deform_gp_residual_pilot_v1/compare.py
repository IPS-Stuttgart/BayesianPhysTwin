...
def fields(data: dict) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    residual = np.einsum(
        "ntvi,nij->ntvj", data["targets"] - data["predictions"], frames
    )[:, :, 2:-2]
    return features, frames, residual


def corrected(
    data: dict, frames: np.ndarray, mean: np.ndarray, shrinkage: float
) -> np.ndarray:
    ...
    if not np.array_equal(
        prediction[:, :, [0, 1, -2, -1]], data["predictions"][:, :, [0, 1, -2, -1]]
    ):
...
def metrics(prediction: np.ndarray, data: dict) -> dict:
    ...
    return {
        "mean_l1_mm": float(1000 * cases.mean()),
        "free_node_l1_mm": float(1000 * error[:, :, 2:-2].mean()),
        "case_l1_mm": dict(zip(data["names"], (1000 * cases).tolist(), strict=False)),
        "wins_vs_hybrid": int(np.sum(cases < baseline)),
        "relative_gain_vs_hybrid": float(1 - cases.mean() / baseline.mean()),
        "early_middle_late_l1_mm": [
            float(1000 * part.mean()) for part in np.array_split(error, 3, axis=1)
        ],
    }
    ...
    return {
        hashlib.sha256(
            b"".join(
                ...
                for k in ("initial", "action", "predictions")
            )
        ).hexdigest()
...
class GPFamily:
        ...
        if self.shared:
            self.models = [
                SparseGP.prepare(
                    ...
                    inducing_count=CONFIG["shared_inducing_count"],
                )
            ]
        else:
            self.models = [
                SparseGP.prepare(
                    ...
                    residual[:, :, node].reshape(-1, 3),
                    length=length,
                    inducing_count=CONFIG["per_node_inducing_count"],
                )
    ...
    def all_means(
        self, features: np.ndarray, noises: list[float]
        ...
        inputs = (
            [x.reshape(-1, x.shape[-1])]
            if self.shared
            else [x[:, :, node].reshape(-1, x.shape[-1]) for node in range(x.shape[2])]
        )
        for node, (model, values) in enumerate(zip(self.models, inputs, strict=False)):
            ...
            for start in range(0, len(values), 2048):
                ...
                for noise in noises:
                    outputs[noise].append(
                        (phi @ weights[noise]) * model.scale + model.location
                    )
            for noise in noises:
...
def main(cache: Path, output: Path) -> None:
    ...
    def candidate(family: str, spec: dict, prediction: np.ndarray, model) -> None:
        ...
        if (
            ...
            or score["mean_l1_mm"] < selected[family]["validation"]["mean_l1_mm"]
        ):
    ...
    for family, material_lengths in [
        ("gp_per_node", [None]),
        ...
        for length in CONFIG["lengths"]:
            for arc_length in material_lengths:
                ...
                for noise, mean in model.all_means(
                    xv, CONFIG["noise_variances"]
                ).items():
                    for shrinkage in CONFIG["shrinkages"]:
    ...
    xh, rh, _ = fields(held)
    results = {
        "hybrid": metrics(held["predictions"], held),
    ...
    for family in ("gp_per_node", "gp_material_shared"):
        ...
        contrasts[family] = {
            "gain_vs_existing_ridge": float(1 - values.mean() / reference.mean()),
            ...
            "descriptive_trajectory_bootstrap_95_interval_mm": np.quantile(
                difference[bootstrap].mean(axis=1), [0.025, 0.975]
            ).tolist(),
        }
    result = {
        ...
        "original_fit_count": original_fit_count,
        "fit_causal_query_groups": len(groups),
        "validation_count": len(validation["names"]),
        "source_test_count": len(held["names"]),
        "horizon": int(held["predictions"].shape[1]),
        "node_count": int(held["predictions"].shape[2]),
        "gp_selected_family": best_family,
        "selected": selected,
        "held_results": results,
        "gp_vs_ridge": contrasts,
    ...
    for family, score in results.items():
        lines.append(
            f"| {family} | {score['mean_l1_mm']:.6f} | {score['wins_vs_hybrid']}/8 |"
        )
    lines += [
        "",
        f"GP family chosen on validation: **{best_family}**.",
        "",
