...
def local_support(feature: FloatArray, model: Model) -> LocalSupport:
    ...
    quotient = np.bincount(
        classes, weights=kernel_weights, minlength=class_count
    ).astype(np.float64)
...
def fit_probe_quantizer(
    signatures: FloatArray, outcome_count: int, iterations: int
...
def build_probe_bundle(
    ...
    shaped = support.residuals.reshape(
        hypothesis_count, passive_protocol.horizon_frames, -1, 3
    )
    ...
    if frames == 0:
        ...
        labels, signature_mean, signature_scale, centers, radii = fit_probe_quantizer(
            signatures, pilot_protocol.outcome_count, passive_protocol.kmeans_iterations
        )
    ...
    losses = np.empty(
        (hypothesis_count, outcome_count, len(action_scales)), dtype=np.float64
    )
    ...
    for outcome in range(outcome_count):
        ...
        losses[:, outcome, :] = (
            np.mean(np.square(terminal[:, None, :] - actions[None, :, :]), axis=2)
            / denominator[:, None]
        )
    nominal_outcome = support.jeffrey_weights @ likelihood
    candidate = decision_probe_candidate(
        f"probe_{frames}", float(frames), likelihood, losses
    )
    return ProbeBundle(
        frames=frames,
        candidate=candidate,
...
def assign_outcome(
    ...
    distances = np.mean(
        np.square(bundle.standardized_centers - standardized[None, :]), axis=1
    )
    outcome = int(np.argmin(distances))
    distance = float(distances[outcome])
    radius = float(bundle.support_radius[outcome])
    supported = bool(
        distance <= pilot_protocol.target_support_multiplier * max(radius, 1e-12)
    )
    return OutcomeAssignment(
        outcome=outcome,
...
def terminal_mse(
    ...
    return float(
        np.mean(np.square(normalized_terminal_residual - predicted)) * length_scale**2
    )
...
def bootstrap_interval(
    values: FloatArray, replicates: int, seed: int
) -> tuple[float, float]:
...
def method_summary(
    ...
    return {
        ...
        "harm_fraction_vs_fallback": float(np.mean(values > fallback + 1e-12)),
        "mean_probe_frames": float(np.mean(probe_frames)),
        "action_counts": np.bincount(action_array, minlength=action_count).tolist(),
