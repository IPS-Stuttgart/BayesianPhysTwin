...
def test_gaussian_conditioning():
    model = dict(nx=1, weights=np.ones(1), means=np.array([[2., 3.]]),
                 covariances=np.array([[[4., 2.], [2., 5.]]]))
    y, w = m.conditional(model, np.array([[0.], [2.], [4.]]))
    np.testing.assert_allclose(y[:, 0], [2., 3., 4.])
    np.testing.assert_allclose(w, 1.)


def test_mixture_marginal_gate_and_permutation():
    model = dict(nx=1, weights=np.array([.4, .6]), means=np.array([[-3., -5.], [3., 5.]]),
    ...
    permuted = {**model, **{k: model[k][::-1] for k in ('weights', 'means', 'covariances')}}
...
def test_pca_roundtrip_and_reject_invalid():
    ...
    np.testing.assert_allclose(m.decode(transform, m.project(transform, a)), a, atol=1e-12)
    with pytest.raises(ValueError):
        m.compress_fit(np.array([[np.nan], [0.]]), 1)
...
def test_exact_fallback_does_not_evaluate_model():
    base = np.array([[np.pi, -0., 1.]])
    selected = {'shrinkage': 0., 'model': None}
...
def test_future_internal_exclusion():
    ...
    for origin in m.CONFIG['origins']:
        ...
        for horizon in m.CONFIG['horizons']:
            for actual, expected in zip(m.ref.inputs(changed, origin, horizon),
                                         m.ref.inputs(original, origin, horizon), strict=True):
                np.testing.assert_array_equal(actual, expected)


def test_bootstrap_uses_complete_trajectories():
    result = m.bootstrap_delta(np.zeros(24), np.full(24, .001))
    np.testing.assert_allclose(result['ci95_mm'], [-1., -1.])
...
def test_dp_positive_control():
    ...
    x = (2*z+rng.normal(0, .35, len(z)))[:, None]
    y = (3*z+rng.normal(0, .15, len(z)))[:, None]
    model = m.fit_mixture(x, y, 'dp', 4, 1.)
    pred, weights = m.conditional(model, np.array([[-2.], [2.]]))
...
def test_finite_uses_same_expert_family():
    ...
    y = x[:, :1]+rng.normal(0, .2, size=(70, 1))
    model = m.fit_mixture(x, y, 'finite', 2, 1.)
...
def test_target_free_prediction_interface():
    ...
    assert list(inspect.signature(m.predict_family).parameters) == ['selected', 'transform', 'x', 'baseline']
