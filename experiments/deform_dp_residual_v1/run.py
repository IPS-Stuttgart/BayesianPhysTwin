...
CONFIG = {
    ...
    "input_rank": 6,
    "residual_rank": 6,
    "component_caps": [2, 4, 8],
    "concentrations": [0.1, 1.0, 10.0],
    "shrinkages": [0.0, 0.25, 0.5, 1.0],
    "rbf_gamma": [0.125, 0.5, 2.0],
    "rbf_ridge": [0.1, 1.0, 10.0],
    "mixture_mean_precision": 5.0,
    "mixture_covariance_regularizer": 1e-5,
    "n_init": 3,
    "max_iter": 1000,
    "tol": 1e-5,
    "seed": 20260907,
...
def dump(path, obj):
    Path(path).write_text(
        json.dumps(encode(obj), sort_keys=True, indent=2, allow_nan=False) + "\n"
    )
...
def fit_mixture(x, y, kind, cap, alpha):
    ...
    if not estimator.converged_:
        raise RuntimeError(
            f"Mixture did not converge after {estimator.n_iter_} iterations"
        )
    return dict(
        kind="mixture",
        ...
        effective_components=int(np.sum(estimator.weights_ > 0.01)),
    )
...
def select_family(x, y, xv, validation_error, output_transform, family):
    ...
    if family == "single_expert":
        ...
        settings = [
            (k, a) for k in CONFIG["component_caps"] for a in CONFIG["concentrations"]
        ]
    for first, second in settings:
        try:
            if family == "rbf_residual":
                model = dict(
                    ...
                    coef=np.linalg.solve(
                        kernel(x, x, first) + second * np.eye(len(x)), y
                    ),
                )
            else:
                model = fit_mixture(
                    x, y, "dp" if family == "dp_mixture" else "finite", first, second
                )
            ...
            records.append(
                dict(setting=[first, second], status="failed", reason=str(error))
            )
    if selected is None:
        selected = dict(
            model=dict(kind="zero", ny=y.shape[1]),
            shrinkage=0.0,
            setting=None,
            validation_l1_m=float(np.mean(np.abs(validation_error))),
        )
    return selected, records


def predict_family(selected, transform, x, baseline):
    if selected["shrinkage"] == 0.0:
        return np.asarray(baseline).copy()  # Byte-exact fallback, no model evaluation.
    return baseline + selected["shrinkage"] * decode(
        transform, correction(selected["model"], x)
    )
...
def bootstrap_delta(candidate, comparator):
    ...
    indices = np.column_stack(
        [rng.integers(i, i + 12, (CONFIG["bootstrap_replicates"], 12)) for i in (0, 12)]
    )
    return dict(
        difference_mm=float(delta.mean() * 1000),
        ci95_mm=(1000 * np.quantile(delta[indices].mean(1), [0.025, 0.975])).tolist(),
...
def run(root, out):
    ...
    code_hashes = {
        p.name: ref.digest(p) for p in sorted(Path(__file__).parent.glob("*.py"))
    }
    ...
    for dlo in CONFIG["dlos"]:
        ...
        manifest[dlo] = {
            part: [dict(name=p.name, sha256=ref.digest(p)) for p in paths]
            for part, paths in (
                ("fit", fit),
                ("calibration", val),
                ("source_test", test),
    ...
    if (
        ref.digest(out / "input_manifest.json")
        != "9a1ef8d257f4150df842feecc2d45b579eb49a1276e0b023d670221084c0646f"
    ):
    ...
    for dlo in CONFIG["dlos"]:
        ...
        for horizon in CONFIG["horizons"]:
            ...
            for origin in CONFIG["origins"]:
                ...
                residual = np.stack(
                    [
                        a[origin + horizon, 2:10].ravel() - r[1]
            ...
            yp = compress_fit(
                train["truth"] - train["baseline"], CONFIG["residual_rank"]
            )
            x, y = (
                project(xp, train["features"]),
                project(yp, train["truth"] - train["baseline"]),
            )
            ...
            key = f"{dlo}-h{horizon}"
            all_models[key] = dict(
                reference=reference_models, xp=xp, yp=yp, selected=chosen
            )
            ...
            print(
                key,
                {
                    k: (
                        v["setting"],
                        v["shrinkage"],
                        round(v["validation_l1_m"] * 1000, 4),
                    )
    ...
    for dlo, paths in paths_by_dlo.items():
        ...
        for horizon in CONFIG["horizons"]:
            ...
            predictions[key] = {
                "persistence": data["current"],
                "boundary_interpolation": data["simple"],
                "reference_ridge": data["baseline"],
            }
            for family in ARMS[3:]:
                predictions[key][family] = predict_family(
                    m["selected"][family], m["yp"], x, data["baseline"]
                )
    ...
    dump(
        out / "prediction_seal.json",
        {
            ...
            **{
            ...
            "future_internal_values_passed_to_inference": False,
    ...
    for dlo, paths in paths_by_dlo.items():
        ...
        for arm in ARMS:
            error = np.stack(
                [truth[h] - predictions[f"{dlo}-h{h}"][arm] for h in CONFIG["horizons"]]
            )
            ...
            for i, p in enumerate(paths):
                rows.append(
                    dict(
                        ...
                        arm=arm,
                        l1_mm=1000 * l1[i],
                        rmse_mm=1000 * np.sqrt(mse[i]),
                    )
                )
        scoring[dlo] = {arm: float(np.mean(losses[arm][-12:]) * 1000) for arm in ARMS}
    result = dict(
        config=CONFIG,
        per_object_l1_mm=scoring,
        aggregate={
            a: dict(
                l1_mm=1000 * np.mean(losses[a]),
                rmse_mm=1000 * np.sqrt(np.mean(squared[a])),
            )
            for a in ARMS
        },
        dp_minus_comparator={
            a: bootstrap_delta(losses["dp_mixture"], losses[a])
            for a in ARMS
            if a != "dp_mixture"
        },
        accounting=dict(
            objects=2,
            fit_recordings=64,
            validation_recordings=24,
            held_source_recordings=24,
            forecast_contexts=360,
            official_eval_files=0,
        ),
        environment=dict(
            ...
            runner=os.environ.get("RUNNER_NAME", "local-session"),
        ),
    )
    gain = (
        1
        - result["aggregate"]["dp_mixture"]["l1_mm"]
        / result["aggregate"]["reference_ridge"]["l1_mm"]
    )
    result["relative_gain_over_ridge"] = gain
    result["primary_supported"] = bool(
        gain >= 0.01
        and all(
    ...
    text = [
        "# DP residual screen: real-data result",
        "",
        CONFIG["scope"],
        "",
    ...
    text += [
        f"| {a} | {v['l1_mm']:.6f} | {v['rmse_mm']:.6f} |"
        for a, v in result["aggregate"].items()
    ]
    text += [
        "",
        "DP-minus-comparator complete-trajectory bootstrap:",
        "",
