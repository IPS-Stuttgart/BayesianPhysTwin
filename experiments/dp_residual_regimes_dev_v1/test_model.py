...
def test_single_gaussian_median():
    ...
    np.testing.assert_allclose(
        median_of_mixture(np.ones((4, 1)), m, np.ones_like(m)), m[:, 0], atol=1e-10
    )
...
def test_conditional_mean_formula():
    ...
    expected = m[model.dx :] + (a - m[: model.dx]) @ np.linalg.solve(
        s[: model.dx, : model.dx], s[: model.dx, model.dx :]
    )
...
def test_rotation_and_nonfinite_inputs():
    ...
    b = model.predict(x[:3], np.repeat(frame[None], 3, axis=0))["mean"].reshape(
        3, -1, 3
    )
