...
class ConditionalResidual:
    ...
    def fit(self, x: np.ndarray, y: np.ndarray) -> "ConditionalResidual":
        ...
        self.xpca = PCA(n_components=min(6, x.shape[1], len(x) - 1), whiten=True, svd_solver="full")
        self.ypca = PCA(n_components=min(8, y.shape[1], len(y) - 1), whiten=True, svd_solver="full")
        a = self.xpca.fit_transform(self.scaler.transform(x))
        b = self.ypca.fit_transform(y)
        if np.min(self.xpca.explained_variance_) <= 1e-12 or np.min(self.ypca.explained_variance_) <= 1e-16:
            raise ValueError("degenerate fit-only PCA basis")
        self.dx = a.shape[1]
        self.basis = self.ypca.components_ * np.sqrt(self.ypca.explained_variance_)[:, None]
        reconstruction_error = (y - self.ypca.inverse_transform(b)).reshape(len(y), -1, 3)
        self.discarded = np.einsum("npi,npj->pij", reconstruction_error, reconstruction_error) / len(y)
        ...
        if self.spec.family == "ridge":
            ...
            self.metadata = {"converged": True, "active_weight_gt_001": 1, "warnings": []}
            return self
        args = dict(n_components=self.spec.components, covariance_type="full", reg_covar=0.05,
                    n_init=2, max_iter=300, tol=1e-4, random_state=self.seed)
        if self.spec.family == "gmm":
            self.mixture = GaussianMixture(**args)
        elif self.spec.family in ("dp", "finite-bayes"):
            prior_type = "dirichlet_process" if self.spec.family == "dp" else "dirichlet_distribution"
            alpha = self.spec.concentration if self.spec.family == "dp" else self.spec.concentration / self.spec.components
            self.mixture = BayesianGaussianMixture(**args, weight_concentration_prior_type=prior_type,
        ...
        self.metadata = {"converged": bool(self.mixture.converged_),
    ...
    def latent_components(self, x: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        for weight, mean, cov in zip(self.mixture.weights_, self.mixture.means_, self.mixture.covariances_, strict=True):
            xx = cov[:self.dx, :self.dx]
            xy = cov[:self.dx, self.dx:]
            delta = a - mean[:self.dx]
            solve = np.linalg.solve(xx, xy)
            logs.append(np.log(max(weight, 1e-300)) - 0.5 * (
                self.dx * np.log(2 * np.pi) + np.linalg.slogdet(xx)[1]
    ...
    def predict(self, x: np.ndarray, frames: np.ndarray) -> dict[str, np.ndarray]:
        ...
        canonical = (latent_mean @ self.basis + self.ypca.mean_).reshape(len(x), -1, self.basis.shape[1] // 3, 3)
        means = np.einsum("nkpc,nwc->nkpw", canonical, frames).reshape(len(x), weights.shape[1], -1)
        basis = self.basis.reshape(self.basis.shape[0], -1, 3)
        world_basis = np.einsum("qpc,nwc->nqpw", basis, frames).reshape(len(x), self.basis.shape[0], -1)
        variances = np.einsum("nqd,kqr,nrd->nkd", world_basis, latent_cov, world_basis)
        discarded = np.einsum("nwc,pcd,nwd->npw", frames, self.discarded, frames).reshape(len(x), -1)
        variances = np.maximum(variances + discarded[:, None, :], 1e-12)
        return {"mean": np.sum(weights[:, :, None] * means, axis=1),
