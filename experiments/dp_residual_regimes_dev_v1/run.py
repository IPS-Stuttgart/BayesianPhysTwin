...
PINS = {
    "DLO4": (
        "1001cdb688072675eafdba34c8dc7f937be2fb7800c65d756e31214bc244fa17",
        "b858c2f4c0e107d367857dc2c4e17e753b6853e4593af64c4ce489564ae88fca",
        0.01253017906857404,
    ),
    "DLO5": (
        "6bc72a4b3c7dae13c39181d28aebe9228fb1049fbe4bf26871b95fb518997a04",
        "a8f8b3253f2588eed3fb0e92a8812ce534dba857c82a3af9d3f0fe0dadab39b6",
        0.00918328492025507,
    ),
}
FAMILIES = {
...
def direct_source(root: Path, directory: Path) -> None:
    ...
    receipt = {
        "contract": "source-cache-export-v1",
        "official_eval_access": False,
        "source_revision": "9d7383ea56a0a9e3ad6753d1c42fe653cd7e615d",
        "objects": {},
    }
    for dlo, (mh, ph, error) in PINS.items():
        ...
        with np.load(pp, allow_pickle=False) as z:
            ...
            hybrid, candidate = (
                z["physical"].astype(float),
                z["candidate"].astype(float),
            )
        ...
        mutated = raw.copy()
        mutated[:, 2:, 2:-2] += 17.0
        f2, r2 = build_deform_local_residual_features(
            mutated[:, :2], mutated[:, 2:, [0, 1, 10, 11]], hybrid
        )
...
def load(cache: Path) -> dict[str, object]:
    ...
    for dlo, (mh, ph, expected_l1) in PINS.items():
        ...
        if (
            ...
            or entry["source_manifest_sha256"] != mh
            or entry["source_prediction_sha256"] != ph
        ):
            raise ValueError("export provenance mismatch")
        with np.load(p, allow_pickle=False) as z:
            v = {k: z[k] for k in z.files}
        if v["hybrid"].shape != (8, 498, 12, 3) or v["features"].shape != (
            8,
            498,
            8,
            92,
        ...
        if not np.isclose(
            np.abs(v["hybrid"] - v["truth"]).mean(), expected_l1, atol=1e-8, rtol=0
        ):
            raise ValueError("baseline parity failed")
        if not np.array_equal(
            v["hybrid"][:, :, [0, 1, 10, 11]],
            v["legacy_candidate"][:, :, [0, 1, 10, 11]],
        ):
        ...
        for j in range(8):
            ...
            cases.append(
                {"dlo": dlo, "name": str(v["names"][j]), "query_sha256": hashes[-1]}
            )
    ...
    x = np.concatenate(
        [features[:, :, 0], features.mean(axis=2), features[:, :, -1]], axis=-1
    ).reshape(n * chunks, -1)
    ...
    return {
        ...
        "frames": frames,
        "arrays": v,
        "trajectory": np.repeat(np.arange(n), chunks),
        "chunks": chunks,
    }
...
def evaluate(cache: Path, output: Path) -> dict[str, object]:
    ...
    records = []
    protocol = {
        "contract": "dp-residual-regimes-development-v1",
        ...
        "output_pca": 8,
        "seed": SEED,
        "families": {k: [s.__dict__ for s in v] for k, v in FAMILIES.items()},
        "shrinkages": SHRINKAGES,
        "fold_assignment": data["folds"].tolist(),
        "cases": data["cases"],
        "fit_only_transforms": True,
        "outer_test_fold": "f",
        "inner_validation_fold": "(f+1)%4",
        "fit_folds": "remaining two",
        "all_models_fitted_to_residual_of": "frozen legacy candidate",
        ...
        "source_files": {p.name: digest(p) for p in Path(__file__).parent.glob("*.py")},
    }
    ...
    for fold in range(4):
        ...
        fold_result = {
            "fold": fold,
            "fit_trajectories": sorted(set(trajectory[fit].tolist())),
            "validation_trajectories": sorted(set(trajectory[val].tolist())),
            "test_trajectories": test_ids.tolist(),
            "families": {},
        }
        for family, specs in FAMILIES.items():
            ...
            for spec in specs:
                ...
                model = ConditionalResidual(spec, seed=SEED + fold).fit(
                    data["x"][fit], data["y"][fit]
                )
                ...
                option = {
                    "spec": spec.__dict__,
                    "name": spec.name,
                    "shrinkage": SHRINKAGES[chosen],
                    "validation_l1_m": losses[chosen],
                    "validation_curve_l1_m": losses,
                    "fit_metadata": model.metadata,
                    "elapsed_seconds": time.monotonic() - start,
                }
                options.append(option)
                if best is None or (losses[chosen], SHRINKAGES[chosen]) < (
                    best[0],
                    best[1],
                ):
            ...
            predictions[family][test_ids, :, 2:-2] += (
                shrink * test_prediction["median"]
            ).reshape(shape)
            raw_predictions[family][test_ids, :, 2:-2] += test_prediction[
                "median"
            ].reshape(shape)
            mean_predictions[family][test_ids, :, 2:-2] += (
                shrink * test_prediction["mean"]
            ).reshape(shape)
            hard_predictions[family][test_ids, :, 2:-2] += (
                shrink * test_prediction["hard_mean"]
            ).reshape(shape)
            fold_result["families"][family] = {
                **seal,
                "test_average_max_input_weight": float(
                    test_prediction["weights"].max(axis=1).mean()
                ),
            }
            print(
                f"FOLD {fold} {family}: selected {selected_info['name']}, shrinkage={shrink}, active={selected.metadata['active_weight_gt_001']}",
                flush=True,
            )
        records.append(fold_result)
    all_predictions = {
        "hybrid": arrays["hybrid"],
        "legacy": arrays["legacy_candidate"],
        **predictions,
    }
    scores = {}
    for name, prediction in all_predictions.items():
        ...
        scores[name] = {
            "all_node_l1_mm": float(l1.mean() * 1000),
            "free_node_l1_mm": float(np.abs(errors[:, :, 2:-2]).mean() * 1000),
            "per_trajectory_l1_mm": (l1 * 1000).tolist(),
            "per_dlo_l1_mm": {
                d: float(l1[[c["dlo"] == d for c in data["cases"]]].mean() * 1000)
                for d in PINS
            },
        }
        if name in names:
            scores[name]["unshrunk_selected_model_l1_mm"] = float(
                np.abs(raw_predictions[name] - arrays["truth"]).mean() * 1000
            )
            scores[name]["soft_mean_l1_mm"] = float(
                np.abs(mean_predictions[name] - arrays["truth"]).mean() * 1000
            )
            scores[name]["hard_mean_l1_mm"] = float(
                np.abs(hard_predictions[name] - arrays["truth"]).mean() * 1000
            )
    contrasts = {}
    ...
    for name in ("legacy", "ridge", "single-gaussian", "finite-gmm", "finite-bayes"):
        ...
        samples = np.column_stack(
            [rng.integers(0, 8, (5000, 8)), rng.integers(8, 16, (5000, 8))]
        )
        interval = np.quantile(delta[samples].mean(axis=1), [0.025, 0.975])
        contrasts[name] = {
            "delta_mm": float(delta.mean()),
            "relative_improvement_pct": float((1 - dp.mean() / ref.mean()) * 100),
            "wins": int(np.sum(delta < -1e-10)),
            "ties": int(np.sum(np.abs(delta) <= 1e-10)),
            "descriptive_bootstrap_interval_mm": interval.tolist(),
            "n_trajectories": n,
    ...
    result = {
        ...
        "n_trajectories": n,
        "object_count": 2,
        "n_nonoverlapping_blocks": len(data["x"]),
        "scores": scores,
    ...
    rows = [
        "# DP residual-regime development screen",
        "",
    ...
    for name, s in scores.items():
        rows.append(
            f"| {name} | {s['per_dlo_l1_mm']['DLO4']:.6f} | {s['per_dlo_l1_mm']['DLO5']:.6f} | {s['all_node_l1_mm']:.6f} |"
        )
    rows += [
        "",
        "Each added model uses the same input-only features, fit-only low-rank transforms, and recorded training outcomes. Its shrinkage is selected on separate validation trajectories, including exact unchanged-candidate fallback at zero.",
        "",
