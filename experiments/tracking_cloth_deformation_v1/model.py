...
def parameter_bank(protocol: dict[str, Any]) -> list[tuple[float, float]]:
    return list(
        itertools.product(protocol["stiffness_per_mass"], protocol["damping_per_mass"])
    )
...
def rollout(
    ...
    for t in range(start + 1, len(inputs.times)):
        ...
        for sub in range(1, substeps + 1):
            ...
            force = (k * relative_k * (lengths - rest) / np.maximum(lengths, 1e-9))[
                :, None
            ] * delta
            ...
            x[inputs.corners] = (1 - fraction) * inputs.boundary[
                t - 1
...
def predict(inputs: Inputs, protocol: dict[str, Any]) -> Predictions:
    ...
    bank = np.stack(
        [
            rollout(inputs, parameters, protocol, True)
...
def means(
    predictions: Predictions, weights: np.ndarray, protocol: dict[str, Any]
) -> dict[str, np.ndarray]:
    inputs = predictions.inputs
    residual = inputs.prefix[-1] - predictions.nominal[inputs.cutoff]
    nominal_index = parameter_bank(protocol).index(
        tuple(protocol["nominal_parameters"])
    )
    result = {
        "persistence": np.broadcast_to(
            inputs.prefix[-1], predictions.nominal.shape
        ).copy(),
...
def horizon_bins(inputs: Inputs) -> np.ndarray:
    ...
    return np.clip(
        ((inputs.times - inputs.times[inputs.cutoff]) / duration * 3).astype(int), 0, 2
    )


def fit_specimen(
    records: list[tuple[Predictions, np.ndarray]], protocol: dict[str, Any]
) -> dict[str, Any]:
    """Leave one speed/grasp source recording out; never access target outcomes."""
    if len(records) != 4:
        raise ValueError(
            "Exactly four shaking source conditions are required per specimen"
        )
    loss = np.asarray(
        [
            [squared_error(p, truth, masks(pred.inputs, truth)) for p in pred.bank]
    ...
    for held, (prediction, truth) in enumerate(records):
        w = source_weights(
            np.delete(loss, held, axis=0), protocol["measurement_floor_m"]
        )
        ...
        oof.append(
            {
                arm: np.sqrt(squared_error(mean, truth, valid))
                for arm, mean in arm_means.items()
            }
        )
        ensemble_var = np.einsum(
            "k,ktnd->tnd", w, (prediction.bank - arm_means["bayesian_physics"]) ** 2
        )
        for arm, mean in arm_means.items():
            for b in range(3):
    ...
    accepted = bool(
        np.all(candidate <= baseline)
        and candidate.mean() < (1 - protocol["guard_minimum_relative_gain"]) * reference
    )
    noise = {
        arm: [
            max(float(np.mean(values)), protocol["measurement_floor_m"] ** 2)
            for values in bins
        ]
        for arm, bins in residual_squares.items()
    }
    return {
        "source_posterior_weights": source_weights(
            loss, protocol["measurement_floor_m"]
        ).tolist(),
...
def complete_beliefs(
    prediction: Predictions, fit: dict[str, Any], protocol: dict[str, Any]
    ...
    for arm, mean in arm_means.items():
        variance = np.broadcast_to(
            np.asarray(fit["source_residual_variance_m2"][arm])[bins, None, None],
            mean.shape,
        ).copy()
        if arm == "bayesian_physics":
...
def score(
    mean: np.ndarray, variance: np.ndarray, truth: np.ndarray, inputs: Inputs
) -> dict[str, float | int]:
    ...
    return {
        ...
        "coordinate_90_coverage": float(
            np.mean(abs(e) <= 1.6448536269514722 * np.sqrt(var))
        ),
        "mean_full_90_width_mm": 1000
        * float(np.mean(2 * 1.6448536269514722 * np.sqrt(var))),
        "free_marker_samples": int(valid.sum()),
        "missing_free_marker_samples": int(
            (len(inputs.times) - inputs.cutoff - 1) * (len(inputs.order) - 2)
            - valid.sum()
        ),
    }
