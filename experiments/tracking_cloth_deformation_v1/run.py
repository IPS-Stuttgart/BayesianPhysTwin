...
def prepare(
    root: Path, output: Path, protocol: dict[str, Any], stage: str, workers: int = 1
) -> None:
    ...
    report = [
        "# Tracking Cloth Deformation: public-data pilot",
        "",
    ...
    scales = [
        infer_source_scale(c, read_prefix(c, protocol["prefix_seconds"])[1])
        for c in source
    ]
    if len(set(scales)) != 1:
    ...
    for specimen in sorted({c.specimen for c in source}):
        subset = [
            (name, pred, truth)
            for group, name, pred, truth in records
            if group == specimen
        ]
        fitted[specimen] = fit_specimen(
            [(pred, truth) for _, pred, truth in subset], protocol
        )
        fitted[specimen]["source_recordings"] = [name for name, _, _ in subset]
        for name, oof in zip(
            fitted[specimen]["source_recordings"],
            fitted[specimen]["oof_record_rmse_m"],
            strict=True,
        ):
            source_rows.extend(
                {
    ...
    for case in (c for c in cases if c.motion == "twist"):
        inputs = input_view(case, protocol, scale)
        beliefs = complete_beliefs(
            predict(inputs, protocol), fitted[case.specimen], protocol
        )
        ...
        predictions[case.path.name] = {
            ...
            "guard_accepts": fitted[case.specimen]["guard_accepts"],
    ...
    report.extend(
        [
            "## Predictions sealed",
            "",
            "All 32 target batches are sealed before scoring.",
...
def aggregate(
    ...
    summary = {
        arm: {
            metric: float(np.mean([row[metric] for row in table if row["arm"] == arm]))
            for metric in METRICS
        }
        for arm in ARMS
    }
    ...
    for comparator in ("nominal_physics", "last_residual", "map_physics"):
        diffs = np.array(
            [
                next(
                    r["rmse_mm"]
                    for r in table
                    if r["specimen"] == s and r["arm"] == "guarded_bayesian_physics"
                )
                - next(
                    r["rmse_mm"]
                    for r in table
                    if r["specimen"] == s and r["arm"] == comparator
                )
                for s in specimens
            ]
        )
        material_diffs = np.array(
            [
                np.mean(
                    [
                        diffs[i]
                        for i, s in enumerate(specimens)
                        if s.startswith(material + "_")
                    ]
                )
        ...
        material_samples = rng.integers(
            0, 4, size=(protocol["bootstrap_repetitions"], 4)
        )
        contrasts[comparator] = {
            "guarded_minus_comparator_rmse_mm": float(diffs.mean()),
            "specimen_bootstrap_95_interval_mm": np.quantile(
                diffs[resamples].mean(axis=1), [0.025, 0.975]
            ).tolist(),
            "material_cluster_sensitivity_95_interval_mm": np.quantile(
                material_diffs[material_samples].mean(axis=1), [0.025, 0.975]
            ).tolist(),
            "specimen_wins": int((diffs < 0).sum()),
            "specimen_ties": int((diffs == 0).sum()),
...
def score_run(root: Path, output: Path) -> None:
    ...
    if (output / "target_access.json").exists():
        raise ValueError(
            "This run already started target scoring; use a separately identified pilot run"
        )
    ...
    write_json(
        output / "target_access.json",
        {
            ...
            "purpose": "fixed public-data pilot scoring",
        },
    )
    rows = []
    fallback_records = 0
                ...
                rows.append(
                    {
                        ...
                        "grasp": case.grasp,
                        "arm": arm,
                        "guard_accepted": entry["guard_accepts"],
                        **case_scores[arm],
                    }
                )
            if not entry["guard_accepts"]:
                ...
                for field in ("mean", "variance"):
                    if not np.array_equal(
                        arrays[f"guarded_bayesian_physics_{field}"],
                        arrays[f"nominal_physics_{field}"],
                    ):
                        raise ValueError("Exact fallback violated")
                if (
                    case_scores["guarded_bayesian_physics"]
                    != case_scores["nominal_physics"]
                ):
                    raise ValueError("Exact fallback score violated")
            elif (
                case_scores["guarded_bayesian_physics"]["rmse_mm"]
                > case_scores["nominal_physics"]["rmse_mm"]
            ):
                harmful_accepted_records += 1
    table, metrics = aggregate(rows, protocol)
    metrics.update(
        {
            "fallback_recordings": fallback_records,
            "accepted_recordings": 32 - fallback_records,
    ...
    for arm in ARMS:
        ...
        report += (
            f"| {arm} | {values['rmse_mm']:.4f} | {values['coordinate_nll']:.4f} | "
...
def main() -> int:
    ...
    parser.add_argument(
        "--stage", choices=("inventory", "source", "predict", "score"), default="source"
    )
