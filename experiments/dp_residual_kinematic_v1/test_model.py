...
from experiments.dp_residual_kinematic_v1.model import GateSpec, ResidualExperts
...
def test_single_reproduces_direct_ridge():
    ...
    for node in range(2):
        ...
        pen[0, 0] = 0
        expected = design @ np.linalg.solve(
            design.T @ design + pen, design.T @ y[:, :, node].reshape(-1, 3)
        )
        np.testing.assert_allclose(
            pred[:, :, node].reshape(-1, 3), expected, atol=1e-10
        )
...
def test_positive_control_and_probability_normalization():
    ...
    assert err_finite < 0.1 * err_single


def test_dp_and_finite_bayes_are_distinct_priors():
    x, y = fixture()
    dp = ResidualExperts(GateSpec("dp", 4, 0.1), 1).fit(x[:18], y[:18])
    fb = ResidualExperts(GateSpec("finite_bayes", 4, 0.1), 1).fit(x[:18], y[:18])
...
def test_free_future_truth_cannot_change_causal_features():
    ...
    full[..., 1] = np.arange(12)[None, :, None] * 0.001
    ...
    changed, changed_frame = build_deform_local_residual_features(
        changed_initial, changed_action, baseline
    )
...
def test_zero_strength_and_clamped_nodes_remain_exact():
    from experiments.dp_residual_kinematic_v1.run import correction_prediction

    baseline = np.zeros((2, 5, 13, 3))
    local = np.ones((2, 5, 9, 3))
    frames = np.broadcast_to(np.eye(3), (2, 3, 3))
    np.testing.assert_array_equal(
        correction_prediction(baseline, local, frames, 0), baseline
    )
    prediction = correction_prediction(baseline, local, frames, 0.5)
    np.testing.assert_array_equal(
        prediction[:, :, [0, 1, -2, -1]], baseline[:, :, [0, 1, -2, -1]]
    )
    np.testing.assert_allclose(prediction[:, :, 2:-2], 0.5)
