...
class AdapterTests(unittest.TestCase):
    def test_future_suffix_replacement_has_no_effect(self):
        ...
        training = rng.normal(size=(4, 30, 13, 3)) * 0.01
        base = rng.normal(size=(2, 30, 13, 3)) * 0.01
        truth = base + rng.normal(size=base.shape) * 0.01
        ...
        w[0, :2] = np.eye(2)
        dr = Draw(
            w, np.eye(2)[None] * 0.01, np.ones((1, 1)), np.ones(1), np.ones(1), 1, 0.0
        )
        ...
        np.testing.assert_array_equal(
            a[:, :, [0, 1, -2, -1]], base[:, 10:20, [0, 1, -2, -1]]
        )

    def test_prefix_shape_rejects_future(self):
        with self.assertRaises(ValueError):
            make_forecast(
                [], None, np.zeros((1, 20, 13, 3)), np.zeros((1, 20, 13, 3)), 9, 5
            )
