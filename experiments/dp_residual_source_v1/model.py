...
def log_emissions(x: np.ndarray, y: np.ndarray, draw: Draw) -> np.ndarray:
    ...
    for k, (w, q) in enumerate(zip(draw.coef, draw.covariance, strict=False)):
        error = y - x @ w
        inv = np.linalg.inv(q)
        out[..., k] = -0.5 * (
            y.shape[-1] * np.log(2 * np.pi)
...
def forward(
    loge: np.ndarray, transition: np.ndarray, initial: np.ndarray
...
def transition_counts(z: np.ndarray, k: int) -> np.ndarray:
    return np.bincount((z[:, :-1] * k + z[:, 1:]).ravel(), minlength=k * k).reshape(
        k, k
    )
...
def update_beta(
    ...
    for i in range(k):
        for j in range(k):
            ...
            global_tables[i, j] = (
                rng.binomial(tables, alpha * beta[j] / a) if i == j else tables
            )
...
def fit(
    ...
    *,
    ...
    gamma: float = 1.0,
    ridge: float = 5.0,
) -> tuple[list[Draw], dict]:
    if (
        k < 1
        or not 0 <= burn < iterations
        or thin < 1
        or min(alpha, gamma, ridge) <= 0
        or sticky < 0
    ):
    ...
    features = np.concatenate((d[:, 1:-1], 5 * (y - d[:, 1:-1])), axis=-1).reshape(
        n * t, -1
    )
    ...
    for _ in range(12):
        ...
        for j in range(k):
            ...
            centers[j] = (
                points.mean(axis=0)
                if len(points)
                else features[rng.integers(len(features))]
            )
    z = zflat.reshape(n, t)
    beta = np.full(k, 1 / k)
    v0 = ridge * np.eye(p)
    ...
    for iteration in range(iterations):
        ...
        for j in range(k):
            ...
            q = np.atleast_2d(
                invwishart.rvs(df=dim + 2 + len(xx), scale=scale, random_state=rng)
            )
            row_chol = np.linalg.cholesky(np.linalg.inv(precision))
            w.append(
                mean
                + row_chol @ rng.standard_normal((p, dim)) @ np.linalg.cholesky(q).T
            )
            cov.append(q)
        ...
        trans = np.stack(
            [
                rng.dirichlet(counts[j] + alpha * beta + sticky * np.eye(k)[j])
                for j in range(k)
            ]
        )
...
def forecast(
    ...
    *,
    ...
    if (
        not draws
        or future_controls.ndim != 3
        or future_controls.shape[0] != prefix.shape[0]
    ):
    ...
    for draw in draws:
        ...
        for t in range(future_controls.shape[1]):
            ...
            inputs = np.concatenate(
                (future_controls[:, t], np.ones((len(prefix), 1))), axis=-1
            )
            new += next_prob[..., None] * np.einsum(
                "np,kpd->nkd", inputs, draw.coef[:, 2 * dim :]
            )
...
def sample_forecast(
    ...
    *,
    ...
    x, y = design(prefix, past_controls)
    filters = [
        np.exp(forward(log_emissions(x, y, dr), dr.transition, dr.initial)[0][:, -1])
        for dr in draws
    ]
    ...
    for _ in range(samples):
        ...
        for t in range(horizon):
            ...
            xx = np.concatenate(
                (cur, prev, future_controls[:, t], np.ones((n, 1))), axis=-1
            )
            mean = np.einsum("np,npd->nd", xx, dr.coef[z])
            noise = np.einsum(
                "nij,nj->ni",
                np.linalg.cholesky(dr.covariance[z]),
                rng.standard_normal(cur.shape),
            )
